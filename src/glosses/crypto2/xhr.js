 ...
 * XHR API
 */
const xhrApi = module.exports = forge.xhr = forge.xhr || {};
        ...
        if (domain) {
            ...
            for (const key in _clients) {
                ...
                if (http.withinCookieDomain(client.url, domain)) {
                    const cookie = client.getCookie(name, path);
                    if (!is.null(cookie)) {
                        if (is.null(rval)) {
                            rval = cookie;
    ...
    xhrApi.create = function (options) {
      ...
      _state.request.method !== "HEAD") {
            ...
            options.bodyReady = function (e) {
                xhr.readyState = DONE;
                const ct = e.response.getField("Content-Type");
          ...
          ct.indexOf("+xml") !== -1) {
                        try {
                            const doc = new ActiveXObject("MicrosoftXMLDOM");
                            doc.async = false;
     ...
     (xhr.readyState === OPENED && !_state.sendFlag)) {
        ...
        xhr.getAllResponseHeaders = function () {
            let rval = "";
            if (!is.null(_state.response)) {
                const fields = _state.response.fields;
                $.each(fields, (name, array) => {
                    $.each(array, (i, value) => {
                        rval += `${name}: ${value}\r\n`;
                    });
                });
