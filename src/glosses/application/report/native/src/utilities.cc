...
/*******************************************************************************
 * Function to process report config: selection of trigger events.
 ******************************************************************************/
unsigned int ProcessNodeReportEvents(const char *args)
{
    ...
    while (*cursor != '\0')
    {
        ...
        {
            std::cerr << "Unrecognised argument for report events option: " << cursor << "\n";
            return 0;
        }
        if (*cursor == '+')
        {
...
/*******************************************************************************
 * Function to process report config: selection of trigger signal.
 ******************************************************************************/
unsigned int ProcessNodeReportSignal(const char *args)
{
    ...
    if (strlen(args) == 0)
    {
        std::cerr << "Missing argument for report signal option\n";
    }
    else
    {
        ...
        {
            std::cerr << "Unrecognised argument for report signal option: " << args << "\n";
        }
    }
...
/*******************************************************************************
 * Function to process report config: specification of report file name.
 ******************************************************************************/
void ProcessNodeReportFileName(const char *args)
{
    if (strlen(args) == 0)
    {
        std::cerr << "Missing argument for report filename option\n";
        return;
    }
    if (strlen(args) > NR_MAXNAME)
    {
        std::cerr << "Supplied report filename too long (max " << NR_MAXNAME << " characters)\n";
        return;
    }
...
/*******************************************************************************
 * Function to process report config: specification of report directory.
 ******************************************************************************/
void ProcessNodeReportDirectory(const char *args)
{
    if (strlen(args) == 0)
    {
        std::cerr << "Missing argument for report directory option\n";
        return;
    }
    if (strlen(args) > NR_MAXPATH)
    {
        std::cerr << "Supplied report directory path too long (max " << NR_MAXPATH << " characters)\n";
        return;
    }
...
/*******************************************************************************
 * Function to process report config: verbose mode switch.
 ******************************************************************************/
unsigned int ProcessNodeReportVerboseSwitch(const char *args)
{
    if (strlen(args) == 0)
    {
        std::cerr << "Missing argument for report verbose switch option\n";
        return 0;
    }
    // Parse the supplied switch
    ...
    {
        std::cerr << "Unrecognised argument for report verbose switch option: " << args << "\n";
    }
 ...
 * during report module initialisation.
 *******************************************************************************/
void SetVersionString(Isolate *isolate)
{
    ...
    if (!Nan::New<v8::String>("process").ToLocal(&process_prop))
        return;
    v8::Local<v8::Object> global_obj = isolate->GetCurrentContext()->Global();
    v8::Local<v8::Value> process_value;
    if (!Nan::Get(global_obj, process_prop).ToLocal(&process_value))
        return;
    ...
    if (!Nan::New<v8::String>("version").ToLocal(&version_prop))
        return;
    v8::Local<v8::Value> version;
    if (!Nan::Get(process_obj, version_prop).ToLocal(&version))
        return;
    ...
    if (!Nan::New<v8::String>("versions").ToLocal(&versions_prop))
        return;
    v8::Local<v8::Value> versions_value;
    if (!Nan::Get(process_obj, versions_prop).ToLocal(&versions_value))
        return;
    if (!versions_value->IsObject())
        return;
    ...
    if (!Nan::GetOwnPropertyNames(versions_obj).ToLocal(&components))
        return;
    ...
    for (uint32_t i = 0; i < total_components; i++)
    {
        ...
        if (!Nan::Get(components_obj, i).ToLocal(&name_value))
            continue;
        v8::Local<v8::Value> version_value;
        if (!Nan::Get(versions_obj, name_value).ToLocal(&version_value))
            continue;

        Nan::Utf8String component_name(name_value);
        Nan::Utf8String component_version(version_value);
        if (*component_name == nullptr || *component_version == nullptr)
            continue;
...
/*******************************************************************************
 * Function to save the report module load time value. This is called
 * during report module initialisation.
 *******************************************************************************/
void SetLoadTime()
{
...
/*******************************************************************************
 * Function to save the process command line. This is called during report
