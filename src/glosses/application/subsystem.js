...
export default class Subsystem extends adone.event.AsyncEmitter {
    ...
    set name(val) {
        throw new exception.NotAllowed("Subsystem name is immutable");
    }
    ...
    set root(val) {
        throw new exception.NotAllowed("Subsystem root is immutable");
    }
    ...
    set parent(val) {
        throw new exception.NotAllowed("Subsytem parent is immutable");
    }
    ...
    set state(val) {
        throw new exception.NotAllowed("Subsystem state is immutable");
    }
    ...
    waitForState(expectedState, timeout) {
        return new Promise((resolve, reject) => {
            ...
            if (is.number(timeout) && timeout > 0) {
                setTimeout(() => {
                    this.removeListener("state", awaiter);
                    reject(new exception.Timeout(`Timeout occured while waiting for state: ${expectedState}`));
                }, timeout);
            }
        });
    }
    ...
    addSubsystem({ subsystem, name = null, useFilename = false, description = "", group = "subsystem", configureArgs = [], transpile, bind } = {}) {
        ...
        if (instance[OWNED_SYMBOL] === true) {
            throw new exception.NotAllowed("Subsystem already owned by other subsystem");
        }
        ...
        if (this.hasSubsystem(name)) {
            throw new exception.Exists(`Subsystem with name '${name}' already exists`);
        }
        ...
        if (is.string(bind)) {
            if (this[bind]) {
                throw new exception.NotAllowed(`Property with name '${bind}' is already exist`);
            }
            this[bind] = instance;
    ...
    instantiateSubsystem(subsystem, { transpile = false } = {}) {
        ...
        if (is.string(subsystem)) {
            if (!std.path.isAbsolute(subsystem)) {
                throw new exception.NotValid("Path must be absolute");
            }
        ...
        if (!is.subsystem(instance)) {
            throw new exception.NotValid("'subsystem' should be path or instance of adone.application.Subsystem");
        }
    ...
    deleteSubsystem(name, force = false) {
        ...
        if (index < 0) {
            throw new exception.Unknown(`Unknown subsystem: ${name}`);
        }
        if (!force && ![STATE.INITIAL, STATE.UNINITIALIZED, STATE.FAILED].includes(this[SUBSYSTEMS_SYMBOL][index].instance[STATE_SYMBOL])) {
            throw new exception.NotAllowed("The subsystem is used and can not be deleted");
        }
    ...
    async _configureSubsystem(sysInfo) {
        if (sysInfo.instance[STATE_SYMBOL] === STATE.INITIAL) {
            ...
            throw new exception.IllegalState(`Illegal state of '${sysInfo.name}' subsystem for configure: ${humanizeState(sysInfo.instance[STATE_SYMBOL])}`);
        }
    }
    ...
    async _initializeSubsystem(sysInfo) {
        if (sysInfo.instance[STATE_SYMBOL] === STATE.CONFIGURED) {
            ...
            throw new exception.IllegalState(`Illegal state of '${sysInfo.name}' subsystem for initialize: ${humanizeState(sysInfo.instance[STATE_SYMBOL])}`);
        }
    }
    ...
    async _uninitializeSubsystem(sysInfo) {
        if (sysInfo.instance[STATE_SYMBOL] === STATE.INITIALIZED) {
            ...
            throw new exception.IllegalState(`Illegal state of '${sysInfo.name}' subsystem for uninitialize: ${humanizeState(sysInfo.instance[STATE_SYMBOL])}`);
        }
    }
    ...
    async _reinitializeSubsystem(sysInfo) {
        if (sysInfo.instance[STATE_SYMBOL] === STATE.INITIALIZED) {
            ...
            throw new exception.IllegalState(`Illegal state of '${sysInfo.name}' subsystem for reinitialize: ${humanizeState(sysInfo.instance[STATE_SYMBOL])}`);
        }
    }
