...
List._init = function (constructor, object, values, equals, getDefault) {
    ...
    const head = object.head = new object.Node();
    head.next = head;
    head.prev = head;
    object.contentEquals = equals || Object.equals;
    object.getDefault = getDefault || adone.noop;
    object.length = 0;
...
List.prototype.find = function (value, equals, index) {
    ...
    let at = this.scan(index, head.next);
    while (at !== head) {
...
List.prototype.findLast = function (value, equals, index) {
    equals = equals || this.contentEquals;
    const head = this.head;
    let at = this.scan(index, head.prev);
    while (at !== head) {
        if (equals(at.value, value)) {
            return at;
        }
...
List.prototype.get = function (value, equals) {
    const found = this.find(value, equals);
    if (found) {
        return found.value;
    }
...
List.prototype.shift = function (_before, _after) {
    ...
    if (head.prev !== head) {
        ...
        const dispatchValueArray = _before ? _before.call(this, value, 0) : void 0;
        head.next["delete"]();
        this.length--;
...
List.prototype.swap = function (start, length, plus) {
    ...
    // delete minus
    at = start;
    for (let i = 0; i < minus.length; i++, at = at.next) {
        at["delete"]();
    }
    // add plus
    ...
    for (let i = 0; i < plus.length; i++) {
        const node = new this.Node(plus[i]);
        at.addBefore(node);
    }
    // adjust length
