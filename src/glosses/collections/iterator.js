...
export default class Iterator {
    constructor(iterable) {
        const values = iterable && iterable.values && iterable.values();
        ...
        if (iterable instanceof Iterator) {
            ...
            const iterator = iterable.iterate();
            this.next = function () {
...
Iterator.prototype.mapIterator = function (callback /*, thisp*/) {
    ...
    return new Iterator(function () {
        if (self._iterationObject.done !== true) {
            const callbackValue = callback.call(thisp, self.next().value, i++, self);
...
Iterator.prototype.dropWhile = function (callback /*, thisp */) {
    ...
    if (stopped) {
        return new Iterator([stopValue]).concat(self);
    } else {
        return new Iterator([]);
    }
};
...
Iterator.concat = function (iterators) {
    iterators = new Iterator(iterators);
    let next;
    return new Iterator(function () {
        let iteration;
        let nextEntry;
        ...
        if (!nextEntry || nextEntry.done === true) {
            ...
            if (nextEntry.done === false) {
                iteration = new Iterator(nextEntry.value);
...
Iterator.unzip = function (iterators) {
    iterators = new Iterator(iterators).map(Iterator);
...
Iterator.repeat = function (value, times) {
    return Iterator.range(times).mapIterator(function () {
        return value;
    });
};
