...
SortedSet.prototype.add = function (value) {
    ...
    if (this.root) {
        this.splay(value);
        if (!this.contentEquals(value, this.root.value)) {
            const comparison = this.contentCompare(value, this.root.value);
            if (comparison === 0) {
...
SortedSet.prototype.splay = function (value) {
    ...
    const stub = left = right = new this.Node();
    ...
    // root.
    const history = new this.Node();
    root = this.root;

    while (true) {
        const comparison = this.contentCompare(value, root.value);
        if (comparison < 0) {
            if (root.left) {
...
SortedSet.prototype.max = function (at) {
    const greatest = this.findGreatest(at);
    if (greatest) {
...
function Iterator(set, start, end) {
    ...
    this.end = end;
    if (start) {
        const next = this.set.findLeastGreaterThanOrEqual(start);
        if (next) {
