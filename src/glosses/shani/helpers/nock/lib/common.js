...
const isBinaryBuffer = function (buffer) {
  ...
  //  Test if the buffer can be reconstructed verbatim from its utf8 encoding.
    ...
    const reconstructedBuffer = new Buffer(utfEncodedBuffer, "utf8");
...
const overrideRequests = function (newRequest) {
    ...
    ["http", "https"].forEach((proto) => {
        ...
        let moduleName = proto, // 1 to 1 match of protocol and module is fortunate :)
            module = {
                http: require("http"),
                https: require("https")
            }[moduleName],
...
const headersFieldsArrayToLowerCase = function (headers) {
    return _.uniq(_.map(headers, (fieldName) => {
