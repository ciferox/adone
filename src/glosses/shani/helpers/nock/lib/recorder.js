...
function generateRequestAndResponse(req, bodyChunks, options, res, dataChunks) {
  ...
  // Remove any query params from options.path so they can be added in the query() function
    ...
    // Create the query() object
        const queries = req.path.slice(queryIndex + 1).split("&");
...
function record(rec_options) {
  ...
  //  To keep the existing code compatible we take that case into account.
    ...
    const dont_print = (typeof rec_options === "boolean" && rec_options) ||
      (optionsIsObject && rec_options.dont_print);
    const output_objects = optionsIsObject && rec_options.output_objects;
    const enable_reqheaders_recording = optionsIsObject && rec_options.enable_reqheaders_recording;
  ...
  //  We override the requests so that we can save information on them before executing.
    ...
    // twice.
        ...
        var req = overriddenRequest(options, (res) => {
