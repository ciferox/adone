/*!
 * overwriteProperty utility
 * Copyright(c) 2012-2014 Jake Luer <jake@alogicalparadox.com>
 * MIT Licensed
 */

import adone from "adone";
import $assert from "../..";
import flag from "./flag";
import transferFlags from "./transferFlags";

/**
 * ### overwriteProperty (ctx, name, fn)
 *
 * Overwites an already existing property getter and provides
 * access to previous value. Must return function to use as getter.
 *
 *     utils.overwriteProperty(assert.Assertion.prototype, 'ok', function (_super) {
 *       return function () {
 *         var obj = utils.flag(this, 'object');
 *         if (obj instanceof Foo) {
 *           assert.getAssertion(obj.name).to.equal('bar');
 *         } else {
 *           _super.call(this);
 *         }
 *       }
 *     });
 *
 *
 * Can also be accessed directly from `assert.Assertion`.
 *
 *     assert.Assertion.overwriteProperty('foo', fn);
 *
 * Then can be used as any other assertion.
 *
 *     expect(myFoo).to.be.ok;
 *
 * @param {Object} ctx object whose property is to be overwritten
 * @param {String} name of property to overwrite
 * @param {Function} getter function that returns a getter function to be used for name
 * @namespace Utils
 * @name overwriteProperty
 * @api public
 */

export default function (ctx, name, getter) {
    const _get = Object.getOwnPropertyDescriptor(ctx, name);
    let _super = function () { };

    if (_get && adone.is.function(_get.get)) {
        _super = _get.get;
    }

    Object.defineProperty(ctx, name, {
        get: function overwriteProperty() {
            const keep_ssfi = flag(this, "keep_ssfi");
            const old_ssfi = flag(this, "ssfi");
            if (!keep_ssfi && old_ssfi)
                flag(this, "ssfi", overwriteProperty);

            flag(this, "keep_ssfi", true);
            const result = getter(_super).call(this);
            flag(this, "keep_ssfi", false);

            if (result !== undefined) {
                return result;
            }

            const newAssertion = $assert.getAssertion();
            transferFlags(this, newAssertion);
            return newAssertion;
        }
        , configurable: true
    });
}