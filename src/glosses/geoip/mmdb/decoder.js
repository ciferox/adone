...
export default class Decoder {
    constructor(db, baseOffset = 0, opts = {}) {
        if (!db) {
            throw new error.InvalidArguemnt("File stream is required");
        }
        this.db = db;
    ...
    sizeFromCtrlByte(ctrlByte, offset) {
        ...
        return cursor(
            65821 + concat3(this.db[offset], this.db[offset + 1], this.db[offset + 2]),
            offset + 3
        );
    }
    ...
    decodePointer(ctrlByte, offset) {
        ...
        if (pointerSize === 0) {
            packed = concat2(ctrlByte & 7, this.db[offset]);
            ...
            packed = concat3(ctrlByte & 7, this.db[offset], this.db[offset + 1]);
    ...
    decodeBigUint(offset, size) {
        ...
        let integer = new adone.math.BigNumber(0);
        const numberOfLongs = size / 4;
        for (let i = 0; i < numberOfLongs; ++i) {
            integer = integer.shiftLeft(32).or(buffer.readUInt32BE(i << 2, true));
        }
