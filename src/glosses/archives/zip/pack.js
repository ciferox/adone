...
class Entry {
    constructor(metadataPath, isDirectory, options) {
        ...
        if (this.utf8FileName.length > 0xffff) {
            throw new x.InvalidArgument(`utf8 file name too long. ${this.utf8FileName.length} > ${0xffff}`);
        }
    ...
    setFileAttributesMode(mode) {
        if ((mode & 0xffff) !== mode) {
            throw new x.InvalidArgument(`invalid mode. expected: 0 <= ${mode} <= ${0xffff}`);
        }
    ...
    getDataDescriptor() {
        ...
        if (!this.useZip64Format()) {
            const buffer = Buffer.alloc(DATA_DESCRIPTOR_SIZE);
        ...
        // ZIP64 format
        const buffer = Buffer.alloc(ZIP64_DATA_DESCRIPTOR_SIZE);
...
const validateMetadataPath = (metadataPath, isDirectory) => {
    if (metadataPath === "") {
        throw new x.IllegalState("empty metadataPath");
    }
    metadataPath = metadataPath.replace(/\\/g, "/");
    if (/^[a-zA-Z]:/.test(metadataPath) || /^\//.test(metadataPath)) {
        throw new x.IllegalState(`absolute path: ${metadataPath}`);
    }
    if (metadataPath.split("/").indexOf("..") !== -1) {
        throw new x.IllegalState(`invalid relative path: ${metadataPath}`);
    }
    ...
    if (isDirectory) {
        ...
        if (looksLikeDirectory) {
            throw new x.IllegalState(`file path cannot end with '/': ${metadataPath}`);
        }
    }
    return metadataPath;
};
...
export class ZipFile extends EventEmitter {
    ...
    addFile(realPath, metadataPath, options = {}) {
        ...
        fs.stat(realPath).then((stats) => {
            if (!stats.isFile()) {
                return this.emit("error", new x.IllegalState(`not a file: ${realPath}`));
            }
    ...
    addBuffer(buffer, metadataPath, options = {}) {
        ...
        if (buffer.length > 0x3fffffff) {
            throw new x.InvalidArgument(`buffer too large: ${buffer.length} > ${0x3fffffff}`);
        }
        if (!is.nil(options.size)) {
            throw new x.InvalidArgument("options.size not allowed");
        }
        const entry = new Entry(metadataPath, false, options);
    ...
    addEmptyDirectory(metadataPath, options = {}) {
        ...
        if (!is.nil(options.compress)) {
            throw new x.InvalidArgument("options.compress not allowed");
        }
