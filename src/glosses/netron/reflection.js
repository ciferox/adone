...
// decorators
export const Context = (info) => reflect.metadata(CONTEXT_ANNOTATION, info || {});
export const Public = (info) => reflect.metadata(PUBLIC_ANNOTATION, info || {});
export const Property = (name, info) => reflect.metadata(propertyAnnotation(name), info || {});
export const Method = (name, info) => reflect.metadata(methodAnnotation(name), info || {});
...
export class Reflection {
    ...
    static from(instance) {
        if (!is.netronContext(instance) || is.class(instance)) {
            throw new x.NotValid(`'${instance.__proto__.constructor.name}' is not valid instance of netron context`);
        }
