    ...
    async bind(netron) {
        ...
        if (options.secure) {
            ...
            if (is.buffer(options.cert)) {
                ...
                throw new error.NotValid("parameter 'options.cert' is not valid (should be buffer or string)");
            }
            ...
            if (is.buffer(options.key)) {
                ...
                throw new error.NotValid("parameter 'options.key' is not valid (should be buffer or string)");
            }
                ...
                if (e.code === "EADDRINUSE") {
                    if (unixSocket) {
                        ...
                        clientSocket.connect({ path: port }, () => {
                            clientSocket.end();
                            reject(new error.Bind(`address '${port}' already in use`));
                        });
                    } else {
                        reject(new error.Bind(`address '${host}:${port}' already in use`));
                    }
                } else {
                    try {
        ...
        this._wss.on("connection", (ws) => {
            ...
            connHandler(peer).then(() => {
                if (peer.isConnected()) {
                    ws.onerror = (errEvent) => {
                        peer.emit("error", new error.Runtime(errEvent.data));
                    };
                    ws.onclose = () => {
