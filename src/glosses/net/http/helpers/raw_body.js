...
const getDecoder = (encoding) => {
    ...
    try {
        ...
        throw http.x.create(415, "specified encoding unsupported", {
            encoding,
...
const readStream = (stream, encoding, length, limit) => new Promise((resolve, reject) => {
    ...
    if (!is.null(limit) && !is.null(length) && length > limit) {
        return done(http.x.create(413, "request entity too large", {
            expected: length,
            length,
            limit,
    ...
    if (stream._readableState.encoding) {
        // developer error
        return done(http.x.create(500, "stream encoding should not be set", {
    ...
    const onAborted = () => {
        ...
        done(http.x.create(400, "request aborted", {
            code: "ECONNABORTED",
            expected: length,
            length,
            received,
    ...
    const onData = (chunk) => {
        ...
        if (!is.null(limit) && received > limit) {
            done(http.x.create(413, "request entity too large", {
                limit,
                received,
                type: "entity.too.large"
            }));
    ...
    const onEnd = (err) => {
        ...
        if (!is.null(length) && received !== length) {
            done(http.x.create(400, "request size did not match content length", {
                expected: length,
                length,
                received,
