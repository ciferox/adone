...
export default class IncomingForm extends event.Emitter {
    ...
    write(buffer) {
        ...
        if (!this._parser) {
            this._error(new error.IllegalStateException("uninitialized parser"));
            return;
        }
        ...
        if (bytesParsed !== buffer.length) {
            this._error(new error.IllegalStateException(`parser error, ${bytesParsed} of ${buffer.length} bytes parsed`));
        }
    ...
    handlePart(part) {
        if (is.undefined(part.filename)) {
            ...
            part.on("data", (buffer) => {
                ...
                if (this._fieldsSize > this.maxFieldsSize) {
                    this._error(new error.IllegalStateException(`maxFieldsSize exceeded, received ${this._fieldsSize} bytes of field data`));
                    return;
                }
        ...
        part.on("data", (buffer) => {
            ...
            if (this._fileSize > this.maxFileSize) {
                this._error(new error.IllegalStateException(`maxFileSize exceeded, received ${this._fileSize} bytes of file data`));
                return;
            }
            if (buffer.length === 0) {
                return;
    ...
    _parseContentType() {
        ...
        if (!this.headers["content-type"]) {
            this._error(new error.IllegalStateException("bad content-type header, no content-type"));
            return;
        }
        ...
        if (this.headers["content-type"].match(/multipart/i)) {
            ...
            if (m) {
                ...
                this._error(new error.IllegalStateException("bad content-type header, no multipart boundary"));
            }
            return;
        }
        ...
        this._error(new error.UnknownException(`bad content-type header, unknown content-type: ${this.headers["content-type"]}`));
    }
    ...
    _initMultipart(boundary) {
        ...
        parser.onHeadersEnd = () => {
            switch (part.transferEncoding) {
                ...
                default: {
                    return this._error(new error.UnknownException(`unknown transfer-encoding: ${part.transferEncoding}`));
                }
            }
