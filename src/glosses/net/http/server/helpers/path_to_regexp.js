        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (is.nil(value)) {
                if (token.optional) {
                    ...
                    throw new error.IllegalStateException(`Expected "${token.name}" to be defined`);
                }
            }
            ...
            if (is.array(value)) {
                if (!token.repeat) {
                    throw new error.IllegalStateException(`Expected "${token.name}" to not repeat, but received \`${JSON.stringify(value)}\``);
                }
                ...
                if (value.length === 0) {
                    if (token.optional) {
                        ...
                        throw new error.IllegalStateException(`Expected "${token.name}" to not be empty`);
                    }
                }
                ...
                for (let j = 0; j < value.length; j++) {
                    ...
                    if (!matches[i].test(segment)) {
                        throw new error.IllegalStateException(`Expected all "${token.name}" to match "${token.pattern}", but received \`${JSON.stringify(segment)}\``);
                    }
            ...
            if (!matches[i].test(segment)) {
                throw new error.IllegalStateException(`Expected "${token.name}" to match "${token.pattern}", but received "${segment}"`);
            }
