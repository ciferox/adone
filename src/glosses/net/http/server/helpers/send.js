...
export default async function send(ctx, path, opts = {}) {
    if (!ctx) {
        throw new error.InvalidArgumentException("context is required");
    }
    if (!path) {
        throw new error.InvalidArgumentException("pathname is required");
    }
    ...
    if (setHeaders && !is.function(setHeaders)) {
        throw new error.InvalidArgumentException("option setHeaders must be function");
    }
    ...
    if (extensions && !/\.[^/]*$/.exec(path)) {
        for (let ext of extensions) {
            if (!is.string(ext)) {
                throw new error.InvalidArgumentException("option extensions must be array of strings or false");
            }
            if (!/^\./.exec(ext)) {
                ext = `.${ext}`;
            }
