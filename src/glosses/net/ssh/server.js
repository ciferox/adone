...
function Server(cfg, listener) {
    ...
    if (algorithms.serverHostKey === undefined) {
        let hostKeyAlgos = Object.keys(hostKeys);
    ...
    let oldDebug = streamcfg.debug;
    let cfgKeys = Object.keys(streamcfg);
    ...
    this._srv = new net.Server(function(socket) {
        ...
        let sshstream = new SSH2Stream(conncfg);
        let client = new Client(sshstream, socket);
...
function Client(stream, socket) {
    ...
    let unsentGlobalRequestsReplies = [];
...
function openChannel(self, type, opts, cb) {
    ...
    sshstream.once("CHANNEL_OPEN_CONFIRMATION:" + localChan, function(info) {
        ...
        let err = new Error(`(SSH) Channel open failure: ${info.reason} ${info.description}`);
