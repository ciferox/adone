...
Client.prototype.connect = function(cfg) {
    ...
    let debug = this.config.debug = (typeof cfg.debug === "function" ?
    ...
    if (this.config.privateKey) {
        let privKeyInfo = parseKey(this.config.privateKey);
    ...
    let stream = this._sshstream = new SSH2Stream({
        algorithms: algorithms,
        debug: (debug === DEBUG_NOOP ? undefined : debug)
    });
    let sock = this._sock = (cfg.sock || new Socket());
    ...
    // keepalive-related
    let kainterval = this.config.keepaliveInterval;
    let kacountmax = this.config.keepaliveCountMax;
    let kacount = 0;
    let katimer;
    ...
    sock.on("connect", function() {
        ...
        // is received
        let chanNos = Object.keys(self._channels);
        self._channels = {};
    ...
    if (typeof cfg.hostVerifier === "function") {
        ...
        let hashCb = cfg.hostVerifier;
        let hasher = crypto.createHash(cfg.hostHash);
        stream.once("fingerprint", function(key, verify) {
            hasher.update(key);
            let ret = hashCb(hasher.digest("hex"), verify);
            if (ret !== undefined)
                verify(ret);
        });
    }
    ...
    function onUSERAUTH_INFO_REQUEST(name, instructions, lang, prompts) {
        let nprompts = (Array.isArray(prompts) ? prompts.length : 0);
        if (nprompts === 0) {
    ...
    function onUSERAUTH_PK_OK() {
        if (curAuth === "agent") {
            let agentKey = agentKeys[agentKeyPos];
            let keyLen = agentKey.readUInt32BE(0, true);
            let pubKeyFullType = agentKey.toString("ascii", 4, 4 + keyLen);
            let pubKeyType = pubKeyFullType.slice(4);
                    ...
                    agentQuery(self.config.agent, agentKey, pubKeyType, buf,
                        function(err, signed) {
                            if (err) {
                                ...
                                let sigFullTypeLen = signed.readUInt32BE(0, true);
                                if (4 + sigFullTypeLen + 4 < signed.length) {
                                    let sigFullType = signed.toString("ascii", 4, 4 + sigFullTypeLen);
    ...
    if (!cfg.sock) {
        ...
        let forceIPv4 = this.config.forceIPv4;
        let forceIPv6 = this.config.forceIPv6;
        ...
        if ((!forceIPv4 && !forceIPv6) || (forceIPv4 && forceIPv6))
            ...
            dnsLookup(host, (forceIPv4 ? 4 : 6), function(err, address, family) {
                if (err) {
                    let error = new Error("Error while looking up " +
                        (forceIPv4 ? "IPv4" : "IPv6") +
                        " address for host " +
                        host +
                        ": " + err);
...
Client.prototype.sftp = function(cb) {
    ...
    return openChannel(this, "session", function(err, chan) {
        ...
        reqSubsystem(chan, "sftp", function(err, stream) {
            ...
            let serverIdentRaw = self._sshstream._state.incoming.identRaw;
            let cfg = {
                debug: self.config.debug
            };
            let sftp = new SFTPStream(cfg, serverIdentRaw);
...
Client.prototype.unforwardIn = function(bindAddr, bindPort, cb) {
    ...
    if (wantReply) {
        this._callbacks.push(function(had_err) {
            if (had_err) {
                return cb(had_err !== true ?
                    had_err : new Error("Unable to unbind from " + bindAddr + ":" + bindPort)
                );
            }
...
function openChannel(self, type, opts, cb) {
    ...
    let localChan = nextChannel(self);
    let initWindow = Channel.MAX_WINDOW;
    let maxPacket = Channel.PACKET_SIZE;
    let ret = true;
...
function reqX11(chan, screen, cb) {
    ...
    if (wantReply) {
        chan._callbacks.push(function(had_err) {
            if (had_err) {
                return cb(had_err !== true ? had_err : new Error("Unable to request X11"));
            }
...
function reqPty(chan, opts, cb) {
    ...
    if (wantReply) {
        chan._callbacks.push(function(had_err) {
            if (had_err) {
                return cb(had_err !== true ?
                    had_err : new Error("Unable to request a pseudo-terminal"));
            }
            cb();
        });
    }
...
function reqAgentFwd(chan, cb) {
    ...
    chan._callbacks.push(function(had_err) {
        if (had_err) {
            ...
            wantReply && cb(had_err !== true ? had_err : new Error("Unable to request agent forwarding"));
            return;
        }
...
function reqShell(chan, cb) {
    ...
    chan._callbacks.push(function(had_err) {
        if (had_err) {
            return cb(had_err !== true ? had_err : new Error("Unable to open shell"));
        }
        chan.subtype = "shell";
        cb(undefined, chan);
    });
...
function reqSubsystem(chan, name, cb) {
    ...
    chan._callbacks.push(function(had_err) {
        if (had_err) {
            return cb(had_err !== true ? had_err : new Error("Unable to start subsystem: " + name));
        }
...
function onCHANNEL_OPEN(self, info) {
    ...
    function accept() {
        ...
        let stream = new Channel(chaninfo, self);
