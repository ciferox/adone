const { toType, toString } = require("./types");
...
name.encode = function (n, buf, offset) {
    if (!buf) {
        buf = Buffer.allocUnsafe(name.encodingLength(n));
    }
    if (!offset) {
        offset = 0;
    }
...
answer.encode = function (a, buf, offset) {
    ...
    let klass = is.undefined(a.class) ? 1 : a.class;
    if (a.flush) {
        klass |= FLUSH_MASK;
...
question.encode = function (q, buf, offset) {
    ...
    buf.writeUInt16BE(is.undefined(q.class) ? 1 : q.class, offset);
    offset += 2;
