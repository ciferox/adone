    ...
    switch (type) {
        ...
        case "object": {
            if (is.null(val)) {
                ...
                tag("A");
                offset += encodeArray(buffer, val, offset); // eslint-disable-line
                ...
                tag("F");
                offset += encodeTable(buffer, val, offset); // eslint-disable-line
            }
            break;
        }
        case "boolean":
        ...
        case "decimal":
            tag("D");
            if (
                val.hasOwnProperty("places") && val.hasOwnProperty("digits") && val.places >= 0 && val.places < 256) {
...
export const encodeTable = (buffer, val, offset) => {
    const start = offset;
    offset += 4; // leave room for the table length
    for (const key in val) {
        if (!is.undefined(val[key])) {
...
const encodeArray = (buffer, val, offset) => {
    const start = offset;
    offset += 4;
...
// fields.
export const decodeFields = (slice) => {
    const fields = {};
    let offset = 0;
    ...
    const decodeFieldValue = () => {
        ...
        switch (tag) {
            ...
            case "A":
                len = slice.readUInt32BE(offset); offset += 4;
                decodeArray(offset + len); // eslint-disable-line
