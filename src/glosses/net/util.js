...
const util = adone.o({
    isLocal: (port, host) => ((is.nil(port) && is.nil(host)) || is.string(port)),
    normalizeAddr: (port, host, defaultPort) => {
        if (is.string(port)) {
    ...
    async getFreePort({ exclude = null, lbound = 40000, rbound = 65000, rounds = 1000 } = { }) {
        while (rounds--) {
