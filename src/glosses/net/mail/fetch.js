...
function fetch(url, options) {
    ...
    options.fetchRes = options.fetchRes || new adone.std.stream.PassThrough();
    ...
    req.on("response", (res) => {
        ...
        switch (res.headers["content-encoding"]) {
            case "gzip":
            case "deflate":
                inflate = adone.std.zlib.createUnzip();
                break;
        }
        ...
        if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
            ...
            return fetch(adone.std.url.resolve(url, res.headers.location), options);
        }
