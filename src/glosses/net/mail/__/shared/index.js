const { is, std: { util, fs, url: urllib }, net: { mail: { __: { fetch } } } } = adone;
 ...
 */
const resolveStream = (stream, callback) => {
    let responded = false;
    const chunks = [];
    let chunklen = 0;
...
const createDefaultLogger = (levels) => {
 ...
 */
export const callbackPromise = (resolve, reject) => (err, ...args) => {
    if (err) {
        reject(err);
    } else {
        resolve(...args);
...
export const resolveContent = (data, key, callback) => {
    ...
    let contentStream;
    const encoding = (is.object(data[key]) && data[key].encoding || "utf8")
        .toString()
        .toLowerCase()
        .replace(/[-_\s]/g, "");
    ...
    if (is.string(data[key].content) && !["utf8", "usascii", "ascii"].includes(encoding)) {
...
export const assign = (target, ...sources) => {
    ...
    sources.forEach((source) => {
        Object.keys(source || {}).forEach((key) => {
            if (["tls", "auth"].includes(key) && source[key] && is.object(source[key])) {
