...
export default function fetch(_url, options) {
    ...
    if (options.timeout) {
        req.setTimeout(options.timeout, () => {
            ...
            finished = true;
            req.abort();
            const err = new error.TimeoutException("Request Timeout");
            err.type = "FETCH";
            err.sourceUrl = _url;
            fetchRes.emit("error", err);
        });
    ...
    req.on("response", (res) => {
        ...
        if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
            ...
            if (options.redirects > options.maxRedirects) {
                finished = true;
                const err = new error.IllegalStateException("Maximum redirect count exceeded");
                err.type = "FETCH";
                err.sourceUrl = _url;
                fetchRes.emit("error", err);
                req.abort();
        ...
        if (res.statusCode >= 300 && !options.allowErrorResponse) {
            finished = true;
            const err = new error.IllegalStateException(`Invalid status code ${res.statusCode}`);
            err.type = "FETCH";
            err.sourceUrl = _url;
            fetchRes.emit("error", err);
            req.abort();
