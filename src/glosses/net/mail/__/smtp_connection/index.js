...
export default class SMTPConnection extends event.Emitter {
    constructor(options) {
        ...
        this.secureConnection = Boolean(this.options.secure);
        this.alreadySecured = Boolean(this.options.secured);

        this.port = this.options.port || (this.secureConnection ? 465 : 587);
        this.host = this.options.host || "localhost";

        if (is.undefined(this.options.secure) && this.port === 465) {
         ...
         * @private
         */
        this.secure = Boolean(this.secureConnection);
    ...
    connect(connectCallback) {
        if (is.function(connectCallback)) {
            this.once("connect", () => {
                this.logger.debug({
                    tnx: "smtp"
    ...
    _formatError(message, type, response, command) {
        ...
        const responseCode = is.string(response) && Number((response.match(/^\d+/) || [])[0]) || false;
        if (responseCode) {
    ...
    _setEnvelope(envelope, callback) {
        ...
        this._envelope.to = util.arrify(this._envelope.to || []).map((to) => (to && to.address || to || "").toString().trim());
    ...
    _actionGreeting(str) {
        ...
        if (str.substr(0, 3) !== "220") {
            this._onError(new error.Exception(`Invalid greeting from server:\n${str}`), "EPROTOCOL", str, "CONN");
            return;
        }
    ...
    _actionLHLO(str) {
        if (str.charAt(0) !== "2") {
            this._onError(new error.Exception(`Invalid response for LHLO:\n${str}`), "EPROTOCOL", str, "LHLO");
            return;
        }
    ...
    _actionEHLO(str) {
        ...
        if (str.substr(0, 3) === "421") {
            this._onError(new error.Exception(`Server terminates connection:\n${str}`), "ECONNECTION", str, "EHLO");
            return;
        }

        if (str.charAt(0) !== "2") {
            if (this.options.requireTLS) {
                this._onError(new error.Exception(`EHLO failed but HELO does not support required STARTTLS:\n${str}`), "ECONNECTION", str, "EHLO");
                return;
            }
    ...
    _actionHELO(str) {
        if (str.charAt(0) !== "2") {
            this._onError(new error.Exception(`Invalid response for EHLO/HELO:\n${str}`), "EPROTOCOL", str, "HELO");
            return;
        }
    ...
    _actionSTARTTLS(str) {
        if (str.charAt(0) !== "2") {
            ...
            this._onError(new error.Exception("Error upgrading connection with STARTTLS"), "ETLS", str, "STARTTLS");
            return;
        }

        this._upgradeConnection((err, secured) => {
            if (err) {
                this._onError(new error.Exception(`Error initiating TLS - ${err.message || err}`), "ETLS", false, "STARTTLS");
                return;
            }
    ...
    _actionAUTH_CRAM_MD5(str, callback) {
        ...
        const hmacHEX = hmacMD5.digest("hex");
        const prepended = `${this._auth.credentials.user} ${hmacHEX}`;
    ...
    _actionAUTH_LOGIN_PASS(str, callback) {
        ...
        this._sendCommand(Buffer.from(String(this._auth.credentials.pass), "utf-8").toString("base64"));
    }
    ...
    _actionAUTHComplete(str, isRetry, callback) {
        if (!callback && is.function(isRetry)) {
            callback = isRetry;
            isRetry = false;
        }
