...
Connection.prototype._setDefaultWindow = function _setDefaultWindow(settings) {
    ...
    Object.keys(state.stream.map).forEach((id) => {
        let stream = state.stream.map[id];
        let window = stream._spdyState.window;
...
Connection.prototype.request = function request(uri, callback) {
    var stream = this.reserveStream(uri, (err) => {
        if (err) {
            if (callback) {
                callback(err);
            } else {
        ...
        stream.send(function (err) {
            if (err) {
                if (callback) { return callback(err) } return stream.emit('error', err)
            }
...
Connection.prototype._goaway = function _goaway(params) {
    ...
    function finish() {
        ...
        if (state.stream.count === 0 || params.code !== "OK") {
            ...
            process.nextTick(() => {
                let err = new Error("Fatal error: " + params.code);
