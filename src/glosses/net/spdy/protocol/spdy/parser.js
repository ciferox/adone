...
Parser.prototype.onSynHeadFrame = function onSynHeadFrame(type,
    ...
    this.parseKVs(body, (err, headers) => {
        ...
        let id = head.readUInt32BE() & 0x7fffffff;
        ...
        let associated = stream ? head.readUInt32BE() & 0x7fffffff : 0;
        let priority = stream
            ? head.readUInt8() >> 5
            : utils.weightToPriority(constants.DEFAULT_WEIGHT);
        let fin = (flags & constants.flags.FLAG_FIN) !== 0;
        let unidir = (flags & constants.flags.FLAG_UNIDIRECTIONAL) !== 0;
        let path = headers[":path"];

        let isPush = stream && associated !== 0;

        let weight = utils.priorityToWeight(priority);
        let priorityInfo = {
            weight,
            exclusive: false,
            parent: 0
        ...
        let filteredHeaders = self._filterHeader(headers, ":status");
...
Parser.prototype.parseKVs = function parseKVs(buffer, callback) {
    ...
    this.decompress.write(buffer.toChunks(), (err, chunks) => {
        ...
        let buffer = new OffsetBuffer();
        for (let i = 0; i < chunks.length; i++) {
            buffer.push(chunks[i]);
        }

        let size = self.version === 2 ? 2 : 4;
        if (!buffer.has(size)) {
        ...
        while (count > 0) {
            ...
            if (self.version < 3) {
                let isInternal = /^(method|version|url|host|scheme|status)$/.test(key);
                if (key === "url") {
                    key = "path";
                }
                if (isInternal) {
