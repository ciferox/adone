...
const { protocol: { base: { utils, Parser }, http2: { constants } } } = spdy;

export default class Http2Parser extends Parser {
    constructor(options) {
        super(options);
    ...
    onUnknownFrame(header, buffer, callback) {
    ...
    unpadData(header, body, callback) {
    ...
    onDataFrame(header, body, callback) {
    ...
    initHeaderBlock(header, frame, block, callback) {
    ...
    queueHeaderBlock(header, block, callback) {
        const self = this;
    ...
    onHeadersFrame(header, body, callback) {
        const self = this;
    ...
    onContinuationFrame(header, body, callback) {
        this.queueHeaderBlock(header, body, callback);
    }

    onRSTFrame(header, body, callback) {
        if (body.size !== 4) {
            return callback(this.error(constants.error.FRAME_SIZE_ERROR,
    ...
    onSettingsFrame(header, body, callback) {
        if (header.id !== 0) {
            return callback(this.error(constants.error.PROTOCOL_ERROR,
    ...
    onPushPromiseFrame(header, body, callback) {
        if (header.id === 0) {
            return callback(this.error(constants.error.PROTOCOL_ERROR,
    ...
    onPingFrame(header, body, callback) {
        if (body.size !== 8) {
            return callback(this.error(constants.error.FRAME_SIZE_ERROR,
                "PING length != 8"));
        }
    ...
    onGoawayFrame(header, body, callback) {
        if (!body.has(8)) {
            return callback(this.error(constants.error.FRAME_SIZE_ERROR,
                "GOAWAY length < 8"));
        }
    ...
    onPriorityFrame(header, body, callback) {
        if (body.size !== 5) {
            return callback(this.error(constants.error.FRAME_SIZE_ERROR,
                "PRIORITY length != 5"));
        }
    ...
    onWindowUpdateFrame(header, body, callback) {
        if (body.size !== 4) {
            return callback(this.error(constants.error.FRAME_SIZE_ERROR,
    ...
    onXForwardedFrame(header, body, callback) {
        callback(null, {
    ...
    static create(options) {
        return new Http2Parser(options);
    }
}
