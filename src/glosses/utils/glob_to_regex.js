...
const SEP = is.windows ? "\\\\+" : "\\/";
const SEP_ESC = is.windows ? "\\\\" : "/";
const GLOBSTAR = "((?:[^/]*(?:/|$))*)";
const WILDCARD = "([^/]*)";
 ...
 */
export default function (glob, { extended = false, globstar = false, strict = false, filepath = false, flags = "" } = {}) {
    let regex = "";
    let segment = "";
        ...
        if (c === "*") {
            ...
            if (!globstar) {
                ...
                const isGlobstar =
                    ...
                    (prevChar === "/" || is.undefined(prevChar)) && // from the start of the segment
                    (nextChar === "/" || is.undefined(nextChar)); // to the end of the segment
                if (isGlobstar) {
