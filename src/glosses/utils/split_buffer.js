...
const bufferIndexOf = function (buffer, search, offset, encoding) {
    if (!is.buffer(buffer)) {
        throw new adone.error.InvalidArgument("'buffer' is not a Buffer");
    }
    ...
    if (is.string(search)) {
        ...
        throw new adone.error.InvalidArgument("'search' is not a bufferable object");
    }
    ...
    if (is.undefined(offset) || (is.number(offset) && isNaN(offset))) {
        ...
        throw new adone.error.InvalidArgument("'offset' is not a number");
    }
