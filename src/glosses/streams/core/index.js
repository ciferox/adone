...
const _checkDestroyed = function () {
    if (this._destroyed) {
        throw new error.IllegalState("Stream was destroyed");
    }
};
...
export class Stream extends event.Emitter {
    ...
    map(callback) {
        if (!is.function(callback)) {
            throw new error.InvalidArgument("'callback' must be a function");
        }
    ...
    mapIf(condition, callback) {
        if (!is.function(condition)) {
            throw new error.InvalidArgument("'condition' must be a function");
        }
    ...
    unique(prop = null) {
        if (!is.null(prop) && !is.function(prop)) {
            throw new error.InvalidArgument("'prop' must be a function or null");
        }
        const cache = new Set();
        return this.throughSync(function (x) {
    ...
    stash(name, filter) {
        if (is.function(name)) {
            ...
            throw new error.InvalidArgument("'filter' must be a function");
        }
    ...
    unstash(name) {
        ...
        if (!name) {
            ...
            if (!stashes.named.has(name)) {
                throw new error.Unknown(`unknown stash stream '${name}'`);
            }
    ...
    static merge(streams, { end = true, sourceOptions } = {}) {
        ...
        streams = streams.filter((x, i) => {
            ...
            throw new error.InvalidArgument(`Invalid stream at ${i}`);
        });
