...
const getCodec = function (c) {
    ...
    const co = codecs[c];
    if (!co) {
        throw new Error(`Invalid codec ${c}`);
    }
    return co;
};

const SIOSource = function (sio, id, opt) {
    const q = Queue();
...
const SIOSink = function (sio, id, opt) {
    const q = Queue();
    ...
    sio.once(sioname("accept", id), () => {
        ...
        const loop = function () {
            q.get((_, val) => {
                ...
                if (err) {
                    return sio.emit(sioname("error", id), err);
                }
                if (data) {
                    sio.emit(sioname("queue", id), doCodec(codec, data));
                }
                loop();
            });
        };
