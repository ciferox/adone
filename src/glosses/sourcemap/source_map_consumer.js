...
export class BasicSourceMapConsumer extends SourceMapConsumer {
    constructor(aSourceMap, aSourceMapURL) {
        return super(INTERNAL).then((that) => {
            ...
            that._absoluteSources = ArraySet.fromArray(that._sources.toArray().map((s) => {
    ...
    eachMapping(aCallback, aContext, aOrder) {
        ...
        this._wasm.withMappingCallback(
            (mapping) => {
                if (!is.null(mapping.source)) {
                    mapping.source = this._absoluteSources.at(mapping.source);

                    if (!is.null(mapping.name)) {
                        mapping.name = this._names.at(mapping.name);
                    }
                }
                if (this._computedColumnSpans && is.null(mapping.lastGeneratedColumn)) {
    ...
    allGeneratedPositionsFor(aArgs) {
        ...
        this._wasm.withMappingCallback(
            (m) => {
                ...
                if (this._computedColumnSpans && is.null(lastColumn)) {
                    lastColumn = Infinity;
                }
                mappings.push({
...
export class IndexedSourceMapConsumer extends SourceMapConsumer {
    ...
    generatedPositionFor(aArgs) {
        ...
        if (generatedPosition && !is.null(generatedPosition.line)) {
            ...
            if (generatedPosition.line === 1) {
                ...
                if (is.number(generatedPosition.lastColumn)) {
        ...
        this._sections.forEach((section, index) => {
            ...
            section.consumer.eachMapping(function (mapping) {
                if (mapping.generatedLine === 1) {
                    ...
                    if (is.number(mapping.lastGeneratedColumn)) {
