...
class Terminfo {
    constructor() {
        this.terminal = process.env.COLORTERM || process.env.TERM;
        ...
        switch (this.terminal) {
            ...
            case "xterm-256color":
                ...
                for (const envVar in process.env) {
                    if (envVar.match(/KONSOLE/)) {
                        this.terminal = is256color ? "konsole-256color" : "konsole";
                        break;
                    }
                }
                if (Number.parseInt(process.env.VTE_VERSION) >= 3803) {
                    this.terminal = is256color ? "gnome-256color" : "gnome";
                    break;
                }
                break;
            case "linux":
            ...
            case "urxvt-ml":
                if (process.env.TERM === "rxvt") {
                    this.terminal = "rxvt-256color";
                } else {
                    this.terminal = is256color ? "rxvt-256color" : "rxvt";
                }
                break;
            case "xfce":
            case "xfce-terminal":
            ...
            case "eterm":
            case "Eterm":
                this.terminal = is256color ? "eterm-256color" : "eterm";
                break;
            case "atomic-terminal":
                this.terminal = "atomic-terminal";
                break;
            default:
                break;
        }
...
export default class Terminal extends adone.EventEmitter {
    ...
    _attr(param, val) {
        ...
        switch (param) {
            ...
            default: {
                ...
                if (m) {
                    if (m[2] === "fg") {
                        return this.styles.color.ansi16m.hex(m[1]);
                    }
                    if (m[2] === "bg") {
                        return this.styles.bgColor.ansi16m.hex(m[1]);
                    }
                }
                ...
                if (m) {
                    ...
                    if (m[1] === "=") {
                        if (m[3] === "fg") {
                            return this.styles.color.ansi256.rgb(color, color, color);
                        }
                        if (m[3] === "bg") {
                            return this.styles.bgColor.ansi256.rgb(color, color, color);
                        }
                    } else if (m[1] === "~") {
                        if (m[3] === "fg") {
                            return this.styles.color.ansi16m.rgb(color, color, color);
                        }
                        if (m[3] === "bg") {
                            return this.styles.bgColor.ansi16m.rgb(color, color, color);
                        }
                    } else {
                        return null;
                    }
    ...
    styleReset() {
        this.write(this.styles.reset.open);
        return this;
    }
...
const proto = defineProps(function chalk() { }, styles_);
