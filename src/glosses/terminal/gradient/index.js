...
const multilineGradient = function (str, gradient, opts) {
    const options = validateOptions(opts);
...
export const create = function (...args) {
    const grad = new __.Gradient(...args);
...
for (const a in aliases) { // eslint-disable-line guard-for-in
    module.exports[a] = (str) => create(aliases[a].colors)(str, aliases[a].options);
    module.exports[a].multiline = (str) => create(aliases[a].colors).multiline(str, aliases[a].options);
}
