...
const isOnePointZero = (n) => is.string(n) && n.includes(".") && parseFloat(n) === 1;

// Check to see if string passed in is a percentage
const isPercentage = (n) => is.string(n) && n.includes("%");
...
const convertDecimalToHex = (d) => Math.round(parseFloat(d) * 255).toString(16);
...
const isValidCSSUnit = (color) => Boolean(matchers.CSS_UNIT.exec(color));
...
const stringInputToObject = function (color) {
    ...
    if ((match = matchers.hex4.exec(color))) {
        return {
            ...
            a: convertHexToDecimal(String(match[4]) + match[4]),
            format: named ? "name" : "hex8"
        };
    }
...
const validateWCAG2Parms = function (parms) {
...
const rgbToHex = function (r, g, b, allow3Char) {
    const hex = [
...
const rgbaToHex = function (r, g, b, a, allow4Char) {
    const hex = [
        pad2(mathRound(r).toString(16)),
        pad2(mathRound(g).toString(16)),
        pad2(mathRound(b).toString(16)),
...
const rgbaToArgbHex = function (r, g, b, a) {
    const hex = [
        pad2(convertDecimalToHex(a)),
        pad2(mathRound(r).toString(16)),
        pad2(mathRound(g).toString(16)),
...
//
const inputToRGB = function (color) {
    let rgb = { r: 0, g: 0, b: 0 };
    let a = 1;
    let s = null;
    let v = null;
...
const desaturate = function (color, amount) {
...
const saturate = function (color, amount) {
    amount = (amount === 0) ? 0 : (amount || 10);
    const hsl = new Color(color).toHsl();
    hsl.s += amount / 100;
    hsl.s = clamp01(hsl.s);
...
const greyscale = (color) => new Color(color).desaturate(100);

const lighten = function (color, amount) {
    amount = (amount === 0) ? 0 : (amount || 10);
    const hsl = new Color(color).toHsl();
    hsl.l += amount / 100;
    hsl.l = clamp01(hsl.l);
...
const brighten = function (color, amount) {
    amount = (amount === 0) ? 0 : (amount || 10);
    const rgb = new Color(color).toRgb();
...
const spin = function (color, amount) {
    const hsl = new Color(color).toHsl();
...
const complement = function (color) {
    const hsl = new Color(color).toHsl();
    hsl.h = (hsl.h + 180) % 360;
    return new Color(hsl);
};

const triad = function (color) {
    const hsl = new Color(color).toHsl();
    const h = hsl.h;
    return [
        new Color(color),
...
const splitcomplement = function (color) {
    const hsl = new Color(color).toHsl();
    const h = hsl.h;
    return [
        new Color(color),
...
const analogous = function (color, results, slices) {
    results = results || 6;
    slices = slices || 30;

    const hsl = new Color(color).toHsl();
    const part = 360 / slices;
    const ret = [new Color(color)];
...
const monochromatic = function (color, results) {
    results = results || 6;
    const hsv = new Color(color).toHsv();
    const h = hsv.h;
    const s = hsv.s;
    let v = hsv.v;
    const ret = [];
...
export default class Color {
    ...
    static mix(color1, color2, amount) {
        ...
        const rgb1 = new Color(color1).toRgb();
        const rgb2 = new Color(color2).toRgb();
    ...
    //    Color.isReadable("#000", "#111",{level:"AA",size:"large"}) => false
    static isReadable(color1, color2, wcag2) {
        const readability = Color.readability(color1, color2);
        let out = false;
    ...
    // *Example*
    //    Color.mostReadable(Color.mostReadable("#123", ["#124", "#125"],{includeFallbackColors:false}).toHexString(); // "#112255"
    //    Color.mostReadable(Color.mostReadable("#123", ["#124", "#125"],{includeFallbackColors:true}).toHexString();  // "#ffffff"
    //    Color.mostReadable("#a8015a", ["#faf3f3"],{includeFallbackColors:true,level:"AAA",size:"large"}).toHexString(); // "#faf3f3"
    //    Color.mostReadable("#a8015a", ["#faf3f3"],{includeFallbackColors:true,level:"AAA",size:"small"}).toHexString(); // "#ffffff"
