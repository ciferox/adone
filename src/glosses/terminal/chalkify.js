...
const isBackground = (style) => Boolean(style.match(/^bg.+$/));

const isNormalColor = (style) => Boolean(style.match(`^(${normalColors.join("|")})$`));

const isModifier = (style) => Boolean(style.match(`^(${modifiers.join("|")})$`));

const isHexColor = (style) => Boolean(style.match(/^#[0-9A-Fa-f]{6}$/));

const isKeyword = (style) => Boolean(cssColors[style]);

const chalkify = (stylePipe) => {
    let paint = adone.terminal.chalk;
...
chalkify.cssColors = cssColors;
chalkify.modifiers = modifiers;
chalkify.normalColors = normalColors;
