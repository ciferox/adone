...
export default class Terminfo {
    ...
    _useXtermCap() {
        return this.injectTermcap(path.join(adone.realm.getRootRealm().env.SHARE_PATH, "term", "xterm.termcap"));
    }

    _useXtermInfo() {
        return this.injectTerminfo(path.join(adone.realm.getRootRealm().env.SHARE_PATH, "term", "xterm"));
    }

    _useInternalInfo(name) {
        name = path.basename(name);
        return this.injectTerminfo(path.join(adone.realm.getRootRealm().env.SHARE_PATH, "term", name));
    }

    _useInternalCap(name) {
        name = path.basename(name);
        return this.injectTermcap(path.join(adone.realm.getRootRealm().env.SHARE_PATH, "term", `${name}.termcap`));
    }
    ...
    _compile(info, key, str) {
        ...
        const header = "var v, dyn = {}, stat = {}, stack = [], out = [];";
        const footer = ";return out.join(\"\");";
        let code = header;
        let val = str;
        let buff = "";
        let cap;
    ...
    getAll() {
        ...
        list.forEach((letter) => {
            const terms = asort(adone.std.fs.readdirSync(path.resolve(dir, letter)));
