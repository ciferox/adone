...
const expandRange = (args, options) => {
    if (is.function(options.expandRange)) {
...
const parse = (input, options) => {
    ...
    const opts = { ...options };
    const max = is.number(opts.maxLength) ? Math.min(MAX_LENGTH, opts.maxLength) : MAX_LENGTH;
    ...
    const append = (token) => {
        state.output += !is.nil(token.output) ? token.output : token.value;
