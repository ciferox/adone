...
minimalMatch.isMatch = (str, pattern, options) => {
    if (!is.string(str)) {
        throw new error.InvalidArgumentException(`expected a string: "${std.util.inspect(str)}"`);
    }
...
minimalMatch.matchKeys = (obj, patterns, options) => {
    if (!is.plainObject(obj)) {
        throw new error.InvalidArgumentException("expected the first argument to be an object");
    }
...
minimalMatch.matcher = (pattern, options) => {
    ...
    if (!is.string(pattern)) {
        throw new error.InvalidArgumentException("expected pattern to be an array, string or regex");
    }
...
minimalMatch.makeRe = (pattern, options) => {
    ...
    if (pattern.length > minimalMatch.MAX_LENGTH) {
        throw new error.LimitExceededException(`expected pattern to be less than ${minimalMatch.MAX_LENGTH} characters`);
    }
