...
const { resolve, normalize: stdNormalize } = adone.path;
...
class DynamicValue {
    ...
    constructor(part) {
        if (part === "**") {
            ...
            if (part[0] === ".") {
                this.isDotted = true;
                this.re = adone.glob.match.makeRe(part.slice(1));
            } else {
                this.isDotted = false;
                this.re = adone.glob.match.makeRe(part, { dot: true });
            }
        }
    ...
    constructor(patterns, {
        ...
        if (ignore.length) {
            ...
            for (const pattern of ignore) {
                ...
                const re = adone.glob.match.makeRe(normalizedPattern, { dot: true });

                // in this case we can exclude entrire subtrees while walking
                if (normalizedPattern.endsWith("/**")) {
                    childPatterns.push(re); // for childs

                    const dirRe = adone.glob.match.makeRe(normalizedPattern.slice(0, -3), { dot: true }); // for child dir
