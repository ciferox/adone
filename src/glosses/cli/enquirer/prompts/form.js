...
class FormPrompt extends SelectPrompt {
    constructor(options) {
        ...
        this.align = [this.options.align, "right"].find((v) => !is.nil(v));
        this.emptyError = "";
        this.values = {};
    }
    ...
    dispatch(char) {
        return Boolean(char) && this.append(char);
    }
    ...
    deleteForward() {
        ...
        const str = `${input}`.slice(0, cursor) + `${input}`.slice(cursor + 1);
    ...
    next() {
        const ch = this.focused;
        if (!ch) {
            return this.alert(); 
        }
        const { initial, input } = ch;
    ...
    async choiceSeparator(choice, i) {
        const sep = await this.resolve(choice.separator, this.state, choice, i) || ":";
    ...
    async renderChoice(choice, i) {
        ...
        const indicator = style(await this.indicator(choice, i)) + (choice.pad || "");

        const indent = this.indent(choice);
        const line = () => [indent, indicator, msg + sep, input, help].filter(Boolean).join(" ");
        ...
        if (choice.format) {
            ...
            const options = { input, initial, pos: cursor, showCursor: focused, color };
