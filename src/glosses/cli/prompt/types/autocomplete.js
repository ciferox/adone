...
export default class AutocompletePrompt extends BasePrompt {
    ...
    render(error) {
        ...
        if (this.firstRender) {
            ...
            content += style.inactive(`(Use arrow keys or type to search${suggestText})`);
        }
        // Render choices or answer depending on the state
        if (this.status === "answered") {
            content += style.primary(this.shortAnswer || this.answerName || this.answer);
        } else if (this.searching) {
            content += style.focus(this.rl.line);
            bottomContent += `  ${style.inactive("Searching...")}`;
    ...
    search(searchTerm) {
        ...
        if (self.searchedOnce) {
            self.searching = true;
            self.currentChoices = new Choices(this.term, []);
        ...
        return thisPromise.then((choices) => {
            ...
            choices = new Choices(this.term, choices.filter((choice) => {
