...
export default class CheckboxPrompt extends BasePrompt {
    constructor(manager, questions, answers) {
        super(manager, questions, answers);
        ...
        this.opt.default = null;
        this.choices = new Choices(this.term, [], this.answers);
    ...
    async executeSource() {
        ...
        if (is.function(this.opt.search)) {
            sourcePromise = await this.opt.search(this.answers, this.rl.line);
        } else if (this.opt.search === true) {
            const input = this.rl.line || "";
            if (input === "") {
        ...
        this.choices = new Choices(this.term, choices, this.answers);
    ...
    render(error) {
        ...
        if (this.status === "answered") {
            message += style.primary(this.selection.join(", "));
        ...
        choices.forEach((choice, index) => {
            ...
            if (choice.disabled) {
                ...
                if (index - separatorOffset === pointer) {
                    output += style.focus(pointer + line);
                } else {
                    output += ` ${line}`;
                }
            }
