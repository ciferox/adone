const rx = require("rx");

const normalizeKeypressEvents = (value, key) => {
    return { value, key: key || {} };
};

module.exports = () => {
    const keypress = rx.Observable.fromEvent(adone.terminal.readline.input, "keypress", normalizeKeypressEvents).filter((e) => {
        // Ignore `enter` key. On the readline, we only care about the `line` event.
        return e.key.name !== "enter" && e.key.name !== "return";
    });

    return {
        line: rx.Observable.fromEvent(adone.terminal.readline, "line"),
        keypress,

        normalizedUpKey: keypress.filter((e) => {
            return e.key.name === "up" || e.key.name === "k" || (e.key.name === "p" && e.key.ctrl);
        }).share(),

        normalizedDownKey: keypress.filter((e) => {
            return e.key.name === "down" || e.key.name === "j" || (e.key.name === "n" && e.key.ctrl);
        }).share(),

        numberKey: keypress.filter((e) => {
            return e.value && "123456789".indexOf(e.value) >= 0;
        }).map((e) => {
            return Number(e.value);
        }).share(),

        spaceKey: keypress.filter((e) => {
            return e.key && e.key.name === "space";
        }).share(),

        aKey: keypress.filter((e) => {
            return e.key && e.key.name === "a";
        }).share(),

        iKey: keypress.filter((e) => {
            return e.key && e.key.name === "i";
        }).share()
    };
};
