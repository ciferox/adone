...
export const getParamBytesForAlg = (alg) => {
...
const signatureAsBuffer = (signature) => {
    if (is.buffer(signature)) {
        return signature;
    } else if (is.string(signature)) {
...
export const derToJose = (signature, alg) => {
...
export const joseToDer = (signature, alg) => {
    signature = signatureAsBuffer(signature);
    const paramBytes = getParamBytesForAlg(alg);
