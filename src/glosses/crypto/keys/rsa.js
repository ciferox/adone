...
const pkcs1ToJwk = function (bytes) {
    ...
    if (result.error) {
        throw new error.IllegalStateException(result.error);
    }
...
const unmarshalPrivateKey = function (key) {
    if (!key) {
        throw new error.NotValidException("Key is invalid");
    }
    return {
        privateKey: key,
        publicKey: {
...
const hashAndSign = function (key, msg) {
    try {
        ...
        throw new error.NotValidException(`Key or message is invalid!: ${err.message}`);
    }
};
