...
export const hash = (password, salt) => {
    if (!is.string(password) || password === "") {
        throw new error.NotValidException("'password' must be a non empty string");
    }
...
export const verify = (password, hashedPassword) => {
    return new Promise((resolve, reject) => {
        ...
        if (key.length !== 4 || !key[2] || !key[3]) {
            throw new error.NotValidException("Hash not formatted correctly");
        }
        if (key[0] !== "pbkdf2" || key[1] !== iterations.toString()) {
            throw new error.NotValidException("Wrong algorithm and/or iterations");
        }
...
export const generate = (length = 10, memorable = true, pattern = /\w/, prefix = "") => {
    ...
    if (!memorable) {
        ...
        if (!validChars.length) {
            throw new error.NotValidException(`Could not find characters that match the pattern: ${pattern.toString()}`);
        }
    }
