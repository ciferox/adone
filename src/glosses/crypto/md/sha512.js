...
const _update = (s, w, bytes) => {
    ...
    while (len >= 128) {
        ...
        for (i = 0; i < 16; ++i) {
            w[i][0] = bytes.readUInt32BE();
            w[i][1] = bytes.readUInt32BE();
        }
        for (; i < 80; ++i) {
     ...
     */
      ...
      md.messageLengthSize);
        ...
        for (let i = 0; i < hlen; ++i) {
            rval.writeUInt32BE(h[i][0]);
            if (i !== hlen - 1 || algorithm !== "SHA-512/224") {
                rval.writeUInt32BE(h[i][1]);
            }
        }
...
export const sha384 = {
    create: () => create("SHA-384")
};

// SHA-512/256
export const sha256 = {
    create: () => create("SHA-512/256")
};

export const sha224 = {
    create: () => create("SHA-512/224")
};
