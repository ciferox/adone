    ...
    const parseHunk = () => {
        ...
        if (options.strict) {
            if (addCount !== hunk.newLines) {
                throw new error.IllegalStateException(`Added line count did not match for hunk at line ${chunkHeaderIndex + 1}`);
            }
            if (removeCount !== hunk.oldLines) {
                throw new error.IllegalStateException(`Removed line count did not match for hunk at line ${chunkHeaderIndex + 1}`);
            }
        }
    ...
    const parseIndex = () => {
        ...
        while (i < diffstr.length) {
            ...
            if (/^(Index:|diff|\-\-\-|\+\+\+)\s/.test(line)) {
                ...
                throw new error.UnknownException(`Unknown line ${i + 1} ${JSON.stringify(line)}`);
            } else {
                i++;
            }
        }
