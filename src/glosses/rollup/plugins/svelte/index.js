...
module.exports = function svelte(options = {}) {
    ...
    fixed_options.sveltePath = options.sveltePath || adone.getPath("lib/glosses/web/browser");

    // handle CSS extraction
    if ("css" in options) {
        if (!is.function(options.css) && !is.boolean(options.css)) {
            throw new Error("options.css must be a boolean or a function");
        }
    }

    const css = options.css && is.function(options.css)
        ? options.css
        : null;
    ...
    return {
        ...
        generateBundle() {
            if (css) {
                ...
                for (const chunk of cssLookup.values()) {
                    if (!chunk.code) {
                        continue;
                    }
                    result += `${chunk.code}\n`;
