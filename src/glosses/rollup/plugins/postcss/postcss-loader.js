...
function ensurePostCSSOption(option) {
    return is.string(option) ? importCwd(option) : option;
}
...
export default {
    ...
    async process({ code, map }) {
                ...
                require("postcss-modules")({
                    ...
                    getJSON(filepath, json, outpath) {
                        ...
                        if (
                            ...
                            is.function(options.modules.getJSON)
                        ) {
        ...
        if (map && postcssOpts.map) {
            postcssOpts.map.prev = is.string(map) ? JSON.parse(map) : map;
        }
        ...
        if (options.namedExports) {
            ...
            const getClassName =
                is.function(options.namedExports) ?
            ...
            for (const name in json) {
                ...
                if (name !== newName && !is.function(options.namedExports)) {
                    this.warn(
