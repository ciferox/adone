...
const bitsreV3 = /\/win-(x86|ia32|x64)\//; // io.js v3.x.x shipped with "ia32" but should have been "x86"

const normalizePath = (p) => path.normalize(p).replace(/\\/g, "/");

const resolveLibUrl = function (name, defaultUrl, arch, versionMajor) {
...
const processRelease = function (argv, gyp, defaultVersion, defaultRelease) {
    ...
    // custom version
    const canGetHeaders = semver.satisfies(versionSemver, headersTarballRange);
    const tarballUrl = url.resolve(baseUrl, `${name}-v${version}${canGetHeaders ? "-headers" : ""}.tar.gz`);
