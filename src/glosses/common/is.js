...
const is = adone.o({
    ...
    class: (value) => (!is.nil(value) && value.length >= 0 && is.string(value.name) && is.propertyDefined(value, "prototype") && is.propertyDefined(value, "__proto__")),
    ...
    utf8: (bytes) => {
        ...
        while (i < bytes.length) {
            ...
            if ((bytes[i] === 0xE0 && (0xA0 <= bytes[i + 1] && bytes[i + 1] <= 0xBF) && (0x80 <= bytes[i + 2] && bytes[i + 2] <= 0xBF)) || // excluding overlongs
            ...
            if ((bytes[i] === 0xF0 && (0x90 <= bytes[i + 1] && bytes[i + 1] <= 0xBF) && (0x80 <= bytes[i + 2] && bytes[i + 2] <= 0xBF) && (0x80 <= bytes[i + 3] && bytes[i + 3] <= 0xBF)) || // planes 1-3
