...
const Patcher = function Patcher(lines) {
    ...
    const self = this; let replacements = [];
    ...
    self.get = function (loc) {
        ...
        let sliceFrom = loc.start; let toConcat = [];
...
Pp.tryToReprintComments = function (newNode, oldNode, print) {
    ...
    if (ableToReprintComments && reprints.length > 0) {
        reprints.forEach((reprint) => {
            let oldComment = reprint.oldPath.getValue();
...
export function getReprinter(path) {
    ...
    return function (print) {
        ...
        reprints.forEach((reprint) => {
            let newNode = reprint.newPath.getValue();
            let oldNode = reprint.oldPath.getValue();
            SourceLocation.assert(oldNode.loc, true);
            let needToPrintNewPathWithComments = !patcher.tryToReprintComments(newNode, oldNode, print);
            ...
            let nls = needsLeadingSpace(lines, oldNode.loc, newLines);
            let nts = needsTrailingSpace(lines, oldNode.loc, newLines);
...
function needsLeadingSpace(oldLines, oldLoc, newLines) {
    ...
    const charBeforeOldLoc = oldLines.prevPos(posBeforeOldLoc) &&
