...
import { fromString, concat } from "./lines";
...
function genericPrintNoParens(path, options, print) {
    ...
    switch (n.type) {
        ...
        case "ForStatement":
            ...
            var init = path.call(print, "init"), sep = init.length > 1 ? ";\n" : "; ", forParen = "for (", indented = fromString(sep).join([
                init,
                path.call(print, "test"),
                path.call(print, "update")
            ]).indentTail(forParen.length), head = concat([forParen, indented, ")"]), clause = adjustClause(path.call(print, "body"), options);
            parts.push(head);
            if (head.length > 1) {
                parts.push("\n");
        ...
        case "JSXFragment":
            ...
            if (n[openingPropName].selfClosing) {
                assert.ok(!n[closingPropName], "unexpected " + closingPropName + " element in self-closing " + n.type);
                return openingLines;
            }
        ...
        case "TSAsExpression": {
            ...
            if (withParens)
                parts.push("(");
            parts.push(path.call(print, "expression"), fromString(" as "), path.call(print, "typeAnnotation"));
            if (withParens)
                parts.push(")");
            return concat(parts);
        }
