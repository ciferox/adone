...
export function composeSourceMaps(formerMap, latterMap) {
    ...
    smcLatter.eachMapping((mapping) => {
        let origPos = smcFormer.originalPositionFor({
            line: mapping.originalLine,
            column: mapping.originalColumn
        });
        let sourceName = origPos.source;
        ...
        let sourceContent = smcFormer.sourceContentFor(sourceName);
...
function fixTemplateLiteral(node, lines) {
    ...
    node.expressions.forEach((expr, i) => {
        ...
        let dollarCurlyPos = lines.skipSpaces(expr.loc.start, true, false);
        if (lines.prevPos(dollarCurlyPos) &&
            lines.charAt(dollarCurlyPos) === "{" &&
            lines.prevPos(dollarCurlyPos) &&
            lines.charAt(dollarCurlyPos) === "$") {
            let quasiBefore = node.quasis[i];
        ...
        let rightCurlyPos = lines.skipSpaces(expr.loc.end, false, false);
        if (lines.charAt(rightCurlyPos) === "}") {
            assert.ok(lines.nextPos(rightCurlyPos));
            // Now rightCurlyPos is technically the position just after the }.
            let quasiAfter = node.quasis[i + 1];
...
export function isTrailingCommaEnabled(options, context) {
    ...
    if (typeof trailingComma === "object") {
        return Boolean(trailingComma[context]);
    }
    return Boolean(trailingComma);
}
