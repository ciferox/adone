...
const traversalMethods = {
     ...
     * @return {Collection}
     */
    getVariableDeclarators(nameGetter) {
...
const mutationMethods = {
     ...
     */
    replaceWith(nodes) {
        return this.forEach((path, i) => {
            const newNodes =
                (is.function(nodes)) ? nodes.call(path, path, i) : nodes;
    ...
    insertBefore(insert) {
        return this.forEach((path, i) => {
            const newNodes =
                (is.function(insert)) ? insert.call(path, path, i) : insert;
