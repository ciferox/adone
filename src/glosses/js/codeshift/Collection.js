...
class Collection {
    ...
    constructor(paths, parent, types) {
        assert.ok(is.array(paths), "Collection is passed an array");
        assert.ok(
        ...
        } else if (!types || is.array(types) && types.length === 0) {
     ...
     */
    isOfType(type) {
        return Boolean(type) && this._types.indexOf(type.toString()) > -1;
    }
}
...
function registerMethods(methods, type) {
    for (const methodName in methods) {
        ...
        if (!type) {
            ...
            astTypes.getSupertypeNames(type).forEach((name) => {
...
function hasConflictingRegistration(methodName, type) {
    ...
    return astTypes.getSupertypeNames(type.toString()).some((name) => {
        return Boolean(registrations[name]);
    });
}
