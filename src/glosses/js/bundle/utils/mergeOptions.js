...
const normalizeObjectOptionValue = function (optionValue) {
    if (!optionValue) {
        return optionValue;
    }
...
export default function mergeOptions({ config, command = {}, deprecateConfig, defaultOnWarnHandler = defaultOnWarn }) {
    ...
    if (is.function(configExternal)) {
        inputOptions.external = (id, ...rest) => configExternal(id, ...rest) || commandExternal.includes(id);
    } else {
    ...
    const outputOptionKeys = is.array(config.output)
        ? config.output.reduce((keys, o) => [...keys, ...Object.keys(o)], [])
