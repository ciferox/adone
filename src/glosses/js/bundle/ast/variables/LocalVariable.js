...
export default class LocalVariable extends Variable {
    ...
    forEachReturnExpressionWhenCalledAtPath(path, callOptions, callback, options) {
        ...
        this.boundExpressions.forEachAtPath(path, (relativePath, node) => !options.hasNodeBeenCalledAtPathWithOptions(relativePath, node, callOptions) &&
    ...
    hasEffectsWhenAccessedAtPath(path, options) {
        return (path.length > MAX_PATH_DEPTH ||
            this.boundExpressions.someAtPath(path, (relativePath, node) => relativePath.length > 0 &&
    ...
    hasEffectsWhenCalledAtPath(path, callOptions, options) {
        return (path.length > MAX_PATH_DEPTH ||
            ...
            this.boundExpressions.someAtPath(path, (relativePath, node) => !options.hasNodeBeenCalledAtPathWithOptions(relativePath, node, callOptions) &&
