...
const addDeprecations = function (deprecations, warn) {
...
const checkInputOptions = function (options) {
...
const checkOutputOptions = function (options) {
...
const getAndCheckOutputOptions = function (inputOptions, rawOutputOptions) {
    if (!rawOutputOptions) {
...
export const create = function (rawInputOptions) {
    try {
        if (!rawInputOptions) {
        ...
        if (!codeSplitting) {
            return graph.buildSingle(inputOptions.input).then((chunk) => {
                ...
                const normalizeOptions = function (rawOutputOptions) {
                    if (!rawOutputOptions) {
                        throw new Error("You must supply an options object");
                    }
                    // since deprecateOptions, adds the output properties
                ...
                const generate = function (rawOutputOptions) {
        ...
        return graph.buildChunks(inputOptions.input).then((bundle) => {
            ...
            const generate = function (rawOutputOptions) {
                ...
                if (is.string(outputOptions.file)) {
                    error({
                        code: "INVALID_OPTION",
                ...
                write(outputOptions) {
                    ...
                    return generate(outputOptions).then((result) => {
                        ...
                        return Promise.all(Object.keys(result).map((chunkName) => {
                            ...
                            return Promise.all(promises).then(() => {
                                return mapSequence(graph.plugins.filter((plugin) => plugin.onwrite), (plugin) => Promise.resolve(plugin.onwrite(assign({ bundle: chunk }, outputOptions), chunk)));
                            }).then(() => { });
                        }));
                    });
                }
