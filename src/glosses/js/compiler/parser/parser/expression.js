...
export default class ExpressionParser extends LValParser {
    ...
    //   +parseBlock: (allowDirectives) => N.BlockStatement;
    ...
    // +parseDecorators: (allowExport) => void;
    ...
    // +parseFunctionParams: (node, allowModifiers) => void;
    ...
    parseExprOp(
        ...
        if (!is.nil(prec) && (!noIn || !this.match(tt._in))) {
            if (prec > minPrec) {
    ...
    parseParenItem(
        node,
        startPos, // eslint-disable-line no-unused-vars
    ...
    isGetterOrSetterMethod(prop, isPattern) {
        return (
            ...
            (this.match(tt.string) || // get "string"() {}
                ...
                Boolean(this.state.type.keyword)) // get debugger() {}
        );
    }
    ...
    parseIdentifierName(pos, liberal) {
        if (!liberal) {
            this.checkReservedWord(
                this.state.value,
                this.state.start,
                Boolean(this.state.type.keyword),
                false,
            );
        }
