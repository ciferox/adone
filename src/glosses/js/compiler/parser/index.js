...
const getParserClass = (pluginsFromOptions: $ReadOnlyArray<string>): Class<Parser> => {
    if (
...
const getParser = (options: ?Options, input: string): Parser => {
    const cls =
        options && options.plugins ? getParserClass(options.plugins) : Parser;
    return new cls(options, input);
};

export const parse = (input: string, options?: Options): File => {
    return getParser(options, input).parse();
};

export const parseExpression = (input: string, options?: Options): Expression => {
