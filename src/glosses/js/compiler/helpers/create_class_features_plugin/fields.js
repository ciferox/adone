const {
    js: { compiler: { template, traverse, types: t, helper: { replaceSupers, memberExpressionToFunctions, optimiseCallExpression } } }
} = adone;
const { environmentVisitor } = replaceSupers;
...
const privateNameVisitor = {
    PrivateName(path) {
        ...
        if (!privateNamesMap.has(node.id.name)) {
            return;
        }
    ...
    Class(path) {
        ...
        for (const prop of body) {
            ...
            if (!privateNamesMap.has(prop.node.key.id.name)) {
                continue;
            }
...
const privateNameHandlerSpec = {
    ...
    call(member, args) {
        ...
        return optimiseCallExpression(this.get(member), this.receiver(member), args);
    }
};
