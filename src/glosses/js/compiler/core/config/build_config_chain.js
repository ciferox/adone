...
function flattenArgumentsOptionsParts(
    ...
    const plugins = is.array(opts.plugins) ? opts.plugins : null;
    const presets = is.array(opts.presets) ? opts.presets : null;
    const passPerPreset =
        is.boolean(opts.passPerPreset) ? opts.passPerPreset : false;
    ...
    if (env) {
        delete innerOpts.env;
    }
    if (plugins) {
        delete innerOpts.plugins;
    }
    if (presets) {
...
function flattenOptionsPartsLookup(
    ...
    const def = parts.filter((part) => is.null(part.activeEnv));
    const lookup = new Map();

    parts.forEach((part) => {
        if (!is.null(part.activeEnv)) {
            lookup.set(part.activeEnv, []);
        }
    });

    for(const [activeEnv, values] of lookup) {
        parts.forEach((part) => {
            if (is.null(part.activeEnv) || part.activeEnv === activeEnv) {
                values.push(part);
            }
        });
    }
...
function flattenOptionsParts(
    ...
    if (!is.nil(rawOpts.ignore) && !is.array(rawOpts.ignore)) {
        throw new Error(
            `.ignore should be an array, ${JSON.stringify(rawOpts.ignore)} given`,
        );
    }
    if (!is.nil(rawOpts.only) && !is.array(rawOpts.only)) {
        throw new Error(
    ...
    if (
        !is.nil(rawOpts.env) &&
        (typeof rawOpts.env !== "object" || is.array(rawOpts.env))
    ) {
    ...
    Object.keys(rawEnv).forEach((envKey) => {
        ...
        if (!is.undefined(envOpts) && !is.null(activeEnv) && activeEnv !== envKey) {
            throw new Error(`Unreachable .env[${envKey}] block detected`);
        }

        if (
            !is.nil(envOpts) &&
            (typeof envOpts !== "object" || is.array(envOpts))
        ) {
            throw new Error(".env[...] block must be an object, null, or undefined");
        }
