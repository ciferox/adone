 ...
 */
export const makeStrongCache = function <ArgT, ResultT> (handler: (ArgT, CacheConfigurator) => ResultT, autoPermacache?: boolean): ArgT => ResultT {
...
const makeCachedFunction = function <ArgT, ResultT, Cache: CacheMap<ArgT, ResultT>> (
    callCache: Cache,
    handler: (ArgT, CacheConfigurator) => ResultT,
    ...
    return function cachedFunction(arg) {
        ...
        if (autoPermacache && !result.configured) {
            cache.forever();
        }
...
const makeCacheConfig = function (): {
