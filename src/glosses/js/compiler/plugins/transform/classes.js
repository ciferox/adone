const { js: { compiler: { types: t, template, traverse: { visitors }, helpers: { ReplaceSupers, defineMap, functionName, optimiseCallExpression } } } } = adone;
...
class VanillaTransformer {
    ...
    wrapSuperCall(bareSuper, superRef, thisRef, body) {
        ...
        if (this.isLoose) {
            ...
            bareSuperNode = optimiseCallExpression(
                t.logicalExpression(
                    "||",
    ...
    pushConstructor(replaceSupers, method: { type: "ClassMethod" }, path) {
    ...
    return {
        visitor: {
            ...
            ClassExpression(path, state) {
                ...
                if (state.opts.loose) {
                    Constructor = LooseClassTransformer;
                }
