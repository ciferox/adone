const { js: { compiler: { template, helpers: { explodeClass } } } } = adone;
...
export default function ({ types: t }) {
    const cleanDecorators = (decorators) => decorators.reverse().map((dec) => dec.expression);

    const transformClass = (path, ref, state) => {
        const nodes = [];
