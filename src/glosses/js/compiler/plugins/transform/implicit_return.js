...
export default declare((api) => {
    ...
    const lastNotEmptyIndex = (nodes) => {
        for (let index = nodes.length - 1; index >= 0; index -= 1) {
            if (!t.isEmptyStatement(nodes[index])) { 
                return index;
            }
        }
    };
    ...
    return {
        visitor: {
            Function(path) {
                ...
                if (unreturnableStatements.has(lastNode.type)) { 
                    return;
                ...
                if (returnableStatements.has(lastNode.type)) {
                    ...
                    completionRecords.forEach((subPath) => {
                        if (!subPath.isExpressionStatement()) { 
                            return;
                        }
                        ...
                        if (isLoop) {
                            if (!returnUid) { 
                                returnUid = path.scope.generateDeclaredUidIdentifier("ret");
