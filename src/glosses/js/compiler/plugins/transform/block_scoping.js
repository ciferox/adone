...
const getTDZStatus = (refPath, bindingPath) => {
...
const isReference = (node, scope, state) => {
...
const ignoreBlock = (path) => t.isLoop(path.parent) || t.isCatchClause(path.parent);
...
const convertBlockScopedToVar = (path, node, parent, scope, moveBindingsToParent = false) => {
    if (!node) {
        node = path.node;
    }
...
const isVar = (node) => t.isVariableDeclaration(node, { kind: "var" }) && !isBlockScoped(node);
...
class BlockScoping {
    ...
    pushDeclar(node: { type: "VariableDeclaration" }) {
        const declars = [];
