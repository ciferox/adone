...
export const download = function (ver, platform, arch) {
    return new Promise((resolve, reject) => {
        ...
        const done = () => resolve({
            fileName,
            installPath: path.join(adone.realm.rootRealm.env.OPT_PATH, "/go-ipfs/")
        });
        ...
        const unpack = (stream) => {
            ...
            if (isWindows) {
                return stream.pipe(
                    unzip
                        .Extract({ path: adone.realm.rootRealm.env.OPT_PATH })
                        .on("close", done)
                );
            }
            ...
            return stream
                ...
                .pipe(
                    tarFS
                        .extract(adone.realm.rootRealm.env.OPT_PATH)
                        .on("finish", done)
                );
        };
