...
exports.kill = exports.getChildPids = exports.exists = exports.shellSync = exports.execSync = exports.shell = exports.execStderr = exports.execStdout = exports.exec = exports.stdio = exports.errnameFallback = exports.errname = void 0;
const {
  is,
  std
} = adone;
adone.asNamespace(exports); // The Node team wants to deprecate `process.bind(...)`.
...
const errname = code => uvErrname(uv, code); // Used for testing the fallback behavior


exports.errname = errname;
const errnameFallback = uvErrname;
exports.errnameFallback = errnameFallback;
...
const resolveCommand = (command, noExtension) => {
  ...
  try {
    resolved = !noExtension ? adone.fs.whichSync(command) : adone.fs.whichSync(command, {
      pathExt: std.path.delimiter + (process.env.PATHEXT || "")
    });
  } catch (e) {
    /* empty */
  }
...
const escapeArgument = (arg, quote) => {
  // Convert to string
  arg = String(arg); // If we are not going to quote the argument,
...
const readShebang = command => {
  let fd; // Check if it is in the cache first
  ...
  const shebang = adone.util.shebang.command(buffer.toString()); // Store the shebang in the cache

  shebangCache.set(command, shebang);
  return shebang;
};

const skipShellRegExp = /\.(?:com|exe)$/i; // Supported in Node >= 6 and >= 4.8

const supportsShellOption = parseInt(process.version.substr(1).split(".")[0], 10) >= 6 || parseInt(process.version.substr(1).split(".")[0], 10) === 4 && parseInt(process.version.substr(1).split(".")[1], 10) >= 8;
...
const handleArgs = (cmd, args, opts) => {
  ...
  if (opts.preferLocal) {
    opts.env = env(Object.assign({}, opts, {
      cwd: opts.localDir
    }));
  }
...
const getStream = (process, stream, encoding, maxBuffer) => {
  ...
  if (encoding) {
    ...
    ret = adone.stream.as.buffer(process[stream], {
      maxBuffer
    });
  }
...
const hookChildProcess = (cp, parsed) => {
  ...
  cp.emit = function (name, arg1) {
    let err; // If emitting "exit" event and exit code is 1, we need to check if
...
const exec = (cmd, args, opts) => {
  ...
  const handlePromise = () => adone.promise.finally(Promise.all([processDone, getStream(spawned, "stdout", encoding, maxBuffer), getStream(spawned, "stderr", encoding, maxBuffer)]).then(arr => {
    const result = arr[0];
    result.stdout = arr[1];
    result.stderr = arr[2];
...
const execStderr = (...args) => {
  // TODO: set `stdout: 'ignore'` when that option is implemented
  return exec.apply(null, args).then(x => x.stderr);
};

exports.execStderr = execStderr;

const shell = (cmd, opts) => handleShell(exec, cmd, opts);
...
const shellSync = (cmd, opts) => handleShell(execSync, cmd, opts);
...
const getChildPids = async pid => {
  ...
  for (const line of lines) {
    ...
    if (!headers) {
      headers = columns; // Rename Win32 header name, to as same as the linux, for compatible.
...
const kill = (input, {
  ...
  const errors = []; // Don't kill ourselves
