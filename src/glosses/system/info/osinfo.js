...
export const uuid = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_linux) {
            exec("( cat /var/lib/dbus/machine-id /etc/machine-id 2> /dev/null || hostname ) | head -n 1 || :", (error, stdout) => {
                if (!error) {
        ...
        if (_freebsd || _openbsd) {
            exec("kenv -q smbios.system.uuid", (error, stdout) => {
                if (!error) {
                    result.os = stdout.toString().split("\n")[0].trim().toLowerCase();
                }
                if (callback) {
        ...
        if (_windows) {
            exec('%windir%\\System32\\reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid', util.execOptsWin, (error, stdout) => {
                if (!error) {
                    parts = stdout.toString().split("\n\r")[0].split("REG_SZ");
...
// --------------------------
// OS Information
export const osInfo = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        const result = {
        ...
        if (_freebsd || _openbsd) {
            ...
            exec("sysctl kern.ostype kern.osrelease kern.osrevision kern.hostuuid", (error, stdout) => {
                if (!error) {
                    const lines = stdout.toString().split("\n");
        ...
        if (_windows) {
            ...
            result.release = result.kernel;
            try {
                util.execWin(`${util.getWmic()} os get /value`, util.execOptsWin, (error, stdout) => {
                    const lines = stdout.toString().split("\r\n");
...
export const versions = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        const result = {
            kernel: os.release(),
        ...
        try {
            ...
            exec("gulp --version", (error, stdout) => {
                if (!error) {
            ...
            exec("grunt --version", (error, stdout) => {
                if (!error) {
...
export const shell = (callback) => new Promise((resolve, reject) => {
    process.nextTick(() => {
        if (_windows) {
