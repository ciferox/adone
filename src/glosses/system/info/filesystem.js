...
export const fsSize = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        const data = [];
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} logicaldisk get Caption,FileSystem,FreeSpace,Size`, util.execOptsWin, (error, stdout) => {
...
const parseBytes = (s) => parseInt(s.substr(s.indexOf(" (") + 2, s.indexOf(" Bytes)") - 10));
...
const blkStdoutToObject = (stdout) => {
...
export const blockDevices = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        let data = [];
        if (_linux) {
            // see https://wiki.ubuntuusers.de/lsblk/
            // exec("lsblk -bo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,TRAN,SERIAL,LABEL,MODEL,OWNER,GROUP,MODE,ALIGNMENT,MIN-IO,OPT-IO,PHY-SEC,LOG-SEC,SCHED,RQ-SIZE,RA,WSAME", function (error, stdout) {
            exec("lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,TRAN,SERIAL,LABEL,MODEL,OWNER", (error, stdout) => {
                if (!error) {
                    ...
                    exec("lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,LABEL,MODEL,OWNER", (error, stdout) => {
                        if (!error) {
                            const lines = blkStdoutToObject(stdout).split("\n");
                            data = parseBlk(lines);
                        }
        ...
        if (_darwin) {
            exec("diskutil info -all", (error, stdout) => {
                if (!error) {
                    const lines = stdout.toString().split("\n");
        ...
        if (_windows) {
            ...
            try {
                util.execWin(`${util.getWmic()} logicaldisk get Caption,Description,DeviceID,DriveType,FileSystem,FreeSpace,Name,Size,VolumeName,VolumeSerialNumber /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
...
export const fsStats = (callback) => new Promise((resolve, reject) => {
    process.nextTick(() => {
        if (_windows) {
            ...
            if (_linux) {
                ...
                exec("lsblk | grep /", (error, stdout) => {
                    if (!error) {
                        ...
                        exec(`cat /proc/diskstats | egrep "${output}"`, (error, stdout) => {
                            if (!error) {
                                const lines = stdout.toString().split("\n");
                                lines.forEach((line) => {
                                    line = line.trim();
...
export const disksIO = (callback) => new Promise((resolve, reject) => {
    process.nextTick(() => {
        if (_windows) {
            const error = new Error(NOT_SUPPORTED);
            if (callback) {
        ...
        if ((_disk_io && !_disk_io.ms) || (_disk_io && _disk_io.ms && Date.now() - _disk_io.ms >= 500)) {
            if (_linux || _freebsd || _openbsd) {
                ...
                const cmd = 'for mount in `lsblk | grep " disk " | sed "s/[│└─├]//g" | awk \'{$1=$1};1\' | cut -d " " -f 1 | sort -u`; do cat /sys/block/$mount/stat | sed -r "s/ +/;/g" | sed -r "s/^;//"; done';
...
const getVendorFromModel = (model) => {
...
export const diskLayout = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        const result = [];
        let cmd = "";

        if (_linux) {
            exec("export LC_ALL=C; lsblk -ablJO; unset LC_ALL", (error, stdout) => {
                if (!error) {
                    try {
                        const out = stdout.toString().trim();
                        const outJSON = JSON.parse(out);
                        if (outJSON && outJSON.hasOwnProperty("blockdevices")) {
                            const devices = outJSON.blockdevices.filter((item) => {
                                return item.group === "disk" && item.size > 0 && !is.null(item.model);
                            });
                            devices.forEach((device) => {
                                let mediumType = "";
        ...
        if (_darwin) {
            exec("system_profiler SPSerialATADataType SPNVMeDataType", (error, stdout) => {
                if (!error) {
                    const parts = stdout.toString().split("NVMExpress:");

                    const devices = parts[0].split(" Physical Interconnect: ");
                    devices.shift();
                    devices.forEach((device) => {
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} diskdrive get /value`, { encoding: "utf8", windowsHide: true }, (error, stdout) => {
                    if (!error) {
                        const devices = stdout.toString().split(/\n\s*\n/);
                        devices.forEach((device) => {
                            const lines = device.split("\r\n");
