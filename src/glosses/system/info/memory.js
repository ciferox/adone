...
export const mem = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_linux) {
            exec("export LC_ALL=C; cat /proc/meminfo ; unset LC_ALL", (error, stdout) => {
                if (!error) {
        ...
        if (_freebsd || _openbsd) {
            exec('/sbin/sysctl -a | grep -E "hw.realmem|hw.physmem|vm.stats.vm.v_page_count|vm.stats.vm.v_wire_count|vm.stats.vm.v_active_count|vm.stats.vm.v_inactive_count|vm.stats.vm.v_cache_count|vm.stats.vm.v_free_count|vm.stats.vm.v_page_size"', (error, stdout) => {
                if (!error) {
                    const lines = stdout.toString().split("\n");
        ...
        if (_windows) {
            ...
            let swapused = 0;
            try {
                exec(`${util.getWmic()} pagefile get AllocatedBaseSize, CurrentUsage`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
...
export const memLayout = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_darwin) {
            exec("system_profiler SPMemoryDataType", (error, stdout) => {
                if (!error) {
                    const devices = stdout.toString().split("        BANK ");
                    devices.shift();
                    devices.forEach((device) => {
                        const lines = device.split("\n");
        ...
        if (_windows) {
            ...
            try {
                exec(`${util.getWmic()} memorychip get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        const devices = stdout.toString().split("BankL");
                        devices.shift();
                        devices.forEach((device) => {
                            const lines = device.split("\r\n");
                            result.push({
