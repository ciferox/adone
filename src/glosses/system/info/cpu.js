...
const cpuBrandManufacturer = (res) => {
...
const getCpu = () => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_darwin) {
            exec("sysctl machdep.cpu hw.cpufrequency_max hw.cpufrequency_min hw.packages hw.physicalcpu_max hw.ncpu", (error, stdout) => {
                // if (!error) {
        ...
        if (_linux) {
            ...
            exec("export LC_ALL=C; lscpu; unset LC_ALL", (error, stdout) => {
                ...
                exec('export LC_ALL=C; dmidecode –t 4 2>/dev/null | grep "Upgrade: Socket"; unset LC_ALL', (error2, stdout2) => {
                    ...
                    lines = [];
                    exec('cat /proc/cpuinfo | grep -E "physical id|core id"', (error2, stdout3) => {
        ...
        if (_freebsd || _openbsd) {
            ...
            exec("export LC_ALL=C; dmidecode -t 4 2>/dev/null; dmidecode -t 7 2>/dev/null; unset LC_ALL", (error, stdout) => {
                let cache = [];
                if (!error) {
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} cpu get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                    ...
                    exec(`${util.getWmic()} path Win32_CacheMemory get CacheType,InstalledSize,Purpose`, (error, stdout) => {
                        if (!error) {
...
export const cpuCurrentspeed = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
...
export const cpuTemperature = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        let result = {
            main: -1.0,
            cores: [],
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} /namespace:\\\\root\\wmi PATH MSAcpi_ThermalZoneTemperature get CurrentTemperature`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        let sum = 0;
                        const lines = stdout.split("\r\n").filter((line) => line.trim() !== "").filter((line, idx) => idx > 0);
                        lines.forEach((line) => {
...
// --------------------------
// CPU Flags
export const cpuFlags = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        let result = "";
        if (_windows) {
            try {
...
// --------------------------
// CPU Cache
export const cpuCache = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} cpu get l2cachesize, l3cachesize /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        const lines = stdout.split("\r\n");
                        result.l1d = 0;
                        result.l1i = 0;
...
const getLoad = () => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (now >= 200) {
            ...
            for (let i = 0; i < _corecount; i++) {
                ...
                totalIdle += cpu.idle;
                totalIrq += cpu.irq;
                const tmp_tick = (_cpus && _cpus[i] && _cpus[i].totalTick ? _cpus[i].totalTick : 0);
                const tmp_load = (_cpus && _cpus[i] && _cpus[i].totalLoad ? _cpus[i].totalLoad : 0);
                const tmp_user = (_cpus && _cpus[i] && _cpus[i].user ? _cpus[i].user : 0);
                const tmp_system = (_cpus && _cpus[i] && _cpus[i].sys ? _cpus[i].sys : 0);
                const tmp_nice = (_cpus && _cpus[i] && _cpus[i].nice ? _cpus[i].nice : 0);
                const tmp_idle = (_cpus && _cpus[i] && _cpus[i].idle ? _cpus[i].idle : 0);
                const tmp_irq = (_cpus && _cpus[i] && _cpus[i].irq ? _cpus[i].irq : 0);
                _cpus[i] = cpu;
...
export const currentLoad = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        getLoad().then((result) => {
            if (callback) {
                callback(result);
...
const getFullLoad = () => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        const result = (totalTicks - totalIdle) / totalTicks * 100.0;

        resolve(result);
    });
});

export const fullLoad = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
