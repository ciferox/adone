const {
    std: { fs, childProcess: { exec, execSync } }
} = adone;
...
export const system = () => new Promise((resolve) => {
    process.nextTick(() => {
        ...
        if (_linux || _freebsd || _openbsd) {
            ...
            echo -n "sys_vendor: "; cat /sys/devices/virtual/dmi/id/sys_vendor 2>/dev/null; echo;`;
                ...
                if (result.manufacturer === "" && result.model === "Computer" && result.version === "") { // still default values
                    exec('dmesg | grep -i virtual | grep -iE "vmware|qemu|kvm|xen"', (error, stdout) => {
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} csproduct get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        ...
                        result.uuid = util.getValue(lines, "uuid", "=");
                        exec(`${util.getWmic()} /namespace:\\\\root\\wmi path MS_SystemInformation get /value`, util.execOptsWin, (error, stdout) => {
                            if (!error) {
                                const lines = stdout.split("\r\n");
...
export const bios = () => new Promise((resolve) => {
    process.nextTick(() => {
        const result = {
            vendor: "",
            version: "",
        ...
        if (_windows) {
            ...
            try {
                exec(`${util.getWmic()} bios get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        const lines = stdout.toString().split("\r\n");
...
export const baseboard = () => new Promise((resolve) => {
    process.nextTick(() => {
        const result = {
            manufacturer: "",
            model: "",
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} baseboard get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        const lines = stdout.toString().split("\r\n");
...
export const chassis = (callback) => new Promise((resolve) => {
    process.nextTick(() => {
        const result = {
            manufacturer: "",
            model: "",
        ...
        if (_windows) {
            try {
                exec(`${util.getWmic()} path Win32_SystemEnclosure get /value`, util.execOptsWin, (error, stdout) => {
                    if (!error) {
                        const lines = stdout.toString().split("\r\n");
