...
export const graphics = (callback) => {
    const parseLinesDarwin = (lines) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].trim() !== "") {
                ...
                if (level === 4) { // display controller details level
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].trim() !== "") {
                if (lines[i][0] !== " " && lines[i][0] !== "\t") { // first line of new entry
                    const isExternal = (pciIDs.indexOf(lines[i].split(" ")[0]) >= 0);
                    let vgapos = lines[i].toLowerCase().indexOf(" vga ");
                    const _3dcontrollerpos = lines[i].toLowerCase().indexOf("3d controller");
                    if (vgapos !== -1 || _3dcontrollerpos !== -1) { // VGA
                        ...
                        const endpos = lines[i].search(/\[[0-9a-f]{4}:[0-9a-f]{4}]|$/);
                        const parts = lines[i].substr(vgapos, endpos - vgapos).split(":");
                        if (parts.length > 1) {
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (_darwin) {
                const cmd = "system_profiler SPDisplaysDataType";
            ...
            if (_linux) {
                ...
                if (util.isRaspberry() && util.isRaspbian()) {
                    const cmd = 'fbset -s | grep \'mode "\'; vcgencmd get_mem gpu; tvservice -s; tvservice -n;';
                    exec(cmd, (error, stdout) => {
                        const lines = stdout.toString().split("\n");
