    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (_linux) {
                ...
                if (battery_path) {
                    exec(`cat ${battery_path}uevent`, (error, stdout) => {
                        if (!error) {
            ...
            if (_darwin) {
                exec('ioreg -n AppleSmartBattery -r | egrep "CycleCount|IsCharging|MaxCapacity|CurrentCapacity|BatterySerialNumber|TimeRemaining"; pmset -g batt | grep %', (error, stdout) => {
                    if (stdout) {
