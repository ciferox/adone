...
function checkChunksIndex(_this, callback) {
    _this.chunks.listIndexes().toArray((error, indexes) => {
        ...
        if (hasChunksIndex) {
            ...
            const indexOptions = getWriteOptions(_this);
...
function checkDone(_this, callback) {
    if (_this.state.streamEnd &&
        ...
        _this.files.insert(filesDoc, getWriteOptions(_this), (error) => {
            if (error) {
...
function doWrite(_this, chunk, encoding, callback) {
    ...
    let spaceRemaining = _this.chunkSizeBytes - _this.pos;
    ...
    while (inputBufRemaining > 0) {
        ...
        if (spaceRemaining === 0) {
            ...
            const doc = createChunkDoc(_this.id, _this.n, _this.bufToStore);
            ...
            _this.chunks.insert(doc, getWriteOptions(_this), (error) => {
                if (error) {
...
function writeRemnant(_this, callback) {
    ...
    // to be.
    const remnant = new Buffer(_this.pos);
    _this.bufToStore.copy(remnant, 0, 0, _this.pos);
    _this.md5.update(remnant);
    const doc = createChunkDoc(_this.id, _this.n, remnant);
