...
WireProtocol.prototype.killCursor = function (bson, ns, cursorId, pool, callback) {
    ...
    // Command namespace
    const commandns = f("%s.$cmd", parts.shift());
    ...
    const killCursorCallback = function (err, result) {
        if (err) {
...
WireProtocol.prototype.getMore = function (bson, ns, cursorState, batchSize, raw, connection, options, callback) {
    ...
    const queryCallback = function (err, result) {
        ...
        const cursorId = typeof r.documents[0].cursor.id === "number"
...
WireProtocol.prototype.command = function (bson, ns, cmd, cursorState, topology, options) {
    ...
    const wireProtocolCommand = typeof options.wireProtocolCommand === "boolean" ? options.wireProtocolCommand : true;
...
const executeFindCommand = function (bson, ns, cmd, cursorState, topology, options) {
