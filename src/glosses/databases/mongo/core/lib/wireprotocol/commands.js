...
const Insert = function (requestId, ismaster, bson, ns, documents, options) {
...
Insert.prototype.toBin = function () {
    ...
    for (let i = 0; i < this.documents.length; i++) {
        const buffer = this.bson.serialize(this.documents[i], {
...
const Update = function (requestId, ismaster, bson, ns, update, options) {
    // Basic options needed to be passed in
    if (ns == null) throw new MongoError("ns must be specified for query");
...
const Remove = function (requestId, ismaster, bson, ns, remove, options) {
    // Basic options needed to be passed in
    if (ns == null) throw new MongoError("ns must be specified for query");
