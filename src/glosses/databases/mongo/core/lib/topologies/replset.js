...
const MongoCR = require("../auth/mongocr");
const X509 = require("../auth/x509");
const Plain = require("../auth/plain");
const GSSAPI = require("../auth/gssapi");
const SSPI = require("../auth/sspi");
const ScramSHA1 = require("../auth/scram");
 ...
 */
const ReplSet = function (seedlist, options) {
    const self = this;
    options = options || {};
    ...
    // Create a logger
    const logger = Logger("ReplSet", options);
    ...
    types.forEach((x) => {
        self.s.replicaSetState.on(x, (t, s) => {
            self.emit(x, t, s);
        });
    });
...
function connectNewServers(self, servers, callback) {
    ...
    // Handle events
    const _handleEvent = function (self, event) {
        return function (err) {
            const _self = this;
            count = count - 1;
            ...
            if (event == "connect" && !self.authenticating) {
                ...
                applyAuthenticationContexts(self, _self, () => {
                    ...
                    // Update the state
                    const result = self.s.replicaSetState.update(_self);
    ...
    function execute(_server, i) {
        setTimeout(() => {
            ...
            const server = new Server(assign({}, self.s.options, {
...
// Ping the server
const pingServer = function (self, server, cb) {
...
function topologyMonitor(self, options) {
    ...
    const servers = Object.keys(self.s.replicaSetState.set);
    ...
    const monitorServer = function (_host, _self, _options) {
        const intervalId = _process(() => {
            ...
            if (_server) {
                return pingServer(_self, _server, (err) => {
                    if (self.state == DESTROYED || self.state === UNREFERENCED) {
                        clearInterval(intervalId);
                        return;
                    }
    ...
    return function () {
        ...
        if (event == "connect") {
            ...
            applyAuthenticationContexts(self, _this, () => {
                ...
                // Update the state
                const result = self.s.replicaSetState.update(_this);
                if (result == true) {
                    // Primary lastIsMaster store it
...
ReplSet.prototype.auth = function (mechanism, db) {
    const allArgs = Array.prototype.slice.call(arguments, 0).slice(0);
    const self = this;
    ...
    function auth(server) {
        ...
        const argsWithoutCallback = [mechanism, db].concat(args.slice(0));
        // Create arguments
...
ReplSet.prototype.logout = function (dbName, callback) {
    ...
    const providers = Object.keys(this.authProviders);
    for (var i = 0; i < providers.length; i++) {
        this.authProviders[providers[i]].logout(dbName);
    }

    // Clear out any contexts associated with the db
    self.s.authenticationContexts = self.s.authenticationContexts.filter((context) => {
