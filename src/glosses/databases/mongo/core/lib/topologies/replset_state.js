...
const ReplSetState = function (options) {
    options = options || {};
    // Add event listener
...
ReplSetState.prototype.update = function (server) {
    ...
    if (ismaster) {
        ...
        let hosts = Array.isArray(ismaster.hosts) ? ismaster.hosts : [];
    ...
    if (!this.primary && ismaster.ismaster && ismaster.setName) {
        ...
        const normalizedHosts = ismaster.hosts.map(function (x) { return x.toLowerCase(); });
        const locationIndex = normalizedHosts.indexOf(serverName);
        ...
        // Get the electionIds
        const currentElectionId = self.set[self.primary.name.toLowerCase()].electionId;
        const currentSetVersion = self.set[self.primary.name.toLowerCase()].setVersion;
        const currentSetName = self.set[self.primary.name.toLowerCase()].setName;
...
const filterByTags = function (readPreference, servers) {
    if (readPreference.tags == null) return servers;
    const filteredServers = [];
    const tagsArray = Array.isArray(readPreference.tags) ? readPreference.tags : [readPreference.tags];

    // Iterate over the tags
    for (let j = 0; j < tagsArray.length; j++) {
        const tags = tagsArray[j];

        // Iterate over all the servers
        for (let i = 0; i < servers.length; i++) {
            const serverTag = servers[i].lastIsMaster().tags || {};
...
function pickNearestMaxStalenessSeconds(self, readPreference) {
    ...
    let servers = [];
    const heartbeatFrequencyMS = self.heartbeatFrequencyMS;
    ...
    for (let i = 0; i < self.secondaries.length; i++) {
