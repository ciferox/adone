 ...
 * */
const specAllowedKeys = ["type", "index", "unique", "sparse", "default", "get", "set", "enumerable"];

function isComplexSpec(spec) {
    return typeof (spec) === "object" && !util.isArray(spec)
        && _.keys(spec).every((x) => {
            return _.contains(specAllowedKeys, x);
        });
}
...
function construct(self, schema) {
    ...
    _.each(schema, (spec, key) => {
        if (spec.get || spec.set) {
            ...
            if (hasVal) {
                self[key] = val;
            } // call the setter the first time if we already had a set value
            return;
        }
        ...
        Object.defineProperty(self, key, {
            ...
            get: spec.type == "array" ?
                function () {
                    construct(val, spec.schema); return val;
                } :
                function () {
                    return val;
                },
...
function canCast(val, spec) {
    if (spec === true || spec == "mixed") {
        return true;
    }
    if (spec == "array" && util.isArray(val)) {
        return true;
    }
    if (typeof (val) === spec) {
        return true;
    }
    if (spec == "string" && val && val.toString) {
        return true;
    }
    //if (spec == "regexp" && typeof(val))
    if (spec == "number" && !isNaN(val)) {
        return true;
    }
    if (spec == "date" && !isNaN(new Date(val).getTime())) {
        return true;
    }
    if (spec == "boolean" && !isNaN(val)) {
        return true;
    }
    if (util.isRegExp(spec) && val.toString && spec.test(val)) {
        return true;
    }
    return false;
}
...
function getIndexes(obj, prefix) {
    ...
    _.each(obj, (spec, key) => {
        if (spec.schema) {
            return indexes = indexes.concat(getIndexes(spec.schema, `${prefix + key}.`));
        }
