...
Mongoose.prototype.createConnection = function (uri, options) {
    ...
    const rsOption = options && (options.replset || options.replSet);
...
Mongoose.prototype.disconnect = function (fn) {
    ...
    return new MongooseThenable(this, new Promise.ES6(((resolve, reject) => {
        let remaining = _this.connections.length;
        if (remaining <= 0) {
            fn && fn();
            resolve();
            return;
        }
        _this.connections.forEach((conn) => {
            conn.close((error) => {
                if (error) {
                    fn && fn(error);
                    reject(error);
...
function MongooseThenable(mongoose, promise) {
    const _this = this;
    for (const key in mongoose) {
        if (is.function(mongoose[key]) && mongoose[key].$hasSideEffects) {
            (function (key) {
                _this[key] = function () {
