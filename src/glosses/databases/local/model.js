...
const lastStepModifierFunctions = {
    ...
    $push: (obj, field, value) => {
        ...
        if (!is.array(obj[field])) {
            throw new error.IllegalStateException("Can't $push an element on non-array values");
        }
        ...
        if (!is.null(value) && valueIsObject && value.$each) {
            ...
            if (valueKeys.length >= 3 || (valueKeys.length === 2 && is.undefined(value.$slice))) {
                throw new error.IllegalStateException("Can only use $slice in cunjunction with $each when $push to array");
            }
            if (!is.array(value.$each)) {
                throw new error.IllegalStateException("$each requires an array value");
            }
    ...
    $addToSet: (obj, field, value) => {
        ...
        if (!is.array(obj[field])) {
            throw new error.IllegalStateException("Can't $addToSet an element on non-array values");
        }
        ...
        if (!is.null(value) && is.object(value) && value.$each) {
            if (util.keys(value).length > 1) {
                throw new error.IllegalStateException("Can't use another field in conjunction with $each");
            }
            if (!is.array(value.$each)) {
                throw new error.IllegalStateException("$each requires an array value");
            }
    ...
    $pop: (obj, field, value) => {
        if (!is.array(obj[field])) {
            throw new error.IllegalStateException("Can't $pop an element from non-array values");
        }
        if (!is.number(value)) {
            throw new error.IllegalStateException(`${value} isn't an integer, can't use it with $pop`);
        }
        if (value === 0) {
            return;
        }
    ...
    $pull: (obj, field, value) => {
        if (!is.array(obj[field])) {
            throw new error.IllegalStateException("Can't $pull an element from non-array values");
        }
    ...
    $inc: (obj, field, value) => {
        if (!is.number(value)) {
            throw new error.IllegalStateException(`${value} must be a number`);
        }
        ...
        if (!is.number(obj[field])) {
            if (!is.propertyOwned(obj, field)) {
                ...
                throw new error.IllegalStateException("Don't use the $inc modifier on non-number fields");
            }
        } else {
            obj[field] += value;
        }
...
const comparisonFunctions = {
    ...
    $in: (a, b) => {
        if (!is.array(b)) {
            throw new error.IllegalStateException("$in operator called with a non-array");
        }
    ...
    $nin: (a, b) => {
        if (!is.array(b)) {
            throw new error.IllegalStateException("$nin operator called with a non-array");
        }
    ...
    $regex: (a, b) => {
        if (!is.regexp(b)) {
            throw new error.IllegalStateException("$regex operator called with non regular expression");
        }
    ...
    $size: (obj, value) => {
        ...
        if (!is.integer(value)) {
            throw new error.IllegalStateException("$size operator called without an integer");
        }
...
const logicalOperators = {
    $or: (obj, query) => {
        if (!is.array(query)) {
            throw new error.IllegalStateException("$or operator used without an array");
        }
    ...
    $and: (obj, query) => {
        if (!is.array(query)) {
            throw new error.IllegalStateException("$and operator used without an array");
        }
    ...
    $where: (obj, fn) => {
        if (!is.function(fn)) {
            throw new error.IllegalStateException("$where operator used without a function");
        }

        const result = fn.call(obj);
        if (!is.boolean(result)) {
            throw new error.IllegalStateException("$where function must return boolean");
        }
...
const matchQueryPart = (obj, queryKey, queryValue, treatObjAsValue) => {
    ...
    if (!is.null(queryValue) && is.object(queryValue) && !is.regexp(queryValue) && !is.array(queryValue)) {
        ...
        if (dollarFirstChars.length !== 0 && dollarFirstChars.length !== firstChars.length) {
            throw new error.IllegalStateException("You cannot mix operators and normal fields");
        }
        ...
        if (dollarFirstChars.length > 0) {
            for (let i = 0; i < keys.length; ++i) {
                if (!comparisonFunctions[keys[i]]) {
                    throw new error.IllegalStateException(`Unknown comparison function ${keys[i]}`);
                }
...
const checkKey = (k, v) => {
    ...
    if (k[0] === "$" && !(k === "$$date" && is.number(v)) && !(k === "$$deleted" && v === true) && k !== "$$indexCreated" && k !== "$$indexRemoved") {
        throw new error.IllegalStateException("Field names cannot begin with the $ character");
    }

    if (k.indexOf(".") !== -1) {
        throw new error.IllegalStateException("Field names cannot contain a .");
    }
};
...
export default class Model {
    ...
    static modify(obj, updateQuery) {
        ...
        if (keys.indexOf("_id") !== -1 && updateQuery._id !== obj._id) {
            throw new error.IllegalStateException("You cannot change a document's _id");
        }

        if (dollarFirstChars.length !== 0 && dollarFirstChars.length !== firstChars.length) {
            throw new error.IllegalStateException("You cannot mix modifiers and normal fields");
        }
        ...
        if (dollarFirstChars.length === 0) {
            ...
            for (let i = 0; i < keys.length; ++i) {
                ...
                if (!modifierFunctions[modifier]) {
                    throw new error.IllegalStateException(`Unknown modifier ${modifier}`);
                }

                if (!is.object(updateQuery[modifier])) {
                    throw new error.IllegalStateException(`Modifier ${modifier}'s argument must be an object`);
                }
        ...
        if (obj._id !== newDoc._id) {
            throw new error.IllegalStateException("You can't change a document's _id");
        }
        return newDoc;
    }
        ...
        for (let i = 0; i < queryKeys.length; ++i) {
            ...
            if (queryKey[0] === "$") {
                if (!logicalOperators[queryKey]) {
                    throw new error.UnknownException(`Unknown logical operator ${queryKey}`);
                }
