const {
    database: { redis },
    util, collection, is, x, o, promise, EventEmitter, noop, lazify
} = adone;
...
export const cluster = lazify({
    Cluster: "./cluster",
    ConnectionPool: "./cluster/connection_pool",
    DelayQueue: "./cluster/delay_queue"
}, null, require);
...
export class Redis extends redis.Commander.mixin(EventEmitter) {
    constructor(a, b, c) {
        super();
        ...
        if (this.options.sentinels) {
            this.connector = new redis.SentinelConnector(this.options);
        } else {
            this.connector = new redis.Connector(this.options);
        }
    ...
    parseOptions(...args) {
        ...
        for (const arg of args) {
            ...
            if (is.object(arg)) {
                ...
                this.options = o(redis.util.parseURL(arg), this.options);
    ...
    connect(callback) {
        return promise.nodeify(new Promise((resolve, reject) => {
            ...
            this.connector.connect((err, stream) => {
                ...
                stream.once(CONNECT_EVENT, lazy.eventHandler.connectHandler(this));
                stream.once("error", lazy.eventHandler.errorHandler(this));
                stream.once("close", lazy.eventHandler.closeHandler(this));
                stream.on("data", lazy.eventHandler.dataHandler(this));
                ...
                if (this.options.connectTimeout) {
                    stream.setTimeout(this.options.connectTimeout, () => {
                        ...
                        err.code = "ETIMEDOUT";
                        err.syscall = "connect";
                        lazy.eventHandler.errorHandler(this)(err);
                    });
                ...
                const connectionCloseHandler = () => {
                    ...
                    reject(new x.Exception(redis.util.CONNECTION_CLOSED_ERROR_MSG));
                };
    ...
    disconnect(reconnect) {
        ...
        if (this.status === "wait") {
            lazy.eventHandler.closeHandler(this)();
        } else {
    ...
    silentEmit(eventName, ...args) {
        ...
        if (eventName === "error") {
            ...
            if (this.manuallyClosing) {
                ...
                if (is.error(error) &&
                    (error.message === redis.util.CONNECTION_CLOSED_ERROR_MSG ||
    ...
    sendCommand(command, stream) {
        ...
        if (this.condition.subscriber && !redis.Command.checkFlag("VALID_IN_SUBSCRIBER_MODE", command.name)) {
        ...
        let writable = (this.status === "ready") ||
                        (!stream &&
                         this.status === "connect" &&
                         redis.commands.hasFlag(command.name, "loading"));
        if (!this.stream) {
            writable = false;
        ...
        if (writable) {
            ...
            if (redis.Command.checkFlag("WILL_DISCONNECT", command.name)) {
                this.manuallyClosing = true;
            }
        ...
        if (command.name === "select" && redis.util.isInt(command.args[0])) {
...
for (const command of ["scan", "sscan", "hscan", "zscan", "scanBuffer", "sscanBuffer", "hscanBuffer", "zscanBuffer"]) {
    Redis.prototype[`${command}Stream`] = function (key, options) {
        ...
        return new redis.ScanStream(o(options, {
            objectMode: true,
            key,
            redis: this,
            command
...
lazify({
    initParser: () => redis.parser.initParser,
    returnError: () => redis.parser.returnError,
    returnReply: () => redis.parser.returnReply
}, Redis.prototype);
