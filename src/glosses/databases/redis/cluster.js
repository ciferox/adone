...
export default class Cluster extends __.Commander.mixin(event.Emitter) {
    constructor(startupNodes, options) {
        ...
        if (
            ...
            throw new error.Exception(`Invalid option scaleReads ${this.options.scaleReads}. Expected "all", "master", "slave" or a custom function`);
        }
    ...
    connect() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const closeListener = () => {
                ...
                reject(new error.Exception("None of startup nodes is available"));
            };
        ...
        try {
            ...
            for (const key of keys) {
                try {
                    ...
                    if (this.status === "END") {
                        throw new error.Exception("Cluster is disconnected.");
                    }
            ...
            const error = new error.Exception("Failed to refresh slots cache.");
    ...
    sendCommand(command, stream, node) {
        ...
        const tryConnection = (random, asking) => {
            if (this.status === "end") {
                command.reject(new error.Exception("Cluster is ended."));
                return;
            }
            let instance;
            ...
            if (instance) {
                ...
                command.reject(new error.Exception("Cluster isn't ready and enableOfflineQueue options is false"));
            }
        };
    ...
    handleError(error, ttl, handlers) {
        ...
        if (ttl.value <= 0) {
            handlers.maxRedirections(new error.Exception(`Too many Cluster redirections. Last error: ${error}`));
            return;
        }
    ...
    async getInfoFromNode(instance) {
        if (!instance) {
            throw new error.Exception("Node is disconnected");
        }
        let result;
        try {
