...
export default class AbstractQuery {
    ...
    static formatBindParameters(sql, values, dialect, replacementFunc, options) {
        ...
        options = options || {};
        if (!is.function(replacementFunc)) {
        ...
        if (!replacementFunc) {
            ...
            if (options.skipValueReplace) {
                ...
                replacementFunc = (match, key, values, timeZone, dialect, options) => {
                    if (!is.undefined(origReplacementFunc(match, key, values, timeZone, dialect, options))) {
                        return match;
                    }
                    return undefined;
                };
        ...
        if (this.options.raw) {
            result = results.map((result) => {
                ...
                if (this.options.nest) {
                    o = _.transform(o, (result, value, key) => {
    ...
    static _groupJoinData(rows, includeOptions, options) {
        ...
        for (rowsI = 0; rowsI < rowsLength; rowsI++) {
            ...
            for (keyI = 0; keyI < keyLength; keyI++) {
                ...
                if (rowsI === 0 && is.undefined(includeMap[key])) {
                    if (!$keyPrefix.length) {
                ...
                // End of key set
                if (!is.undefined($prevKeyPrefix) && $prevKeyPrefix !== $keyPrefix) {
                    if (checkExisting) {
