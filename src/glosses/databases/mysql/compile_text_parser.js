const { is, database: { mysql: { c, helper: { srcEscape } } }, std: { vm } } = adone;
...
export default function compile(fields, options, config) {
    ...
    if (options.nestTables === true) {
        ...
        for (let i = 0; i < resultTablesArray.length; i++) {
            func += `this[${srcEscape(resultTablesArray[i])}] = {};`;
        }
    }

    for (let i = 0; i < fields.length; i++) {
        const fieldName = srcEscape(fields[i].name);
        let lvalue = "";
        ...
        if (is.function(options.typeCast)) {
            func += `${lvalue} = options.typeCast(wrap(fields[${i}], ${srcEscape(typeNames[fields[i].columnType])}, packet, ${encodingExpr}), function() { return ${readCode};});`;
