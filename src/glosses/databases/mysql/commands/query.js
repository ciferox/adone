const {
    fs,
    std: { stream: { Readable } },
    database: { mysql: { compileTextParser, packet, c, command: { Command } } },
    noop
} = adone;

const EmptyPacket = new packet.Packet(0, Buffer.allocUnsafe(4), 0, 4);
...
export default class Query extends Command {
    ...
    readField(p, connection) {
        ...
        if (this._receivedFieldsCount === this._fieldCount) {
            ...
            if (!this._rowParser) {
                this._rowParser = compileTextParser(fields, this.options, connection.config);
