const { x, database: { mysql: { c, command: { Command }, packet } } } = adone;
...
export default class ServerHandshake extends Command {
    ...
    start(_, connection) {
        const serverHelloPacket = new packet.Handshake(this.args);
    ...
    readClientReply(p, connection) {
        // check auth here
        const clientHelloReply = packet.HandshakeResponse.fromPacket(p);
        ...
        switch (commandCode) {
            ...
            case c.command.FIELD_LIST: {
                if (connection.listeners("field_list").length) {
                    ...
                    connection.writeError({
                        code: c.error.ER_WARN_DEPRECATED_SYNTAX,
