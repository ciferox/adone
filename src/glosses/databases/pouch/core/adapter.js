...
function cleanDocs(docs) {
    for (let i = 0; i < docs.length; i++) {
        ...
        if (doc._deleted) {
            ...
            let atts = Object.keys(doc._attachments);
...
function compareByIdThenRev(a, b) {
    let idCompare = compare(a._id, b._id);
    if (idCompare !== 0) {
        return idCompare;
    }
    let aStart = a._revisions ? a._revisions.start : 0;
    let bStart = b._revisions ? b._revisions.start : 0;
...
function allDocsKeysQuery(api, opts, callback) {
    ...
    return Promise.all(keys.map((key) => {
        let subOpts = Object.assign({ key, deleted: "ok" }, opts);
...
function doNextCompaction(self) {
    let task = self._compactionQueue[0];
    let opts = task.opts;
...
AbstractPouchDB.prototype.remove =
    adapterFun("remove", function (docOrId, optsOrRev, opts, callback) {
        ...
        let newDoc = { _id: doc._id, _rev: (doc._rev || opts.rev) };
...
AbstractPouchDB.prototype.revsDiff =
    adapterFun("revsDiff", function (req, opts, callback) {
        ...
        function processDoc(id, rev_tree) {
            ...
            let missingForId = req[id].slice(0);
            traverseRevTree(rev_tree, (isLeaf, pos, revHash, ctx,
                opts) => {
                let rev = `${pos}-${revHash}`;
                let idx = missingForId.indexOf(rev);
                if (idx === -1) {
                    return;
                }
...
AbstractPouchDB.prototype.compactDocument =
    adapterFun("compactDocument", function (docId, maxHeight, callback) {
        ...
        this._getRevisionTree(docId, (err, revTree) => {
            ...
            let height = computeHeight(revTree);
            let candidates = [];
            let revs = [];
...
AbstractPouchDB.prototype.get = adapterFun("get", function (id, opts, cb) {
    ...
    return this._get(id, opts, (err, result) => {
        ...
        if (opts.conflicts) {
            let conflicts = collectConflicts(metadata);
            if (conflicts.length) {
        ...
        if (opts.revs || opts.revs_info) {
            let splittedRev = doc._rev.split("-");
            let revNo = parseInt(splittedRev[0], 10);
            let revHash = splittedRev[1];

            let paths = rootToLeaf(metadata.rev_tree);
            let path = null;

            for (let i = 0; i < paths.length; i++) {
                let currentPath = paths[i];
                let hashIndex = currentPath.ids.map((x) => {
                    return x.id;
                })
                    .indexOf(revHash);
                let hashFoundAtRevPos = hashIndex === (revNo - 1);
            ...
            let indexOfRev = path.ids.map((x) => {
                return x.id;
            })
                .indexOf(doc._rev.split("-")[1]) + 1;
            let howMany = path.ids.length - indexOfRev;
...
AbstractPouchDB.prototype.info = adapterFun("info", function (callback) {
    ...
    this._info((err, info) => {
        ...
        info.auto_compaction = Boolean(self.auto_compaction && !isRemote(self));
...
AbstractPouchDB.prototype.registerDependentDatabase =
    adapterFun("registerDependentDatabase", function (dependentDb,
        callback) {
        let depDB = new this.constructor(dependentDb, this.__opts);
...
AbstractPouchDB.prototype.destroy =
    adapterFun("destroy", function (opts, callback) {
        ...
        let self = this;
        let usePrefix = "use_prefix" in self ? self.use_prefix : true;

        function destroyDb() {
            // call destroy method of the particular adaptor
            self._destroy(opts, (err, resp) => {
                if (err) {
                    return callback(err);
                }
        ...
        self.get("_local/_pouch_dependentDbs", (err, localDoc) => {
            if (err) {
                /* istanbul ignore if */
                if (err.status !== 404) {
                    return callback(err);
            ...
            let dependentDbs = localDoc.dependentDbs;
            let PouchDB = self.constructor;
            let deletedMap = Object.keys(dependentDbs).map((name) => {
                // use_prefix is only false in the browser
                /* istanbul ignore next */
                let trueName = usePrefix ?
