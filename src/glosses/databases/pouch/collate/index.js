...
function collate(a, b) {
    ...
    let ai = collationIndex(a);
    let bi = collationIndex(b);
    if ((ai - bi) !== 0) {
        return ai - bi;
    }
    switch (typeof a) {
...
function parseNumber(str, i) {
    ...
    if (zero) {
        ...
        i++;
        let numAsString = "";
        let magAsString = str.substring(i, i + MAGNITUDE_DIGITS);
...
function pop(stack, metaStack) {
    ...
    if (metaStack.length) {
        ...
        let element = lastMetaElement.element;
        let lastElementIndex = lastMetaElement.index;
...
function numToIndexableString(num) {
    ...
    let magString = padLeft((magForComparison).toString(), "0", MAGNITUDE_DIGITS);
