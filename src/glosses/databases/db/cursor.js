...
Cursor.prototype.exec = function (callback) {
    let res = [], resIds = {}, resById = {}, self = this,
        err = null;
    ...
    let reducer;
    if (self._reduce) {
        reducer = [].concat(self._reduce);
    }
    if (reducer && reducer[1]) {
        reducer[1] = document.deepCopy(self._reduce[1]);
    ...
    const stream = Cursor.getMatchesStream(this.db, this.query, sort, this._prefetched);
    stream.on("error", (e) => {
        callback(e);
    });
    stream.removeListener("ids", stream.trigger);
    stream.on("ids", (ids) => {
        let indexed = ids._indexed, sorted = ids._sorted; // for some reason we cannot access those in on 'ready' - maybe properties get erased from arrays?
        ...
        if (earlyMapReduce) {
            res = reducer[1] || undefined;
        }

        // Error catcher
        const catcher = function (e) {
            stream.close(); err = e; res = undefined;
        };

        stream.on("data", (d) => {
            let v = d.val();

            if (!indexed && !document.match(v, self.query)) {
                return;
            ...
            try {
                if (self._filter && !self._filter(v)) {
                    return;
                } // Apply filter
            } catch (e) {
                catcher(e);
            ...
            if (self._live) {
                resById[v._id] = v; resIds[v._id] = true;
            ...
            if (res) {
                res[d.idx] = v;
            } // res might have been set to undefined because of an error
        ...
        stream.on("ready", () => {
            ...
            try {
                ...
                if (!earlySort) {
                    res = res.sort(sorter || Cursor.getSorter(sort));
                }
                if (!earlyLimit) {
                ...
                if (reducer) {
                    res = res.reduce.apply(res, reducer);
                }
            } catch (e) {
                err = e; res = undefined;
            }
    ...
    function ready() {
        ...
        if (res && self._aggregate) {
            res = self._aggregate(res);
        }
...
Cursor.prototype.live = function (query) {
    ...
    // Refresh live query
    const refresh = _.debounce((callback) => {
        self.exec((err, res) => {
            if (err) {
                console.error(err);
    ...
    function updated(docs) {
        ...
        docs.forEach((doc) => { // Avoid using .some since it would stop iterating after first match and we need to set _prefetched
            const interested = self._count || self._ids[doc._id] || document.match(doc, self.query); // _count queries never set _ids
            if (interested) {
                self._prefetched[doc._id] = doc;
            }
...
Cursor.getMatchesStream = function (db, query, sort, prefetched) {
    ...
    setTimeout((cb) => {
        ...
        if (db._pipe.queue.length == 0 && _.some(db.indexes, (idx) => {
            return !idx.ready;
        })) {
            db._pipe.push((cb) => {
                setTimeout(cb, db.options.indexBuildingDebounce || INDEX_BUILDING_DEBOUNCE);
            }, _.noop);
        }
    ...
    stream.on("ids", stream.trigger = function (ids) {
        ...
        ids.forEach((id, i) => {
            db._retrQueue.push((cb) => {
                Cursor.retriever({ stream, id, idx: i, db, prefetched: prefetched && prefetched[id] }, () => {
                    if (--stream._waiting == 0) {
                        stream.emit("ready");
                    }
                    cb();
                });
...
Cursor.retriever = function (task, cb) {
    let locked = task.db._retrQueue._locked,
        locks = task.db._retrQueue._locks;

    if (task.stream._closed) {
        return cb();
    }
...
Cursor.getIdsForQuery = function (db, query, sort) {
    ...
    sorted = firstKey ? (Boolean(db.indexes[firstKey]) && _.keys(sort).length == 1) : true;
    ...
    function match(key, val) {
        ...
        if (index && (indexed || !db.options.autoIndexing)) {
            ...
            } // Useful when we invoke match() in sort loop
            if (document.isPrimitiveType(val)) {
                return push(index.getMatching(val));
            }
            if (typeof (val) === "object" && !_.keys(val).some((k) => {
            ...
            if (val && val.hasOwnProperty("$lt") || val.hasOwnProperty("$lte") || val.hasOwnProperty("$gt") || val.hasOwnProperty("$gte")) {
            ...
            if (val && val.hasOwnProperty("$regex")) {
                const r = val.$regex; push(index.getAll((n) => {
    ...
    if (!indexed && db.options.autoIndexing) {
        return null;
    }
    if (!res && !db.indexes._id.ready) {
        return false;
    }
...
Cursor.getSorter = function (sort) {
    const criteria = _.map(sort, (val, key) => {
        return { key, direction: val };
    });
    return function (a, b) {
