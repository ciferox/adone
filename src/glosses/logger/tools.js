...
function getPrettyStream(opts, prettifier, dest) {
    ...
    const prettyFactory = adone.logger.pretty;
    ...
    return {
        ...
        flushSync() {
            ...
            dest.write(pretty(Object.assign({
                level: 40, // warn
                msg: "adone.logger.final with prettyPrint does not support flushing",
                time: Date.now()
            }, this.chindings())));
        },
        chindings() {
...
function final(logger, handler) {
    if (is.undefined(logger) || !is.function(logger.child)) {
        throw Error("Expected logger instance");
    }
    ...
    if (!is.function(stream.flushSync)) {
        throw Error("final requires a stream that has a flushSync method, such as adone.logger.destination and adone.logger.extreme");
    }
