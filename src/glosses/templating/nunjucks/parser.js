...
export class Parser {
    ...
    pushToken(tok) {
        if (this.peeked) {
            throw new x.IllegalState("pushToken: can only push one token on between reads");
        }
        this.peeked = tok;
    }

    fail(msg, lineno, colno) {
        if ((is.undefined(lineno) || is.undefined(colno)) && this.peekToken()) {
    ...
    parseCall() {
        ...
        const callerNode = new nodes.Caller(callTok.lineno, callTok.colno,
            callerName,
            callerArgs,
            body);

        // add the additional caller kwarg, adding kwargs if necessary
        const { args: { children: args } } = macroCall;
    ...
    parseStatement() {
        ...
        if (this.breakOnBlocks && this.breakOnBlocks.includes(tok.value)) {
            return null;
        }
            ...
            default: {
                if (this.extensions.length) {
                    for (let i = 0; i < this.extensions.length; i++) {
                        ...
                        if (ext.tags && ext.tags.includes(tok.value)) {
    ...
    parseRaw(tagName = "raw") {
        ...
        while ((matches = this.tokens._extractRegex(rawBlockRegex)) && rawLevel > 0) {
            const [all, pre, blockName] = matches;
    ...
    parseCompare() {
        ...
        while (true) {  // eslint-disable-line no-constant-condition
            ...
            if (!tok) {
                ...
                ops.push(new nodes.CompareOperand(tok.lineno, tok.colno,
                    this.parseConcat(),
                    tok.value));
            } else {
                this.pushToken(tok);
    ...
    parseNodes() {
        ...
        while ((tok = this.nextToken())) {
            if (tok.type === lexer.TOKEN_DATA) {
                ...
                const c = tok.value[tok.value.length - this.tokens.tags.COMMENT_END.length - 1];
                this.dropLeadingWhitespace = c === "-";
            } else {
...
export const parse = (src, extensions, opts) => {
    const p = new Parser(lexer.lex(src, opts));
    if (!is.undefined(extensions)) {
