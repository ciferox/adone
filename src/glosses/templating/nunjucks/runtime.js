...
export const callWrap = (obj, name, context, args) => {
    if (!obj) {
        throw new error.InvalidArgumentException(`Unable to call \`${name}\`, which is undefined or falsey`);
    } else if (!is.function(obj)) {
        throw new error.InvalidArgumentException(`Unable to call \`${name}\`, which is not a function`);
    }
...
export const inOperator = (key, val) => {
    ...
    throw new error.IllegalStateException(`Cannot use "in" operator to search for "${key}" in unexpected types.`);
};
