...
export default class Sequelize {
    ...
    constructor(database, username, password, options) {
        ...
        if (arguments.length === 1 && is.object(database)) {
            // new Sequelize({ ... options })
            options = database;
            config = _.pick(options, "host", "port", "database", "username", "password");
        } else if (arguments.length === 1 && is.string(database) || arguments.length === 2 && typeof username === "object") {
        ...
        const requestedDialect = this.getDialect();
        if (requestedDialect !== "abstract" && is.propertyOwned(__.dialect, requestedDialect)) {
            Dialect = __.dialect[requestedDialect];
        } else {
            const supported = Object.keys(__.dialect).filter((x) => x !== "abstract");
            throw new Error(`The dialect ${this.getDialect()} is not supported. Supported dialects: ${supported.join(", ")}.`);
        }

        this.dialect = new Dialect(this);
        this.dialect.QueryGenerator.typeValidation = options.typeValidation;
        if (this.options.operatorsAliases === true) {
            util.deprecate("String based operators are now deprecated. Please use Symbol based operators for better security, read more at http://docs.sequelizejs.com/manual/tutorial/querying.html#operators");
            this.dialect.QueryGenerator.setOperatorsAliases(operator.LegacyAliases); //Op.LegacyAliases should be removed and replaced by Op.Aliases by v5.0 use
        } else {
            this.dialect.QueryGenerator.setOperatorsAliases(this.options.operatorsAliases);
        }

        this.queryInterface = new __.QueryInterface(this);

        /**
         * Models are stored here under the name given to `sequelize.define`
         */
        this.models = {};
        this.modelManager = new __.ModelManager(this);
    ...
    refreshTypes() {
        this.connectionManager.refreshTypeParser(type);
    }
     ...
     */
    getQueryInterface() {
        this.queryInterface = this.queryInterface || new __.QueryInterface(this);
    ...
    define(modelName, attributes, options) {
        ...
        const model = class extends __.Model { };
    ...
    import(path) {
        ...
        if (Path.normalize(path) !== Path.resolve(path)) {
            ...
            const callerFilename = util.stack()[1].getFileName();
        ...
        if (!this.importCache[path]) {
            ...
            this.importCache[path] = defineCall(this, type);
        }
    ...
    async query(sql, options) {
        ...
        if (options.replacements) {
            if (is.array(options.replacements)) {
                sql = util.format([sql].concat(options.replacements), this.options.dialect);
            } else {
                sql = util.formatNamedParameters(sql, options.replacements, this.options.dialect);
            }
        }
     ...
     */
    static and() {
        return { [operator.and]: util.sliceArgs(arguments) };
    }
     ...
     */
    static or() {
        return { [operator.or]: util.sliceArgs(arguments) };
    }
    ...
    static json(conditionsOrPath, value) {
        return new util.Json(conditionsOrPath, value);
    }
    ...
    static where(attr, comparator, logic) {
        return new util.Where(attr, comparator, logic);
    }
    ...
    log() {
        let options;
        let args = util.sliceArgs(arguments);
    ...
    normalizeDataType(Type) {
        let type = is.function(Type) ? new Type() : Type;
        ...
        if (type instanceof orm.type.ARRAY && dialectTypes[type.type.key]) {
    ...
    normalizeAttribute(attribute) {
        ...
        if (attribute.hasOwnProperty("defaultValue")) {
            if (is.function(attribute.defaultValue) && (
                attribute.defaultValue === type.NOW ||
                attribute.defaultValue === type.UUIDV1 ||
                attribute.defaultValue === type.UUIDV4
            )) {
 ...
 * @see {@link QueryTypes}
 */
Sequelize.prototype.queryType = queryType;
 ...
 * @see  {@link Operators}
 */
Sequelize.prototype.operator = Sequelize.operator = operator;
 ...
 * @see {@link Sequelize#transaction}
 */
Sequelize.prototype.Deferrable = Sequelize.Deferrable = orm.Deferrable;

/**
 * Allow hooks to be defined on Sequelize + on sequelize instance as universal hooks to run on all models
 * and on Sequelize/sequelize methods e.g. Sequelize(), Sequelize#define()
 */
__.Hooks.applyTo(Sequelize);
__.Hooks.applyTo(Sequelize.prototype);

/**
 * Expose various errors available
 */
for (const error of Object.keys(x)) {
    if (x[error] === x.BaseError) {
        Sequelize.prototype.Error = Sequelize.Error = x.BaseError;
    } else {
        Sequelize.prototype[error] = Sequelize[error] = x[error];
    }
}
