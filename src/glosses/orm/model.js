...
export default class Model {
    ...
    static _paranoidClause(model, options) {
        ...
        const now = util.now(this.sequelize.options.dialect);
        ...
        if (util.isWhereEmpty(options.where)) {
            options.where = deletedAtObject;
        } else {
            options.where = { [operator.and]: [deletedAtObject, options.where] };
        }
    ...
    static _conformInclude(include, self) {
        ...
        if (include instanceof Association.Base) {
    ...
    static _expandIncludeAllElement(includes, include) {
        ...
        if (nested) {
            ...
            if (!include.include) {
                include.include = [];
            } else if (!is.array(include.include)) {
        ...
        (function addAllIncludes(parent, includes) {
            _.forEach(parent.associations, (association) => {
                ...
                // include this model
                const thisInclude = util.cloneDeep(include);
    ...
    static _validateIncludedElements(options, tableNames) {
        ...
        options.include = options.include.map((include) => {
            ...
            if (is.undefined(include.duplicating)) {
        ...
        for (const include of options.include) {
            ...
            if (options.topModel === options.model && is.undefined(options.subQuery) && options.topLimit) {
                if (include.subQuery) {
        ...
        if (options.topModel === options.model && is.undefined(options.subQuery)) {
    ...
    static _validateIncludedElement(include, tableNames, options) {
        ...
        if (include.attributes && !options.raw) {
            ...
            include = util.mapFinderOptions(include, include.model);
            ...
            if (include.attributes.length) {
                _.each(include.model.primaryKeys, (attr, key) => {
                    ...
                    if (!_.some(include.attributes, (includeAttr) => {
                        if (attr.field !== key) {
                            return is.array(includeAttr) && includeAttr[0] === attr.field && includeAttr[1] === key;
                        }
        ...
        if (include._pseudo) {
            ...
            return util.mapFinderOptions(include, include.model);
        }
        ...
        if (include.association.through && Object(include.association.through.model) === include.association.through.model) {
            ...
            if (through.scope) {
                include.through.where = include.through.where ? { [operator.and]: [include.through.where, through.scope] } : through.scope;
            }
        ...
        if (is.undefined(include.required)) {
            include.required = Boolean(include.where);
        }

        if (include.association.scope) {
            include.where = include.where ? { [operator.and]: [include.where, include.association.scope] } : include.association.scope;
        }

        if (include.limit && is.undefined(include.separate)) {
            include.separate = true;
        }

        if (include.separate === true && !(include.association instanceof Association.HasMany)) {
    ...
    static init(attributes, options) { // testhint options:none
        ...
        options = util.merge(_.cloneDeep(globalOptions.define), options);
        ...
        options = util.merge({
            name: {
                plural: util.pluralize(options.modelName),
                singular: util.singularize(options.modelName)
            },
            indexes: [],
        ...
        if (!this.options.tableName) {
            this.tableName = this.options.freezeTableName ? this.name : util.underscoredIf(util.pluralize(this.name), this.options.underscoredAll);
        } else {
        ...
        this.rawAttributes = _.mapValues(attributes, (attribute, name) => {
            ...
            if (is.undefined(attribute.type)) {
        ...
        if (this.options.timestamps) {
            if (this.options.createdAt !== false) {
                this._timestampAttributes.createdAt = this.options.createdAt || util.underscoredIf("createdAt", this.options.underscored);
            }
            if (this.options.updatedAt !== false) {
                this._timestampAttributes.updatedAt = this.options.updatedAt || util.underscoredIf("updatedAt", this.options.underscored);
            }
            if (this.options.paranoid && this.options.deletedAt !== false) {
                this._timestampAttributes.deletedAt = this.options.deletedAt || util.underscoredIf("deletedAt", this.options.underscored);
            }
        }
        if (this.options.version) {
            this._versionAttribute = is.string(this.options.version) ? this.options.version : "version";
        }
    ...
    static refreshAttributes() {
        ...
        _.each(this.rawAttributes, (definition, name) => {
            ...
            if (is.undefined(definition.field)) {
            ...
            if (definition.type instanceof type.BOOLEAN) {
                this._booleanAttributes.push(name);
            } else if (definition.type instanceof type.DATE || definition.type instanceof type.DATEONLY) {
                this._dateAttributes.push(name);
            } else if (definition.type instanceof type.HSTORE || type.ARRAY.is(definition.type, type.HSTORE)) {
                this._hstoreAttributes.push(name);
            } else if (definition.type instanceof type.RANGE || type.ARRAY.is(definition.type, type.RANGE)) {
                this._rangeAttributes.push(name);
            } else if (definition.type instanceof type.JSON) {
                this._jsonAttributes.push(name);
            } else if (definition.type instanceof type.VIRTUAL) {
                this._virtualAttributes.push(name);
            } else if (definition.type instanceof type.GEOMETRY) {
                this._geometryAttributes.push(name);
            }

            if (definition.hasOwnProperty("defaultValue")) {
                this._defaultValues[name] = _.partial(util.toDefaultValue, definition.defaultValue, this.sequelize.options.dialect);
            }
            ...
            if (definition.hasOwnProperty("unique") && definition.unique) {
                ...
                } else if (is.string(definition.unique)) {
            ...
            if (definition.index === true && definition.type instanceof type.JSONB) {
        ...
        this._hasBooleanAttributes = Boolean(this._booleanAttributes.length);
        this._isBooleanAttribute = _.memoize((key) => this._booleanAttributes.indexOf(key) !== -1);

        this._hasDateAttributes = Boolean(this._dateAttributes.length);
        this._isDateAttribute = _.memoize((key) => this._dateAttributes.indexOf(key) !== -1);

        this._hasHstoreAttributes = Boolean(this._hstoreAttributes.length);
        this._isHstoreAttribute = _.memoize((key) => this._hstoreAttributes.indexOf(key) !== -1);

        this._hasRangeAttributes = Boolean(this._rangeAttributes.length);
        this._isRangeAttribute = _.memoize((key) => this._rangeAttributes.indexOf(key) !== -1);

        this._hasJsonAttributes = Boolean(this._jsonAttributes.length);
        this._isJsonAttribute = _.memoize((key) => this._jsonAttributes.indexOf(key) !== -1);

        this._hasVirtualAttributes = Boolean(this._virtualAttributes.length);
        this._isVirtualAttribute = _.memoize((key) => this._virtualAttributes.indexOf(key) !== -1);

        this._hasGeometryAttributes = Boolean(this._geometryAttributes.length);
    ...
    static scope(option) {
        ...
        for (const option of options) {
            ...
            if (_.isPlainObject(option)) {
                if (option.method) {
                    if (is.array(option.method) && Boolean(self.options.scopes[option.method[0]])) {
            ...
            if (scope) {
                _.assignWith(self._scope, scope, (objectValue, sourceValue, key) => {
                    if (key === "where") {
                        return is.array(sourceValue) ? sourceValue : Object.assign(objectValue || {}, sourceValue);
                    } else if (["attributes", "include", "group"].indexOf(key) >= 0 && is.array(objectValue) && is.array(sourceValue)) {
    ...
    static async findAll(options) {
        ...
        util.mapFinderOptions(options, this);
        ...
        const originalOptions = util.cloneDeep(options);
    ...
    static warnOnInvalidOptions(options, validColumnNames) {
        ...
        if (!options.where && unexpectedModelAttributes.length > 0) {
            util.warn(`Model attributes (${unexpectedModelAttributes.join(", ")}) passed into finder method options of model ${this.name}, but the options.where object is empty. Did you forget to use options.where?`);
        }
    }
    ...
    static async findById(param, options) {
        ...
        if (is.number(param) || is.string(param) || is.buffer(param)) {
            options.where = {};
    ...
    static aggregate(attribute, aggregateFunction, options) {
        ...
        if (!options.dataType) {
            if (attrOptions) {
                ...
                options.dataType = new type.FLOAT();
            }
        } else {
            options.dataType = this.sequelize.normalizeDataType(options.dataType);
        }

        util.mapOptionFieldNames(options, this);
        options = this._paranoidClause(this, options);
    ...
    static async count(options) {
        options = _.defaults(util.cloneDeep(options), { hooks: true });
        if (options.hooks) {
        ...
        options.dataType = new type.INTEGER();
    ...
    static async findAndCount(options) {
        ...
        const countOptions = util.cloneDeep(options);
    ...
    static bulkBuild(valueSets, options) { // testhint options:none
        ...
        if (options.attributes) {
            options.attributes = options.attributes.map((attribute) => is.array(attribute) ? attribute[1] : attribute);
        }
    ...
    static async findOrCreate(options) {
        ...
        try {
            let instance = await this.findOne(util.defaults({ transaction }, options));
            ...
            if (err instanceof this.sequelize.UniqueConstraintError) {
                const flattenedWhere = util.flattenObjectDeep(options.where);
                ...
                const instance = await this.findOne(util.defaults({
    ...
    static async upsert(values, options) {
        ...
        const insertValues = util.mapValueFieldNames(instance.dataValues, instance.attributes, this);
        const updateValues = util.mapValueFieldNames(updatedDataValues, options.fields, this);
        const now = util.now(this.sequelize.options.dialect);
        ...
        if (options.returning === true && primaryKey) {
            const record = await this.findById(primaryKey, options);
            val = [record, created];
        } else {
            val = created;
        }
        ...
        if (options.hooks) {
            await this.runHooks("afterUpsert", val, options);
        }
    ...
    static async bulkCreate(records, options) {
        ...
        for (const instance of instances) {
            ...
            instance.dataValues = util.mapValueFieldNames(values, options.fields, this);
        }
    ...
    static async destroy(options) {
        ...
        if (!options.truncate && !_.isPlainObject(options.where) && !_.isArray(options.where) && !(options.where instanceof util.SequelizeMethod)) {
        ...
        options.type = queryType.BULKDELETE;
        this._injectScope(options);
        ...
        if (this._timestampAttributes.deletedAt && !options.force) {
            ...
            attrValueHash[field] = util.now(this.sequelize.options.dialect);
    ...
    static async update(values, options) {
        ...
        if (this._timestampAttributes.updatedAt && !options.silent) {
            values[this._timestampAttributes.updatedAt] = this._getDefaultTimestamp(this._timestampAttributes.updatedAt) || util.now(this.sequelize.options.dialect);
        }
        ...
        if (!results) {
            valuesUse = util.mapValueFieldNames(valuesUse, options.fields, this);
            options = util.mapOptionFieldNames(options, this);
    ...
    static _getDefaultTimestamp(attr) {
        if (Boolean(this.rawAttributes[attr]) && Boolean(this.rawAttributes[attr].defaultValue)) {
            return util.toDefaultValue(this.rawAttributes[attr].defaultValue, this.sequelize.options.dialect);
        }
        return undefined;
    }
    ...
    static _injectScope(options) {
        const scope = util.cloneDeep(this._scope);

        const filteredScope = _.omit(scope, "include"); // Includes need special treatment

        util.defaults(options, filteredScope);
        util.defaults(options.where, filteredScope.where);
    ...
    static async increment(fields, options) {
        ...
        if (!options.silent && updatedAtAttr && !values[updatedAtAttr]) {
            options.attributes[updatedAtAttribute.field || updatedAtAttr] = this._getDefaultTimestamp(updatedAtAttr) || util.now(this.sequelize.options.dialect);
        }
        if (versionAttr) {
    ...
    static _optionsMustContainWhere(options) {
        ...
        assert(_.isPlainObject(options.where) || _.isArray(options.where) || options.where instanceof util.SequelizeMethod, "Expected plain object, array or sequelize method in the options.where parameter");
    }
        ...
        if (options.isNewRecord) {
            ...
            if (this.constructor._hasDefaultValues) {
                defaults = _.mapValues(this.constructor._defaultValues, (valueFn) => {
                    const value = valueFn();
                    return value && value instanceof util.SequelizeMethod ? value : _.cloneDeep(value);
                });
            }
            ...
            if (this.constructor._timestampAttributes.createdAt && defaults[this.constructor._timestampAttributes.createdAt]) {
                this.dataValues[this.constructor._timestampAttributes.createdAt] = util.toDefaultValue(defaults[this.constructor._timestampAttributes.createdAt], this.sequelize.options.dialect);
                delete defaults[this.constructor._timestampAttributes.createdAt];
            }

            if (this.constructor._timestampAttributes.updatedAt && defaults[this.constructor._timestampAttributes.updatedAt]) {
                this.dataValues[this.constructor._timestampAttributes.updatedAt] = util.toDefaultValue(defaults[this.constructor._timestampAttributes.updatedAt], this.sequelize.options.dialect);
                delete defaults[this.constructor._timestampAttributes.updatedAt];
            }

            if (this.constructor._timestampAttributes.deletedAt && defaults[this.constructor._timestampAttributes.deletedAt]) {
                this.dataValues[this.constructor._timestampAttributes.deletedAt] = util.toDefaultValue(defaults[this.constructor._timestampAttributes.deletedAt], this.sequelize.options.dialect);
            ...
            if (Object.keys(defaults).length) {
                for (key in defaults) {
                    if (is.undefined(values[key])) {
                        this.set(key, util.toDefaultValue(defaults[key], this.sequelize.options.dialect), defaultsOptions);
                        delete values[key];
                    }
                }
            }
    ...
    where(checkVersion) {
        ...
        return util.mapWhereFieldNames(where, this.constructor);
    }
    ...
    setDataValue(key, value) {
        ...
        if (!util.isPrimitive(value) || value !== originalValue) {
    ...
    get(key, options) { // testhint options:none
        if (is.undefined(options) && typeof key === "object") {
            options = key;
            key = undefined;
        }
        ...
        if (key) {
            ...
            if (options.plain && this._options.include && this._options.includeNames.indexOf(key) !== -1) {
                if (is.array(this.dataValues[key])) {
    ...
    set(key, value, options) { // testhint options:none
        ...
        if (typeof key === "object" && !is.null(key)) {
            values = key;
            options = value || {};
            ...
            if (!options.raw && this._customSetters[key]) {
                ...
                if (!util.isPrimitive(newValue) && !is.null(newValue) || newValue !== originalValue) {
                ...
                if (!options.raw) {
                    ...
                    if (!this._isAttribute(key)) {
                        if (key.indexOf(".") > -1 && this.constructor._isJsonAttribute(key.split(".")[0])) {
                            const previousDottieValue = _.get(this.dataValues, key);
                ...
                if (
                    ...
                    (
                        ...
                        value instanceof util.SequelizeMethod ||
                        // Check for data type type comparators
                        !(value instanceof util.SequelizeMethod) && this.constructor._dataTypeChanges[key] && this.constructor._dataTypeChanges[key].call(this, value, originalValue, options) ||
                        // Check default
                        !this.constructor._dataTypeChanges[key] && (!util.isPrimitive(value) && !is.null(value) || value !== originalValue)
                    )
                ) {
                    this._previousDataValues[key] = originalValue;
                    this.changed(key, true);
    ...
    _setInclude(key, value, options) {
        ...
        if (is.undefined(include.originalAttributes) || include.originalAttributes.length) {
            if (association.isSingleAssociation) {
                if (is.array(value)) {
                    value = value[0];
                }

                isEmpty = value && is.null(value[primaryKeyAttribute]) || is.null(value);
                this[accessor] = this.dataValues[accessor] = isEmpty ? null : include.model.build(value, childOptions);
            } else {
                isEmpty = value[0] && is.null(value[0][primaryKeyAttribute]);
    ...
    async save(options) {
        ...
        if (is.undefined(options.returning)) {
        ...
        if (this.isNewRecord === false) {
            if (primaryKeyName && is.undefined(this.get(primaryKeyName, { raw: true }))) {
        ...
        if (
            ...
            await Promise.all(this._options.include.filter((include) => include.association instanceof Association.BelongsTo).map((include) => {
                const instance = this.get(include.as);
                if (!instance) {
                    return Promise.resolve();
                }

                const includeOptions = _(util.cloneDeep(include))
                    .omit(["association"])
                    .defaults({
                        transaction: options.transaction,
                        logging: options.logging,
        ...
        let values = util.mapValueFieldNames(this.dataValues, options.fields, this.constructor);
        let query = null;
        let args = [];
        let where;
        ...
        if (this.isNewRecord) {
            ...
            where = util.mapValueFieldNames(where, Object.keys(where), this.constructor);
            if (versionAttr) {
        ...
        if (wasNewRecord && this._options.include && this._options.include.length) {
            ...
            await Promise.all(this._options.include.filter((include) => !(include.association instanceof Association.BelongsTo)).map(async (include) => {
                ...
                await Promise.all(instances.map(async (instance) => {
                    if (include.association instanceof Association.BelongsToMany) {
    ...
    update(values, options) {
        ...
        options = util.cloneDeep(options);
        const setOptions = util.cloneDeep(options);
    ...
    async destroy(options) {
        ...
        if (this.constructor._timestampAttributes.deletedAt && options.force === false) {
            ...
            result = await this.sequelize.getQueryInterface().delete(this, this.constructor.getTableName(options), where, _.assign({ type: queryType.DELETE, limit: null }, options));
        }
    ...
    isSoftDeleted() {
        ...
        const deletedAtIsInTheFuture = adone.datetime(deletedAt).isAfter(now);
...
_.extend(Model, Association.mixin);
Hooks.applyTo(Model);
