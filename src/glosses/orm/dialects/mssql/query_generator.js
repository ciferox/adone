...
const QueryGenerator = {
    ...
    bulkInsertQuery(tableName, attrValueHashes, options, attributes) {
        ...
        _.forEach(attrValueHashes, (attrValueHash) => {
            ...
            if (fields.length === 1 && firstAttr && firstAttr.autoIncrement && is.null(attrValueHash[fields[0]])) {
    ...
    upsertQuery(tableName, insertValues, updateValues, where, model) {
        ...
        for (const index of model.options.indexes) {
            if (index.unique && index.fields) {
                for (const field of index.fields) {
                    const fieldName = is.string(field) ? field : field.name || field.attribute;
        ...
        identityAttrs.forEach((key) => {
            if (updateValues[key] && !is.null(updateValues[key])) {
                needIdentityInsertWrapper = true;
                /*
    ...
    selectFromTableFragment(options, model, attributes, tables, mainTableAs, where) {
        ...
        if (adone.semver.valid(this.sequelize.options.databaseVersion) && adone.semver.lt(this.sequelize.options.databaseVersion, "11.0.0")) {
            if (options.limit) {
        ...
        if (options.tableHint && __.TableHints[options.tableHint]) {
            mainFragment += ` WITH (${__.TableHints[options.tableHint]})`;
        }
