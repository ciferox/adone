...
function SchemaType(path, options, instance) {
    ...
    for (const prop in options) {
        if (this[prop] && is.function(this[prop])) {
            ...
            const opts = is.array(val) ? val : [val];
...
SchemaType.prototype.doValidate = function (value, fn, scope) {
    ...
    this.validators.forEach((v) => {
        ...
        let validatorProperties = utils.clone(v);
        ...
        if (validator instanceof RegExp) {
            ...
            if (validatorProperties.isAsync) {
                ...
                if (ok && is.function(ok.then)) {
                    ok.then(
                        (ok) => { validate(ok, validatorProperties); },
                        (error) => {
                            validatorProperties.reason = error;
                            ok = false;
...
function asyncValidate(validator, scope, value, props, cb) {
    let called = false;
    var returnVal = validator.call(scope, value, (ok, customMsg) => {
        if (called) {
            return;
        }
        called = true;
