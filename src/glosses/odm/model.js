...
Model.prototype.$__save = function (options, callback) {
    ...
    _this.$__handleSave(options, (error, result) => {
        ...
        if (_this.$__.version && !_this.$__.inserting) {
            let doIncrement = VERSION_INC === (VERSION_INC & _this.$__.version);
            _this.$__.version = undefined;

            if (numAffected <= 0) {
                // the update failed. pass an error back
                let err = new VersionError(_this);
                return callback(err);
            }

            // increment version if was successful
            if (doIncrement) {
                let key = _this.schema.options.versionKey;
                let version = _this.getValue(key) | 0;
...
Model.translateAliases = function translateAliases(fields) {
    let aliases = this.schema.aliases;
...
Model.insertMany = function (arr, options, callback) {
    ...
    parallelLimit(toExecute, limit, (error, docs) => {
        ...
        let docAttributes = docs.filter((doc) => {
      return doc != null;
    });
        ...
        let docObjects = docAttributes.map((doc) => {
...
Model.mapReduce = function mapReduce(o, callback) {
    ...
    return new Promise.ES6(((resolve, reject) => {
        if (!Model.mapReduce.schema) {
            let opts = { noId: true, noVirtualId: true, strict: false };
...
Model.geoNear = function (near, options, callback) {
    ...
    if (!near) {
        return new Promise.ES6(((resolve, reject) => {
            let error = new Error("Must pass a near option to geoNear");
            reject(error);
            callback && callback(error);
        }));
    }
...
Model.__subclass = function subclass(conn, schema, collection) {
    ...
    const s = schema && !is.string(schema)
        ? schema
