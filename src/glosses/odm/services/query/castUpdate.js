        ...
        if (val && val.constructor.name === "Object") {
            ...
            if (schematype && schematype.caster && op in castOps) {
                ...
                if ("$each" in val) {
                    ...
                    if (Boolean(val.$position) || val.$position === 0) {
            ...
            const pathDetails = schema._getPathType(checkPath);
            let isStrict = strict;
            if (useNestedStrict &&
                pathDetails &&
                pathDetails.schema &&
...
function castUpdateVal(schema, val, op, $conditional, context) {
    ...
    const cond = schema.caster && op in castOps &&
        (utils.isObject(val) || is.array(val));
    if (cond) {
    ...
    if (overwriteOps[op]) {
        return schema.castForQueryWrapper({
            val,
            context,
            $skipQueryCastForUpdate: !is.nil(val) && schema.$isMongooseArray
        });
    }
