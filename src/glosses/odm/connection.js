...
Connection.prototype._handleOpenSetArgs = function (uris, database, options, callback) {
    ...
    if (!this.name) {
        const err = new Error("No database name provided for replica set");
        this.error(err, callback);
        throw err;
    }
...
Connection.prototype.openUri = function (uri, options, callback) {
    ...
    if (options) {
        ...
        const autoIndex = options.config && !is.nil(options.config.autoIndex) ?
        ...
        if (!is.nil(options.bufferCommands)) {
...
Connection.prototype.close = function (force, callback) {
    ...
    this.$wasForceClosed = Boolean(force);

    return new Promise.ES6(((resolve, reject) => {
        _this._close(force, (error) => {
            callback && callback(error);
            if (error) {
                reject(error);
                return;
...
Connection.prototype.shouldAuthenticate = function () {
    return (!is.null(this.user) && this.user !== void 0) &&
        ((!is.null(this.pass) || this.pass !== void 0) || this.authMechanismDoesNotRequirePassword());
};
