...
const _getPathsToValidate = function (doc) {
    let i;
    let len;

    // only validate required fields when necessary
    let paths = Object.keys(doc.$__.activePaths.states.require).filter((path) => {
        if (!doc.isSelected(path) && !doc.isModified(path)) {
            return false;
        }
        const p = doc.schema.path(path);
        if (is.function(p.originalRequiredValue)) {
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (doc.schema.nested[pathToCheck]) {
            ...
            const _subpaths = Object.keys(flat).map((p) => `${pathToCheck}.${p}`);
...
const minimize = function (obj) {
    ...
    while (i--) {
        ...
        if (utils.isObject(val) && !is.buffer(val)) {
...
const applyGetters = function (self, json, type, options) {
...
export default class Document {
    constructor(obj, fields, skipId, options) {
    ...
    $__buildDoc(obj, fields, skipId) {
        ...
        if (fields && utils.getFunctionName(fields.constructor) === "Object") {
            ...
            if (ki === 1 && keys[0] === "_id") {
                exclude = Boolean(fields[keys[ki]]);
            } else {
                while (ki--) {
    ...
    */
    ...
    $set(path, val, type, options) {
        ...
        // if (path === "surnames.docarray") {
        //     adone.log(path, val);
        //     // adone.log(new Error().stack);
        // }
        ...
        if (parts.length <= 1) {
            ...
            for (i = 0; i < parts.length; ++i) {
                ...
                // adone.log(subpath, this.isDirectModified(subpath));
                if (/*this.isDirectModified(subpath) // earlier prefixes that are already marked as dirty have precedence
                    || */is.null(this.get(subpath))) {
                    pathToMark = subpath;
                    break;
                }
            }
        ...
        const priorVal = constructing ? undefined : this.getValue(path);
        ...
        try {
            ...
            let popOpts;
            if (schema.options &&
                is.array(schema.options[this.schema.options.typeKey]) &&
        ...
        if (schema.$isSingleNested && (this.isDirectModified(path) || is.nil(val))) {
            cleanModifiedSubpaths(this, path);
        }
     ...
     * @memberOf Document
     */
    $__shouldModify(pathToMark, path, constructing, parts, schema, val, priorVal) {
        if (this.isNew) {
            return true;
        }
     ...
     * @memberOf Document
     */
    $__set(pathToMark, path, constructing, parts, schema, val, priorVal) {
        ...
        if (shouldModify) {
            ...
            if (val && val.isMongooseArray) {
                ...
                this.$__.activePaths.forEach((modifiedPath) => {
        ...
        for (; i < l; i++) {
            ...
            if (last) {
                ...
                if (obj[parts[i]] && utils.getFunctionName(obj[parts[i]].constructor) === "Object") {
                    ...
                    obj = obj[parts[i]];
                } else if (obj[parts[i]] && is.array(obj[parts[i]])) {
                    obj = obj[parts[i]];
                } else {
    ...
    markModified(path, scope) {
        ...
        if (!is.nil(scope) && !this.ownerDocument) {
    ...
    modifiedPaths() {
        ...
        return directModifiedPaths.reduce((list, path) => {
            const parts = path.split(".");
            return list.concat(parts.reduce((chains, part, i) => {
    ...
    isModified(paths) {
        if (paths) {
            ...
            const directModifiedPaths = Object.keys(this.$__.activePaths.states.modify);
            const isModifiedChild = paths.some((path) => {
                return Boolean(~modified.indexOf(path));
            });
            return isModifiedChild || paths.some((path) => {
                return directModifiedPaths.some((mod) => {
    ...
    $isDeleted(val) {
        if (arguments.length === 0) {
            return Boolean(this.$__.isDeleted);
        }

        this.$__.isDeleted = Boolean(val);
        return this;
    }
    ...
    */
    $__validate(callback) {
        ...
        if (paths.length === 0) {
            return process.nextTick(() => {
                ...
                if (error) {
                    return _this.schema.s.hooks.execPost("validate:error", _this, [_this], { error }, (error) => {
                        callback(error);
                    });
                }
                callback();
    ...
    validateSync(pathsToValidate) {
        ...
        if (is.string(pathsToValidate)) {
            pathsToValidate = pathsToValidate.split(" ");
        }

        // only validate required fields when necessary
        let paths = _getPathsToValidate(this);
    ...
    */
    ...
    $__reset() {
        ...
        this.errors = undefined;
        _this = this;
        this.schema.requiredPaths().forEach((path) => {
    ...
    $__dirty() {
        ...
        let all = this.$__.activePaths.map("modify", (path) => {
            return {
                path,
        ...
        all = all.concat(this.$__.activePaths.map("default", (path) => {
    ...
    $__getAllSubdocs() {
        ...
        const docReducer = function (doc, seed, path) {
            const val = doc[path];
            ...
            if (val && val.$isSingleNested) {
                seed = Object.keys(val._doc).reduce((seed, path) => {
            ...
            if (val && val.isMongooseDocumentArray) {
                val.forEach(function _docReduce(doc) {
                    ...
                    seed = Object.keys(doc._doc).reduce((seed, path) => {
                        return docReducer(doc._doc, seed, path);
                    }, seed);
                });
            } else if (val instanceof Document && val.$__isNested) {
                if (val) {
                    seed = Object.keys(val).reduce((seed, path) => {
        ...
        const _this = this;
        const subDocs = Object.keys(this._doc).reduce((seed, path) => {
    ...
    $toObject(options, json) {
        ...
        if (transform === true ||
            ...
            if (opts) {
                transform = (is.function(options.transform) ? options.transform : opts.transform);
            }
        } else {
     ...
     * @api public
     */
    populated(path, val, options) {
        // val and options are internal

        if (is.null(val) || val === void 0) {
    ...
    depopulate(path) {
        ...
        for (let i = 0; i < path.length; i++) {
            const populatedIds = this.populated(path[i]);
            if (!populatedIds) {
                continue;
            }
    ...
    if (k === "post") {
        ...
        Document.prototype.$pre = Document.$pre = function mongoosePreWrapper() {
            if (arguments[0] === "set") {
                ...
                const $setArgs = Array.prototype.slice.call(arguments);
                $setArgs[0] = "$set";
