...
MongooseArray.mixin = {
   ...
   */
    ...
    _cast(value) {
        ...
        if (populated && !is.null(value) && !is.undefined(value)) {
            ...
            let isDisc = value.schema && value.schema.discriminatorMapping &&
                !is.undefined(value.schema.discriminatorMapping.key);
            if (!isDisc) {
   ...
   */
    ...
    _registerAtomic(op, val) {
        ...
        let atomics = this._atomics;

        // reset pop/shift after save
        if (op === "$pop" && !("$pop" in atomics)) {
            let _this = this;
            this._parent.once("save", () => {
        ...
        if (op === "$pullAll" || op === "$pushAll" || op === "$addToSet") {
            ...
            let pullOp = atomics.$pull || (atomics.$pull = {});
   ...
   */
    ...
    $__getAtomics() {
        let ret = [];
        let keys = Object.keys(this._atomics);
        let i = keys.length;
   ...
   */
    ...
    push() {
        ...
        let ret = [].push.apply(this, values);
   ...
   */
    ...
    pull() {
        ...
        if (values[0] instanceof EmbeddedDocument) {
            this._registerAtomic("$pullDocs", values.map((v) => {
                return v._id || v;
            }));
        } else {
   ...
   */
    ...
    sort() {
        let ret = [].sort.apply(this, arguments);
        this._registerAtomic("$set", this);
        this._markModified();
        return ret;
    },
...
function _checkManualPopulation(arr, docs) {
    const ref = arr._schema.caster.options && arr._schema.caster.options.ref;
    if (arr.length === 0 &&
        docs.length > 0) {
