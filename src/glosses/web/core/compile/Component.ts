...
export default class Component {
		...
		if (result) {
			...
			result = result
				...
				.replace(compile_options.generate === 'ssr' ? /(@+|#+)(\w*(?:-\w*)?)/g : /(@+)(\w*(?:-\w*)?)/g, (match: string, sigil: string, name: string) => {
					if (sigil === '@') {
						if (adone.web.core.internalExports.has(name)) {
							if (compile_options.dev && adone.web.core.internalExports.has(`${name}Dev`)) name = `${name}Dev`;
							this.helpers.add(name);
						}
	...
	get_unique_name_maker() {
		...
		adone.web.core.internalExports.forEach(add);
