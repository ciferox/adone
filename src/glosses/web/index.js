...
const validateBodyLimitOption = function (bodyLimit) {
...
export const createServer = function (options) {
    options = options || {};
    ...
    const fourOhFourFallBack = function (req, res) {
        ...
        const reply = new Reply(res, { onSend: [], onError: [] }, request, res.log);
    ...
    const getTrustProxyFn = function () {
    ...
    const buildRoutePrefix = function (instancePrefix, pluginPrefix) {
        if (!pluginPrefix) {
    ...
    const override = function (old, fn, opts) {
    ...
    const throwIfAlreadyStarted = function (msg) {
        if (started) {
            throw new Error(msg);
        }
    };
    ...
    const hasContentTypeParser = function (contentType, fn) {
        return this[kContentTypeParser].hasParser(contentType);
    };

    const setSchemaCompiler = function (schemaCompiler) {
    ...
    const beforeHandlerWarning = function () {
        if (beforeHandlerWarning.called) {
            return;
        }
    ...
    const basic404 = function (req, reply) {
        reply.code(404).send(new Error("Not Found"));
    };

    const onRunMiddlewares = function (err, req, res, reply) {
        if (!is.nil(err)) {
            reply.send(err);
            return;
        }
    ...
    const buildMiddie = function (middlewares) {
    ...
    const Context = function (schema, handler, Reply, Request, contentTypeParser, config, errorHandler, bodyLimit, logLevel, attachValidation) {
        this.schema = schema;
    ...
    const middlewareCallback = function (err, request, reply) {
        if (reply.sent === true) {
            return;
        }
        if (!is.nil(err)) {
    ...
    const routeHandler = function (req, res, params, context) {
        if (closing === true) {
            res.writeHead(503, {
                "Content-Type": "application/json",
                "Content-Length": "80",
        ...
        const request = new context.Request(params, req, query, req.headers, req.log);
        const reply = new context.Reply(res, context, request, res.log);
    ...
    const _setNotFoundHandler = function (prefix, opts, handler) {
        ...
        if (!is.null(this[kFourOhFourContext]) && prefix === "/") {
    ...
    const setNotFoundHandler = function (opts, handler) {
    ...
    const addSchema = function (name, schema) {
    ...
    const addContentTypeParser = function (contentType, opts, parser) {
    ...
    const setErrorHandler = function (func) {
    ...
    const logServerAddress = function (address, isHttps) {
        const isUnixSocket = is.string(address);
        if (!isUnixSocket) {
    ...
    const listenPromise = function (port, address, backlog) {
        if (listening) {
    ...
    const listen = function (port, address, backlog, cb) {
        /* Deal with listen (cb) */
        if (is.function(port)) {
            cb = port;
            port = 0;
        ...
        if (is.function(backlog)) {
            cb = backlog;
            backlog = undefined;
        }

        if (is.undefined(cb)) {
            return listenPromise(port, address, backlog);
        }

        const wrap = function (err) {
    ...
    const _route = function (_fastify, method, url, options, handler) {
    ...
    const route = function (opts) {
        ...
        const afterRouteAdded = function (path, notHandledErr, done) {
