...
export default class RollupResult implements CompileResult {
	...
	constructor(duration: number, compiler: RollupCompiler, sourcemap: boolean | 'inline') {
		...
		this.summary = compiler.chunks.map(chunk => {
			const size_color = chunk.code.length > 150000 ? chalk.bold.red : chunk.code.length > 50000 ? chalk.bold.yellow : chalk.bold.white;
			const size_label = left_pad(adone.pretty.size(chunk.code.length), 10);
