        ...
        return super.dest((file) => {
            ...
            for (let i = 0; i < this._matchers.length; ++i) {
                if (this._matchers[i](sourcePath)) {
                    if (!is.null(match)) {
                        throw new adone.error.Exception(`Ambiguity. This file "${sourcePath}" has more than one possible source: "${this._mappings[match].from}" or "${this._mappings[i].from}"`);
                    }
                    match = i;
                }
            }
            if (is.null(match)) {
                throw new adone.error.Exception(`Invalid file: "${sourcePath}". There is no matching source`);
            }
