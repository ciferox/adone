...
if (global.raspiTest) {
    ...
    procInfo = adone.std.fs.readFileSync("/proc/cpuinfo").toString();
}
let pins = null;
let rev = null;
const aliases = {};
...
export const getBoardRevision = () => BOARD_REVISIONS[rev] || VERSION_UNKNOWN;
export const getPins = () => pins;

export const getPinNumber = (alias) => {
    if (!is.number(alias) && !is.string(alias)) {
        return null;
    }
    ...
    if (is.undefined(alias)) {
        return null;
    }
    return alias;
};

export const getGpioNumber = (alias) => {
