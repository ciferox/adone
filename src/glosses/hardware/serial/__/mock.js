...
export default class MockBindings extends adone.hardware.serial.__.BaseBinding {
    constructor(opt) {
        super(opt);
    ...
    open(path, opt) {
        ...
        return super.open(path, opt).then(() => {
            if (!port) {
    ...
    close() {
        ...
        return super.close().then(() => {
            delete port.openOpt;
            // reset data on close
            port.data = Buffer.allocUnsafe(0);
    ...
    read(buffer, offset, length) {
        return super.read(buffer, offset, length).then(() => {
