...
Animation.prototype.loopFunction = function (loop) {
    ...
    const progress = this.calculateProgress(loop.calledAt);
...
Animation.prototype.findIndices = function (progress) {
    ...
    indices.right = this.cuePoints.findIndex((point) => {
...
Animation.prototype.tweenedValue = function (indices, progress) {
    ...
    const result = this.normalizedKeyFrames.map(function (keyFrame) {
        ...
        memberIndices.right = keyFrame.findIndex((frame, index) => {
        ...
        let left = keyFrame[memberIndices.left],
        ...
        if (right.position) {
            // This is a tuple
            calcValue = right.position.map((value, index) => {
