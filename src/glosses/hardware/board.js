...
Board.prototype.snapshot = function (reducer) {
    ...
    return this.register.reduce((accum, component) => {
        // Don't include collections or multi/imu wrappers
        if (typeof component.components === UNDEFINED) {
            accum.push(
                Object.getOwnPropertyNames(component).reduce((data, prop) => {
...
Board.prototype.shiftOut = function (dataPin, clockPin, isBigEndian, value) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        if (isBigEndian) {
            this.io.digitalWrite(dataPin, Boolean(value & (1 << (7 - i))) | 0);
        } else {
            this.io.digitalWrite(dataPin, Boolean(value & (1 << i)) | 0);
        }
...
function Boards(opts) {
    ...
    const initialized = {};
    const noRepl = ports.some((port) => {
        return port.repl === false;
    });
    const noDebug = ports.some((port) => {
...
Board.Virtual = function (opts) {
    let temp;

    if (opts instanceof adone.hardware.Expander) {
        temp = {
            io: opts
        };
    } else {
    ...
    return new adone.hardware.Board(
        Object.assign({}, {
            repl: false,
            debug: false,
            sigint: false
