...
const nanosleep = require("./sleep").nano;
const Fn = require("./fn");
const priv = new Map();
    ...
    PCA9685: {
        ...
        servoWrite: {
            value(pin, value) {
                ...
                const off = Fn.map(value, 0, 180, this.pwmRange[0] / 4, this.pwmRange[1] / 4);
    ...
    GROVEPI: {
        ...
        ioRead: {
            value(pin, type, callback) {
                ...
                const handler = function (pinIndex, value) {
                    const pin = this.pins[pinIndex];
                    let canEmit = true;
        ...
        pingRead: {
            value(settings, callback) {
                const state = priv.get(this);
                const pinIndex = state.pinMap[settings.pin];
