...
function Pin(opts) {
    ...
    const pinValue = typeof opts === "object" ? (opts.addr || opts.pin || 0) : opts;
    let isAnalogInput = Pin.isAnalog(opts);
    let isDTOA = false;
    ...
    const isPin = typeof opts !== "object";
    const addr = isDTOA ? pinValue : (isPin ? opts : opts.addr);
    const type = opts.type || (isAnalogInput ? "analog" : "digital");
...
function read(pin) {
    ...
    pin.io[`${pin.type}Read`](pin.addr, (data) => {
        state.value = data;
    });
...
Pin.prototype.high = function () {
    const value = this.type === "analog" ? 255 : 1;
