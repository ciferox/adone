const {
    is,
    p2p: { util: { abortableIterator } },
    stream: { pull: { pipe, itrPushable } }
} = adone;

const log = require("debug")("libp2p:mplex");
const Coder = require("./coder");
const restrictSize = require("./restrict_size");
const { MessageTypes, MessageTypeNames } = require("./message_types");
...
class Mplex {
    ...
    constructor(options) {
        options = options || {};
        options = is.function(options) ? { onStream: options } : options;
    ...
    newStream(name) {
        ...
        name = is.nil(name) ? id.toString() : String(name);
    ...
    /**
     * Creates a sink with an abortableIterator source. Incoming messages will
    ...
    _createSink() {
        return async (source) => {
            if (this._options.signal) {
                source = abortableIterator(source, this._options.signal);
            }
    ...
    _createSource() {
        ...
        const source = itrPushable({ onEnd, writev: true });
