const {
    is,
    p2p: { util: { abortableIterator, AbortController } },
    stream: { pull: { itrPushable } }
} = adone;

const log = require("debug")("libp2p:mplex:stream");
const { InitiatorMessageTypes, ReceiverMessageTypes } = require("./message_types");
...
module.exports = ({ id, name, send, onEnd = () => { }, type = "initiator" }) => {
    ...
    name = String(is.nil(name) ? id : name);
    ...
    const stream = {
        ...
        sink: async (source) => {
            source = abortableIterator.multi(source, [
        ...
        source: itrPushable(onSourceEnd),
        timeline: {
            open: Date.now(),
            close: null
        },
