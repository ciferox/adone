...
Timestamp.prototype.toString = function (opt_radix) {
    ...
    if (this.isNegative()) {
        if (this.equals(Timestamp.MIN_VALUE)) {
            ...
            let radixTimestamp = Timestamp.fromNumber(radix);
            let div = this.div(radixTimestamp);
    ...
    let radixToPower = Timestamp.fromNumber(Math.pow(radix, 6));
...
Timestamp.prototype.div = function (other) {
    ...
    while (rem.greaterThanOrEqual(other)) {
        ...
        let approxRes = Timestamp.fromNumber(approx);
...
Timestamp.fromInt = function (value) {
    if (-128 <= value && value < 128) {
        let cachedObj = Timestamp.INT_CACHE_[value];
        if (cachedObj) {
            return cachedObj;
        }
    }

    let obj = new Timestamp(value | 0, value < 0 ? -1 : 0);
    if (-128 <= value && value < 128) {
...
Timestamp.fromString = function (str, opt_radix) {
    ...
    // minimize the calls to the very expensive emulated div.
    let radixToPower = Timestamp.fromNumber(Math.pow(radix, 8));

    let result = Timestamp.ZERO;
    for (let i = 0; i < str.length; i += 8) {
        let size = Math.min(8, str.length - i);
        let value = parseInt(str.substring(i, i + size), radix);
        if (size < 8) {
            let power = Timestamp.fromNumber(Math.pow(radix, size));
