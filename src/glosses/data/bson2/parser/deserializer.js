...
const JS_INT_MAX_LONG = Long.fromNumber(bson2.JS_INT_MAX);
const JS_INT_MIN_LONG = Long.fromNumber(bson2.JS_INT_MIN);
...
const deserializeObject = function (buffer, index, options, isArray) {
    ...
    while (!done) {
        ...
        if (elementType === bson2.BSON_DATA_STRING) {
            const stringSize =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
        ...
        } else if (elementType === bson2.BSON_DATA_OID) {
            const oid = new Buffer(12);
            buffer.copy(oid, 0, index, index + 12);
            object[name] = new ObjectId(oid);
            index = index + 12;
        } else if (elementType === bson2.BSON_DATA_INT && promoteValues === false) {
            object[name] = new Int32(
                buffer[index++] | (buffer[index++] << 8) | (buffer[index++] << 16) | (buffer[index++] << 24)
            );
        } else if (elementType === bson2.BSON_DATA_INT) {
            object[name] =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
                (buffer[index++] << 24);
        } else if (elementType === bson2.BSON_DATA_NUMBER && promoteValues === false) {
            object[name] = new Double(buffer.readDoubleLE(index));
            index = index + 8;
        } else if (elementType === bson2.BSON_DATA_NUMBER) {
            object[name] = buffer.readDoubleLE(index);
            index = index + 8;
        } else if (elementType === bson2.BSON_DATA_DATE) {
            const lowBits =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
        ...
        } else if (elementType === bson2.BSON_DATA_BOOLEAN) {
            if (buffer[index] !== 0 && buffer[index] !== 1) {
                throw new Error("illegal boolean type value");
            }
            object[name] = buffer[index++] === 1;
        } else if (elementType === bson2.BSON_DATA_OBJECT) {
            const _index = index;
            const objectSize =
                buffer[index] |
                (buffer[index + 1] << 8) |
        ...
        } else if (elementType === bson2.BSON_DATA_ARRAY) {
            const _index = index;
            const objectSize =
                buffer[index] |
                (buffer[index + 1] << 8) |
        ...
        } else if (elementType === bson2.BSON_DATA_UNDEFINED) {
            object[name] = undefined;
        } else if (elementType === bson2.BSON_DATA_NULL) {
            object[name] = null;
        } else if (elementType === bson2.BSON_DATA_LONG) {
        ...
        } else if (elementType === bson2.BSON_DATA_DECIMAL128) {
        ...
        } else if (elementType === bson2.BSON_DATA_BINARY) {
            let binarySize =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
        ...
        } else if (elementType === bson2.BSON_DATA_REGEXP && bsonRegExp === false) {
            // Get the start search index
            i = index;
            // Locate the end of the c string
            while (buffer[i] !== 0x00 && i < buffer.length) {
        ...
        } else if (elementType === bson2.BSON_DATA_REGEXP && bsonRegExp === true) {
            // Get the start search index
            i = index;
            // Locate the end of the c string
            while (buffer[i] !== 0x00 && i < buffer.length) {
        ...
        } else if (elementType === bson2.BSON_DATA_SYMBOL) {
            const stringSize =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
            ...
            index = index + stringSize;
        } else if (elementType === bson2.BSON_DATA_TIMESTAMP) {
            const lowBits =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
        ...
        } else if (elementType === bson2.BSON_DATA_MIN_KEY) {
            object[name] = new MinKey();
        } else if (elementType === bson2.BSON_DATA_MAX_KEY) {
            object[name] = new MaxKey();
        } else if (elementType === bson2.BSON_DATA_CODE) {
            const stringSize =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
            ...
            index = index + stringSize;
        } else if (elementType === bson2.BSON_DATA_CODE_W_SCOPE) {
            const totalSize =
                buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
        ...
        } else if (elementType === bson2.BSON_DATA_DBPOINTER) {
            // Get the code string size
            const stringSize =
                buffer[index++] |
                (buffer[index++] << 8) |
...
export default function (buffer, options, isArray) {
