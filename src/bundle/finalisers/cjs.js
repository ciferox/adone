...
export default function cjs(magicString, { graph, isEntryModuleFacade, namedExportsMode, hasExports, intro, outro, dependencies, exports }, options) {
    ...
    if (options.compact) {
        ...
        importBlock = '';
        dependencies.forEach(({ id, namedExportsMode, isChunk, name, reexports, imports, exportsNames, exportsDefault }) => {
        ...
        importBlock = dependencies
            .map(({ id, namedExportsMode, isChunk, name, reexports, imports, exportsNames, exportsDefault }) => {
            if (!reexports && !imports)
                return `require('${id}');`;
            if (!interop || isChunk || !exportsDefault || !namedExportsMode)
    ...
    const exportBlock = getExportBlock(exports, dependencies, namedExportsMode, options.interop, options.compact, `module.exports${_}=${_}`);
