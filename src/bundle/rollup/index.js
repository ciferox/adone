...
function applyBuildStartHook(graph) {
    return Promise.all(graph.plugins.map(plugin => plugin.buildStart && plugin.buildStart.call(graph.pluginContext))).then(() => { });
}
function applyBuildEndHook(graph, err) {
    return Promise.all(graph.plugins.map(plugin => plugin.buildEnd && plugin.buildEnd.call(graph.pluginContext, err))).then(() => { });
}
...
export default function rollup(rawInputOptions) {
    try {
        ...
        return applyBuildStartHook(graph)
            .then(() => {
            return graph.build(inputOptions.input, inputOptions.manualChunks, inputOptions.inlineDynamicImports, inputOptions.experimentalPreserveModules);
        })
            .catch(err => {
            ...
            let singleChunk;
            const singleInput = typeof inputOptions.input === 'string' ||
            ...
            function generate(rawOutputOptions, isWrite) {
                ...
                const inputBase = commondir(chunks.filter(chunk => chunk.entryModule).map(chunk => chunk.entryModule.id));
                return createAddons(graph, outputOptions)
                    ...
                    return Promise.all(chunks.map(chunk => {
                        ...
                        return chunk.render(outputOptions, addons).then(rendered => {
                            const outputChunk = outputBundle[chunkId];
                            outputChunk.code = rendered.code;
                            outputChunk.map = rendered.map;
                            return Promise.all(graph.plugins
                                .filter(plugin => plugin.ongenerate)
                                .map(plugin => plugin.ongenerate.call(graph.pluginContext, { bundle: outputChunk, ...outputOptions }, outputChunk)));
                        });
                    })).then(() => { });
                })
                    .then(() => {
                    ...
                    return Promise.all(generateBundlePlugins.map(plugin => plugin.generateBundle.call(generateBundleContext, outputOptions, outputBundle, isWrite))).then(() => {
            ...
            const result = {
                ...
                generate: ((rawOutputOptions) => {
                    const promise = generate(rawOutputOptions, false).then(result => inputOptions.experimentalCodeSplitting
                        ? { output: result }
                        : result[chunks[0].id]);
                ...
                write: ((outputOptions) => {
                    ...
                    else if (!inputOptions.experimentalCodeSplitting &&
                    ...
                    return generate(outputOptions, true).then(result => Promise.all(Object.keys(result).map(chunkId => {
                        return writeOutputFile(graph, chunkId, result[chunkId], outputOptions);
                    })).then(() => inputOptions.experimentalCodeSplitting
                        ? { output: result }
                        : result[chunks[0].id]));
                })
            };
...
function writeOutputFile(graph, outputFileName, outputFile, outputOptions) {
    ...
    return writeFile(filename, source)
        ...
        .then(() => isOutputChunk(outputFile) &&
        Promise.all(graph.plugins
            .filter(plugin => plugin.onwrite)
            .map(plugin => plugin.onwrite.call(graph.pluginContext, { bundle: outputFile, ...outputOptions }, outputFile))))
        .then(() => { });
}
