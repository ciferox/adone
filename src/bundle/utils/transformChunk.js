...
export default function transformChunk(graph, chunk, code, sourcemapChain, options) {
    return graph.plugins.reduce((promise, plugin) => {
        if (!plugin.transformBundle && !plugin.transformChunk)
            return promise;
        return promise.then(code => {
            return Promise.resolve()
                .then(() => (plugin.transformChunk || plugin.transformBundle).call(graph.pluginContext, code, options, chunk))
                .then(result => {
                if (result == null)
                    return code;
