...
export default class Graph {
    constructor(options) {
        ...
        this.handleMissingExport = firstSync(this.plugins
            ...
            .filter(Boolean)
            .map(missingExport => {
            return (exportName, importingModule, importedModule, importerStart) => {
                return missingExport.call(this.pluginContext, importingModule.id, exportName, importedModule, importerStart);
            };
        })
    ...
    includeMarked(modules) {
        if (this.treeshake) {
            let needsTreeshakingPass, treeshakingPass = 1;
            do {
    ...
    loadEntryModules(entryModules, manualChunks) {
        ...
        return Promise.all(entryAndManualChunkIds.map(id => this.loadModule(id))).then(entryAndChunkModules => {
    ...
    build(entryModules, manualChunks, inlineDynamicImports, preserveModules) {
        // Phase 1 – discovery. We load the entry module and find which
        // modules it imports, and import those, until we have all
        // of the entry module's dependencies
        timeStart('parse modules', 2);
        return this.loadEntryModules(entryModules, manualChunks).then(({ entryModules, entryModuleAliases, manualChunkModules }) => {
            ...
            this.link();
            const { orderedModules, dynamicImports, dynamicImportAliases } = this.analyseExecution(entryModules, !preserveModules && !inlineDynamicImports, inlineDynamicImports, manualChunkModules);
            ...
            if (!preserveModules) {
                ...
                for (const entryHashSum in chunkModules) {
                    ...
                    const chunkModulesOrdered = chunkModuleList.sort((moduleA, moduleB) => (moduleA.execIndex > moduleB.execIndex ? 1 : -1));
    ...
    analyseExecution(entryModules, graphColouring, inlineDynamicImports, chunkModules) {
        let curEntry, curEntryHash;
        const allSeen = {};
        const orderedModules = [];
        const dynamicImports = [];
        const dynamicImportAliases = [];
        let parents;
        const visit = (module) => {
...
Try defining "${chunkName}" first in the manualChunks definitions of the Rollup configuration.`
    ...
    fetchAllDependencies(module) {
        ...
        const fetchDynamicImportsPromise = Promise.all(module.getDynamicImportExpressions().map((dynamicImportExpression, index) => {
            return Promise.resolve(this.resolveDynamicImport.call(this.pluginContext, dynamicImportExpression, module.id)).then(replacement => {
                if (!replacement) {
                ...
                const alias = getAliasName(replacement, typeof dynamicImportExpression === 'string' ? dynamicImportExpression : undefined);
        ...
        return Promise.all(module.sources.map(source => {
