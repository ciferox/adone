...
export default class Chunk {
    ...
    populateEntryExports(preserveModules) {
    ...
    traceImport(exportName, module) {
        ...
        if (traced.variable.isNamespace) {
            const namespaceVariables = traced.variable.originals ||
    ...
    getChunkDependencyDeclarations(options, inputBase) {
        ...
        this.dependencies.forEach(dep => {
            const importSpecifiers = Array.from(this.imports.entries()).filter(([, module]) => (module instanceof Module ? module.chunk === dep : module === dep));
            let imports;
     ...
     */
    postVisitChunkDependencies(visitor) {
        const seen = new Set();
    ...
    computeFullHash(addons, options) {
        const hash = sha256();
    ...
    generateIdPreserveModules(preserveModulesRelativeDir) {
    ...
    generateId(pattern, patternName, addons, options, existingNames) {
        const outName = makeUnique(renderNamePattern(pattern, patternName, type => {
            switch (type) {
                case 'hash':
    ...
    render(options, addons) {
        ...
        return transformChunk(this.graph, this, prevCode, chunkSourcemapChain, options).then((code) => {
