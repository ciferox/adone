...
export default class extends Subsystem {
    ...
    async bench(args, opts) {
        ...
        const formatEventMessage = (event, oldResult) => {
            ...
            if (oldResult) {
                ...
                if (diff >= 0) {
                    message += term.parse(` ({#4CAF50-fg}+${formatNumber(diff.toFixed(diff < 100 ? 2 : 0))} : +${percent}%{/})`);
                } else {
                    message += term.parse(` ({#F44336-fg}${formatNumber(diff.toFixed(diff > -100 ? 2 : 0))} : ${percent}%{/})`);
                }
            }
            ...
            if (oldResult) {
                ...
                if (diff >= 0) {
                    message += term.parse(` ({#4CAF50-fg}+${diff}{/})`);
                } else {
                    message += term.parse(` ({#F44336-fg}${diff}{/})`);
                }
            }
        ...
        const defaultOptions = {
            ...
            onCycle: (event) => {
                term.column(0);
                term.eraseLine();
                term.write(`${formatEventMessage(event)}`);
            },
            onComplete: () => {
                term.column(0);
                term.eraseLine();
        ...
        for (const suite of suites) {
            term.print(`Suite: {escape}${suite.name}{/escape}\n`);
            const result = {};
    ...
    _getSuites(funcs, defaultOptions, suiteName = this.filename) {
        const suites = [];
        const suite = new adone.benchmark.Benchmark.Suite(suiteName);
