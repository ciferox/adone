 ...
 * @private
 */
const DsName = (name) => new datastore.Key(keyPrefix + name);
 ...
 * @private
 */
const DsInfoName = (name) => new datastore.Key(infoPrefix + name);
...
export default class Keychain {
    ...
    static generateOptions() {
        ...
        options.dek.salt = std.crypto.randomBytes(saltLength).toString("base64");
        return options;
    }
    ...
    createKey(name, type, size, callback) {
        ...
        self.store.has(dsname, (err, exists) => {
            ...
            try {
                const keypair = crypto.keys.generateKeyPair(type, size);
                const kid = keypair.id();
                const pem = keypair.export(this._());
                const keyInfo = {
                    name,
                    id: kid
                };
    ...
    exportKey(name, password, callback) {
        ...
        this.store.get(dsname, (err, res) => {
            ...
            const pem = res.toString();
            try {
                const privateKey = crypto.keys.import(pem, this._());
                callback(null, privateKey.export(password));
            } catch (err) {
                return _error(callback, err);
            }
        });
    ...
    importKey(name, pem, password, callback) {
        ...
        self.store.has(dsname, (err, exists) => {
            ...
            try {
                const privateKey = crypto.keys.import(pem, password);
                try {
                    const kid = privateKey.id();
                    const pem = privateKey.export(this._());
                    const keyInfo = {
                        name,
                        id: kid
                    };
