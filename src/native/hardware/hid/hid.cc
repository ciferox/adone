...
static string narrow(wchar_t *wide)
{
    wstring ws(wide);
    ostringstream os;
...
class HID : public Nan::ObjectWrap
{
  public:
    ...
    void write(const databuf_t &message) throw(JSException)
    {
    ...
    void setNonBlocking(int message) throw(JSException)
    {
        int res;
  ...
  private:
    ...
    static NAN_METHOD(getFeatureReport)
    {
        ...
        if (info.Length() != 2 || info[1]->ToUint32()->Value() == 0)
        {
...
NODE_MODULE(HID, HID::Initialize);
