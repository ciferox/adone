...
static inline size_t StringOrBufferLength(v8::Local<v8::Value> obj)
{
    ...
    return (!obj->ToObject().IsEmpty() && node::Buffer::HasInstance(obj->ToObject()))
...
static inline void DisposeStringOrBufferFromSlice(
    Nan::Persistent<v8::Object> &handle, leveldb::Slice slice)
{
    Nan::HandleScope scope;
...
static inline void DisposeStringOrBufferFromSlice(
    v8::Local<v8::Value> handle, leveldb::Slice slice)
{
...
#define LD_STRING_OR_BUFFER_TO_SLICE(to, from, name)                                     \
    ...
    else if (!from->ToObject().IsEmpty() && node::Buffer::HasInstance(from->ToObject())) \
    {                                                                                    \
    ...
    {                                                                                    \
        ...
        to##Str->WriteUtf8(                                                              \
            to##Ch_, -1, NULL, v8::String::NO_NULL_TERMINATION);                         \
    }                                                                                    \
...
#define LD_STRING_OR_BUFFER_TO_COPY(to, from, name)                                 \
    size_t to##Sz_;                                                                 \
    char *to##Ch_;                                                                  \
    if (!from->ToObject().IsEmpty() && node::Buffer::HasInstance(from->ToObject())) \
    {                                                                               \
        to##Sz_ = node::Buffer::Length(from->ToObject());                           \
        to##Ch_ = new char[to##Sz_];                                                \
...
#define LD_METHOD_SETUP_COMMON(name, optionPos, callbackPos)                                    \
    ...
    else if (optionPos != -1 && info[optionPos]->IsObject() && info[callbackPos]->IsFunction()) \
    {                                                                                           \
