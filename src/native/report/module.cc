...
// Default report option settings
...
NAN_METHOD(TriggerReport) {
  ...
  if (info[0]->IsString()) {
    ...
    if (filename_parameter.length() < NR_MAXNAME) {
      ...
      Nan::ThrowError("report: filename parameter is too long");
    }
    err_index++;
  }
...
/*******************************************************************************
 * External JavaScript APIs for report configuration
 *
 ******************************************************************************/
...
static void SignalDumpInterruptCallback(Isolate* isolate, void* data) {
  if (report_signal != 0) {
    if (nodereport_verbose) {
      fprintf(stdout, "report: SignalDumpInterruptCallback handling signal\n");
    }
    if (nodereport_events & NR_SIGNAL) {
      if (nodereport_verbose) {
        fprintf(stdout, "report: SignalDumpInterruptCallback triggering report\n");
      }
...
static void SignalDumpAsyncCallback(uv_async_t* handle) {
  if (report_signal != 0) {
    if (nodereport_verbose) {
      fprintf(stdout, "report: SignalDumpAsyncCallback handling signal\n");
    }
    if (nodereport_events & NR_SIGNAL) {
      if (nodereport_verbose) {
        fprintf(stdout, "report: SignalDumpAsyncCallback triggering NodeReport\n");
      }
...
static int StartWatchdogThread(void* (*thread_main)(void* unused)) {
  ...
  if (err != 0) {
    fprintf(stderr, "report: StartWatchdogThread pthread_create() failed: %s\n", strerror(err));
    fflush(stderr);
    return -err;
  }
  return 0;
...
inline void* ReportSignalThreadMain(void* unused) {
  for (;;) {
    ...
    if (nodereport_verbose) {
      fprintf(stdout, "report: signal %s received\n", node::signo_string(report_signal));
    }
...
static void SetupSignalHandler() {
  ...
  if (rc != 0) {
    fprintf(stderr, "report: initialization failed, uv_sem_init() returned %d\n", rc);
    Nan::ThrowError("report: initialization failed, uv_sem_init() returned error\n");
  }
  rc = uv_mutex_init(&node_isolate_mutex);
  if (rc != 0) {
    fprintf(stderr, "report: initialization failed, uv_mutex_init() returned %d\n", rc);
    Nan::ThrowError("report: initialization failed, uv_mutex_init() returned error\n");
  }
  ...
  if (StartWatchdogThread(ReportSignalThreadMain) == 0) {
    ...
    if (rc != 0) {
      fprintf(stderr, "report: initialization failed, uv_async_init() returned %d\n", rc);
      Nan::ThrowError("report: initialization failed, uv_async_init() returned error\n");
    }
...
void Initialize(v8::Local<v8::Object> exports) {
  ...
  if (nodereport_verbose) {
#ifdef _WIN32
    fprintf(stdout, "report: initialization complete, event flags: %#x\n",
            nodereport_events);
#else
    fprintf(stdout, "report: initialization complete, event flags: %#x signal flag: %#x\n",
