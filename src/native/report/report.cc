...
/*******************************************************************************
 * Functions to process report configuration options:
...
/*******************************************************************************
 * Function to save the report module load time value
 *******************************************************************************/
void SetLoadTime() {
#ifdef _WIN32
...
void TriggerNodeReport(Isolate* isolate, DumpEvent event, const char* message, const char* location, char* name, MaybeLocal<Value> error) {
  ...
  if (name != nullptr && strlen(name) > 0) {
    ...
    snprintf(filename, sizeof(filename), "%s", "report");
    seq++;
#ifdef _WIN32
...
static void PrintVersionInformation(std::ostream& out) {
  ...
  // Print report module version
  // e.g. report version: 1.0.6 (built against Node.js v6.9.1)
  out << std::endl << "report version: " << NODEREPORT_VERSION
...
void PrintNativeStack(std::ostream& out) {
  ...
  // Print the native frames, omitting the top 3 frames as they are in report code
