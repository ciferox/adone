...
export const currentStack = function (stackTraceLimit = 0) {
    const e = Error;
    ...
    e.prepareStackTrace = (prepare, stack) => stack;
    // We remove stack at top since it's always this function.
    const stack = new e().stack.slice(1);
...
export const formatStackTrace = function (trace) {
    let s = "";
    for (const site of trace) {
        s += `at ${site.getFunctionName()}(${site.getFileName()}:${site.getLineNumber()}:${site.getColumnNumber()})\n`;
    }
    return s;
};
