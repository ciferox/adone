...
describe("realm", () => {
    ...
    it("get default manager", async () => {
        runtimeRealmManager = realm.getManager();
        const anotherInstance = realm.getManager();
    ...
    it("initialize realm manager", async () => {
        ...
        tasks.forEach((name) => {
            assert.false(runtimeRealmManager.hasTask(name));
        });
        
        await runtimeRealmManager.initialize();
        
        tasks.forEach((name) => {
            assert.true(runtimeRealmManager.hasTask(name));
        });
    });
