...
const jsVersion = adone.ipfs.version;
const versions = {
    js: `js-ipfs version: ${jsVersion}`,
    go: `ipfs version ${adone.ipfs.go.defaultVersion}`,
    proc: jsVersion
};
    ...
    tests.forEach((fOpts) => describe(`${fOpts.type}`, () => {
        ...
        describe("custom config options", () => {
            it("custom config", function (done) {
                ...
                const options = { config, initOptions: { bits: fOpts.bits, profile: "test" } };
        ...
        describe("f.spawn with args", () => {
            ...
            it("check that pubsub was enabled", (done) => {
                ...
                const handler = (msg) => {
                    ...
                    expect(is.buffer(msg.seqno)).to.eql(true);
