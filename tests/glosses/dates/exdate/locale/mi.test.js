...
describe("mi", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "i roto i te hēkona ruarua", "prefix");
        assert.equal(adone.date(0).from(30000), "te hēkona ruarua i mua", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "te hēkona ruarua i mua", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "i teie mahana, i 12:00", "today at the same time");
