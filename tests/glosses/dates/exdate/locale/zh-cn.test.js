...
describe("zh-cn", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "几秒内", "prefix");
        assert.equal(adone.date(0).from(30000), "几秒前", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "几秒前", "now from now should display as in the past");
    });
    ...
    it("weeks year starting sunday format", () => {
        assert.equal(adone.date([2012, 0, 1]).format("w ww wo"), "52 52 52周", "Jan  1 2012 应该是第52周");
        assert.equal(adone.date([2012, 0, 7]).format("w ww wo"), "1 01 1周", "Jan  7 2012 应该是第 1周");
        assert.equal(adone.date([2012, 0, 14]).format("w ww wo"), "2 02 2周", "Jan 14 2012 应该是第 2周");
    });
});
