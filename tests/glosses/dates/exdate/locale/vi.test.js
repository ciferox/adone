...
describe("vi", () => {
    ...
    it("parse", () => {
        ...
        function equalTest(input, mmm, i) {
            assert.equal(adone.date(input, mmm).month(), i, input + " should be month " + i);
        }
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "vài giây tới", "prefix");
        assert.equal(adone.date(0).from(30000), "vài giây trước", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "vài giây trước", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "Hôm nay lúc 12:00", "today at the same time");
