...
describe("ms-my", () => {
    ...
    it("parse", () => {
        ...
        function equalTest(input, mmm, i) {
            assert.equal(adone.date(input, mmm).month(), i, input + " sepatutnya bulan " + (i + 1));
        }
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "dalam beberapa saat", "prefix");
        assert.equal(adone.date(0).from(30000), "beberapa saat yang lepas", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "beberapa saat yang lepas", "waktu sekarang dari sekarang sepatutnya menunjukkan sebagai telah lepas");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "Hari ini pukul 12.00", "hari ini pada waktu yang sama");
    ...
    it("weeks year starting sunday format", () => {
        assert.equal(adone.date([2012, 0, 1]).format("w ww wo"), "1 01 1", "Jan  1 2012 sepatutnya minggu 1");
        assert.equal(adone.date([2012, 0, 7]).format("w ww wo"), "2 02 2", "Jan  7 2012 sepatutnya minggu 2");
        assert.equal(adone.date([2012, 0, 8]).format("w ww wo"), "2 02 2", "Jan  8 2012 sepatutnya minggu 2");
        assert.equal(adone.date([2012, 0, 14]).format("w ww wo"), "3 03 3", "Jan 14 2012 sepatutnya minggu 3");
        assert.equal(adone.date([2012, 0, 15]).format("w ww wo"), "3 03 3", "Jan 15 2012 sepatutnya minggu 3");
    });
});
