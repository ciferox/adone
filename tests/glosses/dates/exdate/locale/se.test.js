...
describe("se", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "moadde sekunddat geažes", "prefix");
        assert.equal(adone.date(0).from(30000), "maŋit moadde sekunddat", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "maŋit moadde sekunddat", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "otne ti 12:00", "Today at the same time");
