...
describe("sl", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "čez nekaj sekund", "prefix");
        assert.equal(adone.date(0).from(30000), "pred nekaj sekundami", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "pred nekaj sekundami", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "danes ob 12:00", "today at the same time");
