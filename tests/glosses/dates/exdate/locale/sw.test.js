...
describe("sw", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "hivi punde baadaye", "prefix");
        assert.equal(adone.date(0).from(30000), "tokea hivi punde", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "tokea hivi punde", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "leo saa 12:00", "today at the same time");
