...
describe("et", () => {
    ...
    it("parse", () => {
        ...
        function equalTest(input, mmm, i) {
            assert.equal(adone.date(input, mmm).month(), i, input + " peaks olema kuu " + (i + 1));
        }
        for (i = 0; i < 12; i++) {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "mõne sekundi pärast", "prefix");
        assert.equal(adone.date(0).from(30000), "mõni sekund tagasi", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "mõni sekund tagasi", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "Täna, 12:00", "today at the same time");
