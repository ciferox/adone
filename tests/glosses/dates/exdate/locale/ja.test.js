...
describe("ja", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "数秒後", "prefix");
        assert.equal(adone.date(0).from(30000), "数秒前", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "数秒前", "now from now should display as in the past");
    });
