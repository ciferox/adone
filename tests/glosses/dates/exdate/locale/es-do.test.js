...
describe("es-do", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "en unos segundos", "prefix");
        assert.equal(adone.date(0).from(30000), "hace unos segundos", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "hace unos segundos", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "hoy a las 12:00 PM", "today at the same time");
