...
describe("id", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "dalam beberapa detik", "prefix");
        assert.equal(adone.date(0).from(30000), "beberapa detik yang lalu", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "beberapa detik yang lalu", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "Hari ini pukul 12.00", "today at the same time");
