...
describe("nn", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.date(30000).from(0), "om nokre sekund", "prefix");
        assert.equal(adone.date(0).from(30000), "nokre sekund sidan", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.date().fromNow(), "nokre sekund sidan", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.date(a).calendar(), "I dag klokka 12:00", "today at the same time");
