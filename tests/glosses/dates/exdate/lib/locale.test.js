...
describe("locale", () => {
    ...
    it("library getters and setters", () => {
        const r = adone.date.locale("en");

        assert.equal(r, "en", "locale should return en by default");
        assert.equal(adone.date.locale(), "en", "locale should return en by default");

        adone.date.locale("fr");
        assert.equal(adone.date.locale(), "fr", "locale should return the changed locale");

        adone.date.locale("en-gb");
        assert.equal(adone.date.locale(), "en-gb", "locale should return the changed locale");

        adone.date.locale("en");
        assert.equal(adone.date.locale(), "en", "locale should reset");

        adone.date.locale("does-not-exist");
        assert.equal(adone.date.locale(), "en", "locale should reset");

        adone.date.locale("EN");
        assert.equal(adone.date.locale(), "en", "Normalize locale key case");

        adone.date.locale("EN_gb");
        assert.equal(adone.date.locale(), "en-gb", "Normalize locale key underscore");
    });

    it("library setter array of locales", () => {
        assert.equal(adone.date.locale(["non-existent", "fr", "also-non-existent"]), "fr", "passing an array uses the first valid locale");
        assert.equal(adone.date.locale(["es", "fr", "also-non-existent"]), "es", "passing an array uses the first valid locale");
    });

    it("library setter locale substrings", () => {
        assert.equal(adone.date.locale("fr-crap"), "fr", "use substrings");
        assert.equal(adone.date.locale("fr-does-not-exist"), "fr", "uses deep substrings");
        assert.equal(adone.date.locale("fr-CA-does-not-exist"), "fr-ca", "uses deepest substring");
    });

    it("library getter locale array and substrings", () => {
        assert.equal(adone.date.locale(["en-CH", "fr"]), "en", "prefer root locale to shallower ones");
        assert.equal(adone.date.locale(["en-gb-leeds", "en-CA"]), "en-gb", "prefer root locale to shallower ones");
        assert.equal(adone.date.locale(["en-fake", "en-CA"]), "en-ca", "prefer alternatives with shared roots");
        assert.equal(adone.date.locale(["en-fake", "en-fake2", "en-ca"]), "en-ca", "prefer alternatives with shared roots");
        assert.equal(adone.date.locale(["fake-CA", "fake-MX", "fr"]), "fr", "always find something if possible");
        assert.equal(adone.date.locale(["fake-CA", "fake-MX", "fr"]), "fr", "always find something if possible");
        assert.equal(adone.date.locale(["fake-CA", "fake-MX", "fr-fake-fake-fake"]), "fr", "always find something if possible");
        assert.equal(adone.date.locale(["en", "en-CA"]), "en", "prefer earlier if it works");
    });

    it("library ensure inheritance", () => {
        adone.date.locale("made-up", {
        ...
        assert.equal(adone.date([2012, 5, 6]).format("MMMM"), "July", "Override some of the configs");
        assert.equal(adone.date([2012, 5, 6]).format("MMM"), "Jun", "But not all of them");
    });

    it("library ensure inheritance LT L LL LLL LLLL", () => {
        const locale = "test-inherit-lt";

        adone.date.defineLocale(locale, {
            longDateFormat: {
                LT: "-[LT]-",
                L: "-[L]-",
                LL: "-[LL]-",
        ...
        adone.date.locale("es");

        assert.equal(adone.date().locale(locale).calendar(), "sameDay -LT-", "Should use instance locale in LT formatting");
        assert.equal(adone.date().add(1, "days").locale(locale).calendar(), "nextDay -L-", "Should use instance locale in L formatting");
        assert.equal(adone.date().add(-1, "days").locale(locale).calendar(), "lastDay -LLL-", "Should use instance locale in LL formatting");
        assert.equal(adone.date().add(4, "days").locale(locale).calendar(), "nextWeek -LL-", "Should use instance locale in LLL formatting");
        assert.equal(adone.date().add(-4, "days").locale(locale).calendar(), "lastWeek -LLLL-", "Should use instance locale in LLLL formatting");
    });

    it("library localeData", () => {
        adone.date.locale("en");

        const jan = adone.date([2000, 0]);

        assert.equal(adone.date.localeData().months(jan), "January", "no arguments returns global");
        assert.equal(adone.date.localeData("zh-cn").months(jan), "一月", "a string returns the locale based on key");
        assert.equal(adone.date.localeData(adone.date().locale("es")).months(jan), "enero", "if you pass in a adone.date it uses the adone.date's locale");
    });
    ...
    it("instance locale method", () => {
        ...
        assert.equal(adone.date([2012, 5, 6]).format("MMMM"), "June", "Normally default to global");
        assert.equal(adone.date([2012, 5, 6]).locale("es").format("MMMM"), "junio", "Use the instance specific locale");
        assert.equal(adone.date([2012, 5, 6]).format("MMMM"), "June", "Using an instance specific locale does not affect other moments");
    });

    it("instance locale method with array", () => {
        let m = adone.date().locale(["non-existent", "fr", "also-non-existent"]);
        assert.equal(m.locale(), "fr", "passing an array uses the first valid locale");
        m = adone.date().locale(["es", "fr", "also-non-existent"]);
    ...
    it("instance locale persists with cloning", () => {
        ...
        const a = adone.date([2012, 5, 6]).locale("es");
        const b = a.clone();

        assert.equal(b.format("MMMM"), "junio", "using adone.date.fn.clone()");
        assert.equal(b.format("MMMM"), "junio", "using adone.date()");
    });
    ...
    it("changing the global locale doesn't affect existing duration instances", () => {
        const mom = adone.date.duration();
        adone.date.locale("fr");
        assert.equal("en", mom.locale());
    });

    it("from and fromNow with invalid date", () => {
        assert.equal(adone.date(NaN).from(), "Invalid date", "adone.date.from with invalid adone.date");
        assert.equal(adone.date(NaN).fromNow(), "Invalid date", "adone.date.fromNow with invalid adone.date");
    });
    ...
    it("instance locale used with from", () => {
        ...
        const a = adone.date([2012, 5, 6]).locale("es");
        const b = adone.date([2012, 5, 7]);

        assert.equal(a.from(b), "hace un día", "preserve locale of first adone.date");
        assert.equal(b.from(a), "in a day", "do not preserve locale of second adone.date");
    });
    ...
    it("month name callback function", () => {
        ...
        assert.equal(adone.date().format("[test] dd ddd dddd MMM MMMM"), "test test test test test test", "format month name function should be able to access the format string");
        assert.equal(adone.date([2011, 0, 1]).format("dd ddd dddd MMM MMMM"), "date date date date date", "format month name function should be able to access the adone.date object");
        assert.equal(adone.date([2011, 0, 2]).format("dd ddd dddd MMM MMMM"), "default default default default default", "format month name function should be able to access the adone.date object");
    });

    it("changing parts of a locale config", () => {
        adone.date.locale("partial-lang", {
            months: "a b c d e f g h i j k l".split(" ")
        });

        assert.equal(adone.date([2011, 0, 1]).format("MMMM"), "a", "should be able to set locale values when creating the localeuage");

        adone.date.updateLocale("partial-lang", {
            monthsShort: "A B C D E F G H I J K L".split(" ")
        });

        assert.equal(adone.date([2011, 0, 1]).format("MMMM MMM"), "a A", "should be able to set locale values after creating the localeuage");

        adone.date.defineLocale("partial-lang", null);
    });

    it("start/endOf week feature for first-day-is-monday locales", () => {
        adone.date.locale("monday-lang", {
            week: {
        ...
        adone.date.locale("monday-lang");
        assert.equal(adone.date([2013, 0, 1]).startOf("week").day(), 1, "for locale monday-lang first day of the week should be monday");
        assert.equal(adone.date([2013, 0, 1]).endOf("week").day(), 0, "for locale monday-lang last day of the week should be sunday");
        adone.date.defineLocale("monday-lang", null);
    });
    ...
    it("meridiem parsing", () => {
        ...
        adone.date.locale("meridiem-parsing");
        assert.equal(adone.date("2012-01-01 3b", "YYYY-MM-DD ha").hour(), 15, "Custom parsing of meridiem should work");
        assert.equal(adone.date("2012-01-01 3d", "YYYY-MM-DD ha").hour(), 3, "Custom parsing of meridiem should work");
        adone.date.defineLocale("meridiem-parsing", null);
    });

    it("invalid date formatting", () => {
        adone.date.locale("has-invalid", {
            invalidDate: "KHAAAAAAAAAAAN!"
        });

        assert.equal(adone.date.invalid().format(), "KHAAAAAAAAAAAN!");
        assert.equal(adone.date.invalid().format("YYYY-MM-DD"), "KHAAAAAAAAAAAN!");
        adone.date.defineLocale("has-invalid", null);
    });

    it("return locale name", () => {
        const registered = adone.date.locale("return-this", {});

        assert.equal(registered, "return-this", "returns the locale configured");
        adone.date.locale("return-this", null);
    });

    it("changing the global locale doesn't affect existing instances", () => {
        const mom = adone.date();
        adone.date.locale("fr");
        assert.equal("en", mom.locale());
    });

    it("setting a language on instance returns the original adone.date for chaining", () => {
        const mom = adone.date();

        assert.equal(mom.locale("it"), mom, "setting the language (locale) returns the original adone.date for chaining");
    });

    it("adone.date#locale(false) resets to global locale", () => {
        const m = adone.date();

        adone.date.locale("fr");
        m.locale("it");

        assert.equal(adone.date.locale(), "fr", "global locale is it");
        assert.equal(m.locale(), "it", "instance locale is it");
        m.locale(false);
        assert.equal(m.locale(), "fr", "instance locale reset to global locale");
    });

    it("adone.date().locale with missing key doesn't change locale", () => {
        assert.equal(adone.date().locale("boo").localeData(), adone.date.localeData(),
    ...
    //     assert.equal(adone.date('A', 'MMMM', true).month(), 0, 'parse long month 0 with MMMM');
    //     assert.equal(adone.date('AA', 'MMMM', true).month(), 1, 'parse long month 1 with MMMM');
    //     assert.equal(adone.date('AAA', 'MMMM', true).month(), 2, 'parse long month 2 with MMMM');
    //     assert.equal(adone.date('B B', 'MMMM', true).month(), 4, 'parse long month 4 with MMMM');
    //     assert.equal(adone.date('BB  B', 'MMMM', true).month(), 5, 'parse long month 5 with MMMM');
    //     assert.equal(adone.date('C-C', 'MMMM', true).month(), 7, 'parse long month 7 with MMMM');
    //     assert.equal(adone.date('C,C2C', 'MMMM', true).month(), 8, 'parse long month 8 with MMMM');
    //     assert.equal(adone.date('D+D', 'MMMM', true).month(), 10, 'parse long month 10 with MMMM');
    //     assert.equal(adone.date('D`D*D', 'MMMM', true).month(), 11, 'parse long month 11 with MMMM');

    //     assert.equal(adone.date('E', 'MMM', true).month(), 0, 'parse long month 0 with MMM');
    //     assert.equal(adone.date('EE', 'MMM', true).month(), 1, 'parse long month 1 with MMM');
    //     assert.equal(adone.date('EEE', 'MMM', true).month(), 2, 'parse long month 2 with MMM');

    //     assert.equal(adone.date('A', 'MMM').month(), 0, 'non-strict parse long month 0 with MMM');
    //     assert.equal(adone.date('AA', 'MMM').month(), 1, 'non-strict parse long month 1 with MMM');
    //     assert.equal(adone.date('AAA', 'MMM').month(), 2, 'non-strict parse long month 2 with MMM');
    //     assert.equal(adone.date('E', 'MMMM').month(), 0, 'non-strict parse short month 0 with MMMM');
    //     assert.equal(adone.date('EE', 'MMMM').month(), 1, 'non-strict parse short month 1 with MMMM');
    //     assert.equal(adone.date('EEE', 'MMMM').month(), 2, 'non-strict parse short month 2 with MMMM');
    // });
});
