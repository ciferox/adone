describe("quarter", () => {
    ...
    it("quarter setter only month changes", () => {
        const m = adone.date([2014, 4, 11, 1, 2, 3, 4]).quarter(4);
    ...
    it("quarter setter bubble to next year", () => {
        const m = adone.date([2014, 4, 11, 1, 2, 3, 4]).quarter(7);
    ...
    it("quarter diff", () => {
        assert.equal(adone.date("2014-01-01").diff(adone.date("2014-04-01"), "quarter"),
                -1, "diff -1 quarter");
        assert.equal(adone.date("2014-04-01").diff(adone.date("2014-01-01"), "quarter"),
                1, "diff 1 quarter");
        assert.equal(adone.date("2014-05-01").diff(adone.date("2014-01-01"), "quarter"),
                1, "diff 1 quarter");
        assert.ok(Math.abs((4 / 3) - adone.date("2014-05-01").diff(
                        adone.date("2014-01-01"), "quarter", true)) < 0.00001,
                "diff 1 1/3 quarter");
        assert.equal(adone.date("2015-01-01").diff(adone.date("2014-01-01"), "quarter"),
                4, "diff 4 quarters");
    });

    it("quarter setter bubble to previous year", () => {
        const m = adone.date([2014, 4, 11, 1, 2, 3, 4]).quarter(-3);
