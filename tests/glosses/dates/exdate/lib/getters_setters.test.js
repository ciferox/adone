describe("getters and setters", () => {
    ...
    it("getters", () => {
        const a = adone.date([2011, 9, 12, 6, 7, 8, 9]);
    ...
    it("setters", () => {
        ...
        // Test month() behavior. See https://github.com/timrwood/adone.date/pull/822
        a = adone.date("20130531", "YYYYMMDD");
        a.month(3);
    ...
    it("setters strings", () => {
        const a = adone.date([2012]).locale("en");
    ...
    it("day setter", () => {
        let a = adone.date([2011, 0, 15]);
        assert.equal(adone.date(a).day(0).date(), 9, "set from saturday to sunday");
        assert.equal(adone.date(a).day(6).date(), 15, "set from saturday to saturday");
        assert.equal(adone.date(a).day(3).date(), 12, "set from saturday to wednesday");

        a = adone.date([2011, 0, 9]);
        assert.equal(adone.date(a).day(0).date(), 9, "set from sunday to sunday");
        assert.equal(adone.date(a).day(6).date(), 15, "set from sunday to saturday");
        assert.equal(adone.date(a).day(3).date(), 12, "set from sunday to wednesday");

        a = adone.date([2011, 0, 12]);
        assert.equal(adone.date(a).day(0).date(), 9, "set from wednesday to sunday");
        assert.equal(adone.date(a).day(6).date(), 15, "set from wednesday to saturday");
        assert.equal(adone.date(a).day(3).date(), 12, "set from wednesday to wednesday");

        assert.equal(adone.date(a).day(-7).date(), 2, "set from wednesday to last sunday");
        assert.equal(adone.date(a).day(-1).date(), 8, "set from wednesday to last saturday");
        assert.equal(adone.date(a).day(-4).date(), 5, "set from wednesday to last wednesday");

        assert.equal(adone.date(a).day(7).date(), 16, "set from wednesday to next sunday");
        assert.equal(adone.date(a).day(13).date(), 22, "set from wednesday to next saturday");
        assert.equal(adone.date(a).day(10).date(), 19, "set from wednesday to next wednesday");

        assert.equal(adone.date(a).day(14).date(), 23, "set from wednesday to second next sunday");
        assert.equal(adone.date(a).day(20).date(), 29, "set from wednesday to second next saturday");
        assert.equal(adone.date(a).day(17).date(), 26, "set from wednesday to second next wednesday");
    });
    ...
    it("setters across DST +1", () => {
        const oldUpdateOffset = adone.date.updateOffset;
        // Based on a real story somewhere in America/Los_Angeles
        const dstAt = adone.date("2014-03-09T02:00:00-08:00").parseZone();
        let m;
        ...
        m = adone.date("2014-03-15T00:00:00-07:00").parseZone();
        m.year(2013);
        ...
        m = adone.date("2014-03-09T03:05:00-07:00").parseZone();
        m.hour(0);
        assert.equal(m.format(), "2014-03-09T00:05:00-08:00", "hour across +1");

        adone.date.updateOffset = oldUpdateOffset;
    });

    it("setters across DST -1", () => {
        const oldUpdateOffset = adone.date.updateOffset;
        // Based on a real story somewhere in America/Los_Angeles
        const dstAt = adone.date("2014-11-02T02:00:00-07:00").parseZone();
        let m;
        ...
        m = adone.date("2014-11-15T00:00:00-08:00").parseZone();
        m.year(2013);
        ...
        m = adone.date("2014-11-02T03:30:00-08:00").parseZone();
        m.hour(0);
