describe("create", () => {
    ...
    it("array", () => {
        assert.ok(adone.date([2010]).toDate() instanceof Date, "[2010]");
        assert.ok(adone.date([2010, 1]).toDate() instanceof Date, "[2010, 1]");
        assert.ok(adone.date([2010, 1, 12]).toDate() instanceof Date, "[2010, 1, 12]");
        assert.ok(adone.date([2010, 1, 12, 1]).toDate() instanceof Date, "[2010, 1, 12, 1]");
    ...
    it("array with invalid arguments", () => {
        assert.ok(!adone.date([2010, null, null]).isValid(), "[2010, null, null]");
        assert.ok(!adone.date([1945, null, null]).isValid(), "[1945, null, null] (pre-1970)");
    });

    it("array copying", () => {
        const importantArray = [2009, 11];
        adone.date(importantArray);
    ...
    it("multi format array copying", () => {
        ...
        adone.date("1999-02-13", importantArray);
        assert.deepEqual(importantArray, ["MM/DD/YYYY", "YYYY-MM-DD", "MM-DD-YYYY"], "initializer should not mutate the original array");
    });

    it("number", () => {
        assert.ok(adone.date(1000).toDate() instanceof Date, "1000");
        assert.equal(adone.date(1000).valueOf(), 1000, "asserting valueOf");
        assert.equal(adone.date.utc(1000).valueOf(), 1000, "asserting valueOf");
    });

    it("unix", () => {
        assert.equal(adone.date.unix(1).valueOf(), 1000, "1 unix timestamp == 1000 Date.valueOf");
        assert.equal(adone.date(1000).unix(), 1, "1000 Date.valueOf == 1 unix timestamp");
        assert.equal(adone.date.unix(1000).valueOf(), 1000000, "1000 unix timestamp == 1000000 Date.valueOf");
        assert.equal(adone.date(1500).unix(), 1, "1500 Date.valueOf == 1 unix timestamp");
        assert.equal(adone.date(1900).unix(), 1, "1900 Date.valueOf == 1 unix timestamp");
        assert.equal(adone.date(2100).unix(), 2, "2100 Date.valueOf == 2 unix timestamp");
        assert.equal(adone.date(1333129333524).unix(), 1333129333, "1333129333524 Date.valueOf == 1333129333 unix timestamp");
        assert.equal(adone.date(1333129333524000).unix(), 1333129333524, "1333129333524000 Date.valueOf == 1333129333524 unix timestamp");
    });

    it("date", () => {
        assert.ok(adone.date(new Date()).toDate() instanceof Date, "new Date()");
        assert.equal(adone.date(new Date(2016, 0, 1), "YYYY-MM-DD").format("YYYY-MM-DD"), "2016-01-01", "If date is provided, format string is ignored");
    });
    ...
    it("date with a format as an array", () => {
        ...
        for (let i = 0; i < tests.length; i++) {
            assert.equal(adone.date(tests[i]).format(), adone.date(tests[i], ["MM/DD/YYYY"], false).format(), "Passing date with a format array should still return the correct date");
        }
    });

    it("date mutation", () => {
        const a = new Date();
        assert.ok(adone.date(a).toDate() !== a, "the date adone.date uses should not be the date passed in");
    });

    it("adone.date", () => {
        assert.ok(adone.date(adone.date()).toDate() instanceof Date, "adone.date(adone.date())");
        assert.ok(adone.date(adone.date(adone.date())).toDate() instanceof Date, "adone.date(adone.date(adone.date()))");
    });

    it("cloning adone.date should only copy own properties", () => {
        assert.ok(!adone.date().clone().hasOwnProperty("month"), "Should not clone prototype methods");
    });

    it("cloning respects adone.date.customProperties", () => {
        const m = adone.date();

        assert.equal(m.clone()._special, undefined, "cloning ignores extra properties");
        m._special = "bacon";
        adone.date.customProperties.push("_special");
        assert.equal(m.clone()._special, "bacon", "cloning respects customProperties");
        adone.date.customProperties.pop();
    });

    it("undefined", () => {
        assert.ok(adone.date().toDate() instanceof Date, "undefined");
    });

    it("iso with bad input", () => {
        assert.ok(!adone.date("a", adone.date.ISO_8601).isValid(), "iso parsing with invalid string");
        assert.ok(!adone.date("a", adone.date.ISO_8601, true).isValid(), "iso parsing with invalid string, strict");
    });
    ...
    it("string without format - json", () => {
        assert.equal(adone.date("Date(1325132654000)").valueOf(), 1325132654000, "Date(1325132654000)");
        assert.equal(adone.date("Date(-1325132654000)").valueOf(), -1325132654000, "Date(-1325132654000)");
        assert.equal(adone.date("/Date(1325132654000)/").valueOf(), 1325132654000, "/Date(1325132654000)/");
        assert.equal(adone.date("/Date(1325132654000+0700)/").valueOf(), 1325132654000, "/Date(1325132654000+0700)/");
        assert.equal(adone.date("/Date(1325132654000-0700)/").valueOf(), 1325132654000, "/Date(1325132654000-0700)/");
    });
    ...
    it("string with format dropped am/pm bug", () => {
        ...
        assert.equal(adone.date("05/1/2012 12:25:00", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"), "05/01/2012", "should not break if am/pm is left off from the parsing tokens");
        assert.equal(adone.date("05/1/2012 12:25:00 am", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"), "05/01/2012", "should not break if am/pm is left off from the parsing tokens");
        assert.equal(adone.date("05/1/2012 12:25:00 pm", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"), "05/01/2012", "should not break if am/pm is left off from the parsing tokens");

        assert.ok(adone.date("05/1/2012 12:25:00", "MM/DD/YYYY h:m:s a").isValid());
        assert.ok(adone.date("05/1/2012 12:25:00 am", "MM/DD/YYYY h:m:s a").isValid());
        assert.ok(adone.date("05/1/2012 12:25:00 pm", "MM/DD/YYYY h:m:s a").isValid());
    });

    it("empty string with formats", () => {
        assert.equal(adone.date("", "MM").format("YYYY-MM-DD HH:mm:ss"), "Invalid date");
        assert.equal(adone.date(" ", "MM").format("YYYY-MM-DD HH:mm:ss"), "Invalid date");
        assert.equal(adone.date(" ", "DD").format("YYYY-MM-DD HH:mm:ss"), "Invalid date");
        assert.equal(adone.date(" ", ["MM", "DD"]).format("YYYY-MM-DD HH:mm:ss"), "Invalid date");

        assert.ok(!adone.date("", "MM").isValid());
        assert.ok(!adone.date(" ", "MM").isValid());
        assert.ok(!adone.date(" ", "DD").isValid());
        assert.ok(!adone.date(" ", ["MM", "DD"]).isValid());
    });

    it("undefined argument with formats", () => {
        assert.equal(adone.date(undefined, "MM").format("YYYY-MM-DD HH:mm:ss"), "Invalid date");
        assert.equal(adone.date(undefined, "DD").format("YYYY-MM-DD HH:mm:ss"), "Invalid date");
        assert.equal(adone.date(undefined, ["MM", "DD"]).format("YYYY-MM-DD HH:mm:ss"), "Invalid date");

        assert.ok(!adone.date(undefined, "MM").isValid());
        assert.ok(!adone.date(undefined, "MM").isValid());
        assert.ok(!adone.date(undefined, "DD").isValid());
        assert.ok(!adone.date(undefined, ["MM", "DD"]).isValid());
    });
    ...
    it("2 digit year with YYYY format", () => {
        assert.equal(adone.date("9/2/99", "D/M/YYYY").format("DD/MM/YYYY"), "09/02/1999", "D/M/YYYY ---> 9/2/99");
        assert.equal(adone.date("9/2/1999", "D/M/YYYY").format("DD/MM/YYYY"), "09/02/1999", "D/M/YYYY ---> 9/2/1999");
        assert.equal(adone.date("9/2/68", "D/M/YYYY").format("DD/MM/YYYY"), "09/02/2068", "D/M/YYYY ---> 9/2/68");
        assert.equal(adone.date("9/2/69", "D/M/YYYY").format("DD/MM/YYYY"), "09/02/1969", "D/M/YYYY ---> 9/2/69");
    });
    ...
    it("unix offset milliseconds", () => {
        assert.equal(adone.date("1234567890123", "x").valueOf(), 1234567890123, "x matches unix offset in milliseconds");
    });

    it("milliseconds format", () => {
        assert.equal(adone.date("1", "S").get("ms"), 100, "deciseconds");
        // assert.equal(adone.date('10', 'S', true).isValid(), false, 'deciseconds with two digits');
        // assert.equal(adone.date('1', 'SS', true).isValid(), false, 'centiseconds with one digits');
        assert.equal(adone.date("12", "SS").get("ms"), 120, "centiseconds");
        // assert.equal(adone.date('123', 'SS', true).isValid(), false, 'centiseconds with three digits');
        assert.equal(adone.date("123", "SSS").get("ms"), 123, "milliseconds");
        assert.equal(adone.date("1234", "SSSS").get("ms"), 123, "milliseconds with SSSS");
        assert.equal(adone.date("123456789101112", "SSSS").get("ms"), 123, "milliseconds with SSSS");
    });
    ...
    it("string with format (timezone)", () => {
        assert.equal(adone.date("5 -0700", "H ZZ").toDate().getUTCHours(), 12, "parse hours '5 -0700' ---> 'H ZZ'");
        assert.equal(adone.date("5 -07:00", "H Z").toDate().getUTCHours(), 12, "parse hours '5 -07:00' ---> 'H Z'");
        assert.equal(adone.date("5 -0730", "H ZZ").toDate().getUTCMinutes(), 30, "parse hours '5 -0730' ---> 'H ZZ'");
        assert.equal(adone.date("5 -07:30", "H Z").toDate().getUTCMinutes(), 30, "parse hours '5 -07:0' ---> 'H Z'");
        assert.equal(adone.date("5 +0100", "H ZZ").toDate().getUTCHours(), 4, "parse hours '5 +0100' ---> 'H ZZ'");
        assert.equal(adone.date("5 +01:00", "H Z").toDate().getUTCHours(), 4, "parse hours '5 +01:00' ---> 'H Z'");
        assert.equal(adone.date("5 +0130", "H ZZ").toDate().getUTCMinutes(), 30, "parse hours '5 +0130' ---> 'H ZZ'");
        assert.equal(adone.date("5 +01:30", "H Z").toDate().getUTCMinutes(), 30, "parse hours '5 +01:30' ---> 'H Z'");
    });
    ...
    it("string with format (timezone offset)", () => {
        ...
        const e = adone.date.utc("Fri, 20 Jul 2012 17:15:00", "ddd, DD MMM YYYY HH:mm:ss");
        const f = adone.date.utc("Fri, 20 Jul 2012 10:15:00 -0700", "ddd, DD MMM YYYY HH:mm:ss ZZ");
        assert.equal(e.hours(), f.hours(), "parse timezone offset in utc");
    });

    it("string with timezone around start of year", () => {
        assert.equal(adone.date("2000-01-01T00:00:00.000+01:00").toISOString(), "1999-12-31T23:00:00.000Z", "+1:00 around 2000");
        assert.equal(adone.date("2000-01-01T00:00:00.000-01:00").toISOString(), "2000-01-01T01:00:00.000Z", "-1:00 around 2000");
        assert.equal(adone.date("1970-01-01T00:00:00.000+01:00").toISOString(), "1969-12-31T23:00:00.000Z", "+1:00 around 1970");
        assert.equal(adone.date("1970-01-01T00:00:00.000-01:00").toISOString(), "1970-01-01T01:00:00.000Z", "-1:00 around 1970");
        assert.equal(adone.date("1200-01-01T00:00:00.000+01:00").toISOString(), "1199-12-31T23:00:00.000Z", "+1:00 around 1200");
        assert.equal(adone.date("1200-01-01T00:00:00.000-01:00").toISOString(), "1200-01-01T01:00:00.000Z", "-1:00 around 1200");
    });
    ...
    it("string with array of formats", () => {
        ...
        assert.equal(adone.date("1999-02-11", ["MM/DD/YYYY", "YYYY MM DD", "MM-DD-YYYY"]).format("MM DD YYYY"), "02 11 1999", "year first");

        assert.equal(adone.date("02-11-1999", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"), "02 11 1999", "year last");
        assert.equal(adone.date("1999-02-11", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"), "02 11 1999", "year first");
        assert.equal(adone.date("02-11-1999", ["YYYY MM DD", "MM/DD/YYYY"]).format("MM DD YYYY"), "02 11 1999", "year last");
        assert.equal(adone.date("1999-02-11", ["YYYY MM DD", "MM/DD/YYYY"]).format("MM DD YYYY"), "02 11 1999", "year first");

        assert.equal(adone.date("13-11-1999", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"), "11 13 1999", "second must be month");
        assert.equal(adone.date("11-13-1999", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"), "11 13 1999", "first must be month");
        assert.equal(adone.date("01-02-2000", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"), "01 02 2000", "either can be a month, month first format");
        assert.equal(adone.date("02-01-2000", ["DD/MM/YYYY", "MM/DD/YYYY"]).format("MM DD YYYY"), "01 02 2000", "either can be a month, day first format");
        ...
        assert.equal(adone.date("gibberish", []).format("MM DD YYYY"), "Invalid date", "doest throw for an empty array");

        assert.equal(adone.date(
        ...
        assert.equal(adone.date("11-02-10", ["MM.DD.YY", "DD-MM-YY"]).format("MM DD YYYY"), "02 11 2010", "escape RegExp special characters on comparing");

        assert.equal(adone.date("13-10-98", ["DD MM YY", "DD MM YYYY"])._f, "DD MM YY", "use two digit year");
        assert.equal(adone.date("13-10-1998", ["DD MM YY", "DD MM YYYY"])._f, "DD MM YYYY", "use four digit year");
    ...
    it("string with array of formats + ISO", () => {
        ...
        assert.equal(adone.date("2014-05-05", [adone.date.ISO_8601, "YYYY-MM-DD"]).parsingFlags().iso, true, "iso: edge case array precedence iso");
        assert.equal(adone.date("2014-05-05", ["YYYY-MM-DD", adone.date.ISO_8601]).parsingFlags().iso, false, "iso: edge case array precedence not iso");
    });

    it("string with format - years", () => {
        assert.equal(adone.date("67", "YY").format("YYYY"), "2067", "67 > 2067");
        assert.equal(adone.date("68", "YY").format("YYYY"), "2068", "68 > 2068");
        assert.equal(adone.date("69", "YY").format("YYYY"), "1969", "69 > 1969");
        assert.equal(adone.date("70", "YY").format("YYYY"), "1970", "70 > 1970");
    });

    it("implicit cloning", () => {
        const momentA = adone.date([2011, 10, 10]);
        const momentB = adone.date(momentA);
        momentA.month(5);
        assert.equal(momentB.month(), 10, "Calling adone.date() on a adone.date will create a clone");
        assert.equal(momentA.month(), 5, "Calling adone.date() on a adone.date will create a clone");
    });
    ...
    it("cloning carrying over utc mode", () => {
        assert.equal(adone.date().local().clone()._isUTC, false, "An explicit cloned local adone.date should have _isUTC == false");
        assert.equal(adone.date().utc().clone()._isUTC, true, "An cloned utc adone.date should have _isUTC == true");
        assert.equal(adone.date().clone()._isUTC, false, "An explicit cloned local adone.date should have _isUTC == false");
        assert.equal(adone.date.utc().clone()._isUTC, true, "An explicit cloned utc adone.date should have _isUTC == true");
        assert.equal(adone.date(adone.date().local())._isUTC, false, "An implicit cloned local adone.date should have _isUTC == false");
        assert.equal(adone.date(adone.date().utc())._isUTC, true, "An implicit cloned utc adone.date should have _isUTC == true");
        assert.equal(adone.date(adone.date())._isUTC, false, "An implicit cloned local adone.date should have _isUTC == false");
        assert.equal(adone.date(adone.date.utc())._isUTC, true, "An implicit cloned utc adone.date should have _isUTC == true");
    });
    ...
    it("non iso 8601 strings", () => {
        assert.ok(!adone.date("2015-10T10:15", adone.date.ISO_8601, true).isValid(), "incomplete date with time");
        assert.ok(!adone.date("2015-W10T10:15", adone.date.ISO_8601, true).isValid(), "incomplete week date with time");
        assert.ok(!adone.date("201510", adone.date.ISO_8601, true).isValid(), "basic YYYYMM is not allowed");
        assert.ok(!adone.date("2015W10T1015", adone.date.ISO_8601, true).isValid(), "incomplete week date with time (basic)");
        assert.ok(!adone.date("2015-10-08T1015", adone.date.ISO_8601, true).isValid(), "mixing extended and basic format");
        assert.ok(!adone.date("20151008T10:15", adone.date.ISO_8601, true).isValid(), "mixing basic and extended format");
        assert.ok(!adone.date("2015-10-1", adone.date.ISO_8601, true).isValid(), "missing zero padding for day");
    });

    it("parsing iso week year/week/weekday", () => {
        assert.equal(adone.date.utc("2007-W01").format(), "2007-01-01T00:00:00Z", "2008 week 1 (1st Jan Mon)");
        assert.equal(adone.date.utc("2008-W01").format(), "2007-12-31T00:00:00Z", "2008 week 1 (1st Jan Tue)");
        assert.equal(adone.date.utc("2003-W01").format(), "2002-12-30T00:00:00Z", "2008 week 1 (1st Jan Wed)");
        assert.equal(adone.date.utc("2009-W01").format(), "2008-12-29T00:00:00Z", "2009 week 1 (1st Jan Thu)");
        assert.equal(adone.date.utc("2010-W01").format(), "2010-01-04T00:00:00Z", "2010 week 1 (1st Jan Fri)");
        assert.equal(adone.date.utc("2011-W01").format(), "2011-01-03T00:00:00Z", "2011 week 1 (1st Jan Sat)");
        assert.equal(adone.date.utc("2012-W01").format(), "2012-01-02T00:00:00Z", "2012 week 1 (1st Jan Sun)");
    });
    ...
    it("parsing week year/week/weekday (dow 1, doy 4)", () => {
        ...
        assert.equal(adone.date.utc("2003-01", "gggg-ww").format(), "2002-12-30T00:00:00Z", "2003 week 1 (1st Jan Wed)");
        assert.equal(adone.date.utc("2009-01", "gggg-ww").format(), "2008-12-29T00:00:00Z", "2009 week 1 (1st Jan Thu)");
        assert.equal(adone.date.utc("2010-01", "gggg-ww").format(), "2010-01-04T00:00:00Z", "2010 week 1 (1st Jan Fri)");
        assert.equal(adone.date.utc("2011-01", "gggg-ww").format(), "2011-01-03T00:00:00Z", "2011 week 1 (1st Jan Sat)");
        assert.equal(adone.date.utc("2012-01", "gggg-ww").format(), "2012-01-02T00:00:00Z", "2012 week 1 (1st Jan Sun)");
    ...
    it("parsing week year/week/weekday (dow 1, doy 7)", () => {
        ...
        assert.equal(adone.date.utc("2003-01", "gggg-ww").format(), "2002-12-30T00:00:00Z", "2003 week 1 (1st Jan Wed)");
        assert.equal(adone.date.utc("2009-01", "gggg-ww").format(), "2008-12-29T00:00:00Z", "2009 week 1 (1st Jan Thu)");
        assert.equal(adone.date.utc("2010-01", "gggg-ww").format(), "2009-12-28T00:00:00Z", "2010 week 1 (1st Jan Fri)");
        assert.equal(adone.date.utc("2011-01", "gggg-ww").format(), "2010-12-27T00:00:00Z", "2011 week 1 (1st Jan Sat)");
        assert.equal(adone.date.utc("2012-01", "gggg-ww").format(), "2011-12-26T00:00:00Z", "2012 week 1 (1st Jan Sun)");
        adone.date.defineLocale("dow:1,doy:7", null);
    });
    ...
    it("parsing week year/week/weekday (dow 0, doy 6)", () => {
        ...
        assert.equal(adone.date.utc("2003-01", "gggg-ww").format(), "2002-12-29T00:00:00Z", "2003 week 1 (1st Jan Wed)");
        assert.equal(adone.date.utc("2009-01", "gggg-ww").format(), "2008-12-28T00:00:00Z", "2009 week 1 (1st Jan Thu)");
        assert.equal(adone.date.utc("2010-01", "gggg-ww").format(), "2009-12-27T00:00:00Z", "2010 week 1 (1st Jan Fri)");
        assert.equal(adone.date.utc("2011-01", "gggg-ww").format(), "2010-12-26T00:00:00Z", "2011 week 1 (1st Jan Sat)");
        assert.equal(adone.date.utc("2012-01", "gggg-ww").format(), "2012-01-01T00:00:00Z", "2012 week 1 (1st Jan Sun)");
        adone.date.defineLocale("dow:0,doy:6", null);
    });
    ...
    it("parsing week year/week/weekday (dow 6, doy 12)", () => {
        ...
        assert.equal(adone.date.utc("2003-01", "gggg-ww").format(), "2002-12-28T00:00:00Z", "2003 week 1 (1st Jan Wed)");
        assert.equal(adone.date.utc("2009-01", "gggg-ww").format(), "2008-12-27T00:00:00Z", "2009 week 1 (1st Jan Thu)");
        assert.equal(adone.date.utc("2010-01", "gggg-ww").format(), "2009-12-26T00:00:00Z", "2010 week 1 (1st Jan Fri)");
        assert.equal(adone.date.utc("2011-01", "gggg-ww").format(), "2011-01-01T00:00:00Z", "2011 week 1 (1st Jan Sat)");
        assert.equal(adone.date.utc("2012-01", "gggg-ww").format(), "2011-12-31T00:00:00Z", "2012 week 1 (1st Jan Sun)");
        adone.date.defineLocale("dow:6,doy:12", null);
    });
    ...
    it("parsing iso with T", () => {
        assert.equal(adone.date("2011-10-08T18")._f, "YYYY-MM-DDTHH", "should include 'T' in the format");
        assert.equal(adone.date("2011-10-08T18:20")._f, "YYYY-MM-DDTHH:mm", "should include 'T' in the format");
        assert.equal(adone.date("2011-10-08T18:20:13")._f, "YYYY-MM-DDTHH:mm:ss", "should include 'T' in the format");
        assert.equal(adone.date("2011-10-08T18:20:13.321")._f, "YYYY-MM-DDTHH:mm:ss.SSSS", "should include 'T' in the format");

        assert.equal(adone.date("2011-10-08 18")._f, "YYYY-MM-DD HH", "should not include 'T' in the format");
        assert.equal(adone.date("2011-10-08 18:20")._f, "YYYY-MM-DD HH:mm", "should not include 'T' in the format");
        assert.equal(adone.date("2011-10-08 18:20:13")._f, "YYYY-MM-DD HH:mm:ss", "should not include 'T' in the format");
        assert.equal(adone.date("2011-10-08 18:20:13.321")._f, "YYYY-MM-DD HH:mm:ss.SSSS", "should not include 'T' in the format");
    });
    ...
    it("parsing iso Z timezone into local", () => {
        const m = adone.date("2011-10-08T18:04:20.111Z");

        assert.equal(m.utc().format("YYYY-MM-DDTHH:mm:ss.SSS"), "2011-10-08T18:04:20.111", "adone.date should be able to parse ISO 2011-10-08T18:04:20.111Z");
    });

    it("parsing iso with more subsecond precision digits", () => {
        assert.equal(adone.date.utc("2013-07-31T22:00:00.0000000Z").format(), "2013-07-31T22:00:00Z", "more than 3 subsecond digits");
    });

    it("null or empty", () => {
        assert.equal(adone.date("").isValid(), false, "adone.date('') is not valid");
        assert.equal(adone.date(null).isValid(), false, "adone.date(null) is not valid");
        assert.equal(adone.date(null, "YYYY-MM-DD").isValid(), false, "adone.date('', 'format') is not valid");
        assert.equal(adone.date("", "YYYY-MM-DD").isValid(), false, "adone.date('', 'format') is not valid");
        assert.equal(adone.date.utc("").isValid(), false, "adone.date.utc('') is not valid");
        assert.equal(adone.date.utc(null).isValid(), false, "adone.date.utc(null) is not valid");
        assert.equal(adone.date.utc(null, "YYYY-MM-DD").isValid(), false, "adone.date.utc(null) is not valid");
        assert.equal(adone.date.utc("", "YYYY-MM-DD").isValid(), false, "adone.date.utc('', 'YYYY-MM-DD') is not valid");
    });

    it("first century", () => {
        assert.equal(adone.date([0, 0, 1]).format("YYYY-MM-DD"), "0000-01-01", "Year AD 0");
        assert.equal(adone.date([99, 0, 1]).format("YYYY-MM-DD"), "0099-01-01", "Year AD 99");
        assert.equal(adone.date([999, 0, 1]).format("YYYY-MM-DD"), "0999-01-01", "Year AD 999");
        assert.equal(adone.date("0 1 1", "YYYY MM DD").format("YYYY-MM-DD"), "0000-01-01", "Year AD 0");
        assert.equal(adone.date("999 1 1", "YYYY MM DD").format("YYYY-MM-DD"), "0999-01-01", "Year AD 999");
        assert.equal(adone.date("0 1 1", "YYYYY MM DD").format("YYYYY-MM-DD"), "00000-01-01", "Year AD 0");
        assert.equal(adone.date("99 1 1", "YYYYY MM DD").format("YYYYY-MM-DD"), "00099-01-01", "Year AD 99");
        assert.equal(adone.date("999 1 1", "YYYYY MM DD").format("YYYYY-MM-DD"), "00999-01-01", "Year AD 999");
    });
    ...
    it("negative four digit years", () => {
        assert.equal(adone.date("-1000-01-01", "YYYYY-MM-DD").toDate().getFullYear(), -1000, "parse BC 1,001");
        assert.equal(adone.date.utc("-1000-01-01", "YYYYY-MM-DD").toDate().getUTCFullYear(), -1000, "parse utc BC 1,001");
    });

    it("strict parsing", () => {
        assert.equal(adone.date("2014-", "YYYY-Q", true).isValid(), false, "fail missing quarter");

        assert.equal(adone.date("2012-05", "YYYY-MM", true).format("YYYY-MM"), "2012-05", "parse correct string");
        assert.equal(adone.date(" 2012-05", "YYYY-MM", true).isValid(), false, "fail on extra whitespace");
        assert.equal(adone.date("foo 2012-05", "[foo] YYYY-MM", true).format("YYYY-MM"), "2012-05", "handle fixed text");
        assert.equal(adone.date("2012 05", "YYYY-MM", true).isValid(), false, "fail on different separator");
        assert.equal(adone.date("2012 05", "YYYY MM DD", true).isValid(), false, "fail on too many tokens");

        assert.equal(adone.date("05 30 2010", ["DD MM YYYY", "MM DD YYYY"], true).format("MM DD YYYY"), "05 30 2010", "array with bad date");
        assert.equal(adone.date("05 30 2010", ["", "MM DD YYYY"], true).format("MM DD YYYY"), "05 30 2010", "array with invalid format");
        assert.equal(adone.date("05 30 2010", [" DD MM YYYY", "MM DD YYYY"], true).format("MM DD YYYY"), "05 30 2010", "array with non-matching format");

        assert.equal(adone.date("2010.*...", "YYYY.*", true).isValid(), false, "invalid format with regex chars");
        assert.equal(adone.date("2010.*", "YYYY.*", true).year(), 2010, "valid format with regex chars");
        assert.equal(adone.date(".*2010.*", ".*YYYY.*", true).year(), 2010, "valid format with regex chars on both sides");

        //strict tokens
        assert.equal(adone.date("-5-05-25", "YYYY-MM-DD", true).isValid(), false, "invalid negative year");
        assert.equal(adone.date("2-05-25", "YYYY-MM-DD", true).isValid(), false, "invalid one-digit year");
        assert.equal(adone.date("20-05-25", "YYYY-MM-DD", true).isValid(), false, "invalid two-digit year");
        assert.equal(adone.date("201-05-25", "YYYY-MM-DD", true).isValid(), false, "invalid three-digit year");
        assert.equal(adone.date("2010-05-25", "YYYY-MM-DD", true).isValid(), true, "valid four-digit year");
        assert.equal(adone.date("22010-05-25", "YYYY-MM-DD", true).isValid(), false, "invalid five-digit year");

        assert.equal(adone.date("12-05-25", "YY-MM-DD", true).isValid(), true, "valid two-digit year");
        assert.equal(adone.date("2012-05-25", "YY-MM-DD", true).isValid(), false, "invalid four-digit year");

        assert.equal(adone.date("-5-05-25", "Y-MM-DD", true).isValid(), true, "valid negative year");
        assert.equal(adone.date("2-05-25", "Y-MM-DD", true).isValid(), true, "valid one-digit year");
        assert.equal(adone.date("20-05-25", "Y-MM-DD", true).isValid(), true, "valid two-digit year");
        assert.equal(adone.date("201-05-25", "Y-MM-DD", true).isValid(), true, "valid three-digit year");

        assert.equal(adone.date("2012-5-25", "YYYY-M-DD", true).isValid(), true, "valid one-digit month");
        assert.equal(adone.date("2012-5-25", "YYYY-MM-DD", true).isValid(), false, "invalid one-digit month");
        assert.equal(adone.date("2012-05-25", "YYYY-M-DD", true).isValid(), true, "valid one-digit month");
        assert.equal(adone.date("2012-05-25", "YYYY-MM-DD", true).isValid(), true, "valid one-digit month");

        assert.equal(adone.date("2012-05-2", "YYYY-MM-D", true).isValid(), true, "valid one-digit day");
        assert.equal(adone.date("2012-05-2", "YYYY-MM-DD", true).isValid(), false, "invalid one-digit day");
        assert.equal(adone.date("2012-05-02", "YYYY-MM-D", true).isValid(), true, "valid two-digit day");
        assert.equal(adone.date("2012-05-02", "YYYY-MM-DD", true).isValid(), true, "valid two-digit day");

        assert.equal(adone.date("+002012-05-25", "YYYYY-MM-DD", true).isValid(), true, "valid six-digit year");
        assert.equal(adone.date("+2012-05-25", "YYYYY-MM-DD", true).isValid(), false, "invalid four-digit year");

        //thse are kinda pointless, but they should work as expected
        assert.equal(adone.date("1", "S", true).isValid(), true, "valid one-digit milisecond");
        assert.equal(adone.date("12", "S", true).isValid(), false, "invalid two-digit milisecond");
        assert.equal(adone.date("123", "S", true).isValid(), false, "invalid three-digit milisecond");

        assert.equal(adone.date("1", "SS", true).isValid(), false, "invalid one-digit milisecond");
        assert.equal(adone.date("12", "SS", true).isValid(), true, "valid two-digit milisecond");
        assert.equal(adone.date("123", "SS", true).isValid(), false, "invalid three-digit milisecond");

        assert.equal(adone.date("1", "SSS", true).isValid(), false, "invalid one-digit milisecond");
        assert.equal(adone.date("12", "SSS", true).isValid(), false, "invalid two-digit milisecond");
        assert.equal(adone.date("123", "SSS", true).isValid(), true, "valid three-digit milisecond");

        // strict parsing respects month length
        assert.ok(adone.date("1 January 2000", "D MMMM YYYY", true).isValid(), "capital long-month + MMMM");
        assert.ok(!adone.date("1 January 2000", "D MMM YYYY", true).isValid(), "capital long-month + MMM");
        assert.ok(!adone.date("1 Jan 2000", "D MMMM YYYY", true).isValid(), "capital short-month + MMMM");
        assert.ok(adone.date("1 Jan 2000", "D MMM YYYY", true).isValid(), "capital short-month + MMM");
        assert.ok(adone.date("1 january 2000", "D MMMM YYYY", true).isValid(), "lower long-month + MMMM");
        assert.ok(!adone.date("1 january 2000", "D MMM YYYY", true).isValid(), "lower long-month + MMM");
        assert.ok(!adone.date("1 jan 2000", "D MMMM YYYY", true).isValid(), "lower short-month + MMMM");
        assert.ok(adone.date("1 jan 2000", "D MMM YYYY", true).isValid(), "lower short-month + MMM");
    });

    it("parsing into a locale", () => {
        adone.date.defineLocale("parselocale", {
        ...
        assert.equal(adone.date("2012 seven", "YYYY MMM", "parselocale").month(), 6, "should be able to parse in a specific locale");

        adone.date.locale("parselocale");

        assert.equal(adone.date("2012 july", "YYYY MMM", "en").month(), 6, "should be able to parse in a specific locale");

        adone.date.defineLocale("parselocale", null);
    });
    ...
    it("parsing week and weekday information", () => {
        ...
        // lower-order only
        assert.equal(adone.date("22", "ww").week(), 22, "week sets the week by itself");
        assert.equal(adone.date("22", "ww").weekYear(), adone.date().weekYear(), "week keeps this year");
        assert.equal(adone.date("2012 22", "YYYY ww").weekYear(), 2012, "week keeps parsed year");

        assert.equal(adone.date("22", "WW").isoWeek(), 22, "iso week sets the week by itself");
        assert.equal(adone.date("2012 22", "YYYY WW").weekYear(), 2012, "iso week keeps parsed year");
        assert.equal(adone.date("22", "WW").isoWeekYear(), adone.date().isoWeekYear(), "iso week keeps this year");
        ...
        assert.equal(adone.date("1999-W37-4 3:30", "GGGG-[W]WW-E HH:mm").format("YYYY MM DD HH:mm"), "1999 09 16 03:30", "parsing weeks and hours");
    ...
    it("parsing with customized two-digit year", () => {
        const original = adone.date.parseTwoDigitYear;
        try {
    ...
    it("array with strings", () => {
        assert.equal(adone.date(["2014", "7", "31"]).isValid(), true, "string array + isValid");
    });
    ...
    it("utc with array of formats", () => {
        assert.equal(adone.date.utc("2014-01-01", ["YYYY-MM-DD", "YYYY-MM"]).format(), "2014-01-01T00:00:00Z", "adone.date.utc works with array of formats");
    });
    ...
    it("milliseconds", () => {
        assert.equal(adone.date("1", "S").millisecond(), 100);
        assert.equal(adone.date("12", "SS").millisecond(), 120);
        assert.equal(adone.date("123", "SSS").millisecond(), 123);
        assert.equal(adone.date("1234", "SSSS").millisecond(), 123);
        assert.equal(adone.date("12345", "SSSSS").millisecond(), 123);
        assert.equal(adone.date("123456", "SSSSSS").millisecond(), 123);
        assert.equal(adone.date("1234567", "SSSSSSS").millisecond(), 123);
        assert.equal(adone.date("12345678", "SSSSSSSS").millisecond(), 123);
        assert.equal(adone.date("123456789", "SSSSSSSSS").millisecond(), 123);
    });

    it("hmm", () => {
        assert.equal(adone.date("123", "hmm", true).format("HH:mm:ss"), "01:23:00", "123 with hmm");
        assert.equal(adone.date("123a", "hmmA", true).format("HH:mm:ss"), "01:23:00", "123a with hmmA");
        assert.equal(adone.date("123p", "hmmA", true).format("HH:mm:ss"), "13:23:00", "123p with hmmA");

        assert.equal(adone.date("1234", "hmm", true).format("HH:mm:ss"), "12:34:00", "1234 with hmm");
        assert.equal(adone.date("1234a", "hmmA", true).format("HH:mm:ss"), "00:34:00", "1234a with hmmA");
        assert.equal(adone.date("1234p", "hmmA", true).format("HH:mm:ss"), "12:34:00", "1234p with hmmA");

        assert.equal(adone.date("12345", "hmmss", true).format("HH:mm:ss"), "01:23:45", "12345 with hmmss");
        assert.equal(adone.date("12345a", "hmmssA", true).format("HH:mm:ss"), "01:23:45", "12345a with hmmssA");
        assert.equal(adone.date("12345p", "hmmssA", true).format("HH:mm:ss"), "13:23:45", "12345p with hmmssA");
        assert.equal(adone.date("112345", "hmmss", true).format("HH:mm:ss"), "11:23:45", "112345 with hmmss");
        assert.equal(adone.date("112345a", "hmmssA", true).format("HH:mm:ss"), "11:23:45", "112345a with hmmssA");
        assert.equal(adone.date("112345p", "hmmssA", true).format("HH:mm:ss"), "23:23:45", "112345p with hmmssA");

        assert.equal(adone.date("023", "Hmm", true).format("HH:mm:ss"), "00:23:00", "023 with Hmm");
        assert.equal(adone.date("123", "Hmm", true).format("HH:mm:ss"), "01:23:00", "123 with Hmm");
        assert.equal(adone.date("1234", "Hmm", true).format("HH:mm:ss"), "12:34:00", "1234 with Hmm");
        assert.equal(adone.date("1534", "Hmm", true).format("HH:mm:ss"), "15:34:00", "1234 with Hmm");
        assert.equal(adone.date("12345", "Hmmss", true).format("HH:mm:ss"), "01:23:45", "12345 with Hmmss");
        assert.equal(adone.date("112345", "Hmmss", true).format("HH:mm:ss"), "11:23:45", "112345 with Hmmss");
        assert.equal(adone.date("172345", "Hmmss", true).format("HH:mm:ss"), "17:23:45", "112345 with Hmmss");
    });

    it("Y token", () => {
        assert.equal(adone.date("1-1-2010", "M-D-Y", true).year(), 2010, "parsing Y");
    });

    it("parsing flags retain parsed date parts", () => {
        const a = adone.date("10 p", "hh:mm a");
        ...
        const b = adone.date("10:30", ["MMDDYY", "HH:mm"]);
    ...
    it("parsing only meridiem results in invalid date", () => {
        assert.ok(!adone.date("alkj", "hh:mm a").isValid(), "because an a token is used, a meridiem will be parsed but nothing else was so invalid");
        assert.ok(adone.date("02:30 p more extra stuff", "hh:mm a").isValid(), "because other tokens were parsed, date is valid");
        assert.ok(adone.date("1/1/2016 extra data", ["a", "M/D/YYYY"]).isValid(), "took second format, does not pick up on meridiem parsed from first format (good copy)");
    });
