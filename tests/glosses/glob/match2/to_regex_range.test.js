...
const matchRange = (min, max, expected, match, notMatch) => {
    ...
    const actual = toRegexRange(min, max);
...
describe("toRegexRange", () => {
    after(() => {
        console.log();
        console.log("   ", (Number(Number(count.toFixed(2)))).toLocaleString(), "assertions");
    });

    describe("range", () => {
        it("should throw an error when the first arg is invalid:", () => {
            assert.throws(() => toRegexRange(), /expected/);
        });

        it("should throw an error when the second arg is invalid:", () => {
            assert.throws(() => toRegexRange(1, {}), /expected/);
        });

        it("should match the given numbers", () => {
            const oneFifty = toRegex(toRegexRange(1, 150));
            ...
            const oneTwentyFive = toRegex(toRegexRange(1, 125));
    ...
    describe("minimum / maximum", () => {
        it("should reverse `min/max` when the min is larger than the max:", () => {
            assert.equal(toRegexRange(55, 10), "(?:1[0-9]|[2-4][0-9]|5[0-5])");
        });
    });
    ...
    describe("ranges", () => {
        ...
        it("should return a single number when both numbers are equal", () => {
            ...
            assert.equal(toRegexRange("65443", "65443"), "65443");
            assert.equal(toRegexRange("192", "192"), "192");
        ...
        it("should return regex character classes when both args are less than 10", () => {
            assert.equal(toRegexRange(0, 9), "[0-9]");
            assert.equal(toRegexRange(1, 5), "[1-5]");
            assert.equal(toRegexRange(1, 7), "[1-7]");
            assert.equal(toRegexRange(2, 6), "[2-6]");
        });

        it("should support string numbers", () => {
            assert.equal(toRegexRange("1", "5"), "[1-5]");
            assert.equal(toRegexRange("10", "50"), "(?:1[0-9]|[2-4][0-9]|50)");
        });

        it("should support padded ranges:", () => {
            assert.equal(toRegexRange("001", "005"), "0{0,2}[1-5]");
            assert.equal(toRegexRange("01", "05"), "0?[1-5]");
            assert.equal(toRegexRange("001", "100"), "(?:0{0,2}[1-9]|0?[1-9][0-9]|100)");
            assert.equal(toRegexRange("0001", "1000"), "(?:0{0,3}[1-9]|0{0,2}[1-9][0-9]|0?[1-9][0-9]{2}|1000)");
        });

        it("should work when padding is imbalanced:", () => {
            assert.equal(toRegexRange("001", "105"), "(?:0{0,2}[1-9]|0?[1-9][0-9]|10[0-5])");
            assert.equal(toRegexRange("01", "105"), "(?:0{0,2}[1-9]|0?[1-9][0-9]|10[0-5])");
            assert.equal(toRegexRange("010", "105"), "(?:0?1[0-9]|0?[2-9][0-9]|10[0-5])");
            assert.equal(toRegexRange("010", "1005"), "(?:0{0,2}1[0-9]|0{0,2}[2-9][0-9]|0?[1-9][0-9]{2}|100[0-5])");
            assert.equal(toRegexRange("0001", "1000"), toRegexRange("001", "1000"));
            assert.equal(toRegexRange("0001", "1000"), toRegexRange("01", "1000"));
        });

        it("should generate regex strings for negative patterns", () => {
            assert.equal(toRegexRange(-1, 0), "(?:-1|0)");
            assert.equal(toRegexRange(-1, 1), "(?:-1|[01])");
            assert.equal(toRegexRange(-4, -2), "-[2-4]");
            assert.equal(toRegexRange(-3, 1), "(?:-[1-3]|[01])");
            assert.equal(toRegexRange(-2, 0), "(?:-[12]|0)");
        ...
        it("should wrap patterns when options.capture is true", () => {
            assert.equal(toRegexRange(-1, 0, { capture: true }), "(-1|0)");
            assert.equal(toRegexRange(-1, 1, { capture: true }), "(-1|[01])");
            assert.equal(toRegexRange(-4, -2, { capture: true }), "(-[2-4])");
            assert.equal(toRegexRange(-3, 1, { capture: true }), "(-[1-3]|[01])");
            assert.equal(toRegexRange(-2, 0, { capture: true }), "(-[12]|0)");
            assert.equal(toRegexRange(-1, 3, { capture: true }), "(-1|[0-3])");
        });
        ...
        it("should generate regex strings for positive patterns", () => {
            ...
            assert.equal(toRegexRange(65666, 65667), "(?:65666|65667)");
            assert.equal(toRegexRange(12, 3456), "(?:1[2-9]|[2-9][0-9]|[1-9][0-9]{2}|[12][0-9]{3}|3[0-3][0-9]{2}|34[0-4][0-9]|345[0-6])");
            assert.equal(toRegexRange(1, 3456), "(?:[1-9]|[1-9][0-9]{1,2}|[12][0-9]{3}|3[0-3][0-9]{2}|34[0-4][0-9]|345[0-6])");
            assert.equal(toRegexRange(1, 10), "(?:[1-9]|10)");
            assert.equal(toRegexRange(1, 19), "(?:[1-9]|1[0-9])");
            assert.equal(toRegexRange(1, 99), "(?:[1-9]|[1-9][0-9])");
            assert.equal(toRegexRange(1, 100), "(?:[1-9]|[1-9][0-9]|100)");
            assert.equal(toRegexRange(1, 1000), "(?:[1-9]|[1-9][0-9]{1,2}|1000)");
            assert.equal(toRegexRange(1, 10000), "(?:[1-9]|[1-9][0-9]{1,3}|10000)");
            assert.equal(toRegexRange(1, 100000), "(?:[1-9]|[1-9][0-9]{1,4}|100000)");
            assert.equal(toRegexRange(1, 9999999), "(?:[1-9]|[1-9][0-9]{1,6})");
            assert.equal(toRegexRange(99, 100000), "(?:99|[1-9][0-9]{2,4}|100000)");
        ...
        it("should optimize regexes", () => {
            assert.equal(toRegexRange(-9, 9), "(?:-[1-9]|[0-9])");
            assert.equal(toRegexRange(-19, 19), "(?:-[1-9]|-?1[0-9]|[0-9])");
            assert.equal(toRegexRange(-29, 29), "(?:-[1-9]|-?[12][0-9]|[0-9])");
            assert.equal(toRegexRange(-99, 99), "(?:-[1-9]|-?[1-9][0-9]|[0-9])");
            assert.equal(toRegexRange(-999, 999), "(?:-[1-9]|-?[1-9][0-9]{1,2}|[0-9])");
            assert.equal(toRegexRange(-9999, 9999), "(?:-[1-9]|-?[1-9][0-9]{1,3}|[0-9])");
            assert.equal(toRegexRange(-99999, 99999), "(?:-[1-9]|-?[1-9][0-9]{1,4}|[0-9])");
        });
    });

    describe("validate ranges", () => {
        it("should match all numbers in the given range", () => {
            const isMatch = matcher(1, 59);
            for (let i = 0; i < 100; i++) {
                if (i >= 1 && i <= 59) {
                    assert(isMatch(i));
                } else {
