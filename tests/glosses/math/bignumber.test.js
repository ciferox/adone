...
describe("bignumber", () => {
    describe("big", () => {
        it("create", function () {
            expect(BigNumber(1337).toString()).to.be.deep.equal("1337");
            expect(BigNumber("1337").toString()).to.be.deep.equal("1337");
            ...
            expect(Number(BigNumber("1e+100").toString())).to.be.deep.equal(1e+100);
            expect(Number(BigNumber("1e+100").bitLength())).to.be.deep.equal(333);
            expect(Number(BigNumber("1.23e+45").toString())).to.be.deep.equal(1.23e+45);

            for (let i = 0; i < 10; i++) {
                expect(BigNumber("1.23456e+" + i).toString()).to.be.deep.equal("" + Math.floor(1.23456 * Math.pow(10, i)));
            }

            expect(BigNumber("1.23e-45").toString()).to.be.deep.equal("0");
        ...
        it("add", function () {
            for (let i = -10; i < 10; i++) {
                for (let j = -10; j < 10; j++) {
                    ...
                    expect(BigNumber(i).add(j).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).add(js).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).add(BigNumber(j)).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).add(BigNumber(j)).toString()).to.be.deep.equal(ks);
            ...
            expect(
                BigNumber("201781752444966478956292456789265633588628356858680927185287861892" + 
        ...
        it("sub", function () {
            for (let i = -10; i < 10; i++) {
                for (let j = -10; j < 10; j++) {
                    const js = j.toString();
                    const ks = (i - j).toString();
                    expect(BigNumber(i).sub(j).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).sub(js).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).sub(BigNumber(j)).toString()).to.be.deep.equal(ks);
        ...
        it("div", function () {
            for (let i = -10; i < 10; i++) {
                for (let j = -10; j < 10; j++) {
                    ...
                    if (ks.match(/^-?\d+$/)) {
                        // ignore exceptions
                        expect(BigNumber(i).div(j).toString()).to.be.deep.equal(ks);
                        expect(BigNumber(i).div(js).toString()).to.be.deep.equal(ks);
                        expect(BigNumber(i).div(BigNumber(j)).toString()).to.be.deep.equal(ks);
        ...
        it("mod", function () {
            for (let i = 0; i < 10; i++) {
                for (let j = 0; j < 10; j++) {
                    ...
                    if (!isNaN(i % j)) {
                        const ks = (i % j).toString();
                        expect(BigNumber(i).mod(j).toString()).to.be.deep.equal(ks);
                        expect(BigNumber(i).mod(js).toString()).to.be.deep.equal(ks);
                        expect(BigNumber(i).mod(BigNumber(j)).toString()).to.be.deep.equal(ks);
                        expect(BigNumber.mod(i, j).toString()).to.be.deep.equal(ks);
                    }
                }
            }

            expect(BigNumber("486541542410442549118519277483401413").mod("1802185856709793916115771381388554").toString()).to.be.deep.equal("1753546955507985683376775889880387");
        });

        it("cmp", function () {
            for (let i = -10; i <= 10; i++) {
                const bi = BigNumber(i);

                for (let j = -10; j <= 10; j++) {
                    [j, BigNumber(j)].forEach(function (jj) {
        ...
        it("powm", function () {
            const twos = [2, "2", BigNumber(2), BigNumber("2")];
            const tens = [100000, "100000", BigNumber(100000), BigNumber(100000)];
            twos.forEach(function (two) {
                tens.forEach(function (ten) {
                    expect(BigNumber("111111111").powm(two, ten).toString()).to.be.deep.equal("54321");
                });
            });

            expect(BigNumber("624387628734576238746587435").powm(2732, "457676874367586").toString()).to.be.deep.equal("335581885073251");
        });

        it("pow", function () {
            [2, "2", BigNumber(2), BigNumber("2")].forEach(function (two) {
                expect(BigNumber("111111111").pow(two).toString()).to.be.deep.equal("12345678987654321");
            });

            expect(BigNumber("3487438743234789234879").pow(22).toString()).to.be.deep.equal("");
        });
        ...
        it("and", function () {
            for (let i = 0; i < 256; i += 7) {
                for (let j = 0; j < 256; j += 7) {
                    const js = j.toString();
                    const ks = (i & j).toString();
                    expect(BigNumber(i).and(j).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).and(js).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).and(BigNumber(j)).toString()).to.be.deep.equal(ks);
                    expect(BigNumber.and(i, j).toString()).to.be.deep.equal(ks);

                    expect(BigNumber(-1 * i).and(j).toString()).to.be.deep.equal((-1 * i & j).toString());
                    expect(BigNumber(i).and(-1 * j).toString()).to.be.deep.equal((i & -1 * j).toString());
                    expect(BigNumber(-1 * i).and(-1 * j).toString()).to.be.deep.equal((-1 * i & -1 * j).toString());
                }
            }
            expect(BigNumber.and(BigNumber("111111", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111111");
            expect(BigNumber.and(BigNumber("111110", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111110");
            expect(BigNumber.and(BigNumber("111112", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111110");
            expect(BigNumber.and(BigNumber("111121", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111101");
            expect(BigNumber.and(BigNumber("111131", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111111");

            expect(BigNumber.and(BigNumber("-111111", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("01");
            expect(BigNumber.and(BigNumber("111111", 16), BigNumber("-111111", 16)).toString(16)).to.be.deep.equal("01");
            expect(BigNumber.and(BigNumber("-111111", 16), BigNumber("-111111", 16)).toString(16)).to.be.deep.equal("-111111");
        });
        ...
        it("or", function () {
            for (let i = 0; i < 256; i += 7) {
                for (let j = 0; j < 256; j += 7) {
                    const js = j.toString();
                    const ks = (i | j).toString();
                    expect(BigNumber(i).or(j).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).or(js).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).or(BigNumber(j)).toString()).to.be.deep.equal(ks);
                    expect(BigNumber.or(i, j).toString()).to.be.deep.equal(ks);

                    expect(BigNumber(-1 * i).or(j).toString()).to.be.deep.equal((-1 * i | j).toString());
                    expect(BigNumber(i).or(-1 * j).toString()).to.be.deep.equal((i | -1 * j).toString());
                    expect(BigNumber(-1 * i).or(-1 * j).toString()).to.be.deep.equal((-1 * i | -1 * j).toString());
                }
            }
            expect(BigNumber.or(BigNumber("111111", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111111");
            expect(BigNumber.or(BigNumber("111110", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111111");
            expect(BigNumber.or(BigNumber("111112", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111113");
            expect(BigNumber.or(BigNumber("111121", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("111131");

            expect(BigNumber.or(BigNumber("-111111", 16), BigNumber("111111", 16)).toString(16)).to.be.deep.equal("-01");
            expect(BigNumber.or(BigNumber("111111", 16), BigNumber("-111111", 16)).toString(16)).to.be.deep.equal("-01");
            expect(BigNumber.or(BigNumber("-111111", 16), BigNumber("-111111", 16)).toString(16)).to.be.deep.equal("-111111");
        });
        ...
        it("xor", function () {
            for (let i = 0; i < 256; i += 7) {
                for (let j = 0; j < 256; j += 7) {
                    const js = j.toString();
                    const ks = (i ^ j).toString();
                    expect(BigNumber(i).xor(j).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).xor(js).toString()).to.be.deep.equal(ks);
                    expect(BigNumber(i).xor(BigNumber(j)).toString()).to.be.deep.equal(ks);
                    expect(BigNumber.xor(i, j).toString()).to.be.deep.equal(ks);

                    expect(BigNumber(-1 * i).xor(j).toString()).to.be.deep.equal((-1 * i ^ j).toString());
                    expect(BigNumber(i).xor(-1 * j).toString()).to.be.deep.equal((i ^ -1 * j).toString());
                    expect(BigNumber(-1 * i).xor(-1 * j).toString()).to.be.deep.equal((-1 * i ^ -1 * j).toString());
                }
            }
            expect(BigNumber.xor(BigNumber("111111", 16), BigNumber("111111", 16)).toString()).to.be.deep.equal("0");
            expect(BigNumber.xor(BigNumber("111110", 16), BigNumber("111111", 16)).toString()).to.be.deep.equal("1");
            expect(BigNumber.xor(BigNumber("111112", 16), BigNumber("111111", 16)).toString()).to.be.deep.equal("3");
            expect(BigNumber.xor(BigNumber("111121", 16), BigNumber("111111", 16)).toString()).to.be.deep.equal("48");

            expect(BigNumber.xor(BigNumber("-111111", 16), BigNumber("111111", 16)).toString()).to.be.deep.equal("-2");
            expect(BigNumber.xor(BigNumber("111111", 16), BigNumber("-111111", 16)).toString()).to.be.deep.equal("-2");
            expect(BigNumber.xor(BigNumber("-111111", 16), BigNumber("-111111", 16)).toString()).to.be.deep.equal("0");
        });
        ...
        it("rand", function () {
            for (let i = 1; i < 1000; i++) {
                const x = BigNumber(i).rand().toNumber();
                expect(x >= 0 && x < i).to.be.true;

                const y = BigNumber(i).rand(i + 10).toNumber();
        ...
        it("primes", function () {
            ...
            for (let i = 0; i <= 10; i++) {
                expect(BigNumber(i).probPrime()).to.be.deep.equal(Boolean(ps[i]));
            }
            ...
            Object.keys(ns).forEach(function (n) {
                expect(BigNumber(n).nextPrime().toString()).to.be.deep.equal(ns[n].toString());
            });
            ...
            [uniques, wagstaff, big].forEach(function (xs) {
                xs.forEach(function (x) {
                    const p = BigNumber(x).probPrime();
                    expect(p === true || p === "maybe").to.be.ok;
                });
            });
        });

        it("isbitset", function () {
            function mkbin(bn) {
                let bin = "";
            ...
            expect(mkbin(BigNumber(127))).to.be.deep.equal("1111111");
            expect(mkbin(BigNumber(-127))).to.be.deep.equal("1111111");

            expect(mkbin(BigNumber(128))).to.be.deep.equal("00000001");
            expect(mkbin(BigNumber(-128))).to.be.deep.equal("00000001");

            expect(mkbin(BigNumber(129))).to.be.deep.equal("10000001");
            expect(mkbin(BigNumber(-129))).to.be.deep.equal("10000001");
        });

        it("invertm", function () {
            // numbers from http://www.itl.nist.gov/fipspubs/fip186.htm appendix 5
            const q = BigNumber("b20db0b101df0c6624fc1392ba55f77d577481e5", 16);
            const k = BigNumber("79577ddcaafddc038b865b19f8eb1ada8a2838c6", 16);
            const kinv = k.invertm(q);
            expect(kinv.toString(16)).to.be.deep.equal("2784e3d672d972a74e22c67f4f4f726ecc751efa");
        });

        it("shift", function () {
            expect(BigNumber(37).shiftLeft(2).toString()).to.be.deep.equal((37 << 2).toString()); // 148
            expect(BigNumber(37).shiftRight(2).toString()).to.be.deep.equal((37 >> 2).toString()); // 9

            expect(BigNumber(2).pow(Math.pow(2, 10)).shiftRight(4).toString()).to.be.equal(BigNumber(2).pow(Math.pow(2, 10)).div(16).toString());
        });

        it("mod", function () {
            expect(BigNumber(55555).mod(2).toString()).to.be.deep.equal("1");
            expect(BigNumber("1234567").mod(BigNumber("4321")).toNumber()).to.be.deep.equal(1234567 % 4321);
        });

        it("endian", function () {
            const a = BigNumber(0x0102030405);
            ...
            const b = BigNumber(0x0102030405);
        ...
        it("bitlength", function () {
            const bl = BigNumber("433593290010590489671135819286259593426549306666324008679782084292" + "2446494189019075159822930571858728009485237489829138626896756141" + "873895833763224917704497568647701157104426").bitLength();

            expect(bl > 0).to.be.equal(true);
        });

        it("gcd", function () {
            const b1 = BigNumber("234897235923342343242");
            const b2 = BigNumber("234790237101762305340234");
            const expected = BigNumber("6");
            expect(b1.gcd(b2).toString()).to.be.equal(expected.toString());
        });

        it("jacobi", function () {
            // test case from p. 134 of D. R. Stinson
            let b1 = BigNumber("7411");
            let b2 = BigNumber("9283");
    ...
    describe("buf", () => {
        ...
        it("bufBe", function () {
            ...
            expect(num).to.be.deep.equal(1 * Math.pow(256, 3) + 2 * Math.pow(256, 2) + 3 * 256 + 4);
        ...
        it("buf_to_from", function () {
            ...
            expect(function () {
                BigNumber(-1).toBuffer(); // can"t pack negative numbers yet
            }).to.throw(Error);
        });

        it("toBuf", function () {
            const buf = new Buffer([0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]);
            const b = BigNumber(0x0a * 256 * 256 * 256 * 256 * 256 + 0x0b * 256 * 256 * 256 * 256 + 0x0c * 256 * 256 * 256 + 0x0d * 256 * 256 + 0x0e * 256 + 0x0f);
            ...
            expect([].slice.call(BigNumber(43135012110).toBuffer({
        ...
        it("toMpint", function () {
            ...
            Object.keys(refs).forEach(function (key) {
                const buf0 = BigNumber(key, 16).toBuffer("mpint");
                const buf1 = refs[key];
    ...
    describe("isbignum", () => {
        it("create", function () {
            const validBn = BigNumber("42");
            let testObj;

            testObj = BigNumber("123");
            expect(BigNumber.isBigNumber(testObj)).to.be.true;

            testObj = {};
            expect(BigNumber.isBigNumber(testObj)).to.be.false;
