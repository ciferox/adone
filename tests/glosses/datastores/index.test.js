...
describe("datastore", () => {
    ...
    describe("Key", () => {
        ...
        describe("basic", () => {
            const validKey = (s) => it(s, () => {
                ...
                const k = new datastore.Key(s);
                expect(k.toString()).to.eql(fixed);
                expect(k).to.eql(new datastore.Key(s));
                expect(k.toString()).to.eql(new datastore.Key(s).toString());
                ...
                const child = new datastore.Key("cchildd");
        ...
        it("ancestry", () => {
            const k1 = new datastore.Key("/A/B/C");
            const k2 = new datastore.Key("/A/B/C/D");
            ...
            const checks = [
                k1.isAncestorOf(k2),
                k2.isDecendantOf(k1),
                new datastore.Key("/A").isAncestorOf(k2),
                new datastore.Key("/A").isAncestorOf(k1),
                !new datastore.Key("/A").isDecendantOf(k2),
                !new datastore.Key("/A").isDecendantOf(k1),
                k2.isDecendantOf(new datastore.Key("/A")),
                k1.isDecendantOf(new datastore.Key("/A")),
                !k2.isAncestorOf(new datastore.Key("/A")),
                !k1.isAncestorOf(new datastore.Key("/A")),
                !k2.isAncestorOf(k2),
                !k1.isAncestorOf(k1)
            ];
            ...
            expect(k1.child(new datastore.Key("D")).toString()).to.eql(k2.toString());
            expect(k1.toString()).to.eql(k2.parent().toString());
            expect(k1.path().toString()).to.eql(k2.parent().path().toString());
        });

        it("type", () => {
            const k1 = new datastore.Key("/A/B/C:c");
            const k2 = new datastore.Key("/A/B/C:c/D:d");
        ...
        it("less", () => {
            const checkLess = (a, b) => {
                const ak = new datastore.Key(a);
                const bk = new datastore.Key(b);
    ...
    describe("utils", () => {
        it("asyncFilter - sync", (done) => {
            pull(
                pull.values([1, 2, 3, 4]),
                datastore.utils.asyncFilter((val, cb) => {
                    cb(null, val % 2 === 0);
                }),
        ...
        it("asyncSort", (done) => {
            pull(
                pull.values([1, 2, 3, 4]),
                datastore.utils.asyncSort((res, cb) => {
                    setTimeout(() => {
                        cb(null, res.reverse());
                    }, 10);
                }),
        ...
        it("replaceStartWith", () => {
            expect(datastore.utils.replaceStartWith("helloworld", "hello")).to.eql("world");
