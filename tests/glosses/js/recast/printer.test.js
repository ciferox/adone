...
const linesModule = require(srcPath("lib/lines"));
...
describe("printer", () => {
    ...
    it("IffeAfterVariableDeclarationEndingInObjectLiteral", () => {
    ...
    it("ArrayExpressionWithTrailingComma", () => {
    ...
    it("MultiLineParams", () => {
        ...
        assert.strictEqual(printer.print(ast).code, multiLineParamsExpected.join(eol));
    });
    it("SimpleVarPrinting", () => {
        const printer = new Printer({ tabWidth: 2 });
        ...
        assert.strictEqual(printer.print(b.program([varDecl])).code, "var x, y, z;");
        const z = varDecl.declarations.pop();
        varDecl.declarations.pop();
        varDecl.declarations.push(z);
        assert.strictEqual(printer.print(b.program([varDecl])).code, "var x, z;");
    });
    it("MultiLineVarPrinting", () => {
        const printer = new Printer({ tabWidth: 2 });
        const varDecl = b.variableDeclaration("var", [
            b.variableDeclarator(b.identifier("x"), null),
    ...
    it("ForLoopPrinting", () => {
        const printer = new Printer({ tabWidth: 2 });
        const loop = b.forStatement(b.variableDeclaration("var", [
        ...
        assert.strictEqual(printer.print(loop).code, `for (var i = 0; i < 3; i++)${eol 
        }  log(i);`);
    });
    it("EmptyForLoopPrinting", () => {
        const printer = new Printer({ tabWidth: 2 });
        const loop = b.forStatement(b.variableDeclaration("var", [
            b.variableDeclarator(b.identifier("i"), b.literal(0))
    ...
    it("ForInLoopPrinting", () => {
        const printer = new Printer({ tabWidth: 2 });
        const loop = b.forInStatement(b.variableDeclaration("var", [
        ...
        assert.strictEqual(printer.print(loop).code, `for (var key in obj)${eol 
        }  log(key);`);
    });
    it("GuessTabWidth", () => {
        const code = [
        ...
        assert.strictEqual(new Printer({
            tabWidth: 4
        }).print(ast).code, explicitFour);
    });
    it("FunctionDefaultsAndRest", () => {
        const printer = new Printer();
        const funExpr = b.functionExpression(b.identifier("a"), [b.identifier("b"), b.identifier("c")], b.blockStatement([]), false, false);
        funExpr.defaults = [null, b.literal(1)];
        funExpr.rest = b.identifier("d");
        assert.strictEqual(printer.print(funExpr).code, "function a(b, c = 1, ...d) {}");
        const arrowFunExpr = b.arrowFunctionExpression([b.identifier("b"), b.identifier("c")], b.blockStatement([]), false);
        arrowFunExpr.defaults = [null, b.literal(1)];
        arrowFunExpr.rest = b.identifier("d");
        assert.strictEqual(printer.print(arrowFunExpr).code, "(b, c = 1, ...d) => {}");
    });
    it("generically prints parsed code and generated code the same way", () => {
        const printer = new Printer();
        const ast = b.program([
            b.expressionStatement(b.literal(1)),
            b.expressionStatement(b.literal(2))
        ]);
        assert.strictEqual(printer.printGenerically(parse("1; 2;")).code, printer.printGenerically(ast).code);
    });
    it("ExportDeclaration semicolons", () => {
        const printer = new Printer();
    ...
    it("export default of IIFE", () => {
        const printer = new Printer();
        let ast = b.exportDefaultDeclaration(b.callExpression(b.functionExpression(null, [], b.blockStatement([])), []));
    ...
    it("Statement list whitespace reuse", () => {
        ...
        const funDecl = b.functionDeclaration(b.identifier("foo"), [], b.blockStatement(ast.program.body));
        assert.strictEqual(printer.print(funDecl).code, linesModule.concat([
            `function foo() {${eol}`,
            linesModule.fromString(stmtListSpacesExpected.replace(/^\s+|\s+$/g, "")).indent(2),
            `${eol}}`
        ]).toString());
    });
    it("should print static methods with the static keyword", () => {
    ...
    it("should print string literals with the specified delimiter", () => {
        const ast = parse([
            "var obj = {",
            "    \"foo's\": 'bar',",
            "    '\"bar\\'s\"': /regex/m",
    ...
    it("should print block comments at head of class once", () => {
        // Given.
        const ast = parse([
            "/**",
    ...
    it("prints trailing commas in object literals", () => {
        const code = [
            "({",
            "  foo: bar,",
            "  bar: foo,",
    ...
    it("prints trailing commas in function calls", () => {
        const code = [
            "call(",
            "  1,",
            "  2,",
    ...
    it("prints trailing commas in array expressions", () => {
        const code = [
            "[",
            "  1,",
            "  2,",
    ...
    it("prints trailing commas in function definitions", () => {
        const code = [
            "function foo(",
            "  a,",
            "  b,",
    ...
    (nodeMajorVersion >= 6 ? it : xit)("shouldn't print a trailing comma for a RestElement", () => {
        const code = [
            "function foo(",
            "  a,",
            "  b,",
    ...
    it("should support AssignmentPattern and RestElement", () => {
        const code = [
    ...
    it("adds parenthesis around spread patterns", () => {
        ...
        const arrowFunction = b.arrowFunctionExpression([], b.identifier("rest"), false);
    ...
    it("adds parenthesis around single arrow function arg when options.arrowParensAlways is true", () => {
        const code = "(a) => {};";
        const fn = b.arrowFunctionExpression([b.identifier("a")], b.blockStatement([]), false);
        const ast = b.program([
    ...
    it("adds parenthesis around arrow function when binding", () => {
        const code = "var a = (x => y).bind(z);";
        const fn = b.arrowFunctionExpression([b.identifier("x")], b.identifier("y"));
    ...
    it("adds parenthesis around async arrow functions with args", () => {
        let code = "async () => {};";
    ...
    it("adds parenthesis around arrow functions with single arg and a type", () => {
        const code = "(a: b) => {};";
    ...
    it("adds parenthesis around arrow functions with single arg and a return type", () => {
        const code = "(a): void => {};";
        const arg = b.identifier("a");
        const fn = b.arrowFunctionExpression([arg], b.blockStatement([]), false);
    ...
    it("prints class property initializers with type annotations correctly", () => {
        const code = [
            "class A {",
            "  foo = (a: b): void => {};",
            "}"
    ...
    it("prints ClassProperty correctly", () => {
        const code = [
            "class A {",
            "  foo: Type = Bar;",
            "}"
    ...
    it("prints static ClassProperty correctly", () => {
        const code = [
            "class A {",
            "  static foo = Bar;",
            "}"
    ...
    it("prints template expressions correctly", () => {
        let code = [
            "graphql`query`;"
        ].join(eol);
        let ast = b.program([
    ...
    it("preserves newlines at the beginning/end of files", () => {
        const code = [
            "",
            "f();",
            ""
    ...
    it("respects options.lineTerminator", () => {
        const lines = [
            "var first = 1;",
            "var second = 2;"
        ];
    ...
    it("preserves indentation in unmodified template expressions", () => {
        const printer = new Printer({
            tabWidth: 2
        });
        const code = [
    ...
    it("preserves indentation in modified template expressions", () => {
        const code = [
            "const fragments = {",
            "  viewer: Relay.QL`",
        ...
        recast.visit(ast, {
            visitTaggedTemplateExpression(path) {
                ...
                path.get("quasi", "quasis").each((quasiPath) => {
    ...
    it("prints commas for flow object types by default", () => {
        const code = [
            "type MyType = {",
            "    message: string,",
            "    isAwesome: boolean,",
        ...
        const ast = b.typeAlias(b.identifier("MyType"), null, b.objectTypeAnnotation([
    ...
    it("shouldn't print a trailing comma for single-line flow object types", () => {
        const code1 = "type MyType = { message: string };";
        const code2 = "type MyType = { [key: string]: string };";
        const ast1 = b.typeAlias(b.identifier("MyType"), null, b.objectTypeAnnotation([
            b.objectTypeProperty(b.identifier("message"), b.stringTypeAnnotation(), false)
        ]));
        const ast2 = b.typeAlias(b.identifier("MyType"), null, b.objectTypeAnnotation([], [
    ...
    it("prints semicolons for flow object types when options.flowObjectCommas is falsy", () => {
        const code = [
            "type MyType = {",
            "    message: string;",
            "    isAwesome: boolean;",
    ...
    it("prints parens for nullable union/intersection types", () => {
        const code = "type MyType = ?(string | number);";
        const ast = b.typeAlias(b.identifier("MyType"), null, b.nullableTypeAnnotation(b.unionTypeAnnotation([b.stringTypeAnnotation(), b.numberTypeAnnotation()])));
        const printer = new Printer({});
        const pretty = printer.printGenerically(ast).code;
        assert.strictEqual(pretty, code);
    });
    (nodeMajorVersion >= 6 ? it : xit)("uses the `arrayBracketSpacing` and the `objectCurlySpacing` option", () => {
    ...
    it("prints no extra semicolons in for-loop heads (#377)", () => {
        ...
        checkWith(require(srcPath("parsers/esprima")));
        checkWith(require(srcPath("parsers/acorn")));
        checkWith(require(srcPath("parsers/babel")));
        checkWith(require(srcPath("parsers/typescript")));
        checkWith(require(srcPath("parsers/flow")));
    });
    it("parenthesizes NumericLiteral MemberExpression objects", () => {
        const nonBabelNode = b.memberExpression(b.literal(1), b.identifier("foo"));
        const babelNode = b.memberExpression(b.numericLiteral(1), b.identifier("foo"));
    ...
    it("reprints modified numeric literals", () => {
        const code = "3 + 4;";
        const ast = parse(code);
    ...
    it("prints flow tuple type annotations correctly, respecting array options", () => {
        const code = [
            "type MyTupleType = [",
            '  "tuple element 1",',
            '  "tuple element 2",',
    ...
    it("adds parenthesis around object expression", () => {
        const code = "({}).x = 1;";
        const assignment = b.assignmentExpression("=", b.memberExpression(b.objectExpression([]), b.identifier("x"), false), b.literal(1));
        const ast = b.program([
    ...
    it("adds parenthesis around conditional", () => {
    ...
    it("prints flow object type internal slots correctly", () => {
        const code = [
            "type MyObjectType = {",
    ...
    it("prints class private methods and properties correctly", () => {
        const code = [
    ...
    it("prints an interpreter directive correctly", () => {
        const code = [
            "#!/usr/bin/env node",
    ...
    it("prints an interface type annotation correctly", () => {
        const code = [
