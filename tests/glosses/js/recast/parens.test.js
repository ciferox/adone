...
describe("parens", () => {
    ...
    it("don't reparenthesize valid IIFEs", () => {
    ...
    it("don't reparenthesize valid object literals", () => {
        const objCode = "(  {    foo   :  42}.  foo )  ;";
        const objAst = parse(objCode);
        const objReprint = printer.print(objAst).code;
        assert.strictEqual(objReprint, objCode);
    });
    it("don't parenthesize return statements with sequence expressions", () => {
        const objCode = "function foo() { return 1, 2; }";
        const objAst = parse(objCode);
        const objReprint = printer.print(objAst).code;
        assert.strictEqual(objReprint, objCode);
    });
    it("NegatedLoopCondition", () => {
        const ast = parse([
            "for (var i = 0; i < 10; ++i) {",
            "  console.log(i);",
            "}"
    ...
    it("should not be added to multiline boolean expressions", () => {
        const code = [
            "function foo() {",
            "  return !(",
            "    a &&",
    ...
    it("should be added to callees that are function expressions", () => {
        check("(()=>{})()");
        check("(function (){})()");
    });
    it("issues #504 and #512", () => {
        check("() => ({})['foo']");
    ...
    it("should be added to bound arrow function expressions", () => {
        check("(()=>{}).bind(x)");
    });
    it("should be added to object destructuring assignment expressions", () => {
        check("({x}={x:1})");
        // Issue #533
        check("({ foo } = bar)");
    });
    it("regression test for issue #327", () => {
        const expr = "(function(){}())";
        check(expr);
        const ast = parse(expr);
    ...
    it("regression test for issue #366", () => {
