const {
    is,
    js: { compiler: { core, helper: { moduleImports: { ImportInjector } } } }
} = adone;

const test = (sourceType, opts, initializer, expectedCode) => {
    if (is.function(opts)) {
    ...
    expect(result.code.replace(/\s+/g, " ").trim()).to.equal((expectedCode || "").replace(/\s+/g, " ").trim());
};
const testScript = test.bind(undefined, "script");
const testModule = test.bind(undefined, "module");

describe("js", "compiler", "helper", "moduleImports", () => {
