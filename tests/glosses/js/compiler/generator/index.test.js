...
const {
    std: { fs, path },
    js: { compiler: { types: t, parse, generate, Printer, CodeGenerator } }
} = adone;
    ...
    describe("generation", () => {
        it("completeness", () => {
            Object.keys(t.VISITOR_KEYS).forEach((type) => {
                assert.ok(Boolean(Printer.prototype[type]), `${type} should exist`);
            });
            ...
            testSuite.tests.forEach((task) => {
                it(task.title, task.disabled ? adone.noop : (() => {
                    ...
                    if (actualCode) {
                        ...
                        if (!taskExpect.code && result.code && fs.statSync(path.dirname(taskExpect.loc)).isDirectory() && !process.env.CI) {
