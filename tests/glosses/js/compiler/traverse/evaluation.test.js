...
describe("js", "compiler", "traverse", "evaluation", () => {
    ...
    it("should evaluate member expressions", () => {
        ...
        const eval_invalid_call = member_expr
            .get("body.1.declarations.0.init")
            .evaluate();
        expect(eval_member_expr.value).to.equal(2);
        expect(eval_invalid_call.confident).to.equal(false);
    });
    ...
    it("should deopt redefined primitives - undefined, NaN and Infinity", () => {
        ...
        const eval_inf = getPath("let Infinity; Infinity;")
            .get("body.1.expression")
            .evaluate();
        expect(eval_undef.confident).to.equal(false);
        expect(eval_nan.confident).to.equal(false);
        expect(eval_inf.confident).to.equal(false);

        expect(eval_undef.deopt.type).to.equal("VariableDeclarator");
        expect(eval_undef.deopt.parentPath.node.kind).to.equal("let");
    });

    it("should work with String.raw", () => {
        expect(
    ...
    `);
        ...
        expect(result.confident).to.equal(false);
        expect(result.deopt.type).to.equal("Identifier");
        expect(result.deopt.node.name).to.equal("foo");
    });
    ...
    `);
        ...
        expect(result.confident).to.equal(true);
        expect(result.deopt).to.null();
