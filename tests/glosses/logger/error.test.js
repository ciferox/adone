...
describe("error", () => {
    ...
    it("err serializer", async () => {
        ...
        const instance = adone.logger({
            serializers: {
                err: adone.logger.stdSerializers.err
            }
        }, stream);
    ...
    it("stack is omitted if it is not set on err", (done) => {
        const err = new Error("myerror");
        delete err.stack;
        const instance = adone.logger(sink((chunk, enc, cb) => {
    ...
    it("stack is rendered as any other property if it's not a string", (done) => {
        const err = new Error("myerror");
        err.stack = null;
        const instance = adone.logger(sink((chunk, enc, cb) => {
            assert.ok(new Date(chunk.time) <= new Date(), "time is greater than Date.now()");
