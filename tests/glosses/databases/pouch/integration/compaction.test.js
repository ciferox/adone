...
adapters.forEach((adapter) => {
    describe(`test.compaction.js-${adapter}`, () => {
        ...
        it("#2913 massively parallel compaction", () => {
            ...
            return testUtils.Promise.all(tasks.map((i) => {
        ...
        it("#2931 - synchronous putAttachment + compact", () => {
            ...
            for (let i = 0; i < 50; i++) {
                ...
                queue.then(() => {
                    const promise = testUtils.Promise.all([
                        db.compact(),
                        db.compact(),
                        db.compact(),
                        db.compact(),
        ...
        it("#2931 - synchronous putAttachment + compact 2", () => {
            ...
            let compactQueue = testUtils.Promise.resolve();
        ...
        it("#2818 md5 collision (sanity check)", () => {
            ...
            return db.put(doc1).then(() => {
                ...
                return testUtils.Promise.all(["doc1", "doc2"].map((id) => {
        ...
        it("#2818 md5 collision between revs (sanity check)", () => {
            ...
            return db.put(doc1).then((res) => {
                ...
                rev2 = res.rev;
                return testUtils.Promise.all([rev1, rev2].map((rev) => {
        ...
        it("#2818 Compaction removes attachments given conflicts", () => {
            ...
            return db.bulkDocs({
                ...
                return testUtils.Promise.all(allDigests.map((digest) => {
                    const doc = {
                        _attachments: {
                            "newatt.txt": {
                                content_type: "text/plain",
        ...
        it("#3251 massively parallel autocompaction while getting", () => {
            ...
            return db.put(doc).then((res) => {
                ...
                let updatePromise = testUtils.Promise.resolve();
