...
adapters.forEach((adapters) => {
    const title = `test.replication_events.js-${adapters[0]}-${adapters[1]}`;
    describe(`suite2 ${title}`, () => {
        ...
        it("#3852 Test errors", (done) => {
            ...
            db.bulkDocs([{ _id: "a" }, { _id: "b" }]).then(() => {
                ...
                const repl = db.replicate.to(remote, {
                    retry: true,
                    live: true,
                    back_off_function() {
                        return 0;
        ...
        describe("#5172 triggering error when replicating", () => {
            ...
            it("from or to a secured database, using live replication", () => {
                if (adapters[0] === "local" && adapters[1] === "local") {
                    return;
                }

                const replication = source.replicate.to(dest, { live: true });
                const invokedHandlers = attachHandlers(replication);

                return replication.then(() => {
                    throw new Error("Resulting promise should be rejected");
                }, () => {
                    assert.deepEqual(invokedHandlers, ["error"], "incorrect handler was invoked");
                });
            });

            it("from or to a secured database, using live replication with checkpoint", () => {
                if (adapters[0] === "local" && adapters[1] === "local") {
                    return;
                }

                const replication = source.replicate.to(dest, { live: true, since: 1234 });
                const invokedHandlers = attachHandlers(replication);
            ...
            it("from or to a secured database, using live replication with retrying", () => {
                if (adapters[0] === "local" && adapters[1] === "local") {
                    return;
                }

                const replication = source.replicate.to(dest, { live: true, retry: true });
                const invokedHandlers = attachHandlers(replication);
            ...
            it("from or to a secured database, using one-shot replication", () => {
                if (adapters[0] === "local" && adapters[1] === "local") {
                    return;
                }

                const replication = source.replicate.to(dest);
                const invokedHandlers = attachHandlers(replication);
