...
adapters.forEach((adapter) => {
    ...
    describe(`test.changes.js-${adapter}`, () => {
        ...
        it("#3136 winningRev has a lower seq, style=all_docs 2", () => {
            ...
            return chain.then(() => {
                ...
                let chain2 = testUtils.Promise.resolve();
...
describe("changes-standalone", () => {

    it("Changes reports errors", function (done) {
        this.timeout(2000);
        const db = new PouchDB("http://infiniterequest.com", { skipSetup: true });
        db.changes({
            timeout: 1000
        }).on("error", (err) => {
            assert.exists(err);
