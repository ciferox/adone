...
adapters.forEach((adapter) => {
    describe(`test.issue3646.js- ${adapter}`, () => {
        ...
        it("Should finish with 0 documents", () => {
            ...
            return db.bulkDocs(data[0], { new_edits: false }).then(() => {
                ...
                assert.lengthOf(res.rows, 0, "all docs length is 0");
                assert.equal(res.total_rows, 0);
                return db.allDocs({ keys: ["b74e3b45"], include_docs: true });
            }).then((res) => {
                const first = res.rows[0];
                assert.equal(first.value.deleted, true, "all docs value.deleted");
                assert.equal(first.value.rev, "6-441f43a31c89dc68a7cc934ce5779bf8");
                assert.equal(res.total_rows, 0);
                return db.info();
            }).then((info) => {
                assert.equal(info.doc_count, 0, "doc_count is 0");
                return db.changes({ include_docs: true });
            }).then((changes) => {
                assert.lengthOf(changes.results, 1);
                const first = changes.results[0];
                assert.equal(first.doc._rev, "6-441f43a31c89dc68a7cc934ce5779bf8");
                assert.equal(first.deleted, true, "changes metadata.deleted");
                assert.equal(first.doc._deleted, true, "changes doc._deleted");
            });
        });
