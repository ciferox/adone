...
describe("glosses", "databases", "mongo", "CRUD API", () => {
    ...
    for (const topology of [
        ...
        describe(topology, () => {
            ...
            specify("find", async () => {
                await db.collection("t").insert([{ a: 1 }, { a: 1 }, { a: 1 }, { a: 1 }]);
                const cursor = db.collection("t").find({});
                cursor.filter({ a: 1 })
            ...
            describe("insert", () => {
                specify("legacy", async () => {
                    const r = await db.collection("t2_1").insert([{ a: 1 }, { a: 2 }]);
                ...
                specify("insertOne", async () => {
                    const r = await db.collection("t2_3").insertOne({ a: 1 }, { w: 1 });
                ...
                specify("insertMany", async () => {
                    const r = await db.collection("t2_4").insertMany([{ a: 1 }, { b: 2 }], { w: 1 });
                    expect(r.result.n).to.be.equal(2);
                ...
                specify("bulkWriteUnOrdered", async () => {
                    let r = await db.collection("t2_5").insertMany([{ c: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t2_5").bulkWrite([
                        { insertOne: { a: 1 } },
                        { insertMany: [{ g: 1 }, { g: 2 }] },
                ...
                specify("bulkWriteUnOrderedSpec", async () => {
                    let r = await db.collection("t2_6").insertMany([{ c: 1 }, { c: 2 }, { c: 3 }], { w: 1 });
                    expect(r.result.n).to.be.equal(3);
                    r = await db.collection("t2_6").bulkWrite([
                ...
                specify("bulkWriteOrdered", async () => {
                    let r = await db.collection("t2_7").insertMany([{ c: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t2_7").bulkWrite([
                        { insertOne: { a: 1 } },
                        { insertMany: [{ g: 1 }, { g: 2 }] },
                        { updateOne: { q: { a: 2 }, u: { $set: { a: 2 } }, upsert: true } },
                        { updateMany: { q: { a: 2 }, u: { $set: { a: 2 } }, upsert: true } },
                ...
                specify("bulkWriteOrderedCrudSpec", async () => {
                    let r = await db.collection("t2_8").insertMany([{ c: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t2_8").bulkWrite([
                        { insertOne: { document: { a: 1 } } },
                        { updateOne: { filter: { a: 2 }, update: { $set: { a: 2 } }, upsert: true } },
            ...
            describe("update", () => {
                specify("legacy", async () => {
                    const r = await db.collection("t3_1").update({ a: 1 }, { $set: { a: 2 } }, { upsert: true });
                    expect(r.result.n).to.be.equal(1);
                });

                specify("updateOne", async () => {
                    let r = await db.collection("t3_2").insertMany([{ c: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t3_2").updateOne({
                        a: 1
                    }, {
                        $set: { a: 1 }
                    }, {
                ...
                specify("replaceOne", async () => {
                    let r = await db.collection("t3_3").replaceOne({
                        a: 1
                    }, {
                        a: 2
                    }, {
                    ...
                    expect(r.upsertedId).to.exist;
                    r = await db.collection("t3_3").replaceOne({
                        a: 2
                    }, {
                        a: 3
                    }, {
                ...
                specify("updateMany", async () => {
                    let r = await db.collection("t3_4").insertMany([{ a: 1 }, { a: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(2);
                    r = await db.collection("t3_4").updateMany({
                        a: 1
                    }, {
                        $set: { a: 2 }
                    }, {
            ...
            describe("remove", () => {
                specify("legacy", async () => {
                    let r = await db.collection("t4_1").insertMany([{ a: 1 }, { a: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(2);
                    r = await db.collection("t4_1").remove({ a: 1 }, { single: true });
                    expect(r.result.n).to.be.equal(1);
                });

                specify("deleteOne", async () => {
                    let r = await db.collection("t4_2").insertMany([{ a: 1 }, { a: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(2);
                    r = await db.collection("t4_2").deleteOne({ a: 1 });
                    expect(r.result.n).to.be.equal(1);
                    expect(r.deletedCount).to.be.equal(1);
                });

                specify("delete many", async () => {
                    let r = await db.collection("t4_3").insertMany([{ a: 1 }, { a: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(2);
                    r = await db.collection("t4_3").deleteMany({ a: 1 });
                    expect(r.result.n).to.be.equal(2);
                    expect(r.deletedCount).to.be.equal(2);
                });
            });
            ...
            describe("findAndModify", () => {
                specify("findOneAndRemove", async () => {
                    let r = await db.collection("t5_1").insertMany([{ a: 1, b: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t5_1").findOneAndDelete({ a: 1 }, { projection: { b: 1 }, sort: { a: 1 } });
                    expect(r.lastErrorObject.n).to.be.equal(1);
                    expect(r.value.b).to.be.equal(1);
                });

                specify("findOneAndReplace", async () => {
                    let r = await db.collection("t5_2").insertMany([{ a: 1, b: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t5_2").findOneAndReplace({ a: 1 }, { c: 1, b: 1 }, {
                        projection: { b: 1, c: 1 },
                        sort: { a: 1 },
                ...
                specify("findOneAndUpdate", async () => {
                    let r = await db.collection("t5_3").insertMany([{ a: 1, b: 1 }], { w: 1 });
                    expect(r.result.n).to.be.equal(1);
                    r = await db.collection("t5_3").findOneAndUpdate({ a: 1 }, { $set: { d: 1 } }, {
                        projection: { b: 1, d: 1 },
                        sort: { a: 1 },
                        returnOriginal: false,
                        upsert: true
            ...
            it("should correctly execute updateOne operations with w:0 and upsert", async () => {
                const r = await db.collection("try").updateOne({ _id: 1 }, { $set: { x: 1 } }, { upsert: true, w: 0 });
                expect(r).to.exist;
            });
        });
    }
