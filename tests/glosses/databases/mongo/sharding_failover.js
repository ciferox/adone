describe("sharding failover", function () {
    ...
    it("should correctly connect to mongos sharded setup and kill the mongos proxy", async () => {
        const mongos = new mongo.Mongos([
            new mongo.Server(this.host, this.port, { autoReconnect: true }),
            new mongo.Server(this.host, this.port + 1, { autoReconnect: true })
        ], { ha: true, haInterval: 500, poolSize: 1 });
        const left = spy();
        const joined = spy();
        mongos.on("left", left);
        mongos.on("joined", joined);
        const db = new mongo.Db("integration_test_", mongos, { w: 0 });
        await db.open();
