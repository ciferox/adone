export function executeCommand(configuration, db, cmd, options) {
    ...
    return new Promise((resolve, reject) => {
        const { Pool, BSON, Query } = adone.database.mongo.core;

        // Attempt to connect
        const pool = new Pool({
            host, port, bson: new BSON()
        });

        // Add event listeners
        pool.on("connect", function (_pool) {
            const query = new Query(new BSON(), `${db}.$cmd`, cmd, { numberToSkip: 0, numberToReturn: 1 });
            _pool.write(query, {
                command: true
            }, function (err, result) {
...
export function locateAuthMethod(configuration) {
    return new Promise((resolve, reject) => {
        const { Pool, BSON, Query } = adone.database.mongo.core;
