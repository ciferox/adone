...
Request.prototype.reply = function (documents, options) {
    options = options || {};
    documents = is.array(documents) ? documents : [documents];

    // Unpack any variables we need
    const cursorId = options.cursorId || Long.ZERO;
    const responseFlags = is.number(options.responseFlags) ? options.responseFlags : 0;
    const startingFrom = is.number(options.startingFrom) ? options.startingFrom : 0;
    const numberReturned = documents.length;

    // Additional response Options
    const killConnectionAfterNBytes = is.number(options.killConnectionAfterNBytes)
    ...
    if (is.nil(killConnectionAfterNBytes)) {
