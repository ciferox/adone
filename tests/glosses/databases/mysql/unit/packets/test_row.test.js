describe("glosses", "databases", "mysql", "unit", "packets", "text row", () => {
    ...
    specify("russian", () => {
        const packet = TextRow.toPacket(["Ну,", "погоди!"], "cesu8");
        assert.equal(packet.buffer.toString("hex", 4), "05d09dd1832c0dd0bfd0bed0b3d0bed0b4d0b821");
    });

    specify("long text", () => {
        const packet = TextRow.toPacket(["Пушкин родился 26 мая (6 июня) 1799 г. в Москве. В метрической книге церкви Богоявления в Елохове (сейчас на её месте находится Богоявленский собор в Елохове) на дату 8 июня 1799 г."], "cesu8");
        assert.equal(
