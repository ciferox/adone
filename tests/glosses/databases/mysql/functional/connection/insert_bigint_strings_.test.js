...
describe("glosses", "databases", "mysql", "functional", "connection", "insert", "bigint", "bignumer strings", () => {
    let connection = null;

    before(async () => {
        connection = await createConnection({ supportBigNumbers: true, bigNumberStrings: true });
    ...
    it("should set correct id (bigint)", async () => {
        ...
        expect(res.insertId).to.be.equal("9007199254740993");
        ...
        expect(res.insertId).to.be.equal("90071992547409925");
    });

    it("should select them as strings", async () => {
        const [rows] = await connection.query("select * from bigs");
