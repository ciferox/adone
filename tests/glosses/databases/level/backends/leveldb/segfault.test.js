...
const testPending = function (name, expectedCount, fn) {
    operations.push(fn)

    it(`close() waits for pending ${name}`, function (done) {
        ...
        db.open(function (err) {
            ...
            db.put('key', 'value', function (err) {
                assert.notExists(err, 'no error from put()')

                fn(db, function (err) {
                    count++
                    assert.notExists(err, 'no error from operation')
                })

                db.close(function (err) {
                    assert.notExists(err, 'no error from close()')
                    assert.equal(count, expectedCount, 'operation(s) finished before close')
                    done()
                })
            })
        })
...
it('iterator() does not segfault if db is not open', function (done) {
    const db = testCommon.factory();
