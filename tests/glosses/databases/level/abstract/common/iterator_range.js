...
export const setUp = function (leveldown, testCommon, data) {
...
export const range = function (leveldown, testCommon, data) {
    describe("iterator range", () => {

        const collectEntries = testCommon.collectEntries;

        const rangeTest = (name, opts, expected) => {
            opts.keyAsBuffer = false;
            opts.valueAsBuffer = false;
            it(name, async () => {
                const result = await collectEntries(db.iterator(opts));
                assert.strictEqual(result.length, expected.length, "correct number of entries");
...
export const all = function (leveldown, testCommon) {
    ...
    setUp(leveldown, testCommon, data);
    range(leveldown, testCommon, data);
    tearDown(testCommon);
};
