...
describe("multiformat", "CID", () => {
    ...
    before(async () => {
        hash = await multihashingAsync(Buffer.from("abc"), "sha2-256");
    });
    ...
    describe("v0", () => {
        ...
        it("handles Buffer multihash", async () => {
            const mh = await multihashingAsync(Buffer.from("hello world"), "sha2-256");
    ...
    describe("conversion v0 <-> v1", () => {
        it("should convert v0 to v1", async () => {
            const hash = await multihashingAsync(Buffer.from(`TEST${Date.now()}`), "sha2-256");
        ...
        it("should not convert v1 to v0 if not sha2-256 multihash", async () => {
            const hash = await multihashingAsync(Buffer.from(`TEST${Date.now()}`), "sha2-512");
        ...
        it("should not convert v1 to v0 if not 32 byte multihash", async () => {
            const hash = await multihashingAsync(Buffer.from(`TEST${Date.now()}`), "sha2-256", 31);
            const cid = new CID(1, "dag-pb", hash);
