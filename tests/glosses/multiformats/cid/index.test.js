...
const srcPath = (...args) => adone.std.path.join(adone.ROOT_PATH, "lib", "glosses", "multiformats", "cid", ...args);

describe("multiformat", "CID", () => {
    let hash;

    before((done) => {
        multihashingAsync(Buffer.from("abc"), "sha2-256", (err, d) => {
            if (err) {
                return done(err);
            }
            hash = d;
    ...
    describe("v0", () => {
        ...
        it("handles Buffer multihash", (done) => {
            multihashingAsync(Buffer.from("hello world"), "sha2-256", (err, mh) => {
    ...
    describe("conversion v0 <-> v1", () => {
        ...
        it("should not convert v1 to v0 if not sha2-256 multihash", (done) => {
            multihashingAsync(Buffer.from(`TEST${Date.now()}`), "sha2-512", (err, hash) => {
                if (err) {
                    return done(err);
                }
        ...
        it("should not convert v1 to v0 if not 32 byte multihash", (done) => {
            multihashingAsync(Buffer.from(`TEST${Date.now()}`), "sha2-256", 31, (err, hash) => {
                if (err) {
                    return done(err);
                }
                const cid = new CID(1, "dag-pb", hash);
