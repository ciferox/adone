...
const srcPath = (...args) => adone.std.path.join(adone.ROOT_PATH, "lib", "glosses", "multiformats", "multihash", ...args);
...
const sample = function (code, size, hex) {
...
describe("multiformat", "multihash", () => {
    ...
    describe("constants", () => {
        it("exported", () => {
            expect(mh.names).to.eql(constants.names);
            expect(mh.codes).to.eql(constants.codes);
            expect(mh.defaultLengths).to.eql(constants.defaultLengths);
        });
