...
describe("model: update:", () => {
    ...
    it("makes copy of conditions and update options", (done) => {
        ...
        let conditions = { "_id": post._id.toString() };
        let update = { "$set": { "some_attrib": post._id.toString() } };
    ...
    it("updates a number to null (gh-640)", (done) => {
        let db = start();
        let B = db.model("BlogPostForUpdates", "wwwwowowo" + random());
    ...
    it("handles $pull from Mixed arrays (gh-735)", (done) => {
        let db = start();
        let schema = new Schema({ comments: [] });
        let M = db.model("gh-735", schema, "gh-735_" + random());
    ...
    it("handles $push with $ positionals (gh-1057)", (done) => {
        ...
        let Project = db.model("1057-project", projectSchema, "1057-" + random());
    ...
    it("handles nested paths starting with numbers (gh-1062)", (done) => {
        let db = start();
        let schema = new Schema({ counts: Schema.Types.Mixed });
        let M = db.model("gh-1062", schema, "1062-" + random());
    ...
    it("handles positional operators with referenced docs (gh-1572)", (done) => {
        ...
        let so = new Schema({ title: String, obj: [String] });
        let Some = db.model("Some" + random(), so);
    ...
    it("use .where for update condition (gh-2170)", (done) => {
        let db = start();
        let so = new Schema({ num: Number });
        let Some = db.model("gh-2170" + random(), so);
    ...
    it("casts empty arrays", (done) => {
        ...
        let so = new Schema({ arr: [] });
        let Some = db.model("1838-" + random(), so);
    ...
    it("works with $set and overwrite (gh-2515)", (done) => {
        ...
        let schema = new Schema({ breakfast: String });
        let M = db.model("gh-2515", schema);
    ...
    it("successfully casts set with nested mixed objects (gh-2796)", (done) => {
        ...
        let schema = new Schema({ breakfast: {} });
    ...
    it("works with overwrite but no $set (gh-2568)", (done) => {
        ...
        let Book = db.model("gh2568", bookSchema);
    ...
    it("works with undefined date (gh-2833)", (done) => {
        ...
        let D = db.model("gh2833", dateSchema);
    ...
    it("does not add virtuals to update (gh-2046)", (done) => {
        ...
        let childSchema = new Schema({ foo: String }, { toObject: { getters: true } });
        let parentSchema = new Schema({ children: [childSchema] });

        childSchema.virtual("bar").get(() => {
            return 'bar';
        });

        let Parent = db.model("gh2046", parentSchema, "gh2046");

        let update = Parent.update({}, { $push: { children: { foo: "foo" } } }, { upsert: true });
    ...
    it("doesnt modify original argument doc (gh-3008)", (done) => {
        ...
        let Model = db.model("gh3008", FooSchema);

        let update = { $set: { values: 2, value: 2 } };
    ...
    describe("bug fixes", () => {
        ...
        it("lets $currentDate go through with updatedAt (gh-5222)", (done) => {
        ...
        it("update with nested id (gh-5640)", (done) => {
            var testSchema = new mongoose.Schema({
                _id: {
                    a: String,
                    b: String
