...
describe("schema.onthefly", () => {
    ...
    it("setting should cache the schema type and cast values appropriately", (done) => {
        let db = start(),
    ...
    it("should be local to the particular document", (done) => {
        let db = start(),
            Decorated = db.model("Decorated", collection);
    ...
    it("querying a document that had an on the fly schema should work", (done) => {
        let db = start(),
            Decorated = db.model("Decorated", collection);
        ...
        post.save((err) => {
            ...
            Decorated.findById(post.id, (err, found) => {
                db.close();
                assert.strictEqual(null, err);
    ...
    it("on the fly Embedded Array schemas should cast properly", (done) => {
        let db = start(),
            Decorated = db.model("Decorated", collection);
    ...
    it("on the fly Embedded Array schemas should get from a fresh queried document properly", (done) => {
        let db = start(),
            Decorated = db.model("Decorated", collection);
