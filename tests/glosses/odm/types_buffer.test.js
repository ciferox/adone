...
describe("types.buffer", () => {
    ...
    it("test that a mongoose buffer behaves and quacks like a buffer", (done) => {
        let a = new MongooseBuffer();

        assert.ok(a instanceof Buffer);
        assert.ok(a.isMongooseBuffer);
        assert.equal(true, is.buffer(a));
    ...
    it("test write markModified", (done) => {
        let db = start(),
            User = db.model("UserBuffer", UserBuffer, `usersbuffer_${  random()}`);
        ...
        function reset(model) {
            ...
            model.schema.requiredPaths().forEach((path) => {
                model.$__.activePaths.require(path);
            });
        }
    });

    it("can be set to null", (done) => {
        let db = start(),
            User = db.model("UserBuffer", UserBuffer, `usersbuffer_${  random()}`);
        const user = new User({ array: [null], required: new Buffer(1) });
        user.save((err, doc) => {
            assert.ifError(err);
            User.findById(doc, (err, doc) => {
                db.close();
                assert.ifError(err);
    ...
    it("can be updated to null", (done) => {
        let db = start(),
            User = db.model("UserBuffer", UserBuffer, `usersbuffer_${  random()}`);
        const user = new User({ array: [null], required: new Buffer(1), serial: new Buffer(1) });
        user.save((err, doc) => {
            assert.ifError(err);
            User.findOneAndUpdate({ _id: doc.id }, { serial: null }, { new: true }, (err, doc) => {
                db.close();
                assert.ifError(err);
    ...
    describe("#toObject", () => {
        it("retains custom subtypes", (done) => {
    ...
    describe("subtype", () => {
        ...
        it("cast from number (gh-3764)", (done) => {
            let schema = new Schema({ buf: Buffer });
            let MyModel = mongoose.model("gh3764", schema);

            let doc = new MyModel({ buf: 9001 });
