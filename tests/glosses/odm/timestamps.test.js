...
describe("timestamps", () => {
    ...
    it("does not override timestamp params defined in schema (gh-4868)", (done) => {
    ...
    it("updatedAt without createdAt (gh-5598)", (done) => {
        const startTime = Date.now();
        const schema = new mongoose.Schema({
            name: String
    ...
    it("updatedAt without createdAt for nested (gh-5598)", (done) => {
        const startTime = Date.now();
        const schema = new mongoose.Schema({
            name: String
        }, { timestamps: { createdAt: null, updatedAt: true } });
        ...
        M.create({ child: { name: "test" } }, (error) => {
            assert.ifError(error);
            M.findOne({}, (error, doc) => {
                assert.ifError(error);
    ...
    it("does not override nested timestamp params defined in schema (gh-4868)", (done) => {
        const startTime = Date.now();
        const schema = new mongoose.Schema({
            ts: {
                createdAt: {
    ...
    it("does not override timestamps in nested schema (gh-4868)", (done) => {
        const startTime = Date.now();
