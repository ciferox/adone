...
describe("schema", () => {
    ...
    describe("nested fields with same name", () => {
        let db, NestedModel;
        ...
        it("don't disappear", (done) => {
            ...
            n.save((err) => {
                assert.ifError(err);
                NestedModel.findOne({ _id: n._id }, (err, nm) => {
                    assert.ifError(err);
    ...
    it('can be created without the "new" keyword', (done) => {
        const schema = new Schema({ name: String });
        assert.ok(schema instanceof Schema);
        done();
    });

    it("does expose a property for duck-typing instanceof", (done) => {
        const schema = new Schema({ name: String });
        assert.ok(schema.instanceOfSchema);
        done();
    });

    it("supports different schematypes", (done) => {
    ...
    it("supports dot notation for path accessors", (done) => {
    ...
    it("allows paths nested > 2 levels", (done) => {
    ...
    it("default definition", (done) => {
        ...
        assert.equal((Number(Test.path("callback").getDefault({ a: "b" }))), 3);
    ...
    it("Mixed defaults can be empty arrays", (done) => {
        const Test = new Schema({
    ...
    describe("casting", () => {
        it("number", (done) => {
            ...
            assert.equal((Number(Tobi.path("age").cast("0"))), 0);
        ...
        it("date", (done) => {
            ...
            assert.ok(is.null(Loki.path("birth_date").cast("")));
            assert.ok(is.null(Loki.path("birth_date").cast(null)));
            done();
        });
    ...
    describe("setters", () => {
        ...
        describe("array", () => {
            it("object setters will be applied for each object in array", (done) => {
                let Tobi = new Schema({
        ...
        it("applying when none have been defined", (done) => {
            const Tobi = new Schema({
                name: String
            });
        ...
        it("assignment of non-functions throw", (done) => {
    ...
    describe("options", () => {
        ...
        it("setting", (done) => {
            ...
            assert.equal(Tobi.options.read.mode, "nearest");
            assert.ok(is.array(Tobi.options.read.tags));
    ...
    describe("virtuals", () => {
        ...
        describe("id", () => {
            it("default creation of id can be overridden (gh-298)", (done) => {
                assert.doesNotThrow(() => {
            ...
            it("disabling", (done) => {
                let schema = new Schema({ name: String }, { noVirtualId: true });
    ...
    describe("#add()", () => {
        it("does not polute existing paths", (done) => {
            let o = { name: String };
            let s = new Schema(o);
        ...
        it("merging nested objects (gh-662)", (done) => {
            const db = start();
            ...
            merged.save((err) => {
                assert.ifError(err);
                Merged.findById(merged.id, (err, found) => {
                    db.close();
                    assert.ifError(err);
    ...
    describe("construction", () => {
        it("array of object literal missing a type is interpreted as DocumentArray", (done) => {
        ...
        it("array of object literal with type.type is interpreted as DocumentArray", (done) => {
            const goose = new mongoose.Mongoose();
            const s = new Schema({
                arr: [
                    { type: { type: String } }
        ...
        it("does not alter original argument (gh-1364)", (done) => {
            const schema = {
        ...
        it("properly gets value of plain objects when dealing with refs (gh-1606)", (done) => {
            const db = start();
            ...
            ele.save((err) => {
                assert.ifError(err);
                let s = new Some({ obj: ele.toObject() });
                s.save((err) => {
                    assert.ifError(err);
    ...
    describe("property names", () => {
        ...
        it("that do not conflict do not throw", (done) => {
            assert.doesNotThrow(() => {
                new Schema({
                    model: String
                });
            ...
            assert.doesNotThrow(() => {
                let s = new Schema({ docs: [{ path: String }] });
                let M = mongoose.model("gh-1245", s);
                new M({ docs: [{ path: "works" }] });
            });

            assert.doesNotThrow(() => {
                let s = new Schema({ setMaxListeners: String });
                let M = mongoose.model("setMaxListeners-as-property-name", s);
        ...
        it("permit _scope to be used (gh-1184)", (done) => {
            const db = start();
    ...
    describe("pathType()", () => {
        ...
        before(() => {
            ...
            schema.virtual("myVirtual").get(() => {
                return 42;
            });
        });

        describe("when called on an explicit real path", () => {
            it('returns "real"', (done) => {
    ...
    it("required() with doc arrays (gh-3199)", (done) => {
    ...
    it("custom typeKey in doc arrays (gh-3560)", (done) => {
        const schema = new Schema({
            test: [{
                name: { $type: String }
            }]
    ...
    it("required for single nested schemas (gh-3562)", (done) => {
    ...
    it("booleans cause cast error for date (gh-3935)", (done) => {
    ...
    it("trim: false works with strings (gh-4042)", (done) => {
        const testSchema = new Schema({
    ...
    it("arrays with typeKey (gh-4548)", (done) => {
        const testSchema = new Schema({
            test: [{ $type: String }]
        }, { typeKey: "$type" });
    ...
    it("arrays of mixed arrays (gh-5416)", (done) => {
        const testSchema = new Schema({
            test: [Array]
        });
    ...
    describe("remove()", () => {
        ...
        it("handles default value = 0 (gh-4620)", (done) => {
            const schema = new Schema({
        ...
        it("type: childSchema (gh-5521)", (done) => {
        ...
        it("Decimal128 type (gh-4759)", (done) => {
        ...
        it("clone() copies methods, statics, and query helpers (gh-5752)", (done) => {
            const schema = new Schema({});

            schema.methods.fakeMethod = function () {
                return "fakeMethod";
            };
            schema.statics.fakeStatic = function () {
                return "fakeStatic";
            };
            schema.query.fakeQueryHelper = function () {
                return "fakeQueryHelper";
            };
        ...
        it("clone() copies validators declared with validate() (gh-5607)", (done) => {
            const schema = new Schema({
                num: Number
            });

            schema.path("num").validate((v) => {
                return v === 42;
            });
        ...
        it("TTL index with timestamps (gh-5656)", (done) => {
        ...
        it("childSchemas prop (gh-5695)", (done) => {
