...
describe("versioning", () => {
    ...
    it("is only added to parent schema (gh-1265)", (done) => {
    ...
    it("versioning without version key", (done) => {
        const V = db.model("Versioning");
        ...
        function test(err) {
            ...
            V.findById(doc).select("numbers comments").exec((err, doc) => {
                assert.ifError(err);
    ...
    it("version works with strict docs", (done) => {
    ...
    it("version works with existing unversioned docs", (done) => {
        const V = db.model("Versioning");

        V.collection.insert({ title: "unversioned", numbers: [1, 2, 3] }, { safe: true }, (err) => {
            assert.ifError(err);
            V.findOne({ title: "unversioned" }, (err, d) => {
                assert.ifError(err);
    ...
    it("versionKey is configurable", (done) => {
    ...
    it("works with numbericAlpha paths", (done) => {
    ...
    describe("doc.increment()", () => {
        it("works without any other changes (gh-1475)", (done) => {
            let V = db.model("Versioning");
    ...
    describe("versioning is off", () => {
        it("when { safe: false } is set (gh-1520)", (done) => {
            let schema1 = new Schema({ title: String }, { safe: false });
            assert.equal(schema1.options.versionKey, false);
            done();
        });
        it("when { safe: { w: 0 }} is set (gh-1520)", (done) => {
            let schema1 = new Schema({ title: String }, { safe: { w: 0 } });
            assert.equal(schema1.options.versionKey, false);
            done();
        });
    });
    ...
    it("can remove version key from toObject() (gh-2675)", (done) => {
    ...
    it("copying doc works (gh-5779)", (done) => {
