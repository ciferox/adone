const {
    multiformat: { CID },
    ipfs: { unixfsExporter, unixfsImporter, UnixFs, ipld: { Ipld: IPLD, dagPb: { DAGLink, DAGNode } }, ipldInMemory: inMemory },
    stream: { pull }
} = adone;
...
describe("exporter sharded", function () {
    ...
    it("exports all files from a sharded directory with subshards", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}`, ipld),
                    collect(cb)
                );
            },
            (exported, cb) => {
    ...
    it("exports one file from a sharded directory", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/file-14`, ipld),
                    collect(cb)
                );
            },
            (exported, cb) => {
    ...
    it("exports one file from a sharded directory sub shard", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/file-30`, ipld),
                    collect(cb)
                );
            },
            (exported, cb) => {
    ...
    it("exports one file from a shard inside a shard inside a shard", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/file-2567`, ipld),
                    collect(cb)
                );
            },
            (exported, cb) => {
    ...
    it("uses maxDepth to only extract a deep folder from the sharded directory", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/foo/bar/baz`, ipld, {
                        maxDepth: 3
                    }),
                    collect(cb)
                );
    ...
    it("uses fullPath extract all intermediate entries from the sharded directory", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/foo/bar/baz/file-1`, ipld, {
                        fullPath: true
                    }),
                    collect(cb)
                );
    ...
    it("exports a file from a sharded directory inside a regular directory inside a sharded directory", (done) => {
        waterfall([
            ...
            (dir, cb) => {
                pull(
                    unixfsExporter(`/ipfs/${dir.toBaseEncodedString()}/normal-dir/shard/file-1`, ipld),
                    collect(cb)
                );
            },
            (exported, cb) => {
