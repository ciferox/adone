const {
    is,
    multiformat: { CID },
    ipfs: { unixfsExporter, unixfsImporter, UnixFs, ipld: { Ipld: IPLD, dagPb: { DAGNode, DAGLink } }, ipldInMemory: inMemory },
    stream: { pull },
    std: { path, fs }
} = adone;
const { zip, pushable: push, streamToPullStream: toPull, pullStreamToStream: toStream } = pull;
...
const { randomBytes, srcPath } = require("./helpers/random_bytes");
...
describe("exporter", () => {
    ...
    it("ensure hash inputs are sanitized", (done) => {
        dagPut((err, result) => {
            ...
            ipld.get(result.cid, (err, res) => {
                ...
                pull(
                    unixfsExporter(result.cid, ipld),
    ...
    it("exports a small file with links", (done) => {
        waterfall([
    ...
    it("exports a chunk of a small file with links", (done) => {
        const offset = 0;
        const length = 5;
    ...
    it("exports a directory one deep", (done) => {
        waterfall([
            (cb) => pull(
                pull.values([{
                    path: "./200Bytes.txt",
    ...
    it("exports a large file > 1mb imported with raw leaves", (done) => {
        waterfall([
            (cb) => pull(
                pull.values([{
                    path: "1.2MiB.txt",
            ...
            (files, cb) => {
                ...
                pull(
                    unixfsExporter(files[0].multihash, ipld),
                    pull.collect(cb)
                );
            },
            (files, cb) => {
    ...
    it("exports a directory containing an empty file whose content gets turned into a ReadableStream", function (done) {
        ...
        waterfall([
            ...
            (result, cb) => {
                ...
                pull(
                    unixfsExporter(dir.multihash, ipld),
                    pull.map((file) => {
                        if (file.content) {
    ...
    it("exports file with data on internal and leaf nodes", (done) => {
        waterfall([
    ...
    it("exports file with data on some internal and leaf nodes", (done) => {
    ...
    it("exports file with data on internal and leaf nodes with an offset that only fetches data from leaf nodes", (done) => {
        waterfall([
            (cb) => createAndPersistNode(ipld, "raw", [0x04, 0x05, 0x06, 0x07], [], cb),
            (leaf, cb) => createAndPersistNode(ipld, "file", [0x00, 0x01, 0x02, 0x03], [
                leaf
