...
function newEngine(network, callback) {
    parallel([
        ...
        engine.start((err) => callback(err, { peer: peerId, engine }));
    });
}
...
describe("Engine", () => {
    ...
    it("partner wants then cancels", function (done) {
        ...
        function partnerWants(dEngine, values, partner, cb) {
            ...
            map(values, (v, cb) => multihashingAsync(Buffer.from(v), "sha2-256", cb), (err, hashes) => {
                expect(err).to.not.exist();
        ...
        createTempRepo((err, repo) => {
            ...
            waterfall([
                (cb) => map(alphabet,
                    (v, cb) => multihashingAsync(Buffer.from(v), "sha2-256", cb),
                    cb
                ),
                (hashes, cb) => each(
                    hashes.map((h, i) => {
    ...
    it("splits large block messages", (done) => {
        ...
        parallel([
            (cb) => newEngine(net, cb),
            (cb) => map(data, (d, cb) => multihashingAsync(d, "sha2-256", (err, hash) => {
                expect(err).to.not.exist();
