const {
    is,
    ipfs: { ipld: { Ipld: IPLD }, ipldInMemory: inMemory, unixfsImporter },
    stream: { pull }
} = adone;
const { values, map, collect, pushable } = pull;
...
describe("importer: flush", () => {
    ...
    it("can push a single root file and flush yields no dirs", (done) => {
        ...
        const importer = unixfsImporter(ipld);
        pull(
            source,
            importer,
            map((node) => {
        ...
        importer.flush((err, hash) => {
            expect(err).to.not.exist();
            expect(is.buffer(hash)).to.be.true();
            source.end();
        });
    });
