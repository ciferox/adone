const {
    multiformat: { CID },
    ipfs: { unixfsImporter, unixfsExporter, ipld: { Ipld: IPLD }, ipldInMemory: inMemory },
    stream: { pull }
} = adone;
const { values, asyncMap, collect, pushable } = pull;
...
describe("builder: directory sharding", () => {
    ...
    describe("basic dirbuilder", () => {
        let nonShardedHash; let shardedHash;
        ...
        it("exporting unsharded hash results in the correct files", (done) => {
            pull(
                unixfsExporter(nonShardedHash, ipld),
                collect((err, nodes) => {
                    try {
                        expect(err).to.not.exist();
                        expect(nodes.length).to.be.eql(2);
        ...
        it("exporting sharded hash results in the correct files", (done) => {
            pull(
                unixfsExporter(shardedHash, ipld),
                collect((err, nodes) => {
                    try {
                        expect(err).to.not.exist();
                        expect(nodes.length).to.be.eql(2);
    ...
    describe("big nested dir", function () {
        ...
        it("exports a big dir with subpath", (done) => {
            ...
            pull(
                unixfsExporter(exportHash, ipld),
                collect(collected)
            );
