const {
    multiformat: { CID },
    ipfs: { ipld: { Ipld: IPLD }, unixfsImporter, unixfsExporter, ipldInMemory: inMemory, UnixFs },
    stream: { pull },
    std: { path }
} = adone;
const { empty, once, values, collect, onEnd } = pull;
...
const collectLeafCids = require("./helpers/collect_leaf_cids");
...
const smallFile = loadFixture(path.join(__dirname, "fixtures/200Bytes.txt"));
    ...
    describe(`importer: ${strategy}`, function () {
        ...
        it("small file (smaller than a chunk) with raw leaves", (done) => {
            pull(
                ...
                unixfsImporter(ipld, Object.assign({}, options, { rawLeaves: true })),
                collect((err, files) => {
                    expect(err).to.not.exist();
