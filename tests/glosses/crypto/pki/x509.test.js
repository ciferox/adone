describe("x509", () => {
    ...
    const findIndex = (array, predicateObj) => {
        let result = -1;
    ...
    it("should convert SHA-1 based certificate to/from PEM", () => {
        const certificate = crypto.pki.certificateFromPem(_pem.certificate);
        assert.equal(crypto.pki.certificateToPem(certificate), _pem.certificate);
    });

    it("should convert SHA-256 based certificate to/from PEM", () => {
        const certificate = crypto.pki.certificateFromPem(_pemSHA256.certificate);
        assert.equal(crypto.pki.certificateToPem(certificate), _pemSHA256.certificate);
    });

    it("should convert SHA-512 based certificate to/from PEM", () => {
        const certificate = crypto.pki.certificateFromPem(_pemSHA512.certificate);
        assert.equal(crypto.pki.certificateToPem(certificate), _pemSHA512.certificate);
    });
    ...
    it("should generate a certificate with authorityKeyIdentifier extension", () => {
        const keys = {
            privateKey: crypto.pki.privateKeyFromPem(_pem.privateKey),
            publicKey: crypto.pki.publicKeyFromPem(_pem.publicKey)
        };
        const attrs = [{
            name: "commonName",
            value: "example.org"
        ...
        const pem = crypto.pki.certificateToPem(cert);
        cert = crypto.pki.certificateFromPem(pem);
        ...
        crypto.pki.verifyCertificateChain(caStore, [cert], (vfd, depth, chain) => {
    ...
    it("should generate and fail to verify a self-signed certificate that is not in the CA store", () => {
        ...
        const caStore = crypto.pki.createCaStore();
        crypto.pki.verifyCertificateChain(caStore, [cert], (vfd, depth, chain) => {
            assert.equal(vfd, crypto.pki.certificateError.unknown_ca);
            return true;
        });
    });
    ...
    it("should verify certificate chain ending with intermediate certificate from CA store", () => {
        ...
        crypto.pki.verifyCertificateChain(caStore, chain, (vfd, depth, chain) => {
            assert.equal(vfd, true);
            return true;
        });
    });
    ...
    it("should fail to verify certificate chain ending with non-CA intermediate certificate from CA store", () => {
        ...
        crypto.pki.verifyCertificateChain(caStore, chain, (vfd, depth, chain) => {
            if (depth === 0) {
                ...
                assert.equal(vfd, crypto.pki.certificateError.bad_certificate);
            }
            return true;
        });
    });
    ...
    it("should verify certificate with sha1WithRSAEncryption signature", () => {
        ...
        "-----END CERTIFICATE-----\r\n";
        const cert = crypto.pki.certificateFromPem(certPem, true);
        const issuer = crypto.pki.certificateFromPem(issuerPem);
    ...
    it("should import certificate with sha256 RSASSA-PSS signature", () => {
        ...
        assert.equal(cert.signatureOid, crypto.pki.oids["RSASSA-PSS"]);
        assert.equal(cert.signatureParameters.hash.algorithmOid, crypto.pki.oids.sha256);
        assert.equal(cert.signatureParameters.mgf.algorithmOid, crypto.pki.oids.mgf1);
        assert.equal(cert.signatureParameters.mgf.hash.algorithmOid, crypto.pki.oids.sha256);
        assert.equal(cert.siginfo.algorithmOid, crypto.pki.oids["RSASSA-PSS"]);
        assert.equal(cert.siginfo.parameters.hash.algorithmOid, crypto.pki.oids.sha256);
        assert.equal(cert.siginfo.parameters.mgf.algorithmOid, crypto.pki.oids.mgf1);
        assert.equal(cert.siginfo.parameters.mgf.hash.algorithmOid, crypto.pki.oids.sha256);
    });
    ...
    it("should export certificate with sha256 RSASSA-PSS signature", () => {
        ...
        "-----END CERTIFICATE-----\r\n";
        const cert = crypto.pki.certificateFromPem(certPem, true);
        assert.equal(crypto.pki.certificateToPem(cert), certPem);
    });
    ...
    describe("public key fingerprints", () => {
        ...
        it("should get a hex MD5 RSAPublicKey fingerprint", () => {
            ...
            const fp = crypto.pki.getPublicKeyFingerprint(
                publicKey,
                { md: crypto.md.md5.create(), type: "RSAPublicKey", encoding: "hex" });
    ...
    describe("CA store", () => {
        it("should add a certificate object to a store", () => {
            const cert = crypto.pki.certificateFromPem(_pem.certificate);
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert);
        ...
        it("should add two certificate objects to a store", () => {
            const cert1 = crypto.pki.certificateFromPem(_pem.certificate);
            const cert2 = crypto.pki.certificateFromPem(_pemSHA256.certificate);
            const caStore = crypto.pki.createCaStore();
        ...
        it("should add two PEM certificates to a store", () => {
            ...
            const cert2 = _pemSHA256.certificate;
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert1);
            caStore.addCertificate(cert2);
            assert.ok(caStore.hasCertificate(cert1));
        ...
        it("should remove a certificate object from a store", () => {
            ...
            assert.equal(crypto.pki.certificateToPem(result), crypto.pki.certificateToPem(cert1));
            assert.ok(!caStore.hasCertificate(cert1));
            assert.ok(caStore.hasCertificate(cert2));
            assert.equal(caStore.listAllCertificates().length, 1);
        });
        ...
        it("should remove a PEM certificate from a store", () => {
            ...
            caStore.addCertificate(cert2);
            const result = caStore.removeCertificate(cert1);
            assert.equal(crypto.pki.certificateToPem(result), cert1);
            assert.ok(!caStore.hasCertificate(cert1));
            assert.ok(caStore.hasCertificate(cert2));
            assert.equal(caStore.listAllCertificates().length, 1);
        });
        ...
        it("should remove a certificate object from a store with two remaining", () => {
            const cert1 = crypto.pki.certificateFromPem(_pem.certificate);
            const cert2 = crypto.pki.certificateFromPem(_pemSHA256.certificate);
            const cert3 = crypto.pki.certificateFromPem(_pemSHA512.certificate);
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert1);
            caStore.addCertificate(cert2);
        ...
        it("should remove a PEM certificate from a store with two remaining", () => {
            const cert1 = _pem.certificate;
            const cert2 = _pemSHA256.certificate;
            const cert3 = _pemSHA512.certificate;
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert1);
            caStore.addCertificate(cert2);
            caStore.addCertificate(cert3);
        ...
        it("should add two certificate objects with the same subject to a store", () => {
            const cert1 = crypto.pki.certificateFromPem(_pemSameSubject[0]);
            const cert2 = crypto.pki.certificateFromPem(_pemSameSubject[1]);
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert1);
            caStore.addCertificate(cert2);
            assert.ok(caStore.hasCertificate(cert1));
        ...
        it("should add two PEM certificates with the same subject to a store", () => {
            const cert1 = _pemSameSubject[0];
            const cert2 = _pemSameSubject[1];
            const caStore = crypto.pki.createCaStore();
            caStore.addCertificate(cert1);
            caStore.addCertificate(cert2);
            assert.ok(caStore.hasCertificate(cert1));
        ...
        it("should fail to remove a PEM certificate from a store", () => {
            ...
            assert.equal(crypto.pki.certificateToPem(result), cert2);
            result = caStore.removeCertificate(cert3);
            assert.equal(crypto.pki.certificateToPem(result), cert3);
