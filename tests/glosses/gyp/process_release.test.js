const { processRelease } = adone.getPrivate(adone.nodejs.gyp);

it("test process release - process.version = 0.8.20", () => {
    const release = processRelease([], { opts: {} }, "v0.8.20", null);

    assert.equal(release.semver.version, "0.8.20");
...
it("test process release - process.version = 0.10.21", () => {
    ...
    assert.equal(release.semver.version, "0.10.21");
    delete release.semver;
...
it("test process release - process.version = 0.12.9", () => {
    ...
    assert.equal(release.semver.version, "0.12.9");
    delete release.semver;
...
it("test process release - process.version = 0.10.41", () => {
    ...
    assert.equal(release.semver.version, "0.10.41");
    delete release.semver;
...
it("test process release - process.release ~ node@0.10.42", () => {
    ...
    assert.equal(release.semver.version, "0.10.42");
    delete release.semver;
...
it("test process release - process.release ~ node@0.12.10", () => {
    ...
    assert.equal(release.semver.version, "0.12.10");
    delete release.semver;
...
it("test process release - process.release ~ node@4.1.23", () => {
    ...
    assert.equal(release.semver.version, "4.1.23");
    delete release.semver;
...
it("test process release - process.version = 1.8.4", () => {
    const release = processRelease([], { opts: {} }, "v1.8.4", null);

    assert.equal(release.semver.version, "1.8.4");
    delete release.semver;
...
it("test process release - process.release ~ iojs@3.2.24", () => {
    const release = processRelease([], { opts: {} }, "v3.2.24", {
        name: "io.js",
        headersUrl: "https://iojs.org/download/release/v3.2.24/iojs-v3.2.24-headers.tar.gz"
    });

    assert.equal(release.semver.version, "3.2.24");
    delete release.semver;
...
it("test process release - process.release ~ iojs@3.2.11 +libUrl32", () => {
    const release = processRelease([], { opts: {} }, "v3.2.11", {
        name: "io.js",
    ...
    assert.equal(release.semver.version, "3.2.11");
    delete release.semver;
...
it("test process release - process.release ~ iojs@3.2.101 +libUrl64", () => {
    const release = processRelease([], { opts: {} }, "v3.2.101", {
        name: "io.js",
    ...
    assert.equal(release.semver.version, "3.2.101");
    delete release.semver;
...
it("test process release - process.release ~ iojs@3.3.0 - borked win-ia32", () => {
    const release = processRelease([], { opts: {} }, "v3.2.101", {
        name: "io.js",
        headersUrl: "https://iojs.org/download/release/v3.2.101/iojs-v3.2.101-headers.tar.gz",
...
it("test process release - process.release ~ node@4.1.23 --target=0.10.40", () => {
    ...
    assert.equal(release.semver.version, "0.10.40");
    delete release.semver;
...
it("test process release - process.release ~ node@4.1.23 --target=1.8.4", () => {
    const release = processRelease([], { opts: { target: "1.8.4" } }, "v4.1.23", {
        name: "node",
        headersUrl: "https://nodejs.org/dist/v4.1.23/node-v4.1.23-headers.tar.gz"
    });
...
it("test process release - process.release ~ node@4.0.0-rc.4", () => {
    ...
    assert.equal(release.semver.version, "4.0.0-rc.4");
    delete release.semver;
...
it("test process release - NVM_NODEJS_ORG_MIRROR", () => {
...
it("test process release - IOJS_ORG_MIRROR", () => {
...
it("test process release - NVM_IOJS_ORG_MIRROR", () => {
