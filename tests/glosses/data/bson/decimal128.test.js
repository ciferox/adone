describe("data", "bson", "decimal 128", () => {
    ...
    describe("decimal128", () => {
        ...
        it("fromString simple", () => {
            ...
            let bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 0
            result = Decimal128.fromString("0");
            bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value -0
            result = Decimal128.fromString("-0");
            bytes = Buffer.from([0xb0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value -1
            result = Decimal128.fromString("-1");
            bytes = Buffer.from([0xb0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 12345678901234567
            result = Decimal128.fromString("12345678901234567");
            bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2b, 0xdc, 0x54, 0x5d, 0x6b, 0x4b, 0x87].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 989898983458
            result = Decimal128.fromString("989898983458");
            bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe6, 0x7a, 0x93, 0xc8, 0x22].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value -12345678901234567
            result = Decimal128.fromString("-12345678901234567");
            bytes = Buffer.from([0xb0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2b, 0xdc, 0x54, 0x5d, 0x6b, 0x4b, 0x87].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 0.12345
            result = Decimal128.fromString("0.12345");
            bytes = Buffer.from([0x30, 0x36, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x39].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 0.0012345
            result = Decimal128.fromString("0.0012345");
            bytes = Buffer.from([0x30, 0x32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x39].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        ...
        it("fromString scientific format", () => {
            ...
            let bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 1e1
            result = Decimal128.fromString("1e1");
            bytes = Buffer.from([0x30, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 10e-1
            result = Decimal128.fromString("10e-1");
            bytes = Buffer.from([0x30, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 12345678901234567e6111
            result = Decimal128.fromString("12345678901234567e6111");
            bytes = Buffer.from([0x5f, 0xfe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2b, 0xdc, 0x54, 0x5d, 0x6b, 0x4b, 0x87].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 1e-6176
            result = Decimal128.fromString("1e-6176");
            bytes = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value "-100E-10
            result = Decimal128.fromString("-100E-10");
            bytes = Buffer.from([0xb0, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x64].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 10.50E8
            result = Decimal128.fromString("10.50E8");
            bytes = Buffer.from([0x30, 0x4c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x1a].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        });
        ...
        it("fromString large format", () => {
            ...
            let bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x02, 0x9d, 0x42, 0xda, 0x3a, 0x76, 0xf9, 0xe0, 0xd9, 0x79].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x40, 0x3c, 0xde, 0x6f, 0xff, 0x97, 0x32, 0xde, 0x82, 0x5c, 0xd0, 0x7e, 0x96, 0xaf, 0xf2].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x5f, 0xff, 0xed, 0x09, 0xbe, 0xad, 0x87, 0xc0, 0x37, 0x8d, 0x8e, 0x63, 0xff, 0xff, 0xff, 0xff].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x00, 0x01, 0xed, 0x09, 0xbe, 0xad, 0x87, 0xc0, 0x37, 0x8d, 0x8e, 0x63, 0xff, 0xff, 0xff, 0xff].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x40, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        });
        ...
        it("fromString exponent normalization", () => {
            ...
            let bytes = Buffer.from([0x30, 0x4c, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x42, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x40, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x37, 0xcc, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 1E-6177
            result = Decimal128.fromString("1E-6177");
            bytes = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        });
        ...
        it("fromString from string zeros", () => {
            ...
            let bytes = Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 0e-611
            result = Decimal128.fromString("0e-611");
            bytes = Buffer.from([0x2b, 0x7a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 0e+6000
            result = Decimal128.fromString("0e+6000");
            bytes = Buffer.from([0x5f, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 1E-6177
            result = Decimal128.fromString("-0e-1");
            bytes = Buffer.from([0xb0, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        });
        ...
        it("fromString from string round", () => {
            ...
            let bytes = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            // Create decimal from string value 15E-6177
            result = Decimal128.fromString("15E-6177");
            bytes = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);

            result = Decimal128.fromString("251E-6178");
            bytes = Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x42, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x44, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x01].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x42, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0x44, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
            ...
            bytes = Buffer.from([0x30, 0xc6, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse());
            expect(bytes).to.be.deep.equal(result.bytes);
        ...
        it("toString infinity", () => {
            let decimal = new Decimal128(Buffer.from([0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("Infinity").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0xf8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("-Infinity").to.be.equal(decimal.toString());
        });

        it("toString NaN", () => {
            let decimal = new Decimal128(Buffer.from([0x7c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("NaN").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0xfc, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("NaN").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x7e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("NaN").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0xfe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("NaN").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x7e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x12].reverse()));
            expect("NaN").to.be.equal(decimal.toString());
        });

        it("toString regular", () => {
            let decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("1").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("0").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02].reverse()));
            expect("2").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0xb0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("-1").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0xb0, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("-0").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("0.1").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x34, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0xd2].reverse()));
            expect("0.001234").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1c, 0xbe, 0x99, 0x1a, 0x14].reverse()));
            expect("123456789012").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x2a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x5a, 0xef, 0x40].reverse()));
            expect("0.00123400000").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x2f, 0xfc, 0x3c, 0xde, 0x6f, 0xff, 0x97, 0x32, 0xde, 0x82, 0x5c, 0xd0, 0x7e, 0x96, 0xaf, 0xf2].reverse()));
            expect("0.1234567890123456789012345678901234").to.be.equal(decimal.toString());
        });

        it("toString scientific", () => {
            let decimal = new Decimal128(Buffer.from([0x5f, 0xfe, 0x31, 0x4d, 0xc6, 0x44, 0x8d, 0x93, 0x38, 0xc1, 0x5b, 0x0a, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("1.000000000000000000000000000000000E+6144").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("1E-6176").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("-1E-6176").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x31, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x09, 0x18, 0x4d, 0xb6, 0x3e, 0xb1].reverse()));
            expect("9.999987654321E+112").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x5f, 0xff, 0xed, 0x09, 0xbe, 0xad, 0x87, 0xc0, 0x37, 0x8d, 0x8e, 0x63, 0xff, 0xff, 0xff, 0xff].reverse()));
            expect("9.999999999999999999999999999999999E+6144").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x00, 0x01, 0xed, 0x09, 0xbe, 0xad, 0x87, 0xc0, 0x37, 0x8d, 0x8e, 0x63, 0xff, 0xff, 0xff, 0xff].reverse()));
            expect("9.999999999999999999999999999999999E-6143").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x40, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff].reverse()));
            expect("5192296858534827628530496329220095").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x4c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x1a].reverse()));
            expect("1.050E+9").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x1a].reverse()));
            expect("1.050E+4").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x69].reverse()));
            expect("105").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x69].reverse()));
            expect("1.05E+3").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x30, 0x46, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01].reverse()));
            expect("1E+3").to.be.equal(decimal.toString());
        });

        it("toString zeros", () => {
            let decimal = new Decimal128(Buffer.from([0x30, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("0").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x32, 0x98, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
            expect("0E+300").to.be.equal(decimal.toString());

            decimal = new Decimal128(Buffer.from([0x2b, 0x90, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00].reverse()));
