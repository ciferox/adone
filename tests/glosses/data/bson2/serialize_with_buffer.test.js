...
describe("data", "bson", "serializeWithBuffer", () => {
    ...
    it("correctly serialize 3 different docs into buffer using serializeWithBufferAndIndex", (done) => {
        ...
        data.forEach((item) => {
            idx =
                bson2.encodeWithBufferAndIndex(item, bf, {
                    index: idx
                }) + 1;
        });

        expect(bson2.decode(bf.slice(0, 23))).to.deep.equal(data[0]);
        expect(bson2.decode(bf.slice(23, 46))).to.deep.equal(data[1]);
        expect(bson2.decode(bf.slice(46, 69))).to.deep.equal(data[2]);
        done();
    });
});
