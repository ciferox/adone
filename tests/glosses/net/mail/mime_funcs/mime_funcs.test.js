...
describe("glosses", "net", "mail", "Mime-Funcs Tests", () => {
    ...
    describe("#buildHeaderParam", () => {
        ...
        it("should encode and decode", () => {
            ...
            const headerLine = `content-disposition: attachment; ${mimeFuncs.buildHeaderParam("filename", input, 50).map((item) => {
                return `${item.key}="${ item.value}"`;
            }).join("; ")}`;
            const parsedHeader = mimeFuncs.parseHeaderValue(headerLine);
