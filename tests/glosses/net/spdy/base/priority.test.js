...
describe("Stream Priority tree", () => {
    let tree;
    beforeEach(() => {
        tree = transport.Priority.create({});
    });

    it("should create basic tree", () => {
        //                   0
        //     [1 p=2]    [2 p=4]    [3 p=2]
        // [4 p=2] [5 p=2]
    ...
    it("should create default node on error", () => {
        const node1 = tree.add({ id: 1, parent: 1 });
        assert.equal(node1.parent.id, 0);
        assert.equal(node1.weight, tree.defaultWeight);

        const node2 = tree.add({ id: 1, parent: 3 });
        assert.equal(node2.parent.id, 0);
        assert.equal(node2.weight, tree.defaultWeight);
    });

    it("should remove empty node", () => {
        const node = tree.add({ id: 1, parent: 0, weight: 1 });
        assert(!is.undefined(tree.get(1)));
    ...
    it("should move children to parent node on removal", () => {
        // Tree from the first test
        const one = tree.add({ id: 1, parent: 0, weight: 2 });
        tree.add({ id: 5, parent: 1, weight: 2 });
        tree.add({ id: 2, parent: 0, weight: 4 });
        tree.add({ id: 3, parent: 0, weight: 2 });
        tree.add({ id: 4, parent: 1, weight: 2 });
        ...
        assert.deepEqual([2, 3, 4, 5].map((id) => {
            return tree.get(id).priority;
        }), [0.4, 0.2, 0.2, 0.19999999999999996]);
    });

    it("should move children on exclusive addition", () => {
        //            0
        //          /   \
        //        1       2
        //      / | \
        ...
        assert.deepEqual([1, 2, 3, 4, 5, 6].map((id) => {
            return tree.get(id).priority;
        }), [0.5, 0.5, 0.25, 0.125, 0.125, 0.5]);
    });

    it("should remove excessive nodes on hitting maximum", () => {
        tree = transport.Priority.create({
            maxCount: 6
        });
        ...
        assert.deepEqual([2, 3, 4, 5, 6].map((id) => {
            return tree.get(id).priority;
