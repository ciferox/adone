...
describe("net", "amqp", () => {
    describe("Implicit encodings", () => {
        ...
        testCases.forEach((tc) => {
            let name = tc[0], val = tc[1], expect = tc[2];
            it(name, () => {
    ...
    const roundtrips = (T) => {
        return forAll(T).satisfy((v) => {
            return roundtripTable({ value: v });
        });
    };
    ...
    const roundtripProperties = (Properties) => {
