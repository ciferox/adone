const { is, net: { ssh: { util } } } = adone;

const fs = adone.std.fs;
const path = adone.std.path;
const fixturesdir = path.join(__dirname, "fixtures");

describe("net", "ssh", "streams", "utils", () => {
    it("readInt - without stream callback - failure #1", () => {
        const r = util.readInt(Buffer.from([0, 0, 0]), 0);
        assert.strictEqual(r, false, `Wrong result: ${r}`);
    });

    it("readInt - without stream callback - failure #2", () => {
        const r = util.readInt(Buffer.from([]), 0);
        assert.strictEqual(r, false, `Wrong result: ${r}`);
    });

    it("readInt - without stream callback - success", () => {
        const r = util.readInt(Buffer.from([0, 0, 0, 5]), 0);
        assert.strictEqual(r, 5, `Wrong result: ${r}`);
    });

    it("readInt - with stream callback", () => {
        ...
        const r = util.readInt(Buffer.from([]), 0, stream, callback);
        assert.strictEqual(r, false, `Wrong result: ${r}`);
        assert(cleanupCalled, "Cleanup not called");
    });

    it("readString - without stream callback - bad length #1", () => {
        const r = util.readString(Buffer.from([0, 0, 0]), 0);
        assert.strictEqual(r, false, `Wrong result: ${r}`);
    });

    it("readString - without stream callback - bad length #2", () => {
        const r = util.readString(Buffer.from([]), 0);
        assert.strictEqual(r, false, `Wrong result: ${r}`);
    });

    it("readString - without stream callback - success", () => {
        const r = util.readString(Buffer.from([0, 0, 0, 1, 5]), 0);
        assert.deepEqual(r, Buffer.from([5]), `Wrong result: ${r}`);
    });

    it("readString - without stream callback - encoding", () => {
        const r = util.readString(Buffer.from([0, 0, 0, 1, 33]), 0, "ascii");
        assert.deepEqual(r, "!", `Wrong result: ${r}`);
    });

    it("readString - with stream callback - no encoding", () => {
        let cleanupCalled = false;
        const callback = function () { };
        const stream = {
            _cleanup(cb) {
        ...
        const r = util.readString(Buffer.from([0, 0, 0, 1]), 0, stream, callback);

        assert.deepEqual(r, false, `Wrong result: ${r}`);
        assert(cleanupCalled, "Cleanup not called");
    });

    it("readString - with stream callback - encoding", () => {
        let cleanupCalled = false;
        const callback = function () { };
        const stream = {
            _cleanup(cb) {
        ...
        const r = util.readString(Buffer.from([0, 0, 0, 1]), 0, "ascii", stream, callback);
    ...
    it.skip("decryptKey - with encrypted RSA PPK", () => {
        ...
        util.decryptKey(keyInfo, passphrase);

        const expPrivOrig = Buffer.from([
        ...
        assert(keyInfo._converted === true, "Expected automatic private PEM generation");
        assert(keyInfo._macresult === true, "Expected successful MAC verification");
        assert(keyInfo._decrypted === true, "Expected decrypted flag");
        assert.deepEqual(keyInfo.privateOrig, expPrivOrig, "Decrypted private PEM data mismatch");
    });

    it("decryptKey - with encrypted DSA PPK", () => {
    ...
    it("decryptKey - with encrypted RSA", () => {
        ...
        assert(is.undefined(keyInfo.ppk), "Unexpected PPK flag");
        assert(is.undefined(keyInfo._converted), "Unexpected automatic private PEM generation");
        assert(is.undefined(keyInfo._macresult), "Unexpected MAC verification");
        assert(keyInfo._decrypted === true, "Expected decrypted flag");
        assert.deepEqual(keyInfo.private, expPriv, "Decrypted private data mismatch");
        assert.deepEqual(keyInfo.privateOrig, expPrivOrig, "Decrypted private PEM data mismatch");
    });

    it("Generate public key from parsed public key", () => {
        const pubkey = [
        ...
        assert.doesNotThrow(() => {
            const res = util.genPublicKey(util.parseKey(pubkey));
