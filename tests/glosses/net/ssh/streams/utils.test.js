...
describe("SSH-Streams", () => {
    describe("Utils", () => {
        ...
        it("readInt - with stream callback", () => {
            ...
            const r = util.readInt(new Buffer([]), 0, stream, callback);
            assert.strictEqual(r, false, `Wrong result: ${r}`);
            assert(cleanupCalled, "Cleanup not called");
        });

        it("readString - without stream callback - bad length #1", () => {
            const r = util.readString(new Buffer([0, 0, 0]), 0);
            assert.strictEqual(r, false, `Wrong result: ${r}`);
        });

        it("readString - without stream callback - bad length #2", () => {
            const r = util.readString(new Buffer([]), 0);
            assert.strictEqual(r, false, `Wrong result: ${r}`);
        });

        it("readString - without stream callback - success", () => {
            const r = util.readString(new Buffer([0, 0, 0, 1, 5]), 0);
            assert.deepEqual(r, new Buffer([5]), `Wrong result: ${r}`);
        });

        it("readString - without stream callback - encoding", () => {
            const r = util.readString(new Buffer([0, 0, 0, 1, 33]), 0, "ascii");
        ...
        it("readString - with stream callback - no encoding", () => {
            ...
            const r = util.readString(new Buffer([0, 0, 0, 1]), 0, stream, callback);
        ...
        it("readString - with stream callback - encoding", () => {
            ...
            const r = util.readString(new Buffer([0, 0, 0, 1]), 0, "ascii", stream, callback);
        ...
        it("decryptKey - with encrypted RSA", () => {
            ...
            assert(keyInfo.ppk === undefined, "Unexpected PPK flag");
            assert(keyInfo._converted === undefined, "Unexpected automatic private PEM generation");
            assert(keyInfo._macresult === undefined, "Unexpected MAC verification");
            assert(keyInfo._decrypted === true, "Expected decrypted flag");
