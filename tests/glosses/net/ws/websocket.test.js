const { net: { ws: { constants, WebSocket, WebSocketServer } }, std: { http, https, fs, crypto } } = adone;
let port = 20000;
...
describe("net", "ws", "WebSocket", () => {
    describe("#ctor", () => {
        it("throws an error when using an invalid url", () => {
            assert.throws(() => new WebSocket("echo.websocket.org"), /^invalid url$/);
        });
    });
    ...
    describe("options", () => {
        ...
        it("accepts the `options` object as the 3rd argument", () => {
            ...
            new WebSocket("ws://localhost", undefined, { agent });
            new WebSocket("ws://localhost", null, { agent });
            new WebSocket("ws://localhost", [], { agent });
        ...
        it("throws an error when using an invalid `protocolVersion`", () => {
            ...
            assert.throws(() => new WebSocket("ws://localhost", options), /^unsupported protocol version: 1000 \(supported versions: 8, 13\)$/);
        });
        ...
        it("accepts the localAddress option whether it was wrong interface", () => {
            assert.throws(() => new WebSocket(`ws://localhost:${port}`, { localAddress: "123.456.789.428" }), /must be a valid IP: 123.456.789.428/);
        });
    ...
    describe("host and origin headers", () => {
        ...
        it("lacks default origin header", (done) => {
            ...
            new WebSocket(`ws://localhost:${port}`, { agent });
        });
        ...
        it("excludes default ports from host header", () => {
            ...
            new WebSocket("wss://localhost:8443", { agent: httpsAgent });
            new WebSocket("wss://localhost:443", { agent: httpsAgent });
            new WebSocket("ws://localhost:88", { agent: httpAgent });
            new WebSocket("ws://localhost:80", { agent: httpAgent });
