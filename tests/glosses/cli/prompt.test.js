...
describe("cli", "prompt", () => {
    ...
    class Collector {
        ...
        data() {
            ...
            return adone.text.stripAnsi(this.chunks.join(""));
        }
    }
    ...
    const createPrompt = (prompts) => {
        manager = new prompt.Manager(term);
        if (prompts) {
            return manager.run(prompts);
        }
        return manager;
    };

    const getOutputCollector = () => new Collector(term.output);
    ...
    const emitKeypress = async (...args) => {
        manager.rl.input.emit("keypress", ...args);
    };
    ...
    describe("Manager", () => {
        it("should take a prompts array and return answers", async () => {
            ...
            const promise = createPrompt(prompts);
        ...
        it("should take a single prompt and return answer", async () => {
            ...
            const promise = createPrompt(prompt);
            emitEnter();
            const answers = await promise;
    ...
    beforeEach(() => {
        fixtures = {
            ...
            list: {
                message: "message",
                name: "name",
                choices: ["foo", new prompt.Separator(), "bar", "bum"]
            },

            rawlist: {
                message: "message",
                name: "name",
                choices: ["foo", "bar", new prompt.Separator(), "bum"]
            },
            ...
            autocomplete: {
                message: "message",
                name: "name",
                source: async () => ["foo", new prompt.Separator(), "bar", "bum"]
            }
        };
    });
        ...
        const tests = {
            filter(type) {
                describe("filter API", () => {
                    it("should filter the user input", async function () {
                        ...
                        const prompt = new this.Prompt(manager, this.fixture);
                    ...
                    it("should pass previous answers to the prompt filter function", async () => {
                        ...
                        const promise = createPrompt(questions);
                        await delay(50);
                        emitLines([
                            "Y",
                            ""
            ...
            message() {
                describe("message API", () => {
                    ...
                    it("should default to name for message", async function () {
                        ...
                        expect(collector.data()).to.contain(`${this.fixture.name}:`);
                    });
                });
            },
            ...
            choices() {
                describe("choices API", () => {
                    it("should print choices to screen", async function () {
                        const p = new this.Prompt(manager, this.fixture);
                        const choices = p.choices;
                        const collector = getOutputCollector();
                        p.run();
                        await delay(50);
                        for (const choice of choices.filter(prompt.Separator.exclude)) {
                ...
                describe("Missing value", () => {
                    it("`name` should throw", function () {
                        const mkPrompt = () => {
                            ...
                            return new this.Prompt(manager, this.fixture);
                        };
        ...
        for (const detail of prompts) {
            ...
            describe(`on ${detail.name} prompt`, () => {
                beforeEach(function () {
                    ...
                    this.Prompt = prompt.type[detail.name];
                });
    ...
    describe("prompts", () => {
        ...
        describe("base", () => {
            it("should not point by reference to the entry `question` object", () => {
                ...
                const base = new prompt.BasePrompt(term, question);
        ...
        describe("checkbox", () => {
            const createCheckbox = (fixture, answers) => new prompt.type.checkbox(manager, fixture, answers);
        ...
        describe("confirm", () => {
            const createConfirm = (fixture) => new prompt.type.confirm(manager, fixture);
        ...
        describe("editor", () => {
            class EditorPrompt extends prompt.type.editor {
            ...
            it("should retrieve temporary files contents", async function () {
                const prompt = new EditorPrompt(manager, this.fixture);
                const promise = prompt.run();
                emitLines(2);
                const answer = await promise;
        ...
        describe("expand", () => {
            const createExpand = (fixture) => new prompt.type.expand(manager, fixture);
        ...
        describe("input", () => {
            const createInput = (fixture, answers) => new prompt.type.input(manager, fixture, answers);
            ...
            it("should use the flags object in the provided transformer", async function () {
                ...
                expect(collector.data()).to.contain("INQUIRER");
            });
        });

        describe("list", () => {
            const createList = (fixture) => new prompt.type.list(manager, fixture);
        ...
        describe("password", () => {
            const createPassword = (fixture) => new prompt.type.password(manager, fixture);
        ...
        describe("rawlist", () => {
            const createRawlist = (fixture) => new prompt.type.rawlist(manager, fixture);
        ...
        describe("autocomplete", () => {
            const createAutocomplete = (fixture) => new prompt.type.autocomplete(manager, fixture);
    ...
    describe("objects", () => {
        describe("Choice", () => {
            const { Choices: { Choice }, Separator } = prompt;
        ...
        describe("Choices", () => {
            const { Choices, Separator } = prompt;
