...
describe("select", () => {
    ...
    describe("rendering", () => {
        it("should render a choice with the correct styles", () => {
            ...
            return prompt.run()
                .then(async (answer) => {
                    const expected = `${cyan(prompt.symbols.pointer)} ${cyan.underline("A")}`;
        ...
        it.skip("should render a choice hint", (cb) => {
            ...
            prompt.once("run", async () => {
                try {
                    const { state, symbols } = prompt;
                    const pointer = cyan(symbols.pointer);
                    const hint = dim("(this is a hint)");
                    const expected = `${pointer} ${cyan.underline("A")}\n  BB ${hint}\n  CCC\n  DDDD`;
