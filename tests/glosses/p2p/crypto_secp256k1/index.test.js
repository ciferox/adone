...
const srcPath = (...args) => adone.std.path.join(adone.ROOT_PATH, "lib", "glosses", "p2p", "crypto_secp256k1", ...args);
...
describe("secp256k1 keys", () => {
    let key;
    const secp256k1 = p2p.cryptoSecp256k1(keysPBM, randomBytes);
...
describe("key generation error", () => {
    ...
    before((done) => {
        ...
        secp256k1 = cryptoSecp256k1(keysPBM, randomBytes, crypto);
...
describe("handles generation of invalid key", () => {
    ...
    before((done) => {
        generateKey = crypto.generateKey;
        crypto.generateKey = (callback) => {
            callback(null, Buffer.from("not a real key"));
        };
        secp256k1 = cryptoSecp256k1(keysPBM, randomBytes, crypto);
...
describe("go interop", () => {
    const secp256k1 = cryptoSecp256k1(keysPBM, randomBytes);
