...
const Responder = require(srcPath("compat/responder"));
const { SERVICE_TAG_LOCAL, MULTICAST_IP, MULTICAST_PORT } = require(srcPath("compat/constants"));

describe("Responder", () => {
    let responder; let mdns;
    const peerAddrs = [
...
function isResponseFrom(res, fromPeerInfo) {
    ...
    if (fromPeerInfo.id.toB58String() !== peerIdStr) {
        return false;
    }
