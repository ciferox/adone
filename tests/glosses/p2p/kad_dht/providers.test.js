...
const createPeerInfo = promisify(require("./utils/create_peer_info"));
const createValues = promisify(require("./utils/create_values"));
...
describe("Providers", () => {
    ...
    it("more providers than space in the lru cache", async () => {
        ...
        const hashes = await Promise.all([...new Array(100)].map((i) => {
            return multihashingAsync(Buffer.from(`hello ${i}`), "sha2-256");
        }));
