...
const servers = [
    "/ip4/127.0.0.1/tcp/15001/ws/p2p-websocket-star",
    "/ip4/127.0.0.1/tcp/15002/ws/p2p-websocket-star"
].concat(!process.env.TRAVIS ? ["/ip6/::1/tcp/15003/ws/p2p-websocket-star"] : []);
...
describe("p2p", "transport", "WSStarMulti", () => {
    describe("websocket-star-multi", () => {
        let ids;
        before((cb) => map(require("./ids.json"), PeerId.createFromJSON, (err, _) => err ? cb(err) : cb(null, (ids = _))));
        let l = [];
        describe("listen", () => {
            let id;
            before(() => {
            ...
            it("should listen on 1 address", (cb) => {
                const listener = new WSStarMulti({
                    id,
                    servers: [servers[0]]
                }).createListener();
                l.push(listener);
