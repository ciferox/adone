...
const { std: { fs, crypto }, collection: { BufferList } } = adone;
...
describe("glosses", () => {
    describe("collections", () => {
        describe("buffer list", () => {
            ...
            it("append chainable", () => {
                const bl = new BufferList();
                assert(bl.append(new Buffer("abcd")) === bl);
                assert(bl.append([new Buffer("abcd")]) === bl);
                assert(bl.append(new BufferList(new Buffer("abcd"))) === bl);
                assert(bl.append([new BufferList(new Buffer("abcd"))]) === bl);
            });
            ...
            it("test readUInt8 / readInt8", () => {
                ...
                const buf2 = new Buffer(3);
                const buf3 = new Buffer(3);
                const bl = new BufferList();
            ...
            it("test toString encoding", () => {
                const bl = new BufferList();
                const b = new Buffer("abcdefghij\xff\x00");
            ...
            it("unicode string", () => {
                ...
                assert.equal(new Buffer(exp).toString("hex"), bl.toString("hex"));
            });
            ...
            it("shallow slice does not make a copy", () => {
                ...
                const buffers = [new Buffer("First"), new Buffer("Second"), new Buffer("Third")];
