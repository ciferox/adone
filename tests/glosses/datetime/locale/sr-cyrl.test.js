...
describe("datetime", "locale", "sr-cyrl", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "за неколико секунди", "prefix");
        assert.equal(adone.datetime(0).from(30000), "пре неколико секунди", "prefix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "пре неколико секунди", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "данас у 12:00", "today at the same time");
