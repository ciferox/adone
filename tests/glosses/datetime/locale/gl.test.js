...
describe("gl", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "nuns segundos", "prefix");
        assert.equal(adone.datetime(0).from(30000), "hai uns segundos", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "hai uns segundos", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "hoxe ás 12:00", "today at the same time");
    ...
    it("regression tests", () => {
        const lastWeek = adone.datetime().subtract({
            d: 4
        }).hours(1);
