...
describe("datetime", "locale", "se", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "moadde sekunddat geažes", "prefix");
        assert.equal(adone.datetime(0).from(30000), "maŋit moadde sekunddat", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "maŋit moadde sekunddat", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "otne ti 12:00", "Today at the same time");
