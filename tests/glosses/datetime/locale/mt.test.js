...
describe("datetime", "locale", "mt", () => {
    ...
    it("from", () => {
        ...
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 44 }), true), "ftit sekondi", "44 seconds = a few seconds");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 45 }), true), "minuta", "45 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 89 }), true), "minuta", "89 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 90 }), true), "2 minuti", "90 seconds = 2 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 44 }), true), "44 minuti", "44 minutes = 44 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 45 }), true), "siegħa", "45 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 89 }), true), "siegħa", "89 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 90 }), true), "2 siegħat", "90 minutes = 2 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 5 }), true), "5 siegħat", "5 hours = 5 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 21 }), true), "21 siegħat", "21 hours = 21 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 22 }), true), "ġurnata", "22 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 35 }), true), "ġurnata", "35 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 36 }), true), "2 ġranet", "36 hours = 2 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 1 }), true), "ġurnata", "1 day = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 5 }), true), "5 ġranet", "5 days = 5 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 25 }), true), "25 ġranet", "25 days = 25 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 26 }), true), "xahar", "26 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 30 }), true), "xahar", "30 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 43 }), true), "xahar", "43 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 46 }), true), "2 xhur", "46 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 74 }), true), "2 xhur", "75 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 76 }), true), "3 xhur", "76 days = 3 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 1 }), true), "xahar", "1 month = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 5 }), true), "5 xhur", "5 months = 5 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 345 }), true), "sena", "345 days = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 548 }), true), "2 sni", "548 days = 2 years");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 1 }), true), "sena", "1 year = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 5 }), true), "5 sni", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "f’ ftit sekondi", "prefix");
        assert.equal(adone.datetime(0).from(30000), "ftit sekondi ilu", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "ftit sekondi ilu", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(adone.datetime().add({ s: 30 }).fromNow(), "f’ ftit sekondi", "in a few seconds");
        assert.equal(adone.datetime().add({ d: 5 }).fromNow(), "f’ 5 ġranet", "in 5 days");
    });

    it("calendar day", () => {
        const a = adone.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(adone.datetime(a).calendar(), "Illum fil-12:00", "today at the same time");
        assert.equal(adone.datetime(a).add({ m: 25 }).calendar(), "Illum fil-12:25", "Now plus 25 min");
        assert.equal(adone.datetime(a).add({ h: 1 }).calendar(), "Illum fil-13:00", "Now plus 1 hour");
        assert.equal(adone.datetime(a).add({ d: 1 }).calendar(), "Għada fil-12:00", "tomorrow at the same time");
        assert.equal(adone.datetime(a).subtract({ h: 1 }).calendar(), "Illum fil-11:00", "Now minus 1 hour");
        assert.equal(adone.datetime(a).subtract({ d: 1 }).calendar(), "Il-bieraħ fil-12:00", "yesterday at the same time");
    });

    it("calendar next week", () => {
        for (let i = 2; i < 7; i++) {
            const m = adone.datetime().add({ d: i });
    ...
    it("calendar last week", () => {
        for (let i = 2; i < 7; i++) {
            const m = adone.datetime().subtract({ d: i });
    ...
    it("calendar all else", () => {
        ...
        let weeksFromNow = adone.datetime().add({ w: 1 });
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(adone.datetime([2012, 0, 1]).format("w ww wo"), "52 52 52º", "Jan  1 2012 should be week 52");
        assert.equal(adone.datetime([2012, 0, 2]).format("w ww wo"), "1 01 1º", "Jan  2 2012 should be week 1");
        assert.equal(adone.datetime([2012, 0, 8]).format("w ww wo"), "1 01 1º", "Jan  8 2012 should be week 1");
