...
describe("sw", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "hivi punde baadaye", "prefix");
        assert.equal(adone.datetime(0).from(30000), "tokea hivi punde", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "tokea hivi punde", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "leo saa 12:00", "today at the same time");
