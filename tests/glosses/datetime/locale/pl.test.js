...
describe("datetime", "locale", "pl", () => {
    ...
    it("parse strict", () => {
        ...
        function equalTest(input, mmm, i) {
            assert.equal(adone.datetime(input, mmm, true).month(), i, `${input} should be month ${i + 1}`);
        }
        for (i = 0; i < 12; i++) {
            tests[i] = tests[i].split(" ");
            equalTest(tests[i][0], "MMMM", i);
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "za kilka sekund", "prefix");
        assert.equal(adone.datetime(0).from(30000), "kilka sekund temu", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "kilka sekund temu", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "Dziś o 12:00", "today at the same time");
