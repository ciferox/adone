...
describe("datetime", "locale", "gom-latn", () => {
    ...
    before(() => {
        adone.datetime.locale("gom-latn");
    });
    ...
    it("from", () => {
        ...
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 44 }), true), "thodde secondanim", "44 seconds = a few seconds");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 45 }), true), "eka mintan", "45 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 89 }), true), "eka mintan", "89 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 90 }), true), "2 mintanim", "90 seconds = 2 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 44 }), true), "44 mintanim", "44 minutes = 44 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 45 }), true), "eka horan", "45 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 89 }), true), "eka horan", "89 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 90 }), true), "2 horanim", "90 minutes = 2 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 5 }), true), "5 horanim", "5 hours = 5 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 21 }), true), "21 horanim", "21 hours = 21 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 22 }), true), "eka disan", "22 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 35 }), true), "eka disan", "35 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 36 }), true), "2 disanim", "36 hours = 2 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 1 }), true), "eka disan", "1 day = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 5 }), true), "5 disanim", "5 days = 5 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 25 }), true), "25 disanim", "25 days = 25 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 26 }), true), "eka mhoinean", "26 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 30 }), true), "eka mhoinean", "30 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 43 }), true), "eka mhoinean", "43 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 46 }), true), "2 mhoineanim", "46 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 74 }), true), "2 mhoineanim", "75 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 76 }), true), "3 mhoineanim", "76 days = 3 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 1 }), true), "eka mhoinean", "1 month = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 5 }), true), "5 mhoineanim", "5 months = 5 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 345 }), true), "eka vorsan", "345 days = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 548 }), true), "2 vorsanim", "548 days = 2 years");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 1 }), true), "eka vorsan", "1 year = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 5 }), true), "5 vorsanim", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "thodde second", "prefix");
        assert.equal(adone.datetime(0).from(30000), "thodde second adim", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "thodde second adim", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(adone.datetime().add({ s: 30 }).fromNow(), "thodde second", "in a few seconds");
        assert.equal(adone.datetime().add({ d: 5 }).fromNow(), "5 dis", "in 5 days");
    });

    it("calendar day", () => {
        const a = adone.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(adone.datetime(a).calendar(), "Aiz donparam 12:00 vazta", "today at the same time");
        assert.equal(adone.datetime(a).add({ m: 25 }).calendar(), "Aiz donparam 12:25 vazta", "Now plus 25 min");
        assert.equal(adone.datetime(a).add({ h: 1 }).calendar(), "Aiz donparam 1:00 vazta", "Now plus 1 hour");
        assert.equal(adone.datetime(a).add({ d: 1 }).calendar(), "Faleam donparam 12:00 vazta", "tomorrow at the same time");
        assert.equal(adone.datetime(a).subtract({ h: 1 }).calendar(), "Aiz sokalli 11:00 vazta", "Now minus 1 hour");
        assert.equal(adone.datetime(a).subtract({ d: 1 }).calendar(), "Kal donparam 12:00 vazta", "yesterday at the same time");
    });
