...
describe("datetime", "locale", "de-ch", () => {
    ...
    it("from", () => {
        ...
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 44 }), true), "ein paar Sekunden", "44 seconds = a few seconds");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 45 }), true), "eine Minute", "45 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 89 }), true), "eine Minute", "89 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 90 }), true), "2 Minuten", "90 seconds = 2 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 44 }), true), "44 Minuten", "44 minutes = 44 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 45 }), true), "eine Stunde", "45 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 89 }), true), "eine Stunde", "89 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 90 }), true), "2 Stunden", "90 minutes = 2 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 5 }), true), "5 Stunden", "5 hours = 5 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 21 }), true), "21 Stunden", "21 hours = 21 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 22 }), true), "ein Tag", "22 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 35 }), true), "ein Tag", "35 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 36 }), true), "2 Tage", "36 hours = 2 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 1 }), true), "ein Tag", "1 day = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 5 }), true), "5 Tage", "5 days = 5 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 25 }), true), "25 Tage", "25 days = 25 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 26 }), true), "ein Monat", "26 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 30 }), true), "ein Monat", "30 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 43 }), true), "ein Monat", "43 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 46 }), true), "2 Monate", "46 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 74 }), true), "2 Monate", "75 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 76 }), true), "3 Monate", "76 days = 3 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 1 }), true), "ein Monat", "1 month = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 5 }), true), "5 Monate", "5 months = 5 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 345 }), true), "ein Jahr", "345 days = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 548 }), true), "2 Jahre", "548 days = 2 years");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 1 }), true), "ein Jahr", "1 year = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 5 }), true), "5 Jahre", "5 years = 5 years");
    });
    ...
    it("fromNow", () => {
        assert.equal(adone.datetime().add({ s: 30 }).fromNow(), "in ein paar Sekunden", "in a few seconds");
        assert.equal(adone.datetime().add({ d: 5 }).fromNow(), "in 5 Tagen", "in 5 days");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).add({ m: 25 }).calendar(), "heute um 12.25 Uhr", "Now plus 25 min");
        assert.equal(adone.datetime(a).add({ h: 1 }).calendar(), "heute um 13.00 Uhr", "Now plus 1 hour");
        assert.equal(adone.datetime(a).add({ d: 1 }).calendar(), "morgen um 12.00 Uhr", "tomorrow at the same time");
        assert.equal(adone.datetime(a).subtract({ h: 1 }).calendar(), "heute um 11.00 Uhr", "Now minus 1 hour");
        assert.equal(adone.datetime(a).subtract({ d: 1 }).calendar(), "gestern um 12.00 Uhr", "yesterday at the same time");
    });
