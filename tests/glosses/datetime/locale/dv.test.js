...
describe("dv", () => {
    ...
    it("format month", () => {
        ...
        for (i = 0; i < expected.length; i++) {
            assert.equal(adone.datetime([2011, i, 1]).format("MMMM"), expected[i]);
        }
    });
    ...
    it("format week", () => {
        ...
        for (i = 0; i < expected.length; i++) {
            assert.equal(adone.datetime([2011, 0, 2 + i]).format("dddd"), expected[i]);
        }
    });
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "ތެރޭގައި ސިކުންތުކޮޅެއް", "prefix");
        assert.equal(adone.datetime(0).from(30000), "ކުރިން ސިކުންތުކޮޅެއް", "suffix");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "މިއަދު 12:00", "today at the same time");
