...
describe("sl", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "čez nekaj sekund", "prefix");
        assert.equal(adone.datetime(0).from(30000), "pred nekaj sekundami", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "pred nekaj sekundami", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "danes ob 12:00", "today at the same time");
