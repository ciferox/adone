...
describe("th", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "อีก ไม่กี่วินาที", "prefix");
        assert.equal(adone.datetime(0).from(30000), "ไม่กี่วินาทีที่แล้ว", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "ไม่กี่วินาทีที่แล้ว", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "วันนี้ เวลา 12:00", "today at the same time");
