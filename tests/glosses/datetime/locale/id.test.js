...
describe("id", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "dalam beberapa detik", "prefix");
        assert.equal(adone.datetime(0).from(30000), "beberapa detik yang lalu", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "beberapa detik yang lalu", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "Hari ini pukul 12.00", "today at the same time");
