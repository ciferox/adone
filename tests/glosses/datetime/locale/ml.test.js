...
describe("ml", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "അൽപ നിമിഷങ്ങൾ കഴിഞ്ഞ്", "prefix");
        assert.equal(adone.datetime(0).from(30000), "അൽപ നിമിഷങ്ങൾ മുൻപ്", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "അൽപ നിമിഷങ്ങൾ മുൻപ്", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "ഇന്ന് ഉച്ച കഴിഞ്ഞ് 12:00 -നു", "today at the same time");
    ...
    it("meridiem", () => {
        assert.equal(adone.datetime([2011, 2, 23, 2, 30]).format("a"), "രാത്രി", "before dawn");
        assert.equal(adone.datetime([2011, 2, 23, 9, 30]).format("a"), "രാവിലെ", "morning");
        assert.equal(adone.datetime([2011, 2, 23, 14, 30]).format("a"), "ഉച്ച കഴിഞ്ഞ്", "during day");
        assert.equal(adone.datetime([2011, 2, 23, 17, 30]).format("a"), "വൈകുന്നേരം", "evening");
        assert.equal(adone.datetime([2011, 2, 23, 19, 30]).format("a"), "വൈകുന്നേരം", "late evening");
        assert.equal(adone.datetime([2011, 2, 23, 21, 20]).format("a"), "രാത്രി", "night");
