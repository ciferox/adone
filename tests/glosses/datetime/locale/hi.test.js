...
describe("hi", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "कुछ ही क्षण में", "prefix");
        assert.equal(adone.datetime(0).from(30000), "कुछ ही क्षण पहले", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "कुछ ही क्षण पहले", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "आज दोपहर १२:०० बजे", "today at the same time");
    ...
    it("meridiem", () => {
        assert.equal(adone.datetime([2011, 2, 23, 2, 30]).format("a"), "रात", "before dawn");
        assert.equal(adone.datetime([2011, 2, 23, 9, 30]).format("a"), "सुबह", "morning");
        assert.equal(adone.datetime([2011, 2, 23, 14, 30]).format("a"), "दोपहर", "during day");
        assert.equal(adone.datetime([2011, 2, 23, 17, 30]).format("a"), "शाम", "evening");
        assert.equal(adone.datetime([2011, 2, 23, 19, 30]).format("a"), "शाम", "late evening");
        assert.equal(adone.datetime([2011, 2, 23, 21, 20]).format("a"), "रात", "night");
