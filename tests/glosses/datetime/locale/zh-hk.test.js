...
describe("datetime", "locale", "zh-hk", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "幾秒內", "prefix");
        assert.equal(adone.datetime(0).from(30000), "幾秒前", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "幾秒前", "now from now should display as in the past");
    });
    ...
    it("meridiem", () => {
        ...
        assert.equal(adone.datetime([2011, 2, 23, 13, 0]).format("a"), "下午", "after noon");
    ...
    it("weeks year starting sunday format", () => {
        assert.equal(adone.datetime([2012, 0, 1]).format("w ww wo"), "1 01 1週", "Jan  1 2012 應該是第 1週");
        assert.equal(adone.datetime([2012, 0, 7]).format("w ww wo"), "1 01 1週", "Jan  7 2012 應該是第 1週");
        assert.equal(adone.datetime([2012, 0, 8]).format("w ww wo"), "2 02 2週", "Jan  8 2012 應該是第 2週");
        assert.equal(adone.datetime([2012, 0, 14]).format("w ww wo"), "2 02 2週", "Jan 14 2012 應該是第 2週");
        assert.equal(adone.datetime([2012, 0, 15]).format("w ww wo"), "3 03 3週", "Jan 15 2012 應該是第 3週");
    });
});
