...
describe("ro", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "peste câteva secunde", "prefix");
        assert.equal(adone.datetime(0).from(30000), "câteva secunde în urmă", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "câteva secunde în urmă", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "azi la 12:00", "today at the same time");
