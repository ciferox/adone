...
describe("datetime", "locale", "az", () => {
    ...
    it("format", () => {
        ...
        const dt = adone.datetime(new Date(2010, 1, 14, 15, 25, 50, 125));
        let DDDoDt;
        let i;
    ...
    it("format ordinal", () => {
        assert.equal(adone.datetime([2011, 0, 1]).format("DDDo"), "1-inci", "1st");
        assert.equal(adone.datetime([2011, 0, 2]).format("DDDo"), "2-nci", "2nd");
        assert.equal(adone.datetime([2011, 0, 3]).format("DDDo"), "3-üncü", "3rd");
        assert.equal(adone.datetime([2011, 0, 4]).format("DDDo"), "4-üncü", "4th");
        assert.equal(adone.datetime([2011, 0, 5]).format("DDDo"), "5-inci", "5th");
        assert.equal(adone.datetime([2011, 0, 6]).format("DDDo"), "6-ncı", "6th");
        assert.equal(adone.datetime([2011, 0, 7]).format("DDDo"), "7-nci", "7th");
        assert.equal(adone.datetime([2011, 0, 8]).format("DDDo"), "8-inci", "8th");
        assert.equal(adone.datetime([2011, 0, 9]).format("DDDo"), "9-uncu", "9th");
        assert.equal(adone.datetime([2011, 0, 10]).format("DDDo"), "10-uncu", "10th");

        assert.equal(adone.datetime([2011, 0, 11]).format("DDDo"), "11-inci", "11th");
        assert.equal(adone.datetime([2011, 0, 12]).format("DDDo"), "12-nci", "12th");
        assert.equal(adone.datetime([2011, 0, 13]).format("DDDo"), "13-üncü", "13th");
        assert.equal(adone.datetime([2011, 0, 14]).format("DDDo"), "14-üncü", "14th");
        assert.equal(adone.datetime([2011, 0, 15]).format("DDDo"), "15-inci", "15th");
        assert.equal(adone.datetime([2011, 0, 16]).format("DDDo"), "16-ncı", "16th");
        assert.equal(adone.datetime([2011, 0, 17]).format("DDDo"), "17-nci", "17th");
        assert.equal(adone.datetime([2011, 0, 18]).format("DDDo"), "18-inci", "18th");
        assert.equal(adone.datetime([2011, 0, 19]).format("DDDo"), "19-uncu", "19th");
        assert.equal(adone.datetime([2011, 0, 20]).format("DDDo"), "20-nci", "20th");

        assert.equal(adone.datetime([2011, 0, 21]).format("DDDo"), "21-inci", "21th");
        assert.equal(adone.datetime([2011, 0, 22]).format("DDDo"), "22-nci", "22th");
        assert.equal(adone.datetime([2011, 0, 23]).format("DDDo"), "23-üncü", "23th");
        assert.equal(adone.datetime([2011, 0, 24]).format("DDDo"), "24-üncü", "24th");
        assert.equal(adone.datetime([2011, 0, 25]).format("DDDo"), "25-inci", "25th");
        assert.equal(adone.datetime([2011, 0, 26]).format("DDDo"), "26-ncı", "26th");
        assert.equal(adone.datetime([2011, 0, 27]).format("DDDo"), "27-nci", "27th");
        assert.equal(adone.datetime([2011, 0, 28]).format("DDDo"), "28-inci", "28th");
        assert.equal(adone.datetime([2011, 0, 29]).format("DDDo"), "29-uncu", "29th");
        assert.equal(adone.datetime([2011, 0, 30]).format("DDDo"), "30-uncu", "30th");

        assert.equal(adone.datetime([2011, 0, 31]).format("DDDo"), "31-inci", "31st");
    });
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "birneçə saniyyə sonra", "prefix");
        assert.equal(adone.datetime(0).from(30000), "birneçə saniyyə əvvəl", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "birneçə saniyyə əvvəl", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "bugün saat 12:00", "today at the same time");
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(adone.datetime([2011, 11, 26]).format("w ww wo"), "1 01 1-inci", "Dec 26 2011 should be week 1");
        assert.equal(adone.datetime([2012, 0, 1]).format("w ww wo"), "1 01 1-inci", "Jan  1 2012 should be week 1");
        assert.equal(adone.datetime([2012, 0, 2]).format("w ww wo"), "2 02 2-nci", "Jan  2 2012 should be week 2");
        assert.equal(adone.datetime([2012, 0, 8]).format("w ww wo"), "2 02 2-nci", "Jan  8 2012 should be week 2");
        assert.equal(adone.datetime([2012, 0, 9]).format("w ww wo"), "3 03 3-üncü", "Jan  9 2012 should be week 3");
    });
});
