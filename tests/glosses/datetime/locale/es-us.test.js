...
describe("datetime", "locale", "es-us", () => {
    ...
    it("from", () => {
        ...
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 44 }), true), "unos segundos", "44 seconds = a few seconds");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 45 }), true), "un minuto", "45 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 89 }), true), "un minuto", "89 seconds = a minute");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ s: 90 }), true), "2 minutos", "90 seconds = 2 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 44 }), true), "44 minutos", "44 minutes = 44 minutes");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 45 }), true), "una hora", "45 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 89 }), true), "una hora", "89 minutes = an hour");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ m: 90 }), true), "2 horas", "90 minutes = 2 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 5 }), true), "5 horas", "5 hours = 5 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 21 }), true), "21 horas", "21 hours = 21 hours");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 22 }), true), "un día", "22 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 35 }), true), "un día", "35 hours = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ h: 36 }), true), "2 días", "36 hours = 2 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 1 }), true), "un día", "1 day = a day");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 5 }), true), "5 días", "5 days = 5 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 25 }), true), "25 días", "25 days = 25 days");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 26 }), true), "un mes", "26 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 30 }), true), "un mes", "30 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 43 }), true), "un mes", "43 days = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 46 }), true), "2 meses", "46 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 74 }), true), "2 meses", "75 days = 2 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 76 }), true), "3 meses", "76 days = 3 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 1 }), true), "un mes", "1 month = a month");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ M: 5 }), true), "5 meses", "5 months = 5 months");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 345 }), true), "un año", "345 days = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ d: 548 }), true), "2 años", "548 days = 2 years");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 1 }), true), "un año", "1 year = a year");
        assert.equal(start.from(adone.datetime([2007, 1, 28]).add({ y: 5 }), true), "5 años", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "en unos segundos", "prefix");
        assert.equal(adone.datetime(0).from(30000), "hace unos segundos", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "hace unos segundos", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(adone.datetime().add({ s: 30 }).fromNow(), "en unos segundos", "en unos segundos");
        assert.equal(adone.datetime().add({ d: 5 }).fromNow(), "en 5 días", "en 5 días");
    });
