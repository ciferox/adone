...
describe("my", () => {
    ...
    it("format ordinal", () => {
        assert.equal(adone.datetime([2011, 0, 1]).format("DDDo"), "၁", "၁");
        assert.equal(adone.datetime([2011, 0, 2]).format("DDDo"), "၂", "၂");
        assert.equal(adone.datetime([2011, 0, 3]).format("DDDo"), "၃", "၃");
        assert.equal(adone.datetime([2011, 0, 4]).format("DDDo"), "၄", "၄");
        assert.equal(adone.datetime([2011, 0, 5]).format("DDDo"), "၅", "၅");
        assert.equal(adone.datetime([2011, 0, 6]).format("DDDo"), "၆", "၆");
        assert.equal(adone.datetime([2011, 0, 7]).format("DDDo"), "၇", "၇");
        assert.equal(adone.datetime([2011, 0, 8]).format("DDDo"), "၈", "၈");
        assert.equal(adone.datetime([2011, 0, 9]).format("DDDo"), "၉", "၉");
        assert.equal(adone.datetime([2011, 0, 10]).format("DDDo"), "၁၀", "၁၀");

        assert.equal(adone.datetime([2011, 0, 11]).format("DDDo"), "၁၁", "၁၁");
        assert.equal(adone.datetime([2011, 0, 12]).format("DDDo"), "၁၂", "၁၂");
        assert.equal(adone.datetime([2011, 0, 13]).format("DDDo"), "၁၃", "၁၃");
        assert.equal(adone.datetime([2011, 0, 14]).format("DDDo"), "၁၄", "၁၄");
        assert.equal(adone.datetime([2011, 0, 15]).format("DDDo"), "၁၅", "၁၅");
        assert.equal(adone.datetime([2011, 0, 16]).format("DDDo"), "၁၆", "၁၆");
        assert.equal(adone.datetime([2011, 0, 17]).format("DDDo"), "၁၇", "၁၇");
        assert.equal(adone.datetime([2011, 0, 18]).format("DDDo"), "၁၈", "၁၈");
        assert.equal(adone.datetime([2011, 0, 19]).format("DDDo"), "၁၉", "၁၉");
        assert.equal(adone.datetime([2011, 0, 20]).format("DDDo"), "၂၀", "၂၀");

        assert.equal(adone.datetime([2011, 0, 21]).format("DDDo"), "၂၁", "၂၁");
        assert.equal(adone.datetime([2011, 0, 22]).format("DDDo"), "၂၂", "၂၂");
        assert.equal(adone.datetime([2011, 0, 23]).format("DDDo"), "၂၃", "၂၃");
        assert.equal(adone.datetime([2011, 0, 24]).format("DDDo"), "၂၄", "၂၄");
        assert.equal(adone.datetime([2011, 0, 25]).format("DDDo"), "၂၅", "၂၅");
        assert.equal(adone.datetime([2011, 0, 26]).format("DDDo"), "၂၆", "၂၆");
        assert.equal(adone.datetime([2011, 0, 27]).format("DDDo"), "၂၇", "၂၇");
        assert.equal(adone.datetime([2011, 0, 28]).format("DDDo"), "၂၈", "၂၈");
        assert.equal(adone.datetime([2011, 0, 29]).format("DDDo"), "၂၉", "၂၉");
        assert.equal(adone.datetime([2011, 0, 30]).format("DDDo"), "၃၀", "၃၀");

        assert.equal(adone.datetime([2011, 0, 31]).format("DDDo"), "၃၁", "၃၁");
    });
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "လာမည့် စက္ကန်.အနည်းငယ် မှာ", "prefix");
        assert.equal(adone.datetime(0).from(30000), "လွန်ခဲ့သော စက္ကန်.အနည်းငယ် က", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "လွန်ခဲ့သော စက္ကန်.အနည်းငယ် က", "ယခုမှစပြီး အတိတ်တွင်ဖော်ပြသလိုဖော်ပြမည်");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "ယနေ. ၁၂:၀၀ မှာ", "ယနေ. ဒီအချိန်");
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(adone.datetime([2012, 0, 1]).format("w ww wo"), "၅၂ ၅၂ ၅၂", "Jan  1 2012 should be week 52");
        assert.equal(adone.datetime([2012, 0, 2]).format("w ww wo"), "၁ ၀၁ ၁", "Jan  2 2012 should be week 1");
        assert.equal(adone.datetime([2012, 0, 8]).format("w ww wo"), "၁ ၀၁ ၁", "Jan  8 2012 should be week 1");
        assert.equal(adone.datetime([2012, 0, 9]).format("w ww wo"), "၂ ၀၂ ၂", "Jan  9 2012 should be week 2");
        assert.equal(adone.datetime([2012, 0, 15]).format("w ww wo"), "၂ ၀၂ ၂", "Jan 15 2012 should be week 2");
    });
});
