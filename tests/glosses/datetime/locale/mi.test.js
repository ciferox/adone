...
describe("mi", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "i roto i te hēkona ruarua", "prefix");
        assert.equal(adone.datetime(0).from(30000), "te hēkona ruarua i mua", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "te hēkona ruarua i mua", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "i teie mahana, i 12:00", "today at the same time");
