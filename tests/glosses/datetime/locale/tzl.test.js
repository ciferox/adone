...
describe("tzl", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "osprei viensas secunds", "prefix");
        assert.equal(adone.datetime(0).from(30000), "ja'iensas secunds", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "ja'iensas secunds", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "oxhi à 12.00", "today at the same time");
