...
describe("datetime", "locale", "lv", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "pēc dažām sekundēm", "prefix");
        assert.equal(adone.datetime(0).from(30000), "pirms dažām sekundēm", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "pirms dažām sekundēm", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "Šodien pulksten 12:00", "today at the same time");
