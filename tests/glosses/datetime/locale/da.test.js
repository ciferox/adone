...
describe("datetime", "locale", "da", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "om få sekunder", "prefix");
        assert.equal(adone.datetime(0).from(30000), "få sekunder siden", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "få sekunder siden", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "i dag kl. 12:00", "today at the same time");
        assert.equal(adone.datetime(a).add({ m: 25 }).calendar(), "i dag kl. 12:25", "Now plus 25 min");
        assert.equal(adone.datetime(a).add({ h: 1 }).calendar(), "i dag kl. 13:00", "Now plus 1 hour");
        assert.equal(adone.datetime(a).add({ d: 1 }).calendar(), "i morgen kl. 12:00", "tomorrow at the same time");
        assert.equal(adone.datetime(a).subtract({ h: 1 }).calendar(), "i dag kl. 11:00", "Now minus 1 hour");
        assert.equal(adone.datetime(a).subtract({ d: 1 }).calendar(), "i går kl. 12:00", "yesterday at the same time");
    });
