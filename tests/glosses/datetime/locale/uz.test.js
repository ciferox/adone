...
describe("uz", () => {
    ...
    it("suffix", () => {
        assert.equal(adone.datetime(30000).from(0), "Якин фурсат ичида", "prefix");
        assert.equal(adone.datetime(0).from(30000), "Бир неча фурсат олдин", "suffix");
    });

    it("now from now", () => {
        assert.equal(adone.datetime().fromNow(), "Бир неча фурсат олдин", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(adone.datetime(a).calendar(), "Бугун соат 12:00 да", "today at the same time");
