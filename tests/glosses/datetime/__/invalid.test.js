describe("invalid", () => {
    ...
    it("invalid", () => {
        const m = adone.datetime.invalid();
    ...
    it("invalid with existing flag", () => {
        const m = adone.datetime.invalid({ invalidMonth: "whatchamacallit" });
        assert.equal(m.isValid(), false);
    ...
    it("invalid with custom flag", () => {
        const m = adone.datetime.invalid({ tooBusyWith: "reiculating splines" });
        assert.equal(m.isValid(), false);
        assert.equal(m.parsingFlags().userInvalidated, false);
    ...
    it("invalid operations", () => {
        const invalids = [
            adone.datetime.invalid(),
            adone.datetime("xyz", "l"),
            adone.datetime("2015-01-35", "YYYY-MM-DD"),
            adone.datetime("2015-01-25 a", "YYYY-MM-DD", true)
        ];

        const valid = adone.datetime();
        ...
        for (let i = 0; i < invalids.length; ++i) {
            ...
            assert.ok(!adone.datetime.min(invalid, valid).isValid());
            assert.ok(!adone.datetime.min(valid, invalid).isValid());
            assert.ok(!adone.datetime.max(invalid, valid).isValid());
            assert.ok(!adone.datetime.max(valid, invalid).isValid());
            ...
            assert.equal(adone.datetime.duration({ from: invalid, to: valid }).asMilliseconds(), 0);
            assert.equal(adone.datetime.duration({ from: valid, to: invalid }).asMilliseconds(), 0);
            assert.equal(adone.datetime.duration({ from: invalid, to: invalid }).asMilliseconds(), 0);
        }
    });
});
