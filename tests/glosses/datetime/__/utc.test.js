describe("datetime", "utc", () => {
    ...
    it("utc and local", () => {
        const m = adone.datetime(Date.UTC(2011, 1, 2, 3, 4, 5, 6));
        m.utc();
        // utc
        ...
        assert.equal(adone.datetime().utc().utcOffset(), 0, "timezone in utc should always be zero");
    });

    it("creating with utc and no arguments", () => {
        const startOfTest = new Date().valueOf();
        const momentDefaultUtcTime = adone.datetime.utc().valueOf();
        const afterMomentCreationTime = new Date().valueOf();

        assert.ok(startOfTest <= momentDefaultUtcTime, "adone.datetime UTC default time should be now, not in the past");
        assert.ok(momentDefaultUtcTime <= afterMomentCreationTime, "adone.datetime UTC default time should be now, not in the future");
    });

    it("creating with utc and a date parameter array", () => {
        let m = adone.datetime.utc([2011, 1, 2, 3, 4, 5, 6]);
        ...
        m = adone.datetime.utc("2011-02-02 3:04:05", "YYYY-MM-DD HH:mm:ss");
        ...
        m = adone.datetime.utc("2011-02-02T03:04:05+00:00");
    ...
    it("creating with utc without timezone", () => {
        let m = adone.datetime.utc("2012-01-02T08:20:00");
        ...
        m = adone.datetime.utc("2012-01-02T08:20:00+09:00");
    ...
    it("cloning with utc offset", () => {
        const m = adone.datetime.utc("2012-01-02T08:20:00");
        assert.equal(adone.datetime.utc(m)._isUTC, true, "the local offset should be converted to UTC");
        assert.equal(adone.datetime.utc(m.clone().utc())._isUTC, true, "the local offset should stay in UTC");

        m.utcOffset(120);
        assert.equal(adone.datetime.utc(m)._isUTC, true, "the explicit utc offset should stay in UTC");
        assert.equal(adone.datetime.utc(m).utcOffset(), 0, "the explicit utc offset should have an offset of 0");
    });

    it("weekday with utc", () => {
        assert.equal(
            adone.datetime("2013-09-15T00:00:00Z").utc().weekday(), // first minute of the day
            adone.datetime("2013-09-15T23:59:00Z").utc().weekday(), // last minute of the day
            "a UTC-adone.datetime's .weekday() should not be affected by the local timezone"
        );
    });
});
