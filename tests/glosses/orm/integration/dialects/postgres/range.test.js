    ...
    describe("[POSTGRES Specific] range datatype", () => {
        describe("stringify", () => {
            ...
            it("should handle date values", () => {
                const Range = new type.postgres.RANGE(type.DATE);
        ...
        describe("parse", () => {
            ...
            it("should handle empty bounds correctly", () => {
                expect(range.parse("(1,)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([1, null], { inclusive: [false, false] }));
                expect(range.parse("(,1)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([null, 1], { inclusive: [false, false] }));
                expect(range.parse("(,)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([null, null], { inclusive: [false, false] }));
            });

            it("should handle infinity/-infinity bounds correctly", () => {
                expect(range.parse("(infinity,1)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([Infinity, 1], { inclusive: [false, false] }));
                expect(range.parse("(1,infinity)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([1, Infinity], { inclusive: [false, false] }));
                expect(range.parse("(-infinity,1)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([-Infinity, 1], { inclusive: [false, false] }));
                expect(range.parse("(1,-infinity)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([1, -Infinity], { inclusive: [false, false] }));
                expect(range.parse("(-infinity,infinity)", type.postgres.INTEGER.parse)).to.deep.equal(_.extend([-Infinity, Infinity], { inclusive: [false, false] }));
            });
            ...
            it("should handle native postgres timestamp format", () => {
                const tsOid = type.postgres.DATE.types.postgres.oids[0],
        ...
        describe("stringify and parse", () => {
            it("should stringify then parse back the same structure", () => {
                ...
                expect(type.postgres.RANGE.parse(stringified, 3904, () => {
                    return type.postgres.INTEGER.parse;
