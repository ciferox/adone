        ...
        it("stores the current date in createdAt", function () {
            ...
            return this.User.bulkCreate(data).then(() => {
                return self.User.findAll({ order: ["id"] }).then((users) => {
                    expect(users.length).to.equal(2);
                    expect(users[0].username).to.equal("Peter");
                    expect(parseInt(Number(users[0].createdAt) / 5000, 10)).to.be.closeTo(parseInt(Number(new Date()) / 5000, 10), 1.5);
                    expect(users[1].username).to.equal("Paul");
                    expect(parseInt(Number(users[1].createdAt) / 5000, 10)).to.be.closeTo(parseInt(Number(new Date()) / 5000, 10), 1.5);
                });
            });
        });
        ...
        it("emits an error when validate is set to true", function () {
            ...
            return Tasks.sync({ force: true }).then(() => {
                return Tasks.bulkCreate([
                    ...
                    expect(error).to.be.instanceof(adone.x.AggregateException);
                    expect(error._errors).to.have.length(2);

                    const e0name0 = error._errors[0].get("name")[0];

                    expect(error._errors[0].record.code).to.equal("1234");
                    expect(e0name0.type || e0name0.origin).to.equal("notNull Violation");

                    expect(error._errors[1].record.name).to.equal("bar");
                    expect(error._errors[1].record.code).to.equal("1");
                    expect(error._errors[1].get("code")[0].message).to.equal("Validation len on code failed");
                });
            });
        });
