describe("bulkCreate", function () {
    ...
    it("should be able to set createdAt and updatedAt if using silent: true", function () {
        ...
        const values = adone.util.range(10).map(() => {
            return {
                createdAt,
                updatedAt
            };
    ...
    it("emits an error when validate is set to true", async function () {
        ...
        const { _errors: errors } = await assert.throws(async () => {
        ...
        expect(errors[0].errors[0].type).to.equal("notNull Violation");

        expect(errors[1].record.name).to.equal("bar");
        expect(errors[1].record.code).to.equal("1");
        expect(errors[1].errors[0].message).to.equal("Validation len on code failed");
    });
    ...
    if (current.dialect.supports.updateOnDuplicate) {
        it("should support the updateOnDuplicate option", function () {
            ...
            return this.User.bulkCreate(data, { fields: ["uniqueName", "secretValue"], updateOnDuplicate: ["secretValue"] }).then(() => {
                ...
                return self.User.bulkCreate(newData, { fields: ["uniqueName", "secretValue"], updateOnDuplicate: ["secretValue"] }).then(() => {
                    return self.User.findAll({ order: ["id"] }).then((users) => {
                        expect(users.length).to.equal(3);
                        expect(users[0].uniqueName).to.equal("Peter");
