...
describe("Auth service", function() {
    ...
    async function addGroup({ name = true, description = true, contexts = true } = { }) {
        ...
        if (is.string(name)) groupData.name = name;
        else if (name === true) groupData.name = getNextGroupName();

        if (is.string(description)) groupData.description = description;
        else if (description === true) groupData.description = srandom(128);

        if (is.array(contexts)) groupData.contexts = contexts;
        else if (contexts === true) groupData.contexts = [srandom(5), srandom(5), srandom(5)];
    ...
    async function addUser( { name = true, email = true, group = true, status = "Enabled", description = true,  password = true } = { }) {
        ...
        if (is.string(name)) userData.name = name;
        else if (name === true) userData.name = srandom(8);

        if (is.string(email)) userData.email = email;
        else if (email === true) userData.email = getNextEmail();

        if (is.string(description)) userData.description = description;
        else if (description === true) userData.description = srandom(128);

        if (is.string(group)) userData.group = group;
        else if (group === true) userData.group = srandom(12);

        if (is.string(status)) userData.status = status;
        else if (status === true) userData.status = srandom(8);

        if (is.string(password)) userData.password = password;
        else if (password === true) userData.password = srandom(32);
    ...
    before(async function() {
        ...
        authConfig = _.merge({}, appConfig.adone.omnitron.services.auth);
        userSchema = _.merge({}, authConfig.options.schemas.user);
        groupSchema = _.merge({}, authConfig.options.schemas.group);
        await omnitronRunner.startOmnitron();
        await omnitronRunner.connectOmnitron();
        iSystemAuth = omnitronRunner.getInterface("$auth");
    ...
    describe("Schemas", function() {
        it("user schema must be invariable", async function() {
            assert.deepEqual(userSchema, await iSystemAuth.getUserSchema());
        });

        it("group schema must be invariable", async function() {
            assert.deepEqual(groupSchema, await iSystemAuth.getGroupSchema());
        });
    });

    describe("Groups", function() {
        it("initially two default groups must be exist", async function() {
            const groups = await iSystemAuth.listGroups();
            assert.isOk(groups.length === 2);
            let userGroupData;
            let adminGroupData;
            if (groups[0].name === authConfig.options.userGroup.name) {
            ...
            assert.equal(authConfig.options.userGroup.name, userGroupData.name);
            assert.equal(authConfig.options.userGroup.description, userGroupData.description);
            assert.sameMembers(authConfig.options.userGroup.contexts, userGroupData.contexts);

            assert.equal(authConfig.options.adminGroup.name, adminGroupData.name);
            assert.equal(authConfig.options.adminGroup.description, adminGroupData.description);
            assert.sameMembers(authConfig.options.adminGroup.contexts, adminGroupData.contexts);
        });

        it("group interface should be singleton", async function() {
        ...
        it("add group with all fields", async function() {
            const { iGroup, groupData } = await addGroup({ });
        ...
        it("add group using defaults", async function() {
        ...
        it("add group without required field", async function() {
            try {
        ...
        it("add group with same unique field", async function() {
        ...
        it("get group by id", async function() {
        ...
        it("get group by name", async function() {
            const { iGroup, groupData } = await addGroup({ });
        ...
        it("remove group by id", async function() {
            const { iGroup } = await addGroup({ });
        ...
        it("remove group by name", async function() {
            const { iGroup, groupData } = await addGroup({ });
        ...
        it("remove nonexisting group", async function() {
            const groupName = getNextGroupName();
            try {
        ...
        it("remove all groups", async function() {
            const { iGroup } = await addGroup({ });
            await iSystemAuth.removeAllGroups();
            const names = await iSystemAuth.listGroupNames();
            assert.isOk(names.length === 2);
            assert.isOk(names.includes(authConfig.options.userGroup.name));
            assert.isOk(names.includes(authConfig.options.adminGroup.name));
        ...
        it("interfaces of removed groups must be inaccessible", async function() {
            const iGroups = [];
            for (let i = 0; i < 7; i++) {
                const { iGroup } = await addGroup({ });
                iGroups.push(iGroup);
        ...
        describe("Group interface", function() {
            ...
            it("set contexts", async function() {
            ...
            it("set same name", async function() {
                const oldName = await iGroup.getName();
                await iGroup.setName(oldName);
                assert.equal(groupName, await iGroup.getName());
            });

            it("set same description", async function() {
            ...
            it("set nonstring name", async function() {
                try {
            ...
            it("set name of another existing group", async function() {
                const { groupData } = await addGroup({ });
                try {
    ...
    describe("Users", function() {
        let groupName = getNextGroupName();
        before(async function() {
            await addGroup({ name: groupName });
        });

        it("initially users shouldn't be exist", async function() {
            const users = await iSystemAuth.listUsers();
            assert.isOk(users.length === 0);
        });

        it("user interface should be singleton", async function() {
        ...
        it("add user with all fields", async function() {
            const { iUser, userData } = await addUser({ group: groupName });
        ...
        it("add user using defaults", async function() {
        ...
        it("add user without name", async function() {
            try {
        ...
        it("add user without group", async function() {
            try {
        ...
        it("add user without email", async function() {
            try {
        ...
        it("add user without password", async function() {
            try {
        ...
        it("add user with nonexistent group", async function() {
            try {
        ...
        it("add user with incorrect email", async function() {
            try {
        ...
        it("add user with incorrect password", async function() {
            try {
        ...
        it("remove user by email", async function() {
            const { iUser, userData } = await addUser({ group: groupName, description: false, status: false });
        ...
        it("remove nonexisting user", async function() {
        ...
        describe("User interface", function() {
            ...
            it("set password several times", async function() {
            ...
            it("set password without specified old password", async function() {
                try {
            ...
            it("set password with incorrect old password", async function() {
                try {
            ...
            it("set password with incorrect type of old paasword", async function() {
                try {
            ...
            it("set same email", async function() {
                const old = await iUser.getEmail();
            ...
            it("set same status", async function() {
                const old = await iUser.getStatus();
            ...
            it("set email of another existing user", async function() {
    ...
    describe("Registration", function() {
        ...
        it("register user with specified status", async function() {
        ...
        it("register user without name", async function() {
            try {
        ...
        it("register user without password", async function() {
            try {
        ...
        it("register user with email of already existing user", async function() {
            const { userData } = await addUser({ group: authConfig.options.userGroup.name });
    ...
    describe("Login", function() {
        it("login when user is disabled", async function() {
            const userData = await registerUser({ });
        ...
        it("login without email and password", async function() {
            const userData = await registerUser({ });
            delete userData.name;
            delete userData.email;
            delete userData.password;
    ...
    describe("Sessions", function() {
        it("initially no sessions should be exist", async function() {
            const sessionDatas = await iSystemAuth.listSessions();
            assert.isOk(sessionDatas.length === 0);
        });

        it("get session on successfull login", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("get user by session key", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("get user by unknown session key", async function() {
        ...
        it("get session by key", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("session interface should be singleton", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("invalid session interface after logout", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("different session keys between logins", async function() {
            const userData = await registerUser({ });
            delete userData.name;
        ...
        it("get user by session key after logout", async function() {
            const userData = await registerUser({ });
            delete userData.name;
    ...
    describe.skip("Stress", function() {
        ...
        describe("Await mode", function() {
            ...
            it(`${limit} groups`, async function() {
            ...
            it(`${limit} sessions`, async function() {
                for (let i = 0; i < limit; i++) {
        ...
        describe("Stealth mode", function() {
            ...
            it(`${limit} users & sessions`, async function() {
                const promises = [];
                for (let i = 0; i < limit; i++) {
                    const userData = adone.o({
                        name: srandom(8),
