...
class Server {
    ...
    createRoom(roomName) {
        if (this.rooms.has(roomName)) {
            throw new adone.x.Exception("A room with this name already exists");
        }
        const room = new Set;
    ...
    getRoom(roomName) {
        if (!this.rooms.has(roomName)) {
            throw new adone.x.Exception("There is no such room");
        }
    ...
    async login(user, api) {
        if (this.users.has(user)) {
            throw new adone.x.Exception("This name has been taken");
        }
...
async function main(type) {
    ...
    const logger = grid.set(0, 0, 11, 12, adone.terminal.widget.Log, {
        top: 0,
        left: 0,
        width: "100%",
        height: "100%",
    ...
    const textarea = grid.set(11, 0, 1, 12, adone.terminal.widget.TextArea, {
        top: 0,
        left: 0,
        width: "100%",
        height: "100%"
